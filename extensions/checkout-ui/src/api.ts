// @ts-nocheck
import { CartItem, DeliveryDate, TimeSlot, DeliveryDatesResponse, TimeSlotsResponse } from './types';
...
async function getSessionToken(): Promise<string> {
  ...
  return new Promise((resolve) => {
    if (typeof window !== 'undefined' && (window as any).__SHOPIFY_SESSION_TOKEN__) {
 ...
 */
export async function fetchTimeSlots(shopId: string, date: string): Promise<TimeSlot[]> {
  try {
    const token = await getSessionToken();
 ...
 */
export function groupSlotsByTime(slots: TimeSlot[]): Record<string, TimeSlot[]> {
