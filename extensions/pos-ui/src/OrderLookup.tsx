...
import {
  ...
  useAsync,
  useDebounce,
} from "@witylogix/extension-core/hooks";
...
export function OrderLookup({ onOrderSelected }: OrderLookupProps) {
  ...
  const [selectedResult, setSelectedResult] = useState<POSSearchResult | null>(
    null,
  );
  ...
  const handleSelectResult = async (result: POSSearchResult) => {
    ...
    try {
      ...
      setSearchError(
        error instanceof Error ? error : new Error("Failed to load order"),
      );
    }
  };
  ...
  return h(
        ...
        results.map((result) =>
          h(
            "div",
            {
              ...
              style: {
                ...styles.resultItem,
                opacity: selectedResult?.id === result.id ? 0.6 : 1,
              },
            ...
            h(
              ...
              h(
                "span",
                { style: getStatusBadgeStyle(result.status) },
                result.status,
              ),
            ),
            h(
    ...
    !isSearching &&
      ...
      results.length === 0 &&
      !searchError &&
      h("div", { style: styles.emptyState }, "No orders found"),
