...
async function getSessionToken(): Promise<string> {
  return new Promise((resolve) => {
    if (typeof window !== 'undefined' && (window as any).__EXTENSION_CONFIG__?.apiKey) {
...
function getShopId(): string {
  if (typeof window !== 'undefined' && (window as any).__EXTENSION_CONFIG__?.shopId) {
 ...
 */
export async function getAvailableDrivers(zoneId: string): Promise<POSDriver[]> {
  try {
    const token = await getSessionToken();
    const shopId = getShopId();
 ...
 */
export async function getDeliverySlots(zoneId: string, date: string): Promise<DeliverySlot[]> {
  try {
    const token = await getSessionToken();
    const shopId = getShopId();
 ...
 * @returns Zone ID
 */
export async function getZoneIdFromAddress(postalCode: string): Promise<string> {
  try {
    const token = await getSessionToken();
    const shopId = getShopId();
