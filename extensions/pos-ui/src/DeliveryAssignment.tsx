 ...
 */
export function DeliveryAssignment({ order, onAssignmentComplete }: DeliveryAssignmentProps) {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const zone = await getZoneIdFromAddress(order.deliveryAddress.postalCode);
        setZoneId(zone);
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      // Assign delivery
      const assignment = await assignDelivery(order.id, selectedDriverId, selectedSlot.id);
      ...
      setError(err instanceof Error ? err : new Error('Failed to assign delivery'));
    } finally {
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h('p', { style: { ...styles.label, marginBottom: '4px' } }, 'Delivery Address'),
        h(
          'div',
    ...
    h(
      ...
      h(
        'select',
        {
          ...
          onChange: (e) => setSelectedDriverId((e.target as HTMLSelectElement).value),
          style: styles.select,
      ...
      selectedDriver && h(
        ...
        selectedDriver.rating && `Rating: ${selectedDriver.rating.toFixed(1)}/5.0`
      )
    ),
        ...
        getAvailableDates().map((date) =>
          h(
            'button',
            {
              ...
              style: {
                ...styles.button,
                ...(selectedDate === date ? styles.buttonPrimary : styles.buttonSecondary),
        ...
        slots.length > 0 ? slots.map((slot) =>
          h(
            ...
            h(
              ...
              h('span', { style: { fontWeight: tokens.typography.weights.medium } }, slot.label),
    ...
    selectedSlot && h(
      'div',
      { style: { ...styles.formGroup, padding: tokens.spacing[4], backgroundColor: tokens.colors.info.bg, borderRadius: tokens.radii.md } },
      h('p', { style: { ...styles.label, marginBottom: '4px' } }, 'Estimated Delivery'),
      h('p', { style: styles.orderNumber }, getEstimatedDelivery(selectedSlot))
    ),

    // Assign button
    h(
      'div',
      { style: { display: 'flex', gap: tokens.spacing[3], marginTop: tokens.spacing[6] } },
      h(
        'button',
        {
          ...
          disabled: !selectedDriverId || !selectedSlot || isAssigning || isLoading
        },
        isAssigning ? h(
          Fragment,
          null,
          h('div', { style: { ...styles.spinner, marginRight: tokens.spacing[2] } }),
          'Assigning...'
        ) : 'Assign & Print Label'
      )
    )
