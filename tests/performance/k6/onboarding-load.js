...
import { generateEmail, generateCompanyName, generateAddress } from './helpers/data-generators.js';
...
export default function () {
  ...
  group('Step 1: Account Creation', () => {
    ...
    const success = check(response, {
      'account creation status 201': (r) => r.status === 201 || r.status === 200,
  ...
  group('Step 2: Shop Creation', () => {
    ...
    const response = http.post(`${BASE_URL}/shops`, JSON.stringify({
      name: companyName,
  ...
  group('Step 3: Business Details', () => {
    ...
    const response = http.post(`${BASE_URL}/business-profile`, JSON.stringify({
    ...
    const success = check(response, {
      'profile creation status 200-201': (r) => r.status === 200 || r.status === 201,
    });
  ...
  group('Step 4: Payment Setup', () => {
    ...
    const response = http.post(`${BASE_URL}/payment-methods`, JSON.stringify({
      type: 'bank_account',
    ...
    const success = check(response, {
      'payment setup status 200-201': (r) => r.status === 200 || r.status === 201,
    });
  ...
  group('Step 5: Team Invitation', () => {
    ...
    const response = http.post(`${BASE_URL}/team-invitations`, JSON.stringify({
