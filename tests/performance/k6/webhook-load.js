...
export default function () {
  ...
  group('Webhook Delivery', () => {
    ...
    const response = http.post(`${WEBHOOK_URL}/inbound`, JSON.stringify(webhookEvent), {
      headers: {
  ...
  if (Math.random() < 0.1) { // 10% of requests are retries
    group('Webhook Retry Storm', () => {
      ...
      const startTime = Date.now();
      const response = http.post(`${WEBHOOK_URL}/inbound`, JSON.stringify({
        ...webhookEvent,
      ...
      const success = check(response, {
        'retry status is 200 or 429': (r) => r.status === 200 || r.status === 429,
      });
