...
import { login, register, refreshToken, requestPasswordReset } from './helpers/auth.js';
...
export default function () {
    ...
    if (success) {
      ...
      group('Token Refresh', () => {
        ...
        const refreshSuccess = check(refreshResponse, {
          ...
          'refresh response has accessToken': (r) => JSON.parse(r.body).accessToken,
          'refresh response has refreshToken': (r) => JSON.parse(r.body).refreshToken,
        });
  ...
  group('Password Reset', () => {
    ...
    const response = http.post(`${BASE_URL}/auth/request-password-reset`, JSON.stringify({
      email: TEST_USER_EMAIL,
    }), {
      headers: {
        'Content-Type': 'application/json',
    ...
    const success = check(response, {
      'password reset status is 200 or 202': (r) => r.status === 200 || r.status === 202,
    });
