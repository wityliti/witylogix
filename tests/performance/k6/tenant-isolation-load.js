...
export default function () {
    ...
    const success = check(response, {
      ...
      "orders belong to tenant": (r) => {
        ...
        return orders.every((order) => {
          ...
          return (
            order.tenantId === currentTenant.tenantId ||
            order.shopId === currentTenant.tenantId
          );
        });
      },
    });
  ...
  group("Tenant: Cross-Tenant Access Prevention", () => {
    const otherTenant = getTenants().find(
      (t) => t.tenantId !== currentTenant.tenantId,
    );
    if (otherTenant) {
  ...
  group("Tenant: Resource Separation", () => {
    ...
    const success = check(response, {
      "get tenant limits status 200": (r) =>
