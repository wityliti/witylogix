...
test.describe('Critical Paths - Business Workflows', () => {
  ...
  test.describe('@critical Order Lifecycle', () => {
    test('should complete full order lifecycle', async ({ page }) => {
      test.step('Setup: Login to admin account', async () => {
        ...
        await page.fill('[data-testid="email-input"]', process.env.ADMIN_EMAIL || 'admin@test.com');
        await page.fill('[data-testid="password-input"]', process.env.ADMIN_PASSWORD || 'TestPass123');
      ...
      test.step('Create new order', async () => {
        ...
        await page.fill('[data-testid="order-recipient-name"]', 'John Delivery');
        await page.fill('[data-testid="order-recipient-phone"]', '555-0100');
        await page.fill('[data-testid="order-recipient-address"]', '123 Main St');
        await page.fill('[data-testid="order-items-description"]', 'Test Package');
        ...
        await expect(page.locator('[data-testid="order-status"]')).toContainText('Draft', { timeout: 5000 });
      });
      ...
      test.step('Assign delivery driver', async () => {
        ...
        const firstDriver = page.locator('[data-testid="driver-option"]').first();
        await firstDriver.click();
        await page.click('[data-testid="confirm-assignment-button"]');
        await expect(page.locator('[data-testid="success-message"]')).toBeVisible({ timeout: 5000 });
        await expect(page.locator('[data-testid="order-status"]')).toContainText('Assigned', { timeout: 5000 });
      });
      ...
      test.step('Mark order as delivered', async () => {
        ...
        await expect(page.locator('[data-testid="success-message"]')).toBeVisible({ timeout: 5000 });
        await expect(page.locator('[data-testid="order-status"]')).toContainText('Delivered', { timeout: 5000 });
      });
      ...
      test.step('Complete order', async () => {
        ...
        await expect(page.locator('[data-testid="success-message"]')).toBeVisible({ timeout: 5000 });
        await expect(page.locator('[data-testid="order-status"]')).toContainText('Completed', { timeout: 5000 });
      });
  ...
  test.describe('@critical Driver Lifecycle', () => {
    test('should complete full driver registration and activation', async ({ page }) => {
      ...
      test.step('Navigate to driver registration', async () => {
        ...
        await expect(page.locator('[data-testid="registration-title"]')).toContainText('Register Driver', { timeout: 5000 });
      });
      ...
      test.step('Activate driver account', async () => {
        ...
        await page.setInputFiles('[data-testid="license-file-input"]', 'tests/fixtures/license.pdf');
      ...
      test.step('Assign delivery and complete', async () => {
        ...
        await page.click('[data-testid="driver-select-dropdown"]');
        const driverOption = page.locator(`[data-testid="driver-option"]:has-text("Test Driver")`);
      ...
      test.step('Rate driver delivery', async () => {
        ...
        await page.fill('[data-testid="rating-comment"]', 'Great driver, on time!');
  ...
  test.describe('@critical Delivery Lifecycle', () => {
    test('should complete full delivery workflow', async ({ page }) => {
      test.step('Setup: Login and navigate to delivery', async () => {
        ...
        await page.fill('[data-testid="email-input"]', process.env.DRIVER_EMAIL || 'driver@test.com');
        await page.fill('[data-testid="password-input"]', process.env.DRIVER_PASSWORD || 'DriverPass123');
        await page.click('[data-testid="login-button"]');
      ...
      test.step('Confirm pickup location', async () => {
        ...
        const activeDelivery = page.locator('[data-testid="active-delivery-card"]').first();
      ...
      test.step('Mark in transit', async () => {
        await page.click('[data-testid="mark-in-transit-button"]');
        await expect(page.locator('[data-testid="delivery-status"]')).toContainText('In Transit', { timeout: 5000 });
      });

      test.step('Navigate to delivery location', async () => {
        await page.click('[data-testid="navigate-to-delivery-button"]');
        await expect(page.locator('[data-testid="delivery-map"]')).toBeVisible({ timeout: 5000 });
      });

      test.step('Confirm delivery and capture POD', async () => {
        await page.click('[data-testid="mark-delivered-button"]');
        await expect(page.locator('[data-testid="pod-signature-canvas"]')).toBeVisible({ timeout: 5000 });
        await page.click('[data-testid="confirm-delivery-button"]');
        await expect(page.locator('[data-testid="success-message"]')).toBeVisible({ timeout: 5000 });
      });
      ...
      test.step('Confirm in system', async () => {
        ...
        await expect(page.locator('[data-testid="delivery-status"]')).toContainText('Delivered', { timeout: 5000 });
      });
  ...
  test.describe('@critical Payment Flow', () => {
    test('should complete payment and reconciliation', async ({ page }) => {
      ...
      test.step('Create invoice', async () => {
        ...
        await page.fill('[data-testid="invoice-customer-select"]', 'Test Customer');
        await page.fill('[data-testid="invoice-amount"]', '1000.00');
        await page.fill('[data-testid="invoice-description"]', 'Delivery Services');
      ...
      test.step('Send invoice to customer', async () => {
        ...
        await page.fill('[data-testid="invoice-email-recipient"]', 'customer@test.com');
      ...
      test.step('Verify invoice status', async () => {
        await page.goto(`/billing/invoices/${invoiceId}`);
        await expect(page.locator('[data-testid="invoice-status"]')).toContainText('Paid', { timeout: 5000 });
      });
  ...
  test.describe('@critical Integration Lifecycle', () => {
    test('should complete full integration setup and activation', async ({ page }) => {
      test.step('Setup: Login to admin', async () => {
        await page.goto('/login');
        await page.fill('[data-testid="email-input"]', process.env.ADMIN_EMAIL || 'admin@test.com');
        await page.fill('[data-testid="password-input"]', process.env.ADMIN_PASSWORD || 'TestPass123');
      ...
      test.step('Configure integration', async () => {
        ...
        const integrationRow = page.locator('[data-testid="integration-row"]').first();
        ...
        await page.fill('[data-testid="config-api-secret"]', 'test-api-secret-67890');
      ...
      test.step('Activate integration', async () => {
        ...
        await expect(page.locator('[data-testid="success-message"]')).toBeVisible({ timeout: 5000 });
        await expect(page.locator('[data-testid="integration-status"]')).toContainText('Active', { timeout: 5000 });
      });
      ...
      test.step('Deactivate integration', async () => {
        ...
        await expect(page.locator('[data-testid="success-message"]')).toBeVisible({ timeout: 5000 });
        await expect(page.locator('[data-testid="integration-status"]')).toContainText('Inactive', { timeout: 5000 });
      });
