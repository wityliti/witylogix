...
function randomCompanyName(): string {
  const prefixes = ['Acme', 'Tech', 'Global', 'Prime', 'Elite', 'Swift', 'Fast', 'Smart'];
  const suffixes = ['Corp', 'Industries', 'Services', 'Solutions', 'Logistics', 'Express', 'Hub'];
...
function randomFirstName(): string {
  const names = [
    ...
    'Robert', 'Maria', 'William', 'Anna', 'Charles', 'Rachel', 'Daniel', 'Lauren',
  ];
...
function randomAddress(): string {
  const streets = [
    '123 Main St', '456 Oak Ave', '789 Elm Rd', '321 Pine Ln',
    '654 Maple Dr', '987 Cedar Way', '159 Birch Ct', '753 Willow Pl',
  ];
  const cities = [
...
export function createMockOrder(overrides: Partial<MockOrder> = {}): MockOrder {
  ...
  return {
    ...
    items: overrides.items || [
      { productId: randomId(), quantity: Math.floor(Math.random() * 5) + 1, price: Math.floor(Math.random() * 200) + 10 },
    ],
...
export function createMockDriver(overrides: Partial<MockDriver> = {}): MockDriver {
  ...
  return {
    ...
    totalDeliveries: overrides.totalDeliveries || Math.floor(Math.random() * 500),
    createdAt: overrides.createdAt || randomTimestamp(),
    updatedAt: overrides.updatedAt || new Date().toISOString(),
  };
}
 ...
 */
export function createMockDelivery(overrides: Partial<MockDelivery> = {}): MockDelivery {
...
export function createMockIntegration(
  ...
  return {
    ...
    webhookUrl: overrides.webhookUrl || `https://webhook.example.com/${randomString(16)}`,
 ...
 */
export function createOrganizationSetup(overrides: {
  ...
  const shops = createMany(() => createMockShop({ organizationId: organization.id }), overrides.shopCount || 3);
  const users = createMany(() => createMockUser('MANAGER', { organizationId: organization.id }), overrides.userCount || 5);
  const drivers = createMany(() => createMockDriver({ organizationId: organization.id }), overrides.driverCount || 10);
