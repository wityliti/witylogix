 ...
 */
async function verifyConnectedBadge(
  page: Page,
  platformName: string,
): Promise<boolean> {
 ...
 */
async function verifySyncedOrdersAppear(
  page: Page,
...
test.describe("Platform Connect Flow - E2E", () => {
  ...
  test("should complete full Shopify integration flow", async () => {
    ...
    await expect(
      page.locator('[data-testid="integrations-header"]'),
    ...
    // Verify form appears
    await expect(
      page.locator('[data-testid="platform-setup-form"]'),
    ).toBeVisible();
  ...
  test("should display connected badge after successful setup", async () => {
    ...
    const statusText = await page
      .locator(`[data-testid="platform-status-shopify"]`)
      .textContent();
  ...
  test("should trigger manual sync from integrations page", async () => {
    ...
    const completionText = await page
      .locator('[data-testid="sync-completion-message"]')
      .textContent();
  ...
  test("should display synced orders in orders page", async () => {
    ...
    const orderSource = await page
      .locator('[data-testid="order-source-0"]')
      .textContent();
  ...
  test("should show sync progress with percentage", async () => {
    ...
    const progressText = await page
      .locator('[data-testid="sync-progress-text"]')
      .textContent();
  ...
  test("should display sync statistics after completion", async () => {
    ...
    await expect(
      page.locator('[data-testid="sync-stats-orders"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="sync-stats-products"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="sync-stats-customers"]'),
    ).toBeVisible();
  });
  ...
  test("should disconnect platform successfully", async () => {
    ...
    await expect(
      page.locator('[data-testid="disconnect-success-message"]'),
    ).toBeVisible();
