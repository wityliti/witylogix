...
test.describe("Order Lifecycle", () => {
  ...
  test("should create new order with all required fields", async ({
    adminPage,
  }) => {
    ordersPage = new OrdersPage(adminPage);
  ...
  test("should edit order details", async ({ adminPage }) => {
    ...
    const senderNameInput = adminPage.locator(
      'input[name*="senderName"], input[value*="Updated Sender"]',
    );
  ...
  test("should assign driver to order", async ({ adminPage }) => {
    ...
    const assignBtn = adminPage.locator(
      'button:has-text("Assign Driver"), [data-testid="assign-driver-btn"]',
    );
    ...
    if (await assignBtn.isVisible().catch(() => false)) {
      ...
      const driverSelect = adminPage.locator(
        'select[name*="driver"], [data-testid="driver-select"]',
      );
      ...
      const confirmBtn = adminPage.locator(
        'button[type="submit"]:has-text("Assign"), button:has-text("Confirm")',
      );
  ...
  test("should track order status changes", async ({ adminPage }) => {
    ...
    const statusElement = adminPage.locator(
  ...
  test("should cancel order", async ({ adminPage }) => {
    ...
    if (await cancelBtn.isVisible().catch(() => false)) {
      ...
      const confirmBtn = adminPage.locator(
        'button[type="submit"]:has-text("Confirm"), button:has-text("Yes")',
      );
      await confirmBtn.click();
