...
test.describe('Webhook Management', () => {
  ...
  test('should display webhook list', async ({ adminPage }) => {
    ...
    const webhookList = adminPage.locator('table, [data-testid="webhook-list"], [role="grid"]');
  ...
  test('should create webhook endpoint', async ({ adminPage }) => {
    ...
    const createBtn = adminPage.locator('button:has-text("Create Webhook"), button:has-text("New Webhook"), [data-testid="create-webhook-btn"]');
    ...
    if (await createBtn.isVisible().catch(() => false)) {
      ...
      // Fill webhook form
      const urlInput = adminPage.locator('input[placeholder*="Webhook URL"], input[name*="url"], input[type="url"]');
      ...
      if (await urlInput.isVisible().catch(() => false)) {
        ...
        // Submit form
        const submitBtn = adminPage.locator('button[type="submit"]:has-text("Create"), button:has-text("Save")');
        ...
        const successMessage = adminPage.locator('[data-testid="success"], .success, [role="alert"]');
        const webhookInList = adminPage.locator('text=example.com/webhooks/test');
  ...
  test('should view webhook details', async ({ adminPage }) => {
    ...
    // Get webhook rows
    const webhookRows = adminPage.locator('tbody tr, [data-testid="webhook-row"], [role="row"]');
    ...
    if (count > 0) {
      ...
      const detailsPanel = adminPage.locator('[data-testid="webhook-details"], section, .panel');
  ...
  test('should view webhook delivery log', async ({ adminPage }) => {
    ...
    if (count > 0) {
      ...
      if (await logSection.isVisible().catch(() => false)) {
        ...
        const logEntries = adminPage.locator('[data-testid="log-entry"], .log-entry, tbody tr');
  ...
  test('should display delivery status in log', async ({ adminPage }) => {
    ...
    if (count > 0) {
      ...
      const logEntries = adminPage.locator('[data-testid="log-entry"], .log-entry');
      const entryCount = await logEntries.count();

      if (entryCount > 0) {
        // Get status from first log entry
        const statusBadge = logEntries.first().locator('[data-testid="status"], .status, [role="status"]');

        if (await statusBadge.isVisible().catch(() => false)) {
          const status = await statusBadge.textContent();
          expect(['SUCCESS', 'FAILED', 'PENDING', 'RETRY'].some((s) => status?.includes(s))).toBeTruthy();
        }
      }
    }
  });
  ...
  test('should retry failed delivery', async ({ adminPage }) => {
    ...
    if (count > 0) {
      ...
      const failedEntries = adminPage.locator('[data-testid="log-entry"]:has-text("FAILED"), [data-testid="log-entry"]:has-text("FAILED")');

      if (await failedEntries.first().isVisible().catch(() => false)) {
        // Click retry button on failed entry
        const retryBtn = failedEntries.first().locator('button:has-text("Retry"), [data-testid="retry-btn"]');
  ...
  test('should update webhook settings', async ({ adminPage }) => {
    ...
    if (count > 0) {
      ...
      const editBtn = adminPage.locator('button:has-text("Edit"), [data-testid="edit-webhook-btn"]');
      ...
      if (await editBtn.isVisible().catch(() => false)) {
        ...
        const descInput = adminPage.locator('input[name*="description"], textarea[name*="description"]');
        ...
        // Save changes
        const saveBtn = adminPage.locator('button[type="submit"]:has-text("Save")');
        await saveBtn.click();
        ...
        const successMessage = adminPage.locator('[data-testid="success"], .success');
  ...
  test('should disable webhook', async ({ adminPage }) => {
    ...
    if (count > 0) {
      ...
      const disableBtn = webhookRows.first().locator('button[title="Disable"], button[data-testid="disable-btn"]');

      if (await disableBtn.isVisible().catch(() => false)) {
        await disableBtn.click();

        // Confirm disable action if prompt appears
        const confirmBtn = adminPage.locator('button:has-text("Confirm"), button:has-text("Yes")');
        ...
        const statusBadge = webhookRows.first().locator('[data-testid="status"], .status');
