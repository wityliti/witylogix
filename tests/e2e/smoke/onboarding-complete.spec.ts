...
test.describe("Complete Onboarding Flow", () => {
  ...
  test("should complete full onboarding flow step by step", async ({
    page,
  }) => {
    // Step 1: Register
    ...
    test.step("Complete email verification step", async () => {
      ...
      if (
        ...
        currentStep.toLowerCase().includes("verify")
      ) {
    ...
    test.step("Fill company information", async () => {
      const currentStep = await onboardingPage.getCurrentStep();
      expect(
        currentStep.toLowerCase().includes("company") ||
          currentStep.toLowerCase().includes("information"),
      ).toBeTruthy();
    ...
    test.step("Select integrations", async () => {
      const currentStep = await onboardingPage.getCurrentStep();
      expect(
        currentStep.toLowerCase().includes("integration") ||
          currentStep.toLowerCase().includes("connect"),
      ).toBeTruthy();
    ...
    test.step("Select dashboard layout", async () => {
      const currentStep = await onboardingPage.getCurrentStep();
      expect(
        currentStep.toLowerCase().includes("layout") ||
          currentStep.toLowerCase().includes("dashboard"),
      ).toBeTruthy();
    ...
    test.step("Skip data import", async () => {
      const currentStep = await onboardingPage.getCurrentStep();
      expect(
        currentStep.toLowerCase().includes("import") ||
          currentStep.toLowerCase().includes("data"),
      ).toBeTruthy();
    ...
    test.step("Review and submit onboarding", async () => {
      ...
      await expect(
        page.locator(`:text("${SMOKE_TEST_DATA.onboarding.company.name}")`),
    ...
    test.step("Verify workspace was provisioned", async () => {
      ...
      const sidebarLinks = await dashboardPage.page
        .locator('[data-testid="sidebar"] a, nav a')
        .allTextContents();
      expect(sidebarLinks.length).toBeGreaterThan(0);

      // Verify company name is displayed
      const userSection = dashboardPage.page.locator(
        '[data-testid="user-section"], [data-testid="workspace-name"]',
      );
    ...
    test.step("Verify navigation works", async () => {
      ...
      await expect(dashboardPage.ordersButton)
        .toBeVisible({ timeout: 5000 })
        .catch(() => {
  ...
  test("should handle onboarding step navigation correctly", async ({
    page,
  }) => {
    // Register
