...
test.describe('Integration Health Checks', () => {
  ...
  test('API health endpoint should return 200', async ({ request }) => {
    ...
    expect(['ok', 'healthy', 'up'].includes(data.status?.toLowerCase() || '')).toBeTruthy();
  });
  ...
  test('Static assets should load correctly', async ({ page }) => {
    ...
    const criticalErrors = errors.filter(
      (e) => !e.includes('WebSocket') && !e.includes('safe-zone') && !e.toLowerCase().includes('deprecat'),
    );

    expect(criticalErrors).toHaveLength(0);
  });

  test('API authentication endpoints should be accessible', async ({ request }) => {
  ...
  test('API CORS headers should be present', async ({ request }) => {
    ...
    const hasCors = Object.keys(headers).some((key) => key.toLowerCase().includes('access-control'));
  ...
  test('Services should be discoverable and healthy', async ({ request }) => {
    ...
    for (const service of services) {
      try {
        const response = await request.get(`${API_BASE_URL}/${service.name}${service.endpoint}`);
