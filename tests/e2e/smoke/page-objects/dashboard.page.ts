...
export class SmokeDashboardPage {
  ...
  constructor(page: Page) {
    ...
    this.deliveryZoneButton = page.locator(
      'button:has-text("Add Zone"), button:has-text("New Zone"), [data-testid="add-zone-btn"]',
    );
    this.driversButton = page.locator(
      'button:has-text("Add Driver"), button:has-text("New Driver"), [data-testid="add-driver-btn"]',
    );
    ...
    this.successNotification = page.locator(
      '[data-testid="success-notification"], .toast-success, [role="status"]',
    );
    this.errorNotification = page.locator(
      '[data-testid="error-notification"], .toast-error, [role="alert"]',
    );
  }
  ...
  async navigateToDeliveryZones(): Promise<void> {
    const link = this.page.locator(
      '[data-testid="sidebar"] a:has-text("Zones"), nav a:has-text("Zones")',
    );
  ...
  async navigateToDrivers(): Promise<void> {
    const link = this.page.locator(
      '[data-testid="sidebar"] a:has-text("Drivers"), nav a:has-text("Drivers")',
    );
    await expect(link).toBeVisible({ timeout: 5000 });
    await link.click();
    await this.waitForPageLoad();
  ...
  async navigateToOrders(): Promise<void> {
    const link = this.page.locator(
      '[data-testid="sidebar"] a:has-text("Orders"), nav a:has-text("Orders")',
    );
    await expect(link).toBeVisible({ timeout: 5000 });
    await link.click();
    await this.waitForPageLoad();
  ...
  async createDeliveryZone(
    ...
    await this.page.fill(
      'input[name="name"], input[placeholder*="Zone name"]',
      name,
    );
    await this.page.fill(
    ...
    await this.page.fill(
      'input[name="radius"], input[type="number"]',
      radius.toString(),
    );
  ...
  async createOrder(
    ...
    await this.page.fill(
      'textarea[placeholder*="Sender Address"]',
      senderAddress,
    );
