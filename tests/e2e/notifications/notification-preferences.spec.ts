...
class NotificationPreferencesPage {
  ...
  constructor(baseUrl: string = 'http://localhost:3000') {
    ...
    this.emailToggle = new MockPageElement('input[data-testid="channel-email"]');
    ...
    this.whatsappToggle = new MockPageElement('input[data-testid="channel-whatsapp"]');
    this.slackToggle = new MockPageElement('input[data-testid="channel-slack"]');

    // Initialize category toggles
    this.orderConfirmationToggle = new MockPageElement('input[data-testid="category-order-confirmation"]');
    this.orderUpdatesToggle = new MockPageElement('input[data-testid="category-order-updates"]');
    this.deliveryNotificationsToggle = new MockPageElement('input[data-testid="category-delivery"]');
    this.promotionsToggle = new MockPageElement('input[data-testid="category-promotions"]');
    this.newslettersToggle = new MockPageElement('input[data-testid="category-newsletters"]');

    // Initialize quiet hours
    this.quietHoursEnabledToggle = new MockPageElement('input[data-testid="quiet-hours-enabled"]');
    this.quietHoursStartTimeInput = new MockPageElement('input[data-testid="quiet-hours-start"]');
    this.quietHoursEndTimeInput = new MockPageElement('input[data-testid="quiet-hours-end"]');
    this.timezoneSelect = new MockPageElement('select[data-testid="timezone"]');

    // Initialize digest frequency
    this.digestFrequencySelect = new MockPageElement('select[data-testid="digest-frequency"]');

    // Initialize test notification
    this.sendTestNotificationButton = new MockPageElement('button[data-testid="send-test"]');
    this.testNotificationDestinationInput = new MockPageElement('input[data-testid="test-destination"]');

    // Initialize action buttons
    this.savePreferencesButton = new MockPageElement('button[data-testid="save-preferences"]');
  ...
  async enableQuietHours(startTime: string, endTime: string, timezone: string): Promise<void> {
  ...
  async setDigestFrequency(frequency: 'IMMEDIATE' | 'HOURLY' | 'DAILY' | 'WEEKLY'): Promise<void> {
    await this.digestFrequencySelect.setValue(frequency);
  }

  async sendTestNotification(destination: string, channel: string = 'email'): Promise<void> {
  ...
  async waitForToast(predicate: (toast: Toast) => boolean, timeoutMs: number = 5000): Promise<Toast> {
...
describe('Notification Preferences E2E', () => {
  ...
  describe('Integrated Preference Management Workflow', () => {
    it('should complete full preference setup workflow', async () => {
      ...
      // Verify success
      const successToasts = (await page.getToasts()).filter((t) => t.type === 'success');
