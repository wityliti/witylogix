...
export class DispatchPage {
  ...
  constructor(page: Page) {
    this.page = page;
    this.statsBar = page.locator('[data-testid="stats-bar"], [data-testid="dispatch-header"]');
    this.activeDriversCount = page.locator('[data-testid="active-drivers-stat"], [data-testid="stat-drivers"]');
    this.totalStopsCount = page.locator('[data-testid="total-stops-stat"], [data-testid="stat-stops"]');
    this.totalKmCount = page.locator('[data-testid="total-km-stat"], [data-testid="stat-km"]');
    this.totalTimeCount = page.locator('[data-testid="total-time-stat"], [data-testid="stat-time"]');
    this.routeTimeline = page.locator('[data-testid="route-timeline"], [data-testid="timeline"]');
    this.driverRows = page.locator('[data-testid="driver-row"], [data-testid="timeline-row"]');
    this.mapContainer = page.locator('[data-testid="dispatch-map"], [data-testid="map-container"]');
    this.mapMarkers = page.locator('[data-testid="map-marker"], .map-marker');
    this.unscheduledTab = page.locator('[data-testid="tab-unscheduled"], button:has-text("Unscheduled")');
    this.scheduledTab = page.locator('[data-testid="tab-scheduled"], button:has-text("Scheduled")');
    this.tabContent = page.locator('[data-testid="tab-content"], [role="tabpanel"]');
    this.stopCards = page.locator('[data-testid="stop-card"], [data-testid="order-card"]');
    this.planRoutesButton = page.locator('[data-testid="plan-routes-btn"], button:has-text("Plan Routes")');
    this.draggableStops = page.locator('[data-testid="draggable-stop"], [draggable="true"]');
    this.routeDropZones = page.locator('[data-testid="route-drop-zone"], [data-testid="drop-target"]');
   ...
   */
  async getStatsBar(): Promise<{ drivers: string; stops: string; km: string; time: string }> {
  ...
  async toggleTab(tabName: 'unscheduled' | 'scheduled'): Promise<void> {
    const tab = tabName === 'unscheduled' ? this.unscheduledTab : this.scheduledTab;
  ...
  > {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const vehicle = await row.locator('[data-testid="vehicle-plate"]').textContent();
      const status = await row.locator('[data-testid="driver-status"]').textContent();
      const stops = await row.locator('[data-testid="stops-count"]').textContent();
      const rating = await row.locator('[data-testid="driver-rating"]').textContent();
  ...
  async selectDriver(driverName: string): Promise<void> {
    const driverRow = this.page.locator(`[data-testid="driver-row"]:has([data-testid="driver-name"]:text("${driverName}"))`);
   ...
   * Get all map markers
   */
  async getMapMarkers(): Promise<Array<{ id: string; type: string; lat: number; lng: number }>> {
    await this.waitForMapLoad();
    const markers: Array<{ id: string; type: string; lat: number; lng: number }> = [];
  ...
  async dragStopToRoute(stopId: string, targetRouteId: string): Promise<void> {
    const stop = this.page.locator(`[data-testid="draggable-stop"][data-id="${stopId}"]`);
    const targetZone = this.page.locator(`[data-testid="route-drop-zone"][data-route-id="${targetRouteId}"]`);
  ...
  async clickStopCard(stopId: string): Promise<void> {
    const stopCard = this.page.locator(`[data-testid="stop-card"][data-id="${stopId}"]`);
  ...
  async getStopDetails(): Promise<{
    ...
    const modal = this.page.locator('[data-testid="stop-detail-modal"], [role="dialog"]');
    await expect(modal).toBeVisible();

    const id = await modal.locator('[data-testid="stop-id"]').textContent();
    const address = await modal.locator('[data-testid="stop-address"]').textContent();
    const phone = await modal.locator('[data-testid="stop-phone"]').textContent();
    const timeWindow = await modal.locator('[data-testid="time-window"]').textContent();
    const notes = await modal.locator('[data-testid="stop-notes"]').textContent();
   ...
   */
  async waitForStatsUpdate(previousStats: { drivers: string; stops: string }, timeout: number = 15000): Promise<boolean> {
   ...
   */
  async expectRouteTimelineWithDrivers(minDriverCount: number = 1): Promise<void> {
    const count = await this.driverRows.count();
    if (count < minDriverCount) {
      throw new Error(`Expected at least ${minDriverCount} driver rows, found ${count}`);
    }
  }
  ...
  async closeStopDetail(): Promise<void> {
    const closeButton = this.page.locator('[data-testid="stop-detail-close"], [aria-label="Close"]');
