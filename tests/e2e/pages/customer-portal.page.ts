...
export class CustomerPortalPage {
  ...
  constructor(page: Page) {
    ...
    this.loginError = page.locator(
      '[data-testid="login-error"], [role="alert"]',
    );
    this.portalHeader = page.locator(
      '[data-testid="portal-header"], [data-testid="customer-header"]',
    );
    this.navMenu = page.locator('[data-testid="nav-menu"], nav');
    this.ordersLink = page.locator(
      '[data-testid="nav-orders"], a:has-text("Orders")',
    );
    this.trackingLink = page.locator(
      '[data-testid="nav-tracking"], a:has-text("Tracking")',
    );
    this.preferencesLink = page.locator(
      '[data-testid="nav-preferences"], a:has-text("Preferences")',
    );
    this.accountLink = page.locator(
      '[data-testid="nav-account"], a:has-text("Account")',
    );
    this.logoutButton = page.locator(
      'button:has-text("Logout"), button:has-text("Sign Out")',
    );
    this.orderList = page.locator(
      '[data-testid="order-list"], [data-testid="orders-container"]',
    );
    this.orderItems = page.locator(
      '[data-testid="order-item"], [data-testid="order-card"]',
    );
    this.orderDetailContainer = page.locator(
      '[data-testid="order-detail"], [data-testid="order-details"]',
    );
    this.deliveryTimeline = page.locator(
      '[data-testid="delivery-timeline"], [data-testid="timeline"]',
    );
    this.timelineEvents = page.locator(
      '[data-testid="timeline-event"], [data-testid="event"]',
    );
    this.podImage = page.locator(
      '[data-testid="pod-image"], [data-testid="proof-of-delivery"]',
    );
    this.trackingMap = page.locator(
      '[data-testid="tracking-map"], [data-testid="map"]',
    );
    this.driverPosition = page.locator(
      '[data-testid="driver-marker"], [data-testid="driver-location"]',
    );
    this.rescheduleButton = page.locator(
      '[data-testid="reschedule-btn"], button:has-text("Reschedule")',
    );
    this.rateDeliveryButton = page.locator(
      '[data-testid="rate-delivery-btn"], button:has-text("Rate")',
    );
    this.ratingStars = page.locator(
      '[data-testid="rating-star"], [aria-label*="star" i]',
    );
    this.feedbackTextarea = page.locator('[data-testid="feedback"], textarea');
    this.submitRatingButton = page.locator(
      '[data-testid="submit-rating"], button:has-text("Submit")',
    );
    this.preferencesForm = page.locator(
      '[data-testid="preferences-form"], [data-testid="prefs"]',
    );
    this.safePlaceToggle = page.locator(
      '[data-testid="safe-place-toggle"], input[type="checkbox"]',
    );
    this.safeInstructions = page.locator(
      '[data-testid="safe-instructions"], textarea',
    );
    this.accessCodeInput = page.locator(
      '[data-testid="access-code"], input[placeholder*="code" i]',
    );
    this.savePreferencesButton = page.locator(
      '[data-testid="save-prefs"], button:has-text("Save")',
    );
    ...
    this.successMessage = page.locator(
      '[data-testid="success"], [data-testid="success-message"]',
    );
    this.errorMessage = page.locator(
      '[data-testid="error"], [data-testid="error-message"]',
    );
  }
  ...
  async getOrderDetail(orderId: string): Promise<{
    ...
    const orderItem = this.page.locator(
      `[data-testid="order-item"][data-order-id="${orderId}"]`,
    );
    await orderItem.click();
    await expect(this.orderDetailContainer).toBeVisible();

    const id = await this.orderDetailContainer
      .locator('[data-testid="order-id"]')
      .textContent();
    const status = await this.orderDetailContainer
      .locator('[data-testid="order-status"]')
      .textContent();
    const address = await this.orderDetailContainer
      .locator('[data-testid="delivery-address"]')
      .textContent();
    const phone = await this.orderDetailContainer
      .locator('[data-testid="delivery-phone"]')
      .textContent();
    const email = await this.orderDetailContainer
      .locator('[data-testid="delivery-email"]')
      .textContent();
    const total = await this.orderDetailContainer
      .locator('[data-testid="order-total"]')
      .textContent();
    const estimatedDelivery = await this.orderDetailContainer
      .locator('[data-testid="estimated-delivery"]')
      .textContent();

    // Get items
    const items: Array<{ name: string; quantity: number; price: string }> = [];
    const itemElements = this.orderDetailContainer.locator(
      '[data-testid="order-item-line"]',
    );
    const itemCount = await itemElements.count();

    for (let i = 0; i < itemCount; i++) {
      const itemLine = itemElements.nth(i);
      const name = await itemLine
        .locator('[data-testid="item-name"]')
        .textContent();
      const quantity = await itemLine
        .locator('[data-testid="item-quantity"]')
        .textContent();
      const price = await itemLine
  ...
  > {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const timestamp = await event
        .locator('[data-testid="event-time"]')
        .textContent();
      const status = await event
        .locator('[data-testid="event-status"]')
        .textContent();
      const description = await event
        .locator('[data-testid="event-description"]')
        .textContent();
      const icon = await event
  ...
  async confirmReschedule(): Promise<void> {
    const confirmButton = this.page.locator(
      '[data-testid="confirm-reschedule"], button:has-text("Confirm")',
    );
  ...
  async getTrackingInfo(orderId: string): Promise<{
    ...
    const driverName = await this.page
      .locator('[data-testid="driver-name"]')
      .textContent();
    const driverPhone = await this.page
      .locator('[data-testid="driver-phone"]')
      .textContent();
    const vehiclePlate = await this.page
      .locator('[data-testid="vehicle-plate"]')
      .textContent();
    const estimatedArrival = await this.page
      .locator('[data-testid="estimated-arrival"]')
      .textContent();
    const currentLocation = await this.page
      .locator('[data-testid="current-location"]')
      .textContent();
