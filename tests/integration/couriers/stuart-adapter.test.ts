...
describe('StuartAdapter', () => {
  ...
  describe('OAuth2 Token Flow', () => {
    ...
    it('should include refresh token for token renewal', () => {
      ...
      expect(mockStuartOAuth2Token.refresh_token).toBe('stuart_refresh_token_abc123');
    });
  ...
  describe('Quote with Multiple Pricing Tiers', () => {
    ...
    it('should include surge multiplier in breakdown', () => {
      const surgeComponent = mockStuartMultiplePricingTiers.pricing_breakdown.find(
    ...
    it('should include surcharge in breakdown', () => {
      const surchargeComponent = mockStuartMultiplePricingTiers.pricing_breakdown.find(
        (p) => p.type === 'surcharge'
      );
  ...
  describe('Status Transitions', () => {
    ...
    it('should include delivery timestamp when completed', () => {
      expect(mockStuartDeliveryCompleted.delivered_at).toBe('2024-03-11T14:25:00Z');
    });

    it('should include delivery proof requirements', () => {
      expect(mockStuartDeliveryCompleted.delivery_proof).toBeDefined();
      expect(mockStuartDeliveryCompleted.delivery_proof.signature_required).toBe(false);
    });

    it('should handle cancellation status', () => {
      expect(mockStuartDeliveryCancelled.status).toBe('cancelled');
      expect(mockStuartDeliveryCancelled.cancellation_reason).toBe('Customer request');
    });
  });
  ...
  describe('Driver Location Tracking', () => {
    ...
    it('should handle delivery without courier location', () => {
      expect(mockStuartDeliveryWithoutCourier.current_courier_location).toBeUndefined();
    });
  ...
  describe('Scheduled Time Windows', () => {
    it('should include pickup scheduled time', () => {
      expect(mockStuartDelivery.pickup.scheduled_at).toBe('2024-03-11T14:00:00Z');
    });
    ...
    it('should include dropoff scheduled time', () => {
      expect(mockStuartDelivery.dropoff.scheduled_at).toBe('2024-03-11T14:30:00Z');
    });
    ...
    it('should calculate time window from schedules', () => {
      ...
      const duration = (dropoffTime.getTime() - pickupTime.getTime()) / 1000 / 60;
  ...
  describe('Weight and Charged Weight', () => {
    ...
    it('should potentially differ from actual weight', () => {
      const chargedVsActual = mockStuartDelivery.charged_weight >= mockStuartDelivery.items.reduce((sum, item) => sum + (item.weight || 0), 0);
  ...
  describe('Estimated Delivery Time', () => {
    ...
    it('should be after dropoff scheduled time', () => {
      const scheduledTime = new Date(mockStuartDelivery.dropoff.scheduled_at).getTime();
      expect(mockStuartDelivery.estimated_delivery_time).toBeGreaterThanOrEqual(scheduledTime);
    });
  });
