...
type OnboardingStep = 'email_verification' | 'company_info' | 'workspace_setup' | 'integrations' | 'complete';
...
class OnboardingService {
  ...
  updateStep(userId: string, step: OnboardingStep, data: Record<string, unknown> = {}): OnboardingState {
  ...
  moveToStep(userId: string, targetStep: OnboardingStep): OnboardingState {
    ...
    const steps: OnboardingStep[] = ['email_verification', 'company_info', 'workspace_setup', 'integrations', 'complete'];
...
class WorkspaceProvisioningService {
  ...
  provision(config: Omit<WorkspaceConfig, 'orgId' | 'apiKey'>): WorkspaceConfig {
...
class InvitationService {
  ...
  createInvitation(email: string, orgId: string): Invitation {
    ...
    const existing = Array.from(this.invitations.values()).find(
      inv => inv.email === email && inv.orgId === orgId && !inv.acceptedAt && !inv.revokedAt
    );
...
describe('Onboarding Service', () => {
  ...
  describe('Onboarding Lifecycle', () => {
    ...
    it('should track step progression', () => {
      ...
      const state = service.updateStep('user-123', 'email_verification', { email: 'user@example.com' });
    ...
    it('should persist data across steps', () => {
      service.startOnboarding('user-123');
      service.updateStep('user-123', 'email_verification', { email: 'user@example.com' });
  ...
  describe('Session Management', () => {
    ...
    it('should track last updated timestamp', () => {
      ...
      expect(state2.lastUpdatedAt.getTime()).toBeGreaterThanOrEqual(initialTime.getTime());
    });
  });
});
...
describe('Invitation System', () => {
  ...
  describe('Invitation Creation', () => {
    it('should create invitation with token', () => {
      const invitation = service.createInvitation('user@example.com', 'org-123');
    ...
    it('should set expiration date', () => {
      ...
      const expiresIn = invitation.expiresAt.getTime() - invitation.createdAt.getTime();
  ...
  describe('Invitation Acceptance', () => {
    ...
    it('should track acceptance timestamp', () => {
      ...
      expect(accepted?.acceptedAt).toBeInstanceOf(Date);
      expect(accepted!.acceptedAt!.getTime()).toBeGreaterThanOrEqual(before.getTime());
    });
  });
