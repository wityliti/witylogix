...
export function createMockVehicleLocation(overrides?: Partial<MockVehicleLocation>): MockVehicleLocation {
  return {
...
export function createMockShipment(overrides?: Partial<MockShipment>): MockShipment {
  return {
    id: randomId('ship'),
    orderId: randomId('order'),
...
export function createMockWebhookEvent(overrides?: Partial<MockWebhookEvent>): MockWebhookEvent {
  return {
    id: randomId('evt'),
...
export function shopifyOrderResponse(overrides?: Partial<MockOrder>): Record<string, any> {
  const order = createMockOrder(overrides);
  return {
    id: parseInt(order.id.replace(/\D/g, '')) || Math.floor(Math.random() * 1000000),
    ...
    line_items: order.items.map((item) => ({
      id: parseInt(item.productId.replace(/\D/g, '')) || Math.floor(Math.random() * 1000000),
      product_id: parseInt(item.productId.replace(/\D/g, '')) || Math.floor(Math.random() * 1000000),
      title: item.name,
...
export function stripeWebhookResponse(event: MockWebhookEvent): Record<string, any> {
  return {
    id: event.id,
    object: 'event',
...
export function generateOrders(count: number, overrides?: Partial<MockOrder>): MockOrder[] {
  return Array.from({ length: count }, () => createMockOrder(overrides));
}

export function generateProducts(count: number, overrides?: Partial<MockProduct>): MockProduct[] {
  return Array.from({ length: count }, () => createMockProduct(overrides));
}

export function generateVehicles(count: number, overrides?: Partial<MockVehicle>): MockVehicle[] {
  return Array.from({ length: count }, () => createMockVehicle(overrides));
}

export function generateVehicleLocations(count: number, vehicleId?: string): MockVehicleLocation[] {
...
export function generateInvoices(count: number, overrides?: Partial<MockInvoice>): MockInvoice[] {
  return Array.from({ length: count }, () => createMockInvoice(overrides));
}

export function generateShipments(count: number, overrides?: Partial<MockShipment>): MockShipment[] {
