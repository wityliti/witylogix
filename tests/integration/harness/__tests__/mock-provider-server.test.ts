...
describe('MockProviderServer', () => {
  ...
  describe('Basic Server Operations', () => {
    ...
    it('should return 404 for missing endpoints', async () => {
      await server.start();
      const response = await makeRequest(server.getBaseUrl(), '/api/v1/missing');
  ...
  describe('API Key Authentication', () => {
    it('should validate API keys from headers', async () => {
      ...
      const noAuth = await makeRequest(server.getBaseUrl(), '/api/v1/protected');
      expect(noAuth.status).toBe(401);

      const withAuth = await makeRequest(server.getBaseUrl(), '/api/v1/protected', {
        headers: { 'x-api-key': 'sk_test_123' },
      });
      expect(withAuth.status).toBe(200);

      const wrongKey = await makeRequest(server.getBaseUrl(), '/api/v1/protected', {
  ...
  describe('Fixture Management', () => {
    it('should add and retrieve fixtures', async () => {
      ...
      const products = await makeRequest(server.getBaseUrl(), '/api/v1/products');
