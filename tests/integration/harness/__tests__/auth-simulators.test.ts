...
describe('APIKeyValidator', () => {
  ...
  it('should validate API keys from headers', () => {
    expect(validator.validateFromHeaders({ 'x-api-key': 'sk_test_123' })).toBe(true);
    expect(validator.validateFromHeaders({ 'x-api-key': 'invalid' })).toBe(false);
  });
  ...
  it('should add new keys', () => {
    ...
    expect(validator.validateFromHeaders({ 'x-api-key': 'sk_test_789' })).toBe(true);
  });

  it('should remove keys', () => {
    validator.removeKey('sk_test_123');
    expect(validator.validateFromHeaders({ 'x-api-key': 'sk_test_123' })).toBe(false);
  });
...
describe('OAuth2Server', () => {
  ...
  it('should generate authorization codes', () => {
    const code = server.generateAuthCode('http://localhost/callback', 'read write');
  ...
  it('should exchange authorization codes for tokens', () => {
    ...
    const token = server.exchangeAuthCode(code, 'client_123', 'secret_456', 'http://localhost/callback');
  ...
  it('should reject invalid client secrets', () => {
    const code = server.generateAuthCode('http://localhost/callback');
    const token = server.exchangeAuthCode(code, 'client_123', 'wrong_secret', 'http://localhost/callback');
  ...
  it('should support refresh tokens', () => {
    ...
    const newToken = server.refreshAccessToken(token!.refresh_token!, 'client_123', 'secret_456');
...
describe('HMACSignatureGenerator', () => {
  it('should generate Stripe-style HMAC signatures', () => {
    ...
    const signature = HMACSignatureGenerator.generateStripeSignature(payload, secret);
  ...
  it('should generate Shopify-style HMAC signatures', () => {
    ...
    const signature = HMACSignatureGenerator.generateShopifySignature(payload, secret);
  ...
  it('should verify HMAC-SHA256 signatures', () => {
    ...
    expect(HMACSignatureGenerator.verifyHMACSHA256(payload, signature, secret)).toBe(true);
    expect(HMACSignatureGenerator.verifyHMACSHA256(payload, 'wrong_signature', secret)).toBe(false);
    expect(HMACSignatureGenerator.verifyHMACSHA256('wrong_payload', signature, secret)).toBe(false);
  });

  it('should generate different signatures for different payloads', () => {
    const secret = 'test_secret';
    const sig1 = HMACSignatureGenerator.generateStripeSignature('payload1', secret);
    const sig2 = HMACSignatureGenerator.generateStripeSignature('payload2', secret);
  ...
  it('should generate different signatures for different secrets', () => {
    ...
    const sig1 = HMACSignatureGenerator.generateStripeSignature(payload, 'secret1');
    const sig2 = HMACSignatureGenerator.generateStripeSignature(payload, 'secret2');
