...
export const createSkuFixture = (
  overrides?: Partial<SkuFixture>,
): SkuFixture => ({
...
export const createHighValueSkuFixture = (
  overrides?: Partial<SkuFixture>,
): SkuFixture => ({
  id: `sku_${Math.random().toString(36).substr(2, 9)}`,
...
export const createFastMovingSkuFixture = (
  overrides?: Partial<SkuFixture>,
): SkuFixture => ({
  id: `sku_${Math.random().toString(36).substr(2, 9)}`,
...
export const createInventoryFixture = (
  skuId: string,
  warehouse: string,
  overrides?: Partial<InventoryFixture>,
): InventoryFixture => ({
  skuId,
  warehouse,
  onHand: 150,
...
export const createLowInventoryFixture = (
  skuId: string,
  warehouse: string,
  overrides?: Partial<InventoryFixture>,
): InventoryFixture => ({
...
export const createWarehouseFixture = (
...
export const createRegionalWarehouseFixture = (
  region: string,
  overrides?: Partial<WarehouseFixture>,
): WarehouseFixture => ({
  id: `wh_${Math.random().toString(36).substr(2, 9)}`,
...
export const createPurchaseOrderFixture = (
  overrides?: Partial<PurchaseOrderFixture>,
...
export const createSalesOrderFixture = (
  overrides?: Partial<SalesOrderFixture>,
): SalesOrderFixture => ({
...
export const createWaveFixture = (
  warehouse: string,
  orderIds: string[],
  overrides?: Partial<WaveFixture>,
): WaveFixture => ({
...
export const createPickListFixture = (
  waveId: string,
  orderId: string,
  overrides?: Partial<PickListFixture>,
): PickListFixture => ({
...
export const createShipmentFixture = (
  orderIds: string[],
  warehouse: string,
  overrides?: Partial<ShipmentFixture>,
): ShipmentFixture => ({
...
export const createReturnFixture = (
  orderIds: string,
  overrides?: Partial<ReturnFixture>,
): ReturnFixture => ({
  id: `ret_${Math.random().toString(36).substr(2, 9)}`,
  originalOrderId: orderIds,
  status: "initiated",
  items: [
    { skuId: "SKU001", quantity: 2, reason: "defective", condition: "damaged" },
  ],
...
export const createDemandHistoryFixture = (
  skuId: string,
  days: number = 30,
): DemandHistoryFixture[] => {
...
export const createSeasonalDemandHistoryFixture = (
  skuId: string,
  days: number = 90,
): DemandHistoryFixture[] => {
  const history: DemandHistoryFixture[] = [];
  ...
  for (let i = 0; i < days; i++) {
    ...
    const dayOfYear = Math.floor(
      (date.getTime() / (24 * 60 * 60 * 1000)) % 365,
    );
    const seasonalFactor = Math.sin((dayOfYear / 365) * 2 * Math.PI);
    const variance = (Math.random() - 0.5) * 10;
    const quantity = Math.max(
      1,
      Math.floor(baseQuantity * (1 + seasonalFactor * 0.3) + variance),
    );
