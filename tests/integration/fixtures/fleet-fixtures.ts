...
export function createMockVehicle(options: MockVehicleOptions = {}): Vehicle {
  ...
  const vin = options.vin || 'WVWZZZ' + randomUUID().substring(0, 11).toUpperCase();
  const licensePlate = options.licensePlate || 'WIT' + Math.floor(Math.random() * 10000).toString().padStart(4, '0');
...
export function createMockVehicles(count: number, overrides: Partial<MockVehicleOptions> = {}): Vehicle[] {
...
export function createMockMaintenanceRecord(
  ...
  const now = new Date();
  const scheduledDate = new Date(now.getTime() + (options.daysIntoSchedule || 7) * 24 * 60 * 60 * 1000);
  const completedDate = options.status === 'COMPLETED' ? new Date(scheduledDate.getTime() - 24 * 60 * 60 * 1000) : undefined;
...
export function createMockMaintenanceSchedule(
  ...
  const nextServiceDate = new Date(lastServiceDate.getTime() + 12 * 30 * 24 * 60 * 60 * 1000); // +12 months
...
export function createMockFuelCard(overrides: Partial<FuelCard> = {}): FuelCard {
  return {
...
export function createMockFleetCost(options: MockFleetCostOptions = {}): FleetCost {
...
export function calculateHealthScore(components: MockHealthScoreComponents): number {
  const weights = {
...
export function createFleetScenario(vehicleCount: number = 5): FleetScenarioFixtures {
...
export function generateLicensePlate(): string {
  return 'WIT' + Math.floor(Math.random() * 10000).toString().padStart(4, '0');
}
