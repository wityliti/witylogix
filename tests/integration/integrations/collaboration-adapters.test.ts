...
describe("Collaboration Adapters Integration Tests", () => {
  describe("Slack Notification Service", () => {
    ...
    it("should reject short bot token", async () => {
      await expect(service.authenticate("short")).rejects.toThrow(
        "Invalid bot token",
      );
    });
    ...
    it("should send block message", async () => {
      ...
      const blocks = [
        {
          type: "section",
          text: { type: "mrkdwn", text: "*Order Dispatched*" },
        },
      ];
  ...
  describe("Microsoft Teams Service", () => {
    ...
    it("should configure webhook URL", async () => {
      const configured = await service.setWebhookUrl(
    ...
    it("should reject invalid webhook URL", async () => {
      await expect(service.setWebhookUrl("not-a-url")).rejects.toThrow(
        "Invalid webhook URL",
      );
    });
  ...
  describe("SendGrid Email Service", () => {
    ...
    it("should authenticate with API key", async () => {
      const authenticated = await service.authenticate(
        "SG.1234567890123456789012345678901234567890123",
      );
      expect(authenticated).toBe(true);
    });
  ...
  describe("Discord Webhook Service", () => {
    ...
    it("should configure webhook", async () => {
      const configured = await service.setWebhook(
    ...
    it("should format message embed", () => {
      const embed = service.formatMessageEmbed(
        "Order Update",
        "Your order is ready",
      );
  ...
  describe("Notification Template Engine", () => {
    ...
    it("should throw on missing template", () => {
      expect(() =>
        engine.render("nonexistent", { recipientName: "John" }),
