...
import type { NotificationPayload, QuietHours } from '../fixtures/notification-fixtures.js';
import {
...
class QuietHoursService {
  ...
  async handleNotification(notification: NotificationPayload): Promise<{ queued: boolean; reason?: string }> {
  ...
  flushQueueAtQuietHoursEnd(recipientId: string): DigestBatch {
    ...
    // Remove from queue
    this.queue = this.queue.filter((q) => q.notification.recipientId !== recipientId);
  ...
  getQueuedNotifications(recipientId?: string): QueuedNotification[] {
    if (recipientId) {
      return this.queue.filter((q) => q.notification.recipientId === recipientId);
    }
    return [...this.queue];
  }
...
describe('Notification Quiet Hours', () => {
  ...
  describe('Quiet Hours Detection', () => {
    it('should detect when in quiet hours (evening)', () => {
      ...
      const inQuietHours = quietHoursService.isInQuietHours(recipientId, eveningTime);
    ...
    it('should detect when not in quiet hours (morning)', () => {
      ...
      const inQuietHours = quietHoursService.isInQuietHours(recipientId, morningTime);
  ...
  describe('Notification Queueing', () => {
    ...
    it('should queue multiple notifications during quiet hours', async () => {
      const notification1 = createNotificationPayload({ recipientId, priority: 'NORMAL' });
      const notification2 = createNotificationPayload({ recipientId, priority: 'NORMAL' });
      const notification3 = createNotificationPayload({ recipientId, priority: 'NORMAL' });
  ...
  describe('CRITICAL Priority Bypass', () => {
    ...
    it('should bypass quiet hours for HIGH priority messages', async () => {
      ...
      const result = await quietHoursService.handleNotification(highPriorityNotification);
  ...
  describe('Timezone Handling', () => {
    it('should respect UTC timezone', () => {
      ...
      const inQuietHours = quietHoursService.isInQuietHours(recipientId, utcEveningTime);
      expect(inQuietHours).toBe(true);
    });
    ...
    it('should respect EST timezone', () => {
      ...
      const inQuietHours = quietHoursService.isInQuietHours(recipientId, estTime);
    ...
    it('should respect PST timezone', () => {
      ...
      const inQuietHours = quietHoursService.isInQuietHours(recipientId, pstTime);
      expect(typeof inQuietHours).toBe('boolean');
    });
    ...
    it('should respect IST timezone', () => {
      ...
      const inQuietHours = quietHoursService.isInQuietHours(recipientId, istTime);
      expect(typeof inQuietHours).toBe('boolean');
    });
  });
  ...
  describe('Digest Batching During Quiet Hours', () => {
    ...
    it('should clear queue after flushing to digest', async () => {
      ...
      const notification = createNotificationPayload({ recipientId, priority: 'NORMAL' });
  ...
  describe('Queue Management', () => {
    it('should retrieve queued notifications by recipient', async () => {
      ...
      const quietHours = createQuietHours({ enabled: true, startTime: '22:00', endTime: '08:00' });
      quietHoursService.setQuietHours(recipientId, quietHours);
