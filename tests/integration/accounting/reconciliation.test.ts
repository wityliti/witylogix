...
describe('Accounting Reconciliation', () => {
  ...
  describe('Invoice Amount Matching', () => {
    it('should match invoice amounts within tolerance', () => {
      ...
      const match: InvoiceMatch = {
        ...
        amount: 1000.00,
        tolerance: TOLERANCE,
        matched: Math.abs(invoice.amount - 1000.00) <= TOLERANCE,
        difference: Math.abs(invoice.amount - 1000.00),
      };
    ...
    it('should validate exact penny amounts', () => {
      const amounts = [100.01, 250.50, 500.75, 1000.99];
  ...
  describe('Payment Allocation', () => {
    it('should allocate full payment correctly', () => {
      ...
      expect(allocation.allocatedAmount).toBe(5000.00);
      expect(allocation.remainingBalance).toBe(0.00);
    ...
    it('should allocate partial payment correctly', () => {
      ...
      expect(allocation.allocatedAmount).toBe(3000.00);
      expect(allocation.remainingBalance).toBe(7000.00);
    ...
    it('should handle overpayment detection', () => {
      ...
      expect(allocation.remainingBalance).toBe(-500.00);
    });
  ...
  describe('Currency Conversion Reconciliation', () => {
    ...
    it('should reconcile EUR invoices with consistent FX rate', () => {
      ...
      const fxRate = 1.10; // EUR to USD
      const usdAmount = invoice.amount * fxRate;

      expect(invoice.currency).toBe('EUR');
      expect(usdAmount).toBeCloseTo(935.00, 2);
    });
    ...
    it('should validate FX rate source consistency', () => {
      ...
      expect(usdEquivalent).toBeCloseTo(635.00, 2);
    });

    it('should handle multi-currency invoices', () => {
      const invoices = [
        createMockInvoice({ amount: 1000.00, currency: 'USD' }),
        createMockInvoice({ amount: 850.00, currency: 'EUR', fxRate: 1.10 }),
        createMockInvoice({ amount: 100.00, currency: 'GBP', fxRate: 1.27 }),
      ];

      expect(invoices).toHaveLength(3);
      expect(invoices.map((inv) => inv.currency)).toEqual(['USD', 'EUR', 'GBP']);
    });
  });
  ...
  describe('Tax Calculation Verification', () => {
    ...
    it('should calculate tax for tax-inclusive amounts', () => {
      const lineItem = createMockLineItem({
        description: 'Product',
        totalAmount: 1100.00, // includes tax
        taxRate: 0.10,
      });
      ...
      expect(baseAmount).toBeCloseTo(1000.00, 2);
      expect(taxAmount).toBeCloseTo(100.00, 2);
    });
    ...
    it('should validate multi-line tax calculation', () => {
      ...
      const totalTax = line1.amount * line1.taxRate + line2.amount * line2.taxRate;
      const totalAmount = line1.amount + line2.amount + totalTax;

      expect(totalTax).toBe(150.00);
      expect(totalAmount).toBe(1650.00);
    });
  ...
  describe('Credit Note Application', () => {
    it('should apply full credit note to invoice', () => {
      ...
      const creditNote = createMockInvoice({
        id: 'credit_001',
        amount: -5000.00, // negative for credit
      });
    ...
    it('should apply partial credit note', () => {
      ...
      const newBalance = invoice.amount + creditNote.amount;
      expect(newBalance).toBe(7000.00);
    });
    ...
    it('should handle multiple credit notes', () => {
      ...
      const credits = [
        createMockInvoice({ amount: -2000.00 }),
        createMockInvoice({ amount: -3000.00 }),
        createMockInvoice({ amount: -1000.00 }),
      ];

      const totalCredits = credits.reduce((sum, c) => sum + c.amount, 0);
      const newBalance = invoice.amount + totalCredits;

      expect(newBalance).toBe(4000.00);
    });
  });
  ...
  describe('Multi-line Invoice Reconciliation', () => {
    it('should reconcile invoice with multiple line items', () => {
      const items = [
        createMockLineItem({ description: 'Item 1', amount: 1000.00, quantity: 2 }),
        createMockLineItem({ description: 'Item 2', amount: 500.00, quantity: 1 }),
        createMockLineItem({ description: 'Item 3', amount: 250.00, quantity: 4 }),
      ];
    ...
    it('should validate line item totals sum to invoice total', () => {
      const invoice = createMockInvoice({
        id: 'inv_013',
        lineItems: [
          createMockLineItem({ amount: 1000.00, quantity: 1 }),
          createMockLineItem({ amount: 2000.00, quantity: 1 }),
          createMockLineItem({ amount: 500.00, quantity: 1 }),
        ],
      });
  ...
  describe('Period-End Reconciliation', () => {
    it('should reconcile month-end statement', () => {
      const invoices = [
        createMockInvoice({ amount: 5000.00, status: 'paid' }),
        createMockInvoice({ amount: 3000.00, status: 'paid' }),
        createMockInvoice({ amount: 2000.00, status: 'outstanding' }),
      ];
      ...
      expect(report.totalPaid).toBe(8000.00);
      expect(report.outstanding).toBe(2000.00);
      expect(report.reconciled).toBe(true);
    });

    it('should identify reconciliation discrepancies', () => {
      const totalExpected = 10000.00;
      const totalReconciled = 9999.50;

      const discrepancy = totalExpected - totalReconciled;
      expect(discrepancy).toBe(0.50);
