...
class MockHl7Service {
  ...
  extractSubField(segment: Hl7Segment, fieldIndex: number, subFieldIndex: number): string {
    if (
      fieldIndex < 0 ||
  ...
  getPatientName(pidSegment: Hl7Segment): { familyName: string; givenName: string } {
  ...
  validateMessage(messageText: string): Array<{ type: 'error' | 'warning'; message: string }> {
    const issues: Array<{ type: 'error' | 'warning'; message: string }> = [];

    // Check minimum structure
    if (!messageText.startsWith('MSH')) {
      issues.push({ type: 'error', message: 'Message must start with MSH segment' });
    }
    ...
    if (segments.length < 2) {
      issues.push({ type: 'warning', message: 'Message contains only one segment' });
    }
...
describe('HL7 Parsing', () => {
  ...
  it('should parse valid HL7 message', () => {
    const hl7Message = 'MSH|^~\\&|SENDING_APP|SENDING_FAC|RECEIVING_APP|RECEIVING_FAC|20240115120000||ADT^A01|MSG123|P|2.4\nPID|||MRN123^^^MRN||DOE^JOHN||19800101|M';
  ...
  it('should extract message control ID', () => {
    const hl7Message = 'MSH|^~\\&|SENDING_APP|SENDING_FAC|RECEIVING_APP|RECEIVING_FAC|20240115120000||ADT^A01|MSG456|P|2.4';
  ...
  it('should parse ORM message', () => {
    const hl7Message = 'MSH|^~\\&|SENDING_APP|SENDING_FAC|RECEIVING_APP|RECEIVING_FAC|20240115120000||ORM^O01|MSG124|P|2.4\nORC|NW|ORDER123';
  ...
  it('should extract PID segment', () => {
    const hl7Message = 'MSH|^~\\&|APP|FAC|APP|FAC|20240115120000||ADT^A01|MSG123|P|2.4\nPID|||MRN123^^^MRN||DOE^JOHN||19800101|M';
  ...
  it('should detect invalid field separator', () => {
    const hl7Message = 'MSHx^~\\&|APP|FAC|APP|FAC|20240115120000||ADT^A01|MSG123|P|2.4';
