...
class InventoryReconciliationService {
   ...
   */
  async syncStockLevel(sku: string, externalQuantity: number, internalQuantity: number): Promise<InventoryLevel> {
   ...
   */
  async updateReservedQuantity(sku: string, platformId: string, reservedQty: number): Promise<InventoryLevel> {
   ...
   */
  async checkOversell(sku: string, requestedQuantity: number, reservedQuantity: number): Promise<boolean> {
   ...
   */
  async propagateAdjustment(sku: string, quantityChange: number, reason: string): Promise<InventoryAdjustment> {
    const adjustment: InventoryAdjustment = {
      id: `adj_${Math.random().toString(36).substr(2, 9)}`,
      sku,
      platform: 'witylogix',
   ...
   */
  async checkLowStock(sku: string, warehouseId?: string): Promise<LowStockAlert | null> {
    ...
    const reorderPoint = 'reorderPoint' in inventory ? inventory.reorderPoint : 50;
    const currentLevel = 'available' in inventory ? inventory.available : inventory.onHand;
...
describe('InventoryReconciliationService', () => {
  ...
  describe('Stock Level Sync Across Platforms', () => {
    ...
    it('should update lastSyncAt timestamp', async () => {
      ...
      expect(level?.lastSyncAt.getTime()).toBeGreaterThanOrEqual(before.getTime());
  ...
  describe('Reserved Quantity Handling', () => {
    it('should update reserved quantity correctly', async () => {
      ...
      const updated = await service.updateReservedQuantity('SKU-007', 'platform_001', 30);
    ...
    it('should prevent available from going negative', async () => {
      ...
      const updated = await service.updateReservedQuantity('SKU-008', 'platform_001', 60);
  ...
  describe('Inventory Adjustment Propagation', () => {
    it('should propagate adjustment to platforms', async () => {
      const adjustment = await service.propagateAdjustment('SKU-014', 10, 'Restock');
    ...
    it('should track negative adjustments', async () => {
      const adjustment = await service.propagateAdjustment('SKU-015', -5, 'Damage');
