...
class MockDemandPlanningService {
  ...
  async recordDemand(skuId: string, quantity: number, date: Date = new Date()): Promise<SkuDemandHistory> {
  ...
  async getDemandHistory(skuId: string, days: number = 30): Promise<SkuDemandHistory[]> {
    const all = this.demandHistory.get(skuId) || [];
    const cutoff = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
    return all.filter(h => h.date >= cutoff);
  }

  async forecastDemand(skuId: string, method: string = 'moving_average'): Promise<DemandForecast> {
    ...
    if (history.length > 0) {
      baselineForecasted = history.reduce((sum, h) => sum + h.quantity, 0) / history.length;
    ...
    const forecast: DemandForecast = {
      ...
      trendComponent,
      seasonalComponent,
      totalForecasted: baselineForecasted * (1 + trendComponent + seasonalComponent),
  ...
  async calculateSafetyStock(
    ...
    const variance = history.length > 1
      ? history.reduce((sum, h) => sum + Math.pow(h.quantity - avgDemand, 2), 0) / (history.length - 1)
      : 0;
  ...
  async calculateReorderPoint(
    ...
    const safetyStock = await this.calculateSafetyStock(skuId, leadTime, serviceLevel);
  ...
  async getClassification(skuId: string): Promise<InventoryClassification | null> {
...
describe('Demand Planning', () => {
  ...
  it('should classify item as A-class', async () => {
    const classification = await service.classifyInventory('SKU010', 60000, 0.15);

    expect(classification.abcClass).toBe('A');
  });

  it('should classify item as B-class', async () => {
    const classification = await service.classifyInventory('SKU011', 20000, 0.3);
  ...
  it('should classify item as X-class (stable)', async () => {
    const classification = await service.classifyInventory('SKU013', 30000, 0.1);

    expect(classification.xyzClass).toBe('X');
  });

  it('should classify item as Y-class (moderate variability)', async () => {
    const classification = await service.classifyInventory('SKU014', 30000, 0.35);

    expect(classification.xyzClass).toBe('Y');
  });

  it('should classify item as Z-class (high variability)', async () => {
    const classification = await service.classifyInventory('SKU015', 30000, 0.7);
