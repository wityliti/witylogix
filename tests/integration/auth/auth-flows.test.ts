...
function validatePasswordStrength(password: string): { strength: 'weak' | 'fair' | 'strong'; score: number } {
  let score = 0;
...
function generateJWT(payload: Partial<JWTPayload>, secret: string = 'test-secret'): string {
...
function verifyJWT(token: string, secret: string = 'test-secret'): JWTPayload | null {
  try {
...
describe('JWT Token Service', () => {
  describe('Token Generation', () => {
    ...
    it('should set correct expiration time', () => {
      ...
      expect(decoded?.exp).toBeLessThanOrEqual(Math.floor(Date.now() / 1000) + 3600);
    });
  });
...
describe('Session Management', () => {
  ...
  describe('Session Validation', () => {
    ...
    it('should reject session with different device fingerprint', () => {
      ...
      const currentFingerprint = createDeviceFingerprint('Firefox', '192.168.1.1');
      const isDeviceMatch = session.deviceFingerprint === currentFingerprint;
