...
class TenantResolver {
  ...
  registerTenant(tenantId: string, context: TenantContext, subdomain?: string): void {
  ...
  resolveFromJWT(payload: { tenantId?: string; orgId?: string }): TenantContext | null {
...
class APIKeyManager {
  ...
  validateKey(keyValue: string): { valid: boolean; tenantId?: string; scopes?: string[] } {
    const keyHash = createHash('sha256').update(keyValue).digest('hex');
...
class UsageMeteringService {
  ...
  aggregateDaily(tenantId: string, date: string): DailyUsageSummary {
    ...
    const dayRecords = this.records.filter(
      r => r.tenantId === tenantId && r.timestamp >= dayStart && r.timestamp < dayEnd
    );
  ...
  checkQuota(tenantId: string, planLimits: Record<string, number>, usage: Record<string, number>): boolean {
  ...
  identifyTopEndpoints(tenantId: string, limit: number = 10): Array<{ endpoint: string; count: number }> {
...
describe('API Key Management', () => {
  ...
  describe('Scope Checking', () => {
    it('should check scopes on validation', () => {
      ...
      expect(manager.checkScopes(key.id, ['read:drivers', 'write:routes'])).toBe(true);
...
describe('Usage Metering', () => {
  ...
  describe('Usage Recording', () => {
    it('should record API usage', () => {
      ...
      const summary = metering.aggregateDaily('shop-123', new Date().toISOString().split('T')[0]);
  ...
  describe('Daily Aggregation', () => {
    ...
    it('should calculate top endpoints', () => {
      ...
      const summary = metering.aggregateDaily('shop-123', new Date(now).toISOString().split('T')[0]);
