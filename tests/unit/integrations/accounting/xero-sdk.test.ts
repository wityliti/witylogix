...
describe('XeroSDKClient', () => {
  ...
  describe('Bank Transaction Operations', () => {
    ...
    it('should validate bank transaction types', () => {
      ...
      types.forEach(type => {
        expect(['ACCRECPAYMENT', 'ACCPAYPAYMENT', 'ARCREDITPAYMENT', 'APCREDICPAYMENT']).toContain(type);
      });
    });
  ...
  describe('Quote Operations', () => {
    ...
    it('should support quote status lifecycle', () => {
      ...
      statuses.forEach(status => {
        expect(['DRAFT', 'SENT', 'ACCEPTED', 'DECLINED', 'EXPIRED']).toContain(status);
      });
    });
  ...
  describe('Webhook Verification', () => {
    ...
    it('should support intent-to-receive webhook flow', () => {
      const intentPayload = JSON.stringify({ eventTypes: ['INVOICES.CONTACTS'] });
