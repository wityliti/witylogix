...
describe('LightspeedPOSSDKClient', () => {
  ...
  describe('createProductVariant', () => {
    it('should create product variant', async () => {
      ...
      const result = await client.createProductVariant(123, 'Small Burger', 1199);
  ...
  describe('getProductPerformanceReport', () => {
    it('should fetch product performance metrics', async () => {
      (global.fetch as any).mockResolvedValueOnce({
        ...
        json: async () => ({
          items: [
            { name: 'Burger', quantitySold: 50, totalSales: 649.50 },
            { name: 'Fries', quantitySold: 75, totalSales: 225.00 },
          ],
        }),
        headers: new Map(),
      });

      const result = await client.getProductPerformanceReport(new Date(), new Date());
  ...
  describe('voidOrder', () => {
    it('should void sale', async () => {
      ...
      const result = await client.voidOrder('loc-123', 'sale-1', 'User request');
