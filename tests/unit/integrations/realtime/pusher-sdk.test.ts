...
import { PusherSDKClient, type PusherEvent } from '../../../../packages/core/src/integrations/realtime/pusher-sdk-client';
...
describe('PusherSDKClient', () => {
  ...
  describe('Configuration', () => {
    ...
    it('should throw error if appId is missing', () => {
      expect(() => new PusherSDKClient({ ...mockConfig, appId: '' })).toThrow('Pusher appId is required');
    });

    it('should throw error if key is missing', () => {
      expect(() => new PusherSDKClient({ ...mockConfig, key: '' })).toThrow('Pusher key is required');
    });
  ...
  describe('Event Triggering', () => {
    ...
    it('should reject if more than 100 channels', async () => {
      ...
      await expect(client.triggerMultiple(channels, 'event', {})).rejects.toThrow(
  ...
  describe('Batch Event Triggering', () => {
    ...
    it('should reject if more than 10 events in batch', async () => {
      ...
      await expect(client.triggerBatch(batch)).rejects.toThrow('Maximum 10 events per batch');
    });

    it('should reject if batch is empty', async () => {
      await expect(client.triggerBatch([])).rejects.toThrow('At least one event is required');
    });
  });
  ...
  describe('Channel Authentication', () => {
    ...
    it('should reject if channel name is missing', () => {
      ...
      expect(() => client.generateChannelAuth(socketId, '')).toThrow('Channel name is required');
    });
  ...
  describe('Channel Info Queries', () => {
    it('should get all channels', async () => {
      const mockResponse = {
        channels: {
          'channel-1': { name: 'channel-1', occupied: true, subscriptionCount: 5 },
          'channel-2': { name: 'channel-2', occupied: false, subscriptionCount: 0 },
        },
      };
    ...
    it('should filter channels by prefix', async () => {
      const mockResponse = {
        channels: {
          'public-1': { name: 'public-1', occupied: true, subscriptionCount: 3 },
        },
      };
  ...
  describe('Presence Management', () => {
    ...
    it('should return null for non-existent member', async () => {
      ...
      const member = await client.getPresenceMember('presence-chat', 'non-existent');
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', async () => {
      ...
      await expect(client.trigger('ch-1', 'event', {})).rejects.toThrow('Pusher API error');
    });
