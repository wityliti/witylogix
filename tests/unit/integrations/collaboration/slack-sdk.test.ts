...
describe("SlackSDKClient", () => {
  ...
  describe("OAuth2 Flow", () => {
    ...
    it("should throw error for invalid OAuth2 config", () => {
      ...
      expect(() =>
        client.getOAuth2AuthorizationUrl(invalidConfig as any),
      ).toThrow();
    });
  });
  ...
  describe("Event Signature Verification", () => {
    ...
    it("should reject signature with wrong timestamp", () => {
      ...
      const isValid = client.verifyEventSignature(
        signature,
        oldTimestamp,
        body,
      );
    ...
    it("should throw error when appSigningSecret is not set", () => {
      ...
      expect(() =>
        clientNoSecret.verifyEventSignature("v0=test", "123456", "{}"),
      ).toThrow("appSigningSecret required for signature verification");
    });
  });
