...
| WIT-566 | feat/WIT-566-settings-maps-raw-fetch-hardening | **settings/maps API persistence + raw fetch auth hardening.** `settings/maps/page.tsx`: full rewrite — removed `localStorage` persistence; now loads via `useApiQuery('/api/v4/shops/me')` and saves via `api.patch('/api/v4/shops/me', { settings: { mapsConfig } })`; added `Header` + `LoadingSkeleton` + `ErrorState`; keyless CARTO info banner; WLMap live preview (lng,lat order); accessible toggle switches; Save button always enabled (not gated on API key). `lib/api.ts`: added `requestBlob()` helper + `api.download()` for authenticated binary blob downloads. `invoices/[id]/page.tsx`: raw PDF `fetch()` (fragile cookie extraction) → `api.download()`. `integrations/connected/[providerId]/page.tsx`: unauthenticated `fetch()` for test-connection → `api.post()` (was silently 401ing in production). | no new endpoints | 4 files, 3 raw auth-bypassing fetch calls → 0, localStorage removed | 2026-06-23 |
| WIT-568 | feat/WIT-568-dashboard-quality-auth-fixes | **Residual quality pass — auth bypass, error states, dead code, hex CSS.** `settings/payments/page.tsx`: removed duplicate `handleTestPayment` using raw `fetch()` + duplicate `toggleSecretVisibility`; replaced hex gradient → `bg-wl-bg-root`. `collections/page.tsx`: `handleRemoveProduct` raw DELETE `fetch()` → `api.delete()` with auth. `admin/audit/page.tsx`: `error` never destructured from `useApiQuery`; added `ErrorState` import + early return. Deleted `components/map/live-tracking-layer.tsx` (orphan Leaflet component, CITY_COORDS dictionary, `Math.random()` jitter — never imported). Deleted `hooks/use-integration-logs.ts` (raw `fetch()` to non-existent `/api/integrations/logs`); removed from `hooks/index.ts`. Hex CSS → WL design tokens across 7 pages: `bg-[#0a0a0f]`/`bg-[#0d0d14]`→`bg-wl-bg-root`, `bg-[#12121a]`/`bg-[#111118]`→`bg-wl-bg-surface`, `bg-[#1a1a2e]`→`bg-wl-bg-elevated`, `border-[#1e1e2e]`→`border-wl-border-default`, gradient `from-[#0a0a0f] to-[#12121a]`→`bg-wl-bg-root`. Zero new TS errors introduced (pre-existing 101 unchanged). | no new endpoints | 14 files, 2 raw auth-bypass fetch calls → 0, 1 missing error state → 0, 2 orphan files deleted, ~20 hex CSS signals → 0 | 2026-07-04 |
| WIT-569 | feat/WIT-569-dashboard-final-hex-token-pass | **Zero-error TypeScript pass — duplicate code elimination + type corrections.** Removed 519 lines of dead duplicated code across 3 pages (drivers/page, collections/page, admin/test-dashboard/page each had 2–4 copies of the same function body pasted in). Fixed all 6 dashboard-specific TypeScript errors: `notification-center.tsx` missing `Volume2`/`VolumeX` imports + `type`/`severity`/`read` fields in `apiNotifs` useMemo; `home/page.tsx` undeclared `recentOrders`/`displayDrivers` + wrong DashboardStats field names (`totalOrdersToday`→`totalOrders`, `pendingDeliveries`→`pendingOrders`, `totalRevenueToday`→`revenue`) + removed non-existent KPICard `trend` props; `collections/page.tsx` duplicate `if(error)` guard after hooks causing `never` type; `live-kpi-counters.tsx` triple-duplicate `metrics` declarations + wrong field names (`completionRate`→`deliveredToday`) + wrong variable names (`sparkColor`→`sparklineColor`, `isUp`→`isPositiveTrend`); `live-order-feed.tsx` duplicate `useState<Order[]>` + switch to `useApiList` with `rawOrders`; `active-delivery-map.tsx` added `DriverStatus` type + `driverStatusColors`/`driverStatusLabels` constants; `analytics/page.tsx` `Globe2` import + `d[dataKey]`→`d.orders`; `route-performance/page.tsx` missing lucide imports; `locations/page.tsx` `view`→`viewMode`; `use-shipment-tracking.ts` missing `const result = await response.json()`; `analytics-widget.tsx` missing `label` prop on EmptyState; `drivers/[id]/page.tsx` missing `useApiQuery` for `profileData`; `settings/payments/page.tsx` `boolean` assigned to `string\|null` state. Built `@witylogix/validators` package to fix module-not-found in zones pages. Result: zero dashboard TS errors, build passes. | no new endpoints | 23 files, 519 lines removed, 0 dashboard TS errors | 2026-07-18 |
...
| Maps | `/settings/maps` | 0 | 0 | ✅ WIT-566 |
...
| WIT-577 | `feat/WIT-577-fix-build-breaking-typecheck-errors` | **Build rescue — zero TypeScript errors.** Automated sprints WIT-501→WIT-566 had appended new implementations to 62 dashboard files without removing old ones, leaving duplicate `const`/`function` declarations, broken JSX, and missing imports. Removed all duplicates, fixed all type mismatches, restored zero-error typecheck. Scope: orders, payments, analytics, campaigns/[id], drivers, locations, profile, activity/event-filters, activity, admin/shops/[id], admin/test-dashboard, analytics/route-performance, supply-chain/orders, invoices/[id], live-kpi-counters, plus 47 further pages/components. Infrastructure: `packages/db` `$allOperations` types, Prisma generated stub, `@witylogix/validators` tsconfig path. | no new endpoints — build fix only | 342 TS errors → 0 | #399 |
