...
| WIT-578 | feat/WIT-578-dashboard-typecheck-fix | **Build fix — 12 files with duplicate variable declarations → 0.** Removed 395 lines of garbled/duplicated code introduced by previous sprint automation. `next build` was failing on every affected route with webpack `Module parse failed: Identifier already declared` errors. Files: `admin/test-dashboard/page.tsx` (3× duplicate stats blocks), `collections/page.tsx` (duplicate handleRemoveProduct + hooks-after-early-returns), `drivers/page.tsx` (dead MapLegend/DriverMapView + duplicate export default), `eld/page.tsx` (duplicate driversLoading from undefined driversResult), `locations/page.tsx` (missing `)}` in JSX conditional), `supply-chain/orders/page.tsx` (duplicate batch progress bar + corrupted returns tab), `notification-stats-widget.tsx` (CHANNEL_COLORS ref, s.dashArray, missing deliveryRate), `active-delivery-map.tsx` (undefined refetchDrivers/refetchDeliveries), `live-kpi-counters.tsx` (2× duplicate metrics), `live-order-feed.tsx` (3× duplicate orders), `notification-center.tsx` (duplicate readIds), `stop-list-editor.tsx` (dangling code outside function). Build ✓ lint ✓ 147/147 tests ✓. | no new endpoints | 12 files, 395 lines removed, build restored | 2026-07-16 |
