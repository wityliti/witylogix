...
| WIT-571 | feat/WIT-571-accentcolor-design-tokens | **accentColor design tokens + audit ErrorState.** 24 hardcoded hex `accentColor` props → WL CSS variables across 8 pages (integrations/crm, supply-chain, fuel, lastmile, analytics, delivery, admin/workflows/[id], analytics/page); `#3b82f6` props omitted where it matched the default; MiniBarChart `color="#6366f1"` → `var(--wl-primary-500)`; progress bar gradient `#818cf8/#6366f1` → `var(--wl-primary-400/500)`; integrations/analytics inline error div → `<ErrorState>`; admin/audit: missing `error` guard → `ErrorState` shown when API fails. PR #390 | no new endpoints | 9 files, 24 hex inline → 0, 1 silent error → ErrorState | 2026-06-25 |
...
| WIT-570 | feat/WIT-570-auth-bypass-hardening | **Auth bypass hardening — 11 raw fetch() calls eliminated.** `api.ts`: added `requestBlob()` + `api.download()` for authenticated binary blob downloads (PDF etc.) with auth header + 60s timeout. `use-crm-connection.ts`: replaced 8 raw `fetch()` calls using wrong paths (`/api/integrations/crm/...`) and no auth headers with `api.post/patch/delete/get` to correct `/api/v4/integrations/crm/...`; added `useOAuthFlow` + `useCrmMetrics` hooks; `crypto.getRandomValues()` for CSRF state. `invoices/[id]/page.tsx`: replaced fragile cookie-extraction raw fetch for PDF download with `api.download()`. `integrations/connected/[providerId]/page.tsx`: replaced unauthenticated raw `fetch()` test-connection with `api.post()`. `settings/maps/page.tsx`: full rewrite — loads `mapsConfig` via `useApiQuery('/api/v4/shops/me')`, persists via `api.patch('/api/v4/shops/me')`; Header/LoadingSkeleton/ErrorState; keyless CARTO info banner; WLMap live preview; accessible toggles; save no longer gated on API key. `oauth-callback-handler.tsx`: fixed wrong path + missing auth → `api.post('/api/v4/integrations/crm/oauth/callback')`. PR #389 | no new endpoints | 6 files, 11 raw fetch auth bypasses → 0, Settings Maps ✅ API persistence | 2026-06-25 |
...
| Maps | `/settings/maps` | 0 | 0 | ✅ WIT-570 |
