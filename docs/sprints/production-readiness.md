...
| WIT-355 | Tracking + Locations + System + ELD/DVIR (mock sweep) | tracking/live, locations/, admin/system, admin/shops/[id], eld/dvir | ✅ Yes (tracking/live: LiveTrackingLayer; locations: LocationPinLayer) | /api/v4/eld/dvir (new, GET+POST — derives from Vehicle.lastMaintenanceAt+status); /api/v4/admin/system (new, real process.memoryUsage + prisma.$queryRaw + redis.ping latency); /api/v4/admin/stores/:id/billing-history (new, from subscription data); api.ts + useApiMutation extended with PUT support | MOCK_INSPECTION_HISTORY (43 lines) → 0; mockServices (6 hardcoded) + mockMetrics → 0; mockShopDetail (36 refs) + mockBillingHistory + mockActivityLog → 0 | LiveTrackingLayer: 60-city coord lookup, jitter, status-coloured circles, popup; LocationPinLayer: type-coloured emoji markers, auto-fit; tracking/live: full-page WLMap + LiveTrackingLayer above orders list; locations: grid/map toggle + LocationPinLayer in both list-map and detail-panel; PR #361 |
