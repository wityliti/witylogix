...
| WIT-613 | fix/WIT-613-broken-nav-links-quality-fixes | **Broken navigation links across dashboard pages — Next.js route group URL prefix bug.** All links inside the `(dashboard)` route group were prefixed with `/dashboard/` (a path segment that doesn't exist — Next.js route groups don't add URL segments), causing 404 on every affected nav link and tab. `field-service/layout.tsx`: tab hrefs `/dashboard/field-service*` → `/field-service*` (active tab never highlighted; Jobs/Dispatch tabs 404). `pos/layout.tsx`: tab hrefs `/dashboard/pos*` → `/pos*`. `crm/page.tsx`: "Add Integration" + Reconfigure `/dashboard/crm/connect*` → `/crm/connect*`. `partners/page.tsx`: viewDetails/configure/onboard/compare all `/dashboard/partners/*` → `/partners/*`. `partners/[id]/page.tsx`: "Install Partner" + "Reconfigure Credentials" `/dashboard/partners/onboard` → `/partners/onboard`. `partners/compare/page.tsx`: "Add a Courier" → `/partners/onboard`. `partners/onboard/page.tsx`: post-submit redirect → `/partners`. `invoices/create/page.tsx`: both post-save/send redirects → `/invoices`. Quality fixes: `inventory/page.tsx` warehouse error `onRetry={() => {}}` (no-op) → real `refetchWarehouses`; `collections/page.tsx` product-removal `.catch(console.error)` → proper catch with `removeError` state rendered inline + loading state on Remove button. Build ✓ typecheck ✓ lint ✓. | no new endpoints | 10 files, 11 broken nav links → 0, 2 quality gaps → 0 | 2026-07-11 |
| WIT-614 | feat/WIT-614-integration-components-quality-pass | **Integration components quality pass — remove fake delays + loading skeleton guards.** 8 integration library components had `new Promise(resolve => setTimeout(resolve, N))` artificial delays: `chaos-scenario-card` (800ms), `circuit-breaker-visualizer` (500ms), `connection-wizard` (1500ms), `credential-form` (1500ms), `credential-rotation-timeline` (600ms), `oauth-flow` (1000ms+1500ms), `sync-status-card` (1000ms), `webhook-config` (1500ms). All delays removed — parent callbacks called directly. `webhook-config.tsx` additionally upgraded to call real `POST /api/v4/outbound-webhooks/test` (actual HTTP request server-side, returns real status code + latency). `credential-form.tsx` test handler replaced misleading "Connection successful!" (always true) with synchronous required-field check by `f.name`, giving honest feedback. `chaos-scenario-card.tsx` hardcoded `#8b0000` hex → `var(--wl-danger-700)` design token. Loading skeleton guards: `integrations/crm/page.tsx` — `TableSkeleton` while providers load + `ErrorState` on failure (was rendering with 0 providers silently). `ai/route-efficiency/page.tsx` — `LoadingSkeleton` guard during routes fetch (was rendering blank list). Build ✓ typecheck (pre-existing validator errors only) ✓ lint ✓ tests ✓. PR #453. | no new endpoints | 10 files, 8 fake setTimeout delays → 0, 1 fake test result → real API, 1 hardcoded hex → design token, 2 missing loading guards → 0 | 2026-07-13 |
| WIT-618 | fix/WIT-618-dashboard-404-broken-home-link | **404 page broken home link.** `(dashboard)/not-found.tsx`: "Back to Dashboard" button used `window.location.href='/dashboard'` — `/dashboard` is not a URL (the `(dashboard)` route group adds no path segment); button always 404'd. Fixed: `import { useRouter }` + `router.push('/home')`. | no new endpoints | 1 file, 1 broken link → fixed | 2026-07-14 |
| WIT-617 | feat/WIT-617-quality-window-location-console-errors | **Quality — all remaining `window.location.reload()`→refetch + `window.location.href`→router.push + search palette SPA nav.** `orders/conflicts`: reload()×2 → `revalidate` from `useConflicts()`. `supply-chain/orders`: reload()×3 → `orders.refetch`/`refetchWaves`/`refetchBatches`. `esignatures`: reload() → `retryAll()` combining 3 refetch calls. `notifications/delivery-log`: reload() → `refetch`. `widget-config`: Discard reload() → `router.refresh()`. `orders/local`: href→`router.push(/orders/:id)`. `routes/plan`: post-save href→`router.push()`. `integrations/connected/[providerId]`: post-disconnect href→`router.push`. `integrations/fuel`: 3× catalog CTAs href→`router.push`. `integrations/collaboration`: 2× catalog CTAs href→`router.push`. Bonus: `search-command-palette` (global Cmd+K) — 4× `window.location.href` in quick actions + handleSelectResult→`router.push()`; quick actions now close palette before navigating. Build ✓ typecheck ✓ lint ✓. | no new endpoints | 11 files, 5 reload()→refetch, 12 href=→router.push | 2026-07-14 | PR #457 ✅ merged |
| WIT-615 | feat/WIT-615-integration-quality-error-states | **Integration components second quality pass — 5 remaining fake delays removed, analytics-widget refresh fix.** `connect-dialog.tsx`: removed `else` branch with 2000ms fake delay — `onConnect?.()` always awaited directly; error caught properly. `provider-card.tsx`: removed 1000ms fake delay + dead `isLoading` state from `handleTest` — calls `onTest?.()` synchronously. `import-export-wizard.tsx`: removed 800ms fake delay + dead `isProcessing` state from `handleNext`; refactored `handleNext`/`handleBack` to use a single `STEP_ORDER` array (eliminates duplicated step-transition chains). `migration-progress-bar.tsx`: removed 600ms fake delay + dead `isRollingBack` state from `handleRollback` — calls `onRollback?.()` synchronously. `integration-card.tsx`: removed 3× 300ms `setTimeout(() => setIsActionLoading(false), 300)` calls + dead `isActionLoading` state — `getActionButton()` now returns plain synchronous handlers. `analytics-widget.tsx`: `handleRefresh` replaced `setTimeout(() => setIsRefreshing(false), 1000)` with try/finally around `await onRefresh?.()`. Build ✓ typecheck ✓ lint ✓. | no new endpoints | 6 files, 5 fake setTimeout delays → 0, 1 fake analytics refresh → real await | 2026-07-14 |
| WIT-580 | feat/WIT-580-dashboard-ux-hardening | **UX hardening — loading/error states, navigation fixes, dead import cleanup.** `routes/create/page.tsx`: 3 `useApiList` calls (drivers/vehicles/orders) now capture `loading`/`error`; selects disabled while loading; orders panel shows spinner + error card; `bg-blue-500` → `bg-wl-primary-500` on step indicator and stats cards. `drivers/[id]/page.tsx`: `historyData`/`profileData` queries capture `loading`/`error`; history chart shows spinner while loading, error message on failure; Live Map tab distinguishes `profileLoading` skeleton from null `mapProps`. `freight/layout.tsx`: `window.location.pathname` → `usePathname()` (SSR hydration fix); `<a href>` → Next.js `<Link>` (eliminates full-page reloads on tab switch). `integrations/analytics/page.tsx`: hardcoded `reportsLoading=false`/`dataSourcesLoading=false` stubs → real `useApiList('/api/v4/integrations/reports')` + `useApiList('/api/v4/integrations/data-sources')`; skeleton loading now activates correctly. `ai/slots/page.tsx`: `zonesError` captured from `useApiList`; zone select disabled + error message rendered on failure. Dead import cleanup: removed 4 unused `useApiList` imports from `orders/conflicts`, `notifications/delivery-log`, `collaboration`, `admin/queues`. Build ✓ typecheck ✓ lint ✓. | no new endpoints | 9 files, 9 UX issues → 0 | 2026-07-15 | PR #459 |
...
| WIT-608 | `feat/WIT-608-dashboard-quality-pass` | Cross-page quality hardening: (1) `integrations/providers`: Save Configuration button wired — added controlled state for rateLimit/timeout/retryPolicy/circuitBreakerThreshold; `updateConfiguration()` now called on save with loading+error feedback; resets on provider switch. (2) `integrations/catalog`: bespoke `animate-pulse` divs → shared `Skeleton`/`CardSkeleton` in sidebar and card grid. (3) `stores`: bespoke `animate-pulse` Cards → `CardSkeleton`. (4) `partners/onboard`: raw `<select>` in Step 3 → design-system `Select` component with `options` prop. (5) `healthcare`: secondary hook errors (`shopError`, `platformError`) surfaced as inline amber banners. | — | 0 new mock signals; 5 dead-wire/skeleton/design-token issues fixed | open |
| WIT-609 | `feat/WIT-609-orders-import-sync-history` | (1) `orders/import`: replace hardcoded `recentSyncJobs: []`/`failedJobs: []` with real data from `useSyncStatus()` hook (polls `/api/v4/orders/sync/status` every 30s); add platform filter + loading skeleton. (2) API `orders-sync.ts`: populate `recentSyncJobs` from `IntegrationEvent` records (eventType=SYNC) — previously always returned `[]`. (3) `integrations/analytics`: fix 3 API paths returning 404 on every load (caused full-page ErrorState); connections now hits `/api/v4/integrations/connections?category=analytics`; align ConnectionStatus enum to lowercase API values. (4) `calendar`: wire "New Rule" button to full create modal (POST `/api/v4/calendar-rules`); wire Trash2 delete button to DELETE with toast+refetch; fix CalendarRule.type enum to match backend. (5) `integrations/connected`: fix empty-state "Browse Marketplace" CTA from `() => {}` to `router.push('/integrations')`. | — | 0 new mock signals; 5 dead buttons/404 paths fixed | #448 |
| WIT-610 | `feat/WIT-610-demand-maps-quality-polish` | (1) `demand/anomalies/page.tsx`: List↔Map toggle; `anomalyDemandData` computes zone severity scores (high=3pts/medium=2pts/low=1pt per zone) via `name→id` mapping from GeoJSON features; `WLMap + DemandZoneLayer` with severity legend (No anomalies=blue/Low=green/Medium=amber/High=orange/Critical=red) + top-8 zone overlay. (2) `demand/capacity/page.tsx`: List↔Map toggle; `capacityDemandData` maps zone status → predictedVolume (understaffed=100/optimal=50/overstaffed=20) via `name→id` mapping; `WLMap + DemandZoneLayer` with capacity legend (Understaffed=red/Optimal=green/Overstaffed=amber) + zone status overlay with live Badge. (3) `home/page.tsx`: inline `bg-red-950/30` error div → shared `ErrorState` component. Both map views keyless CARTO, SSR-disabled, graceful empty states. Build ✓ typecheck ✓ lint ✓. | no new endpoints | 3 files, 2 new map views, 1 inline error div → ErrorState, 0 mock signals | 2026-07-10 |
| WIT-611 | `feat/WIT-611-db-fix-crm-api-quality` | `packages/db/src/index.ts`: fix TS2769 overload-mismatch in `forTenant`, `forOrg`, `forTenantInOrg` — `query(args)` in `$transaction` array-form returns `Promise<unknown>` which is not assignable to `PrismaPromise<any>`; cast `query(args) as never` at all three call-sites. This was the root cause blocking CI (Build + Typecheck) on all open PRs derived from staging. Zero dashboard changes; zero API changes; zero schema changes. | no new endpoints | 1 file, 0 mock signals; unblocks CI for all open PRs | open |
| WIT-612 | `feat/WIT-612-eta-accuracy-map-quality-pass` | `analytics/eta-accuracy/page.tsx`: Charts↔Map toggle (LayoutGrid/Map icons); when Map active, fetches (1) `GET /api/v4/ai/eta-v2/zone-accuracy?days=N` (new endpoint — Prisma `order.findMany` with `timeSlot.deliveryZone` join, groups by zone, computes `onTimePercent`+`avgDelayMinutes`+`sampleCount`); (2) `GET /api/v4/zones?format=geojson` for polygon boundaries. NEW `EtaAccuracyZoneLayer`: MapLibre fill+stroke layers coloured by on-time % (≥90%=emerald/≥75%=green/≥60%=amber/≥45%=orange/<45%=red/no-data=grey), opacity from `hasData` flag, click selection. NEW `EtaAccuracyMapView`: WLMap + EtaAccuracyZoneLayer + AutoFit (Polygon/MultiPolygon coord extraction) + loading overlay + selected-zone tooltip (name/on-time%/avg-delay/samples) + 6-entry colour legend. Both `useApiQuery` calls use `path=null` gate (only fire when view==='map'). Build ✓ typecheck (pre-existing validator errors only) ✓ lint ✓. | 1 new API endpoint (`GET /api/v4/ai/eta-v2/zone-accuracy`) | 4 files (2 new, 2 modified), 1 map view, 0 mock signals | 2026-07-11 |
