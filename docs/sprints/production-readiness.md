...
| WIT-357 | Final Mock Cleanup (realtime + system + components) | admin/system, admin/workflows/[id], invoices/[id], invoices/create, eld/dvir, + 5 realtime + 5 component files | None (no geographic data) | /api/v4/health/deep (system health), /api/v4/workflow/executions/:id (workflow detail), /api/v4/invoices/:id (invoice detail), /api/v4/customers (invoice customer picker), /api/v4/eld/dvir (new), /api/v4/eld/dvir/history (new), /api/v4/notifications (notification-center), /api/v4/analytics/overview?range=today (live-kpi-counters), /api/v4/orders (live-order-feed), /api/v4/drivers/locations (active-delivery-map), /api/v4/notifications/delivery-log (notification-stats-widget) | mockNotifications(5) + simulation interval + mockMetrics(CPU/memory/activeConns/deploymentInfo) + mockServices(6 hardcoded service health) + MOCK_EXECUTION(6 steps) + MOCK_INVOICE + MOCK_CUSTOMERS(5) + MOCK_INSPECTION_HISTORY + mockOrders useEffect + mockDrivers/mockDeliveries useEffects + MOCK_DAILY_STATS/CHANNEL/TEMPLATES + mockInventory prop object + mockTracker prop object + mockEvents(5) + mock_oauth_token_ generation → 0 | ✅ grep scan returns empty on all src/**/*.tsx (tests excluded); realtime simulation intervals removed; pure display components get safe zero defaults not hardcoded values; admin/system wired to /health/deep with normalizeHealth(); DVIR hooks-order violation fixed |
...
- `apps/dashboard/src/components/map/live-tracking-layer.tsx` — status-coloured order circles by city (60-city lookup + jitter), popup with customer/driver/ETA, WIT-355
- `apps/dashboard/src/components/map/location-pin-layer.tsx` — type-coloured emoji markers for warehouses/stores/hubs/depots/pickup-points, popup with status/city, auto-fit, WIT-355
