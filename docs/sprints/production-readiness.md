...
| WIT-567 | feat/WIT-567-profile-fix-raw-fetch-auth | **Auth bypass fix — profile, CRM hook, integrations.** API: `PATCH /api/v4/users/me` (self-service profile name/email update with email-uniqueness check); `DELETE /api/v4/integrations/crm/:id` (disconnect); `POST /api/v4/integrations/crm/:id/sync` (trigger manual sync). Dashboard profile/page: fixed React hooks-after-returns violation (useState/useEffect called after conditional returns); wired Save button to `PATCH /me`; wired Change Password button to `PATCH /:id/password`; fixed `UserMe` interface (`name` not `firstName`/`lastName`). `use-crm-connection.ts`: replaced all 9 raw `fetch()` calls (no auth header) with authenticated `api.delete/post/patch/get` from `@/lib/api`; `useCrmMetrics` now derives metrics from real `/api/v4/integrations/crm/providers` (was calling non-existent `/api/integrations/crm/metrics`). `integrations/connected/[providerId]`: raw `fetch()` for test-connection → `api.post()`. `crm/connect/page.tsx`: hooks-after-returns fixed; removed unused `useApiList` fetching non-existent route. | PR #381 | 6 files, 9 unauthenticated fetches → 0, 1 hooks violation → 0 | 2026-06-23 |
