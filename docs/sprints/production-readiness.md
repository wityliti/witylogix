...
| WIT-570 | feat/WIT-570-dashboard-quality-hardening-error-states | **Quality hardening — error states, loading skeletons, Prisma schema fix, SSR hydration fix.** Prisma schema: `ReturnRequest` back-relations missing from `Shop` and `Order` models (added in WIT-537 without reciprocal fields) — added `returnRequests ReturnRequest[]` to both, unblocking `db:generate`. `loading.tsx`: `Math.random()` skeleton bar widths → deterministic `[85,70,92,65,78]` array (fixes SSR/client hydration mismatch). `notifications/delivery-log/page.tsx`: `error`/`refetch` not destructured from `useDeliveryLog`; added `ErrorState` early return + `TableSkeleton` guard on initial load; removed unused imports (`useApiList`, `CardHeader`, `CardTitle`, `CardDescription`). `integrations/marketplace/page.tsx`: `connectionsLoading` exposed from `useApiList`; header subtitle shows "loading connection status…" during fetch. `ai/page.tsx`: `useFeatureUsageCounts()` now exposes `loading` boolean; `AiFeatureCard` renders animated pulse placeholder instead of `0` while counts load. | no new endpoints | 6 files, 0 new mock signals, 1 Prisma schema fix, 1 SSR hydration fix, 3 missing error/loading states → 0 | 2026-07-14 |
