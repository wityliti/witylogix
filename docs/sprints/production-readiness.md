...
| WIT-579 | fix/WIT-579-ci-unblock | **CI unblock — duplicate declarations and broken JSX across 14 dashboard files.** SWC hard parse errors: `admin/test-dashboard` (4× duplicate `const { stats }` + `const stats` redeclaration → rewrite keeping single correct block); `eld/page` (duplicate `const driversLoading` + missing `</div>` before `</button>` + hooks-after-return); `locations/page` (unterminated regexp from `onReady={setDetailMapId}` on undefined state + unclosed `{detailMapId && (` JSX expr); `supply-chain/orders` (duplicate progress bar + extra `</div>` + corrupt Returns Queue block with wrong-scope `batch.completionRate` + two JSX siblings without Fragment); `drivers/page` (second `export default function DriversPage` inside unclosed stub + nested `MapLegend`/`DriverMapView` using unimported symbols); `stop-list-editor` (orphaned `}, []);` closing a `useCallback` never opened); `active-delivery-map` (same orphaned `useCallback` closing + `useEffect` calling undefined `refetchDrivers`/`refetchDeliveries`). Prisma back-relations: `Shop.returnRequests ReturnRequest[]` + `Order.returnRequests ReturnRequest[]` (P1012 schema errors). Also fixed: `live-kpi-counters`, `live-order-feed`, `notification-center`, `use-shipment-tracking`, `collections/page` (TypeScript/lint errors). `pnpm --filter @witylogix/dashboard build` passes clean. PR #461. | 0 new mock signals (infra fix only) | 14 files, build ✓ | 2026-07-15 |
