...
| WIT-357 | Map Command Center + Admin System/Shop/Workflow | map/, admin/system, admin/shops/[id], admin/workflows/[id] | OrderMarkerLayer (new shared layer, status-coloured circle markers) on orders tab; DriverLocationLayer on drivers tab (live GPS from /api/v4/drivers/locations); WLMap with info overlay on routes tab | POST /api/v4/admin/stores/:id/suspend (was PUT), GET /api/v4/admin/stores/:id/billing (Invoice model), GET /api/v4/admin/stores/:id/activity (ActivityLog model), GET /api/v4/admin/system (DB/Redis/queue health + process.memoryUsage + version) | map: fake SVG MapCanvas + MAP_BOUNDS + pseudoLatLng() → 0; admin/system: mockServices(6) + mockMetrics → 0; admin/shops/[id]: mockShopDetail + mockBillingHistory(6) + mockActivityLog(8) → 0; admin/workflows/[id]: MOCK_EXECUTION → 0 | New shared layer: order-marker-layer.tsx (pending=amber, assigned=purple, in_transit=blue, delivered=emerald, failed=red). Map page: tab counts show only real-GPS items. Admin system: SVG ring memory gauge + uptime gauge from live metrics. |
