...
| WIT-617 | feat/WIT-617-quality-window-location-console-errors | **Quality — all remaining `window.location.reload()`→refetch + `window.location.href`→router.push + search palette SPA nav.** `orders/conflicts`: reload()×2 → `revalidate` from `useConflicts()`. `supply-chain/orders`: reload()×3 → `orders.refetch`/`refetchWaves`/`refetchBatches`. `esignatures`: reload() → `retryAll()` combining 3 refetch calls. `notifications/delivery-log`: reload() → `refetch`. `widget-config`: Discard reload() → `router.refresh()`. `orders/local`: href→`router.push(/orders/:id)`. `routes/plan`: post-save href→`router.push()`. `integrations/connected/[providerId]`: post-disconnect href→`router.push`. `integrations/fuel`: 3× catalog CTAs href→`router.push`. `integrations/collaboration`: 2× catalog CTAs href→`router.push`. Bonus: `search-command-palette` (global Cmd+K) — 4× `window.location.href` in quick actions + handleSelectResult→`router.push()`; quick actions now close palette before navigating. Build ✓ typecheck ✓ lint ✓. | no new endpoints | 11 files, 5 reload()→refetch, 12 href=→router.push | 2026-07-14 |
