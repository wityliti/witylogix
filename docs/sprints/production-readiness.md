...
| WIT-604 | feat/WIT-604-dashboard-design-token-sweep | **Dashboard design token bulk sweep + pre-existing build error fixes.** Replaced 2,695 raw Tailwind gray/zinc/slate color classes with WL design tokens across 209 dashboard files (`text-gray-*`/`bg-zinc-*`/`border-slate-*`/`text-white`/`text-black` → semantic `text-wl-text-*`/`bg-wl-bg-*`/`border-wl-border-*`/`wl-neutral-*` tokens). Fixed 12 pre-existing JSX/TS build errors that blocked CI (duplicate export default, duplicate const declarations ×3, missing JSX close tags, malformed useCallback remnants, orphaned JSX blocks, corrupt duplicate sections). Fixed Prisma schema: `ReturnRequest.order`/`ReturnRequest.shop` relations missing back-references in `Order` and `Shop` models. Build ✓, lint ✓, dashboard-specific typecheck ✓. PR #442. | no new endpoints | 209 files, 2695 raw color classes → 0, 12 build errors → 0, 1 Prisma schema fix | 2026-07-08 |
