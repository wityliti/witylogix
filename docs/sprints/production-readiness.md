...
| WIT-589 | feat/WIT-589-healthcare-map-real-data | **Healthcare dashboard real data + patient map view.** `healthcare/page.tsx`: removed hardcoded `compliance` object (`hipaaCompliant:true`/`encryptionEnabled:true`/`auditLoggingEnabled:true`/`accessControlsConfigured:true`/`outstandingIssues:0`) — `ComplianceStatusCard` now derives `HIPAA Mode`/`Access Controls` status from real `GET /api/v4/shops/me` (`shop.status`); Quick Stats "System Status: Operational" hardcoded string → real `GET /api/v4/admin/system` (`services.every(s=>s.status==='healthy')`); "Data Backup: Completed / Last run: Today 11:00 PM" hardcoded card → real Database status card showing PostgreSQL health + latency from system endpoint. `healthcare/patients/page.tsx`: added List↔Map toggle; dynamic SSR-disabled `PatientsMapView` (reuses `CustomersMapView`) fed by `useCustomerLocations()` (`/api/v4/customers/locations`) — shows all geo-located patients on WLMap+CustomerDensityLayer with tier colour legend; loading spinner while locations fetch; empty state when no coordinates; patient count in map mode subtitle. | no new endpoints (uses existing `/api/v4/admin/system` + `/api/v4/shops/me` + `/api/v4/customers/locations`) | 2 files, 5 hardcoded values → 0, 1 map view | 2026-07-01 |
