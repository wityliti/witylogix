...
| WIT-344 | Returns + Notifications | returns/, returns/[id], notifications/, notifications/log, notifications/delivery-log, notifications/preferences | None (no geographic data) | /api/v4/returns (fully rewritten: list/stats/get/approve/reject/receive/inspect/refund), /api/v4/notifications (inbox: list/mark-read/mark-unread/delete/bulk-delete/mark-all-read), /api/v4/notifications/delivery-log (wired to NotificationLog model), /api/v4/notification-preferences (GET/PATCH, stored in shop.settings) | MOCK_RETURNS (4 entries) + NOTIFICATION_LOGS (5+ hardcoded entries) → 0 | Added ReturnRequest + TenantNotification Prisma models + migration; returns/[id] detail page with timeline + items + approve/reject actions |
...
| ✅ Returns | returns/, returns/[id] | No | WIT-344 — ReturnRequest model + full lifecycle API + detail page, MOCK_RETURNS removed |
| ✅ Notifications | notifications/, notifications/log, notifications/delivery-log, notifications/preferences | No | WIT-344 — TenantNotification model, inbox API, delivery-log wired to NotificationLog, preferences stored in shop settings |
