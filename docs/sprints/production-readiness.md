...
| WIT-355 | Admin system/shops/workflows + ELD DVIR + Invoices | admin/system, admin/shops/[id], admin/workflows/[id], eld/dvir, invoices/[id], invoices/create, activity | None (no geographic data) | /api/v4/admin/system (new — real process metrics + DB/Redis health); /api/v4/eld/dvir (new — FleetVehicle list); /api/v4/eld/dvir/history (new — derived from FleetVehicle.updatedAt); /api/v4/eld/defects (new — vehicleMaintenanceRecord) | admin/system: mockServices(6)+mockMetrics → 0; admin/shops/[id]: mockShopDetail+mockBillingHistory(6)+mockActivityLog(8) → 0; admin/workflows/[id]: MOCK_EXECUTION → 0; activity: dead generateMockEvents() → 0; admin/customers: dead mockCustomers(8) → 0; eld/dvir: MOCK_INSPECTION_HISTORY(2) → 0; invoices/[id]: MOCK_INVOICE → 0; invoices/create: MOCK_CUSTOMERS(5) → 0 | PR #362; typecheck ✅ build ✅; normalizeShop()/normalizeExecution()/normalizeApiInvoice() normalization helpers; loading skeletons + error states on all pages; dynamic degraded-service alert cards |
