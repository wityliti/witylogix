...
| WIT-578 | fix/WIT-578-staging-ts-build-fix | **Restore dashboard TypeScript build — 0 errors across 18 pages.** Fixed all TypeScript build errors accumulated in `apps/dashboard` on the `staging` branch. 18 pages fixed: `activity/page` (duplicate useRef, undefined setEvents/filteredEvents — added filteredEvents useMemo; removed dead useEffect); `admin/shops/[id]` (duplicate Truck/Trash2 imports; added useParams+LoadingSkeleton+ErrorState); `admin/workflows/[id]` (added formatDuration helper, guarded durationMs??0); `analytics/page` (duplicate ArrowUpRight import); `campaigns/[id]` (sentCount/deliveredCount/etc→campaign.stats.*; createdAt→created_at); `collections` (moved useState/useMemo hooks before early-returns; removed duplicate handleRemoveProduct); `drivers/[id]` (optional chaining on profile.orders; DriverScoreResponse field access); `event-filters` (extraneous closing tag causing TS1381); `field-service/dispatch` (duplicate allTechs/filteredTechs after early-returns); `healthcare/records` (apiRecords→records); `integrations/connected/[providerId]` (added useApiQuery import); `notifications/log` (Input import; ChangeEvent types); `orders` (date spread fix; function name prefixes; endIdx); `profile` (state declarations; User field names); `supply-chain/orders` (loading/error destructuring; RawReturn→ReturnItem); `tracking` (view/setView state). `pnpm --filter @witylogix/dashboard typecheck` → 0 errors. `pnpm --filter @witylogix/dashboard build` → ✅. | no new endpoints | 18 files, 0 typecheck errors, build green | PR #429 | 2026-07-05 |
