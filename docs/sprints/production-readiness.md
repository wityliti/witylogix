...
| WIT-355 | Analytics Route Performance + Deliveries | analytics/route-performance | RouteEfficiencyLayer (city on-time % bubbles, log-scaled radius, charts/map toggle) | /api/v4/analytics/route-performance (summary, planned-vs-actual, drivers, efficiency, co2, sla-compliance — all 6 endpoints rewritten with real Prisma $queryRaw); /api/v4/analytics/route-performance/geo (new, 80+ city coord lookup, city-level on-time %); /api/v4/deliveries (real Shipment model — list/get/assign/status/preferences); /api/v4/notifications/stats (new, real NotificationLog + daily trend + channel breakdown + failed templates) | Math.random() in 5 generate*() functions (route-performance.ts) + MOCK_DELIVERIES (3 items, deliveries.ts) + MOCK_DAILY_STATS/MOCK_CHANNEL_BREAKDOWN/MOCK_FAILED_TEMPLATES (notification-stats-widget.tsx) → 0 | route-performance: real Route/RouteStop/Driver Prisma; CO2 factors per vehicleType (BICYCLE/MOTORCYCLE/CAR/VAN/TRUCK); SLA tier from Order.totalPrice (≥200=premium, ≥50=standard, else economy); deliveries: normalizeShipment() adapts Shipment→dispatch UI shape, status bi-map; preferences endpoint: ETA cutoff + zone radius validation; notification-stats-widget wired to /api/v4/notifications/stats |
