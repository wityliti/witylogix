...
| WIT-344 | Returns | returns/, returns/[id] (new) | None (non-geographic) | /api/v4/returns (list, already registered), /api/v4/returns/stats (pipeline counts + total refunded), /api/v4/returns/:id (detail), /api/v4/returns/:id/approve, /api/v4/returns/:id/reject, /api/v4/returns/:id/refund | MOCK_RETURNS (4 hardcoded entries) → 0 | returns/: loading skeletons, EmptyState, ErrorState, status filter, pipeline bar from real stats; returns/[id]: StatusTimeline, items list, gated action buttons (Approve/Reject/Process Refund); use-returns.ts: fixed missing /api/v4/ prefix on all mutation paths, updated ReturnStats type to match API |
...
| ✅ Returns | returns/, returns/[id] | No | WIT-344 — MOCK_RETURNS removed; detail page created; pipeline bar from /stats; gated actions |
