...
| WIT-592 | feat/WIT-592-design-token-cleanup | **Dashboard inline hex → CSS design token cleanup.** Replaced all remaining inline hex color strings (`color="#..."`, `style={{color:"#..."}}`  etc.) with WL CSS design token equivalents across dashboard components. PR #419 merged ✅. | no new endpoints | dashboard files, inline hex → WL CSS vars → 0 | 2026-07-02 |
| WIT-593 | feat/WIT-593-dashboard-returns-api-production | **Returns API production-fix — ReturnRequest Prisma model + full route rewrite.** Root cause: `ReturnRequest` model never existed in Prisma schema; every `/api/v4/returns/*` route used `(prisma as any).returnRequest?.findMany?.()` optional-chaining stubs that silently returned `undefined`/empty. Fix: NEW `packages/db/prisma/schema/71-returns.prisma` (`ReturnRequest` model with all fields, relations to `Shop`/`Order`/`Customer`, 3 indexes); back-refs added to `Shop`/`Order`/`Customer` models; NEW `packages/db/prisma/migrations/20260703_return_requests/migration.sql` (DDL + FK constraints + 3 indexes + RLS policy `tenant_isolation_return_requests` + `updated_at` trigger). `apps/api/src/routes/returns.ts` fully rewritten — all 8 routes now use real `prisma.returnRequest.*`; `reject` sets `rejectedAt`/`rejectionReason`; `refund` computes itemsTotal from JSONB items + 10% restocking fee for damaged/opened; `stats` uses `prisma.returnRequest.aggregate` for real refund totals; `where: any = { shopId }` pattern consistent with other routes. | no new endpoints (existing 8 routes now actually functional) | 6 files, 6 `(prisma as any)` stubs → 0, 1 Prisma model + migration added | 2026-07-03 |
