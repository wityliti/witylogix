...
| WIT-357 | feat/WIT-357-dashboard-profile-support-polish | **Profile page real save/password APIs.** `profile/page.tsx`: complete rewrite — fixed broken save (was calling `refetch()` with no mutation), broken password change (had inputs but no API call), fake sessions (3 hardcoded IPs/cities/browsers). Now: `useApiQuery<UserMe>('/api/v4/users/me')` for load; `useApiMutation('PATCH', '/api/v4/users/me')` for Save Changes; direct `fetch()` to `PATCH /api/v4/users/${user.id}/password` for password change (URL is dynamic so can't use static mutation hook). All `useState` hooks declared before conditional returns (fixed React rules violation). Client-side validation: passwords must match + min 8 chars. Inline success/error banners for both flows. Password visibility toggles on all 3 fields. Sessions section now shows only real current session. Danger Zone replaced fake delete button with support contact instructions. API (`apps/api/src/routes/users.ts`): added `PATCH /me` endpoint — self-service name/email update, no ADMIN role required, ConflictError guard on duplicate email within same shop. | 1 new API endpoint (`PATCH /api/v4/users/me`) | 2 files, broken save + broken password + 3 fake sessions → 0 | 2026-06-22 |
...
| Profile | `/profile` | ✅ WIT-357 (real save/password APIs + API PATCH /me; fake sessions removed in WIT-523) |
| Stores | `/stores` | ✅ |
| Partners | `/partners` | ✅ |
