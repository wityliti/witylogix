...
| WIT-580 | `feat/WIT-580-dashboard-typecheck-fix` | **Build rescue — 188 → 0 TypeScript errors.** Previous sprints (WIT-577+) left a second wave of appended code across 14 dashboard files. Fixes: `notification-center` (merged two conflicting versions + added VolumeX/Volume2 imports); `live-kpi-counters` (local DashboardStats shadowing imported type → removed local, updated field names); `live-order-feed` (duplicate hook declarations); `eld/page` (missing type definitions DriverComplianceStatus/DriverStatus/DriverStatusInfo, driversResult ref); `home/page` (wrong field names: totalOrdersToday→totalOrders, pendingDeliveries→pendingOrders); `collections/page` (duplicate useState = React hooks violation); `test-dashboard` (4× duplicate if(!data)+destructure blocks); `analytics/page` (Globe2→Globe, dangling dataKey); `analytics/route-performance` (missing BarChart2/MapIcon imports); `notification-stats-widget` (CHANNEL_COLORS→CHANNEL_CSS_VARS, wrong var names); `drivers/[id]` (undefined profileData dead block); `settings/payments` (string\|null type mismatch); `locations/page` (view→viewMode, onReady not in WLMapProps). Prisma: ReturnRequest back-relations on Shop+Order. `packages/db`: $transaction array form → interactive callback. validators dist built. | no new endpoints — build fix only | 188 TS errors → 0 | #492 |
