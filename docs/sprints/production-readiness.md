...
| WIT-615 | feat/WIT-615-integration-quality-error-states | **Integration components second quality pass — remove 5 remaining fake delays.** `connect-dialog.tsx`: remove 2s fake delay in else branch; always await `onConnect?.()`. `provider-card.tsx`: remove 1s fake delay + dead `isLoading` state from `handleTest`. `import-export-wizard.tsx`: remove 800ms fake delay + `isProcessing` state; simplify `handleNext`/`handleBack` to use `STEP_ORDER` array, eliminating duplicated step chains. `migration-progress-bar.tsx`: remove 600ms fake delay + dead `isRollingBack` state. `integration-card.tsx`: remove 3× 300ms `setTimeout(setIsActionLoading)` + dead state; `getActionButton` now returns plain synchronous handlers. `analytics-widget.tsx`: replace `setTimeout(setIsRefreshing, 1000)` with try/finally around `await onRefresh?.()` so loading state tracks real async completion. Build ✓ typecheck ✓ lint ✓. | no new endpoints | 6 files, 5 fake setTimeout delays + 4 dead state vars → 0, 1 loading state → real async | 2026-07-14 |
| WIT-616 | feat/WIT-616-finance-events-tracking-fix | **Quality hardening — replace `window.location.reload()` onRetry with hook refetch; replace `window.location.href` internal nav with `router.push()`.** `orders/conflicts/page.tsx`: `window.location.reload()` in ErrorState onRetry + Refresh button → `revalidate` from `useConflicts()`. `notifications/delivery-log/page.tsx`: `window.location.reload()` → `refetch` from `useDeliveryLog()`. `supply-chain/orders/page.tsx`: 3× `window.location.reload()` → `orders.refetch`, `refetchWaves`, `refetchBatches` (added `refetch` to both `useApiList` destructurings). `esignatures/page.tsx`: single reload → combined `retryAll()` calling `refetchEnvelopes + refetchAnalytics + refetchTemplates`. `orders/local/page.tsx`: View Details `window.location.href = /orders/:id` → `router.push()`. `routes/plan/page.tsx`: post-save redirect `window.location.href` → `router.push()`. `integrations/connected/[providerId]/page.tsx`: post-disconnect `window.location.href = /integrations/connected` → `router.push()`. `integrations/fuel/page.tsx`: 3× `window.location.href = /integrations/catalog` → `router.push()`. `integrations/collaboration/page.tsx`: 2× same → `router.push()`. `widget-config/page.tsx`: Discard `window.location.reload()` → `router.refresh()`. Remaining 3 `window.location.href` usages are correct (tel: link, OAuth provider redirect, 404 page). Build ✓ typecheck ✓ lint ✓. | no new endpoints | 10 files, 5 bad reload() → refetch, 8 href= nav → router.push(), 0 mock signals | 2026-07-14 |
