...
| WIT-357 | feat/WIT-357-dashboard-timeslots-system-dvir-invoices | **Sync job history + API-docs path fix.** `GET /api/v4/orders/sync/status`: replaced hardcoded `recentSyncJobs:[]` with real `IntegrationEvent` Prisma query (eventType=SYNC, filtered to ecommerce slugs, last 20, mapped to SyncJob shape via metadata JSON). orders/import page: removed two hardcoded empty arrays (`recentSyncJobs:SyncJob[]=[]`, `failedJobs:SyncJob[]=[]`) and zeroed counters (`failedOrders:0`, `activeConflicts:0`) — now fetches `/api/v4/orders/sync/status` + `/api/v4/orders/sync/metrics` for real values. admin/api-docs: all 6 endpoint paths `/api/v1/` → `/api/v4/`; PUT → PATCH for orders update. | 3 files, 2 hardcoded arrays → 0, 2 hardcoded zeros → 0, 6 wrong API versions → 0 | 2026-06-22 |
