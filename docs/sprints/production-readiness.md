...
| WIT-570 | feat/WIT-570-dashboard-typecheck-ux-hardening | **Build rescue — all dashboard TypeScript compilation errors resolved.** Prior automated sprints had appended duplicate code blocks instead of replacing, causing 40+ TS errors across 15 files. `eld/page.tsx`: import `DriverComplianceStatus`/`DriverStatusInfo` from `use-eld`; remove dead `driversResult` references; fix `DutyStatus` index with `as DutyStatus`. `collections/page.tsx`: hoist all `useState` calls above early returns; remove 2× duplicate `handleRemoveProduct` and 2× duplicate state blocks. `admin/test-dashboard/page.tsx`: remove 3× duplicated `if (!data)` guards and variable declarations. `drivers/page.tsx`: remove dead `DriverMapView`/`MapLegend` functions (used unimported `WLMap`/`DriverLocationLayer`/`MapIcon` — real page uses lazy `DriversMapView` component). `drivers/[id]/page.tsx`: remove dead `{profileData && ...}` block (`profileData` never declared). `home/page.tsx`: fix `recentOrders` shape → `ApiOrder`; fix `displayDrivers` → include `_count: {orders}` field. `analytics/page.tsx`: add `Globe2` to lucide imports; fix `d[dataKey]` undefined index → `d.orders`. `analytics/route-performance/page.tsx`: add `BarChart2`/`MapIcon` lucide imports. Carried from previous session: `notification-center`, `live-kpi-counters`, `live-order-feed`, `active-delivery-map`, `notification-stats-widget`, `settings/payments`, `analytics-widget`, `locations/page`, `supply-chain/orders`, `use-shipment-tracking`, `packages/db`. Result: `tsc --noEmit` exits 0; Next.js build succeeds for all 90+ routes. | no new endpoints | 22 files, `tsc --noEmit` 0 errors, build clean | 2026-07-17 |
