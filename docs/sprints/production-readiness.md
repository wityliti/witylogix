...
| WIT-568 | feat/WIT-568-analytics-zone-map-auth-hardening | **Analytics zone map + auth hardening.** `analytics/page.tsx`: fixed 3 pre-existing TypeScript bugs (`loading` not in scope in `MetricTile`, `val` undefined in `MiniBarChart` tooltip, stray `</span>` in `WeeklyChart`); added Zones map tab (Charts\|Heatmap\|Zones toggle) with dynamic `AnalyticsZoneView`. NEW `analytics/components/analytics-zone-view.tsx`: WLMap wrapper with 150+ city CITY_COORDS lookup; `resolveCity()` (direct/partial/deterministic fallback); `FitBoundsController` inner component; loading/empty states; trend-colour legend overlay; stats overlay; selected-zone popup. NEW `components/map/city-volume-layer.tsx`: shared MapLibre GL layer (modern `useWLMap()` pattern); halo + circle sub-layers; radius 10–40px by order volume; colour by trend %. **Auth hardening:** `collections/page.tsx`: raw `fetch()` DELETE → `api.delete()`. `settings/payments/page.tsx`: raw `fetch()` POST → `api.post()` with proper ok=true/false from throw vs success. **Cleanup:** deleted orphaned Leaflet `zone-analytics-map.tsx` (340 lines, never imported). `analytics/route-performance/page.tsx`: removed duplicated/garbled controls block (referenced nonexistent `viewMode`/`setViewMode` and mixed native `<button>` with `<Button>` component incorrectly); replaced with correct period-selector using `<Button key={p}>{p}</Button>`; removed unused `MapIcon`/`BarChart2` imports. | no new endpoints | 7 files changed, 3 TS bugs → 0, 2 raw fetch calls → 0, 1 orphan file deleted, 1 map view added | 2026-06-30 |
...
| WIT-568 | `feat/WIT-568-dashboard-collections-payments-rawfetch-cleanup` | **collections/page.tsx**: React hooks violation (useState after early returns) → moved to top; raw fetch DELETE → api.delete(); nested tbody → fragment. **settings/payments/page.tsx**: duplicate handleTestPayment/toggleSecretVisibility declarations removed; unified on gateway-specific test endpoint; isTestPaymentLoading boolean→string\|null. **integrations/providers/page.tsx**: uncontrolled config form (defaultValue) → controlled state wired to updateConfiguration(). **hooks/use-integration-logs.ts**: 3 raw fetch() → api client; paths /api/integrations/→/api/v4/integrations/; AbortController removed. **components/map/live-tracking-layer.tsx**: Math.random() jitter → deterministic hash offset. **components/crm/oauth-callback-handler.tsx**: raw fetch wrong path → api.post() correct v4 path | existing endpoints | 6 files; React hooks fix; duplicate fn fix; uncontrolled→controlled form | #386 |
