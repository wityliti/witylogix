...
describe("zoneShapeSchema", () => {
  ...
  it("rejects unknown shape types", () => {
    expect(() =>
      zoneShapeSchema.parse({ type: "square", ring: polygon.ring } as unknown),
    ).toThrow();
  });
});
```
  ...
  const zone = await request.tenantDb.$transaction(async (tx) => {
    const patched = await tx.deliveryZone.update({
      ...
      data: {
        ...updateData,
        ...(shape
          ? { metadata: { ...(existing.metadata as object), shape } }
          : {}),
      },
    });
    if (shape?.type === "polygon" || legacyPoints) {
      const wkt = ringToWkt(
        shape?.type === "polygon" ? shape.ring : legacyPoints!,
      );
      await tx.$executeRaw`
    ...
    $queryRaw: vi
      ...
      .mockImplementation(async (strings: TemplateStringsArray) => {
        ...
        if (q.includes("open_orders"))
          return [
            { zone_id: "z1", open_orders: 12, drivers: 4, sla_pct: 0.92 },
          ];
...
describe("zones overlays handler", () => {
  it("returns zones, heatmap, and hubs", async () => {
    ...
    expect(res.zones[0]).toMatchObject({
      id: "z1",
      openOrders: 12,
      drivers: 4,
      slaPct: 0.92,
  ...
  it("caches results for 30 seconds per shop", async () => {
    ...
    expect(
      (c.tenantDb.$queryRaw as ReturnType<typeof vi.fn>).mock.calls.length,
    ).toBe(3);
  });
});
```
...
export default async function zonesOverlaysRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/", async (request: FastifyRequest) => {
    ...
    return handler(
      {
        tenantDb: request.tenantDb as OverlaysContext["tenantDb"],
        shopId: request.shopId,
      },
      q,
    );
...
export function HeatmapLayer({ points }: HeatmapLayerProps) {
  ...
  useEffect(() => {
    const src = map.getSource("heatmap") as
      ...
      map.addLayer({
        ...
        paint: {
          ...
          "circle-color": [
            ...
            ["get", "status"],
            "open",
            COLOR_BY_STATUS.open,
            "assigned",
            COLOR_BY_STATUS.assigned,
            "in_transit",
            COLOR_BY_STATUS.in_transit,
            "delayed",
            COLOR_BY_STATUS.delayed,
            "#8585a0",
          ],
...
export function DrawLayer({
  ...
  useEffect(() => {
    ...
    map.on(
      "draw.create" as unknown as keyof maplibregl.MapLayerEventType,
      emitPolygon,
    );
    map.on(
      "draw.update" as unknown as keyof maplibregl.MapLayerEventType,
      emitPolygon,
    );
    ...
    if (value?.type === "polygon" && value.ring.length >= 3) {
      draw.add({
        ...
        geometry: {
          ...
          coordinates: [
            [
              ...value.ring.map((p) => [p.longitude, p.latitude]),
...
describe("zones chrome", () => {
  ...
  it("OverlayControls persists state to localStorage", () => {
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ heatmap: false }),
    );
  });
  ...
  it("KpiStrip renders zone/driver/order counts and a slipping pill", () => {
    render(
      <KpiStrip
        stats={{ zones: 4, driversOnline: 13, openOrders: 47, slipping: 1 }}
        onClickSlipping={() => {}}
      />,
    );
...
describe("zones data hooks", () => {
  it("useZonesGeoJson fetches /api/v4/zones?format=geojson", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(() => fetchJson({ type: "FeatureCollection", features: [] })),
    );
  ...
  useEffect(() => {
    (async () => {
      ...
      setZone({
        ...
        freeAbove:
          zRes.data.freeAbove == null ? null : Number(zRes.data.freeAbove),
...
export function LegacyNotice() {
  return (
    <div className="p-8 text-sm" style={{ color: "var(--wl-neutral-300)" }}>
      ...
      <br />
      Set <code>NEXT_PUBLIC_FEATURE_ZONES_MAP=1</code> in your{" "}
