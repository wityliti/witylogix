...
describe('Container', () => {
  ...
  describe('Service Resolution', () => {
    ...
    it('should handle sync resolution for singletons', async () => {
      container.register('singleton', () => ({ value: 'test' }), { lifetime: 'singleton' });
  ...
  describe('Error Handling', () => {
    it('should throw when resolving non-existent service', async () => {
      await expect(container.resolve('doesNotExist')).rejects.toThrow('not found');
    });
    ...
    it('should provide helpful error message', async () => {
      await expect(container.resolve('unknown')).rejects.toThrow('Did you forget to register');
    });
  });
});
