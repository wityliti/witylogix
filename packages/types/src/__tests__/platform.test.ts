 ...
 */
export function normalizePlatformSource(
  source: string,
): PlatformSource | string {
  if (!source) return source;
  const normalized = source.toUpperCase();
  return isPlatformSource(normalized)
    ? (normalized as PlatformSource)
    : normalized;
}
...
describe("Platform Types Usage Patterns", () => {
  ...
  describe("Type Discrimination", () => {
    ...
    it("should handle platform discrimination", () => {
      ...
      const shopifyRefs = references.filter(
        (r) => r.source === PlatformSource.SHOPIFY,
      );
      const wooRefs = references.filter(
        (r) => r.source === PlatformSource.WOOCOMMERCE,
      );
...
describe("normalizePlatformSource() Helper", () => {
  it("should convert lowercase to uppercase", () => {
    ...
    expect(normalizePlatformSource("woocommerce")).toBe(
