...
const validateOrderStep = {
  name: 'Validate Order',
  async invoke(input: CreateDeliveryOrderInput): Promise<StepResult<CreateDeliveryOrderInput>> {
    try {
      // Validate required fields
      if (!input.customerId || input.customerId.trim().length === 0) {
        return { success: false, error: 'Customer ID is required', code: 'INVALID_CUSTOMER' };
      }

      if (!input.addressLine1 || input.addressLine1.trim().length === 0) {
        return { success: false, error: 'Address is required', code: 'INVALID_ADDRESS' };
      }

      if (!input.city || input.city.trim().length === 0) {
        return { success: false, error: 'City is required', code: 'INVALID_CITY' };
      }

      if (!input.postalCode || input.postalCode.trim().length === 0) {
        return { success: false, error: 'Postal code is required', code: 'INVALID_POSTAL_CODE' };
      }

      if (input.items.length === 0) {
        return { success: false, error: 'Order must have at least 1 item', code: 'NO_ITEMS' };
      }
...
const geocodeAddressStep = {
  ...
  ): Promise<StepResult<CreateDeliveryOrderInput & { coordinates: { lat: number; lng: number } }>> {
    try {
      ...
      if (!result.success) {
        return { success: false, error: result.error, code: 'GEOCODING_FAILED' };
      }
...
const checkInventoryStep = {
  ...
  async invoke(
    input: CreateDeliveryOrderInput & { coordinates: { lat: number; lng: number } }
  ): Promise<StepResult<CreateDeliveryOrderInput & { coordinates: { lat: number; lng: number } }>> {
    try {
...
const reserveInventoryStep = {
  ...
  async invoke(
    input: CreateDeliveryOrderInput & { coordinates: { lat: number; lng: number } }
  ): Promise<StepResult<CreateDeliveryOrderInput & { coordinates: { lat: number; lng: number }; reservationId: string }>> {
    try {
...
const calculateRateStep = {
  ...
  async invoke(
    ...
    try {
      ...
      if (!rateData.success) {
        return { success: false, error: 'Rate calculation failed', code: 'RATE_CALC_FAILED' };
      }
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Rate calculation error',
...
const createOrderStep = {
  ...
  async invoke(
    ...
    try {
      ...
      if (!order.id) {
        return { success: false, error: 'Failed to create order', code: 'ORDER_CREATION_FAILED' };
      }
...
describe('Create Delivery Order Workflow', () => {
  ...
  describe('Happy Path', () => {
    it('should create delivery order successfully', async () => {
      const input: CreateDeliveryOrderInput = {
        ...
        postalCode: 'M5V 3A8',
        country: 'CA',
        items: [{ productId: 'prod_1', quantity: 1, weight: 2.5, price: 29.99 }],
        totalPrice: 29.99,
      };
      ...
      const geocoded = await geocodeAddressStep.invoke(validated.data!, mockContext);
      expect(geocoded.success).toBe(true);

      const inventoryChecked = await checkInventoryStep.invoke(geocoded.data!, mockContext);
      expect(inventoryChecked.success).toBe(true);

      const inventoryReserved = await reserveInventoryStep.invoke(inventoryChecked.data!, mockContext);
      expect(inventoryReserved.success).toBe(true);

      const rateCalculated = await calculateRateStep.invoke(inventoryReserved.data!, mockContext);
      expect(rateCalculated.success).toBe(true);

      const orderCreated = await createOrderStep.invoke(rateCalculated.data!, mockContext);
    ...
    it('should call notification step successfully', async () => {
      ...
      const notificationResult = await sendNotificationStep.invoke(orderCreated.data!, mockContext);
  ...
  describe('Geocoding Failure', () => {
    it('should fail on invalid address coordinates', async () => {
      ...
      const validated = await validateOrderStep.invoke(input, mockContext);
      const result = await geocodeAddressStep.invoke(validated.data!, mockContext);
  ...
  describe('Inventory Management', () => {
    it('should fail when inventory is unavailable', async () => {
      ...
      const validated = await validateOrderStep.invoke(input, mockContext);
      const geocoded = await geocodeAddressStep.invoke(validated.data!, mockContext);
      const result = await checkInventoryStep.invoke(geocoded.data!, mockContext);
    ...
    it('should reserve inventory successfully', async () => {
      ...
      const geocoded = await geocodeAddressStep.invoke(validated.data!, mockContext);
      const inventoryChecked = await checkInventoryStep.invoke(geocoded.data!, mockContext);
      const result = await reserveInventoryStep.invoke(inventoryChecked.data!, mockContext);
  ...
  describe('Rate Calculation', () => {
    it('should calculate rate based on weight and zone', async () => {
      ...
      const inventoryChecked = await checkInventoryStep.invoke(geocoded.data!, mockContext);
      const inventoryReserved = await reserveInventoryStep.invoke(inventoryChecked.data!, mockContext);
      const result = await calculateRateStep.invoke(inventoryReserved.data!, mockContext);
  ...
  describe('Order Creation', () => {
    it('should create order with correct details', async () => {
      ...
      const inventoryReserved = await reserveInventoryStep.invoke(inventoryChecked.data!, mockContext);
      const rateCalculated = await calculateRateStep.invoke(inventoryReserved.data!, mockContext);
      const result = await createOrderStep.invoke(rateCalculated.data!, mockContext);
