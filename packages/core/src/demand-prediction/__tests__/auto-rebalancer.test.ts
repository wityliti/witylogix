...
import { AutoRebalancer, type CapacityImbalance, type RebalancingPlan } from '../auto-rebalancer';
...
describe('AutoRebalancer', () => {
  ...
  describe('Imbalance Detection', () => {
    ...
    it('should classify imbalance severity correctly', () => {
      ...
      for (const testCase of testCases) {
        const zones = [{ zoneId: `zone-${testCase.demand}`, demand: testCase.demand, capacity: testCase.capacity }];
        const imbalances = rebalancer.detectImbalance(zones);
  ...
  describe('Rebalancing Plan Generation', () => {
    ...
    it('should respect minimum capacity retention', () => {
      ...
      if (plan) {
        const bToAMove = plan.moves.find((m) => m.fromZoneId === 'zone-b' && m.toZoneId === 'zone-a');
        if (bToAMove) {
  ...
  describe('Plan Execution', () => {
    it('should execute rebalancing plan with auto-approval', () => {
      const plan: RebalancingPlan = {
        ...
        createdAt: new Date(),
        moves: [
          { fromZoneId: 'zone-a', toZoneId: 'zone-b', driverCount: 3 },
        ],
        isAutomatic: true,
  ...
  describe('Impact Estimation', () => {
    it('should estimate cost impact', () => {
      ...
      expect(plan!.impactEstimate.projectedWaitTimeReduction).toBeGreaterThan(0);
    });
  ...
  describe('Plan Rollback', () => {
    it('should rollback a pending plan', () => {
      ...
      const rolledBack = rebalancer.rollbackRebalancing(plan.id, 'User cancelled');

      expect(rolledBack).toBe(true);
    });

    it('should return false for non-existent plan rollback', () => {
      const rolledBack = rebalancer.rollbackRebalancing('non-existent-id', 'test');
