...
describe('Demand Ensemble', () => {
  ...
  beforeEach(async () => {
    ...
    for (let day = 0; day < 60; day++) {
      for (let hour = 0; hour < 24; hour++) {
        ...
        const value = Math.max(0, 50 + seasonalComponent + dowComponent + noise);
  ...
  describe('Predictions', () => {
    ...
    it('should have valid confidence intervals', async () => {
      ...
      for (let i = 0; i < 24; i++) {
        expect(prediction.lower_bound[i]).toBeLessThanOrEqual(prediction.predictions[i]);
        expect(prediction.predictions[i]).toBeLessThanOrEqual(prediction.upper_bound[i]);
    ...
    it('should include model predictions in output', async () => {
      ...
      expect(prediction.model_predictions.some((p) => p.modelName === 'seasonal')).toBe(true);
      expect(prediction.dominant_model).toBeTruthy();
    });

    it('should throw on untrained zone', async () => {
      await expect(ensemble.predict('zone-untrained', new Date())).rejects.toThrow();
    });
  });
  ...
  describe('Prediction explanation', () => {
    ...
    it('should identify dominant model', async () => {
      ...
      expect(['seasonal', 'regression', 'pattern', 'baseline', 'ensemble']).toContain(
        prediction.dominant_model,
      );
    });
  });
  ...
  describe('Seasonal patterns', () => {
    ...
    it('should capture weekly patterns', async () => {
      ...
      const mondayPred = await ensemble.predict('zone-test-1', monday, 'hourly');
      const saturdayPred = await ensemble.predict('zone-test-1', saturday, 'hourly');
  ...
  describe('Edge cases', () => {
    ...
    it('should handle zones with sparse data', async () => {
      ...
      const prediction = await sparseEnsemble.predict('zone-sparse', new Date(), 'hourly');
