...
describe('DataAggregator', () => {
  ...
  describe('Hourly Distribution', () => {
    ...
    it('should identify peak hours', async () => {
      ...
      const peakHour = Object.entries(distribution).sort((a, b) => b[1] - a[1])[0];
    ...
    it('should reflect business hours patterns', async () => {
      ...
      const businessTotal = businessHours.reduce((sum, h) => sum + distribution[h], 0);

      const nightHours = [0, 1, 2, 3, 4, 5, 6];
      const nightTotal = nightHours.reduce((sum, h) => sum + distribution[h], 0);
  ...
  describe('Day of Week Pattern', () => {
    ...
    it('should show weekday vs weekend difference', async () => {
      ...
      const weekdayAvg = (pattern[1] + pattern[2] + pattern[3] + pattern[4] + pattern[5]) / 5;
  ...
  describe('Average Delivery Time', () => {
    ...
    it('should reflect rush hour delays', async () => {
      ...
      const peakAvg = peakHours.reduce((sum, h) => sum + avgTimes[h], 0) / peakHours.length;
      const nightAvg = nightHours.reduce((sum, h) => sum + avgTimes[h], 0) / nightHours.length;
