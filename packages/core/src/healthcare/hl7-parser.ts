...
export class HL7v2Parser {
  ...
  parseSegments(message: HL7Message): SegmentData {
    ...
    message.segments.forEach((segment) => {
      switch (segment.id) {
        case "MSH":
          data.MSH = this.parseMSHSegment(
            segment.fields.join(this.fieldSeparator),
          );
          break;
        case "PID":
  ...
  private parseIN1Segment(fields: string[]): IN1Segment {
    const insuredNameComponents =
      fields[10]?.split(this.componentSeparator) || [];
    return {
...
export class HL7toFHIR {
  ...
  oruToObservation(message: HL7Message): FHIRObservation[] {
    const segments = this.parser.parseSegments(message);
    const obxSegments = Array.isArray(segments.OBX)
      ? segments.OBX
      : [segments.OBX].filter(Boolean);
  ...
  dg1ToCondition(message: HL7Message): FHIRCondition[] {
    const segments = this.parser.parseSegments(message);
    const dg1Segments = Array.isArray(segments.DG1)
      ? segments.DG1
      : [segments.DG1].filter(Boolean);
