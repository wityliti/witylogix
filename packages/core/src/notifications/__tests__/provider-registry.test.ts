...
describe('TenantProviderRegistry', () => {
  ...
  describe('Per-Tenant Isolation', () => {
    ...
    it('should maintain separate fallback chains per tenant', () => {
      ...
      shop2Registry.setFallbackChain('EMAIL', 'mailgun', ['sendgrid', 'aws-ses']);

      expect(shop1Registry.getFallbackChain('EMAIL')).toEqual(['sendgrid', 'mailgun']);
      expect(shop2Registry.getFallbackChain('EMAIL')).toEqual(['mailgun', 'sendgrid', 'aws-ses']);
    });
  });
  ...
  describe('Fallback Chain Management', () => {
    ...
    it('should support multiple channels with different chains', () => {
      ...
      expect(registry.getFallbackChain('EMAIL')).toEqual(['sendgrid', 'mailgun']);
  ...
  describe('Provider Listing', () => {
    it('should list all providers for a channel', () => {
      ...
      expect(providers.map((p) => p.name)).toEqual(expect.arrayContaining(['sendgrid', 'mailgun']));
    });
  ...
  describe('BYOK (Bring Your Own Key) Credential Resolution', () => {
    it('should store and retrieve tenant-specific config', () => {
      ...
      const config = { apiKey: 'sg_tenant_key_12345', fromEmail: 'tenant@example.com' };
