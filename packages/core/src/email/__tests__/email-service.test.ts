...
class EmailService {
  ...
  async sendBulk(
    ...
    for (const recipient of recipients) {
      ...
      if (this.sentEmails.length >= this.rateLimitConfig.maxEmails) {
...
describe("EmailService", () => {
  ...
  describe("sendTemplate()", () => {
    ...
    it("should throw error for invalid email in sendTemplate", async () => {
      await expect(
        emailService.sendTemplate("invalid-email", "Subject", "Body", {
          name: "John",
        }),
      ).rejects.toThrow("Invalid email address");
    });
    ...
    it("should handle multiple template variables", async () => {
      ...
      const template =
        "Hi {{firstName}} {{lastName}}, order {{id}} is {{status}}";
      const data = {
        firstName: "Jane",
        lastName: "Doe",
        id: "ORD123",
  ...
  describe("sendBulk()", () => {
    it("should send emails to multiple recipients", async () => {
      ...
      const result = await emailService.sendBulk(recipients, "Subject", "Body");
  ...
  describe("Integration", () => {
    it("should send templated bulk emails", async () => {
      ...
      for (const recipient of recipients) {
        ...
        await emailService.sendTemplate(recipient, subject, template, {
