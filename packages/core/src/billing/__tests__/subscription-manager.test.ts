...
describe('SubscriptionManager', () => {
  ...
  describe('createSubscription', () => {
    ...
    it('should throw PlanNotFoundError for invalid plan', async () => {
      ...
      await expect(manager.createSubscription('shop-1', 'invalid-plan')).rejects.toThrow(
        PlanNotFoundError
      );
    });
    ...
    it('should throw SubscriptionError when shop already has active subscription', async () => {
      ...
      prisma.billingPlan.findUnique.mockResolvedValue(plan);
      prisma.billingSubscription.findUnique.mockResolvedValue(existingSubscription);

      await expect(manager.createSubscription('shop-1', 'plan-1')).rejects.toThrow(
        SubscriptionError
      );
    });
  ...
  describe('upgradeSubscription', () => {
    ...
    it('should throw SubscriptionNotFoundError for non-existent subscription', async () => {
      ...
      await expect(manager.upgradeSubscription('invalid-sub', 'plan-2')).rejects.toThrow(
        SubscriptionNotFoundError
      );
    });
  });
  ...
  describe('renewSubscription', () => {
    ...
    it('should throw error when renewing cancelled subscription', async () => {
      ...
      await expect(manager.renewSubscription('sub-1')).rejects.toThrow(SubscriptionError);
    });
