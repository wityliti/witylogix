...
describe('QuotaEnforcer', () => {
  ...
  describe('checkQuota', () => {
    it('should return allowed when under limit', async () => {
      ...
      const mockPlan: any = {
        limits: { orders: 100, shipments: 50, api_calls: 1000, storage: 1000, drivers: 10 },
      };
    ...
    it('should handle unlimited resources', async () => {
      ...
      const mockPlan: any = {
        limits: { orders: null, shipments: 50, api_calls: 1000, storage: 1000, drivers: 10 },
      };
  ...
  describe('incrementUsage', () => {
    ...
    it('should throw QuotaExceededError when incrementing past limit', async () => {
      ...
      await expect(enforcer.incrementUsage('shop-1', 'orders', 1)).rejects.toThrow(
        QuotaExceededError
      );
    });
  ...
  describe('enforceQuotaMiddleware', () => {
    ...
    it('should check multiple resources', async () => {
      ...
      const middleware = enforcer.enforceQuotaMiddleware(['orders', 'shipments']);
      await middleware(request, reply);
