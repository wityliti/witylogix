...
function randomToken (prefix: string): { plaintext: string; prefix: string; hash: string } {
...
export async function registerClient (
  ...
  if (invalid.length > 0) {
    throw new OAuthError('invalid_scope', `Unknown scopes: ${invalid.join(', ')}`)
  }
  if (input.redirectUris.length === 0) {
    throw new OAuthError('invalid_request', 'At least one redirect URI is required')
  }

  const isConfidential = (input.clientType ?? 'CONFIDENTIAL') === 'CONFIDENTIAL'
...
export async function rotateClientSecret (clientId: string): Promise<string> {
  ...
  if (client.clientType !== 'CONFIDENTIAL') {
    throw new OAuthError('invalid_request', 'Public clients have no secret to rotate')
  }
  const token = randomToken(CLIENT_SECRET_PREFIX)
...
export async function installApp (input: InstallAppInput): Promise<AppInstallationRow> {
...
export async function revokeInstallation (installationId: string): Promise<void> {
...
export async function exchangeCodeForTokens (
  ...
  if (!verifyPkce(input.codeVerifier, codeRow.codeChallenge, codeRow.codeChallengeMethod)) {
    throw new OAuthError('invalid_grant', 'PKCE verification failed')
  }

  return issueTokenPair(codeRow.installationId, codeRow.scopes, { consumeCodeId: codeRow.id })
}
...
export async function refreshAccessToken (
  ...
  if (!refreshRow) throw new OAuthError('invalid_grant', 'Refresh token not found')
...
export async function validateAccessToken (
  ...
  return { tokenId: row.id, installation: row.installation, scopes: row.scopes }
}
...
async function authenticateClient (
  ...
  if (client.clientType === 'CONFIDENTIAL') {
    ...
    if (!client.clientSecretHash || !safeEqualHex(providedHash, client.clientSecretHash)) {
