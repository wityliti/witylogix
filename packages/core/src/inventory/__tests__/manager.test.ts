...
describe("InventoryManager", () => {
  ...
  describe("Stock Adjustment", () => {
    ...
    it("should update lastUpdated timestamp", () => {
      ...
      const stock = inventory.getStock("SKU001", "WH001");
      expect(stock?.lastUpdated.getTime()).toBeGreaterThanOrEqual(
  ...
  describe("Stock Transfer", () => {
    ...
    it("should fail if source warehouse does not exist", () => {
      expect(() =>
        inventory.transfer("SKU001", "NONEXIST", "WH002", 10),
      ).toThrow("One or both warehouse entries not found");
    });

    it("should fail if destination warehouse does not exist", () => {
      expect(() =>
        inventory.transfer("SKU001", "WH001", "NONEXIST", 10),
      ).toThrow("One or both warehouse entries not found");
    });
  });
  ...
  describe("Reservations", () => {
    ...
    it("should prevent reservation of more stock than available", async () => {
      await expect(
        inventory.reserve("SKU001", "WH001", 150, "ORD001"),
      ).rejects.toThrow("Insufficient stock for reservation");
    });
  ...
  describe("Negative Stock Prevention", () => {
    ...
    it("should prevent reservations that would create negative available stock", async () => {
      await expect(
        inventory.reserve("SKU001", "WH001", 100, "ORD001"),
      ).rejects.toThrow("Insufficient stock for reservation");
    });
