...
describe("AuditLogger", () => {
  ...
  describe("Batch Flushing", () => {
    ...
    it(
      "should flush on timeout when batch not full",
      async () => {
        const events: AuditEvent[] = [];
  ...
  describe("Sensitive Field Masking", () => {
    it("should mask password fields", async () => {
      ...
      expect(events[0].changes?.password).toEqual({
        old: "***REDACTED***",
        new: "***REDACTED***",
      });
    ...
    it("should mask token fields", async () => {
      ...
      expect(events[0].changes?.api_token).toEqual({
        old: "***REDACTED***",
        new: "***REDACTED***",
      });
    ...
    it("should handle multiple sensitive fields", async () => {
      ...
      expect(events[0].changes?.apiKey).toEqual({
        old: "***REDACTED***",
        new: "***REDACTED***",
      });
      expect(events[0].changes?.access_token).toEqual({
        old: "***REDACTED***",
        new: "***REDACTED***",
      });
      expect(events[0].changes?.name).toBe("John");
  ...
  describe("Event Exclusions", () => {
    it("should exclude events from specified resources", async () => {
      const config = new AuditLogger({
        excludeResources: ["system", "internal"],
      });
      let loggedCount = 0;
      config.setFlushCallback(async () => {
        loggedCount++;
  ...
  describe("Convenience Methods", () => {
    it("should log create events", async () => {
      ...
      await logger.logCreate("tenant-1", "user-1", "orders", "order-1", {
        name: "Order 1",
        status: "pending",
      });
