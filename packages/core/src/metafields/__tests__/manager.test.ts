...
class MetafieldsManager {
    ...
    switch (type) {
      case "STRING":
        if (typeof value !== "string") {
          throw new Error(
            `Invalid STRING value: expected string, got ${typeof value}`,
          );
        }
        break;
      case "NUMBER":
        if (typeof value !== "number") {
          throw new Error(
            `Invalid NUMBER value: expected number, got ${typeof value}`,
          );
        }
        break;
      case "BOOLEAN":
        if (typeof value !== "boolean") {
          throw new Error(
            `Invalid BOOLEAN value: expected boolean, got ${typeof value}`,
          );
        }
        break;
      case "JSON":
        if (typeof value !== "object" || value === null) {
          throw new Error(
            `Invalid JSON value: expected object, got ${typeof value}`,
          );
        }
        break;
      case "DATE":
        if (!(value instanceof Date) && typeof value !== "string") {
          throw new Error(
            `Invalid DATE value: expected Date or string, got ${typeof value}`,
          );
        }
        if (typeof value === "string") {
  ...
  create(
    ...
    if (definition.unique) {
      ...
      if (index && index.has(value)) {
        throw new Error(
          `Unique constraint violated: ${key} with value ${value} already exists`,
        );
      }
    }
  ...
  update(
    ...
    if (definition.unique) {
      ...
      if (index && index.has(value) && metafield.value !== value) {
        throw new Error(
          `Unique constraint violated: value ${value} already exists`,
        );
      }
      // Update index
      if (index) {
  ...
  getByNamespace(resourceId: string, namespace: string): Metafield[] {
    ...
    for (const metafield of this.fields.values()) {
      if (
        metafield.resourceId === resourceId &&
  ...
  bulkCreate(
    ...
    for (const [key, value] of Object.entries(data)) {
      results.push(
        this.create(resourceId, resourceType, namespace, key, value),
      );
    }
    return results;
  }
...
describe("MetafieldsManager", () => {
  ...
  describe("Create Operations", () => {
    ...
    it("should set createdAt and updatedAt", () => {
      ...
      expect(field.createdAt.getTime()).toBeGreaterThanOrEqual(
        before.getTime(),
      );
  ...
  describe("Update Operations", () => {
    ...
    it("should update updatedAt timestamp", () => {
      ...
      const field = manager.read("prod_1", "products", "color");
      expect(field?.updatedAt.getTime()).toBeGreaterThanOrEqual(
        before.getTime(),
      );
    });
  ...
  describe("Delete Operations", () => {
    ...
    it("should reject deletion of non-existent metafield", () => {
      expect(() => manager.delete("prod_999", "products", "color")).toThrow(
        "Metafield not found",
      );
    });
  });
