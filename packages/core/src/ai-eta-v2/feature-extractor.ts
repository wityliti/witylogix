...
export class FeatureExtractor {
  ...
  private getWeatherCondition(
    intensity: number,
  ): FeatureVector["weather_condition"] {
  ...
  extractFromHistorical(delivery: HistoricalDelivery): FeatureVector {
    ...
    return {
      ...
      driver_experience_score: this.normalizeExperience(
        delivery.driver_experience_score,
      ),
      vehicle_type: delivery.vehicle_type,
      num_stops_remaining: delivery.num_stops,
      temperature_celsius: this.normalizeTemperature(
  ...
  computeSimilarity(
    ...
    if (features1.weather_condition !== features2.weather_condition)
      sumSquaredDiff += 0.09;
    if (features1.traffic_condition !== features2.traffic_condition)
      sumSquaredDiff += 0.04;
    if (features1.vehicle_type !== features2.vehicle_type)
      sumSquaredDiff += 0.04;
    count += 4;
