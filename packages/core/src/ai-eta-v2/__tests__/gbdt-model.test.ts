...
function makeDelivery(
  overrides: Partial<HistoricalDelivery> = {},
): HistoricalDelivery {
  return {
...
function generateTrainingData(n: number): HistoricalDelivery[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const trafficMultiplier =
      traffic === "heavy" ? 1.8 : traffic === "moderate" ? 1.3 : 1.0;
    const actual =
      distance * 3 * trafficMultiplier +
      (hour >= 7 && hour <= 9 ? 10 : 0) +
      Math.random() * 5;
...
describe("GBDTModel", () => {
  ...
  describe("prediction", () => {
    ...
    it("predicts longer duration for heavier traffic", () => {
      const lightFeatures = makeFeatureVector({
        traffic_condition: "light",
        traffic_multiplier: 1.0,
      });
      const heavyFeatures = makeFeatureVector({
      ...
      expect(heavyPred.predicted_duration_minutes).toBeGreaterThan(
        lightPred.predicted_duration_minutes,
      );
    });

    it("predicts longer duration for longer distance", () => {
      const shortFeatures = makeFeatureVector({
        distance_km: 5,
        historical_avg_minutes: 15,
      });
      const longFeatures = makeFeatureVector({
        distance_km: 30,
        historical_avg_minutes: 90,
      });

      const shortPred = model.predict(shortFeatures);
      const longPred = model.predict(longFeatures);

      expect(longPred.predicted_duration_minutes).toBeGreaterThan(
        shortPred.predicted_duration_minutes,
      );
    });
    ...
    it("returns lower_bound < predicted < upper_bound", () => {
      const pred = model.predict(makeFeatureVector());
      expect(pred.lower_bound_minutes).toBeLessThan(
        pred.predicted_duration_minutes,
      );
      expect(pred.upper_bound_minutes).toBeGreaterThan(
        pred.predicted_duration_minutes,
      );
    });
  ...
  describe("accuracy", () => {
    it("achieves reasonable MAE on test data", () => {
      ...
      for (const d of testData) {
        ...
        const pred = model.predict(features);
        totalError += Math.abs(
          pred.predicted_duration_minutes - d.actual_duration_minutes,
        );
      }
