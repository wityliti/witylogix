...
describe("ETAPipeline", () => {
  ...
  describe("traffic adjustment", () => {
    it("should apply traffic delay", () => {
      ...
      expect(result.adjustedDurationMin).toBeGreaterThan(
        result.baseDurationMin,
      );
    });
    ...
    it("should not apply traffic adjustment when disabled", () => {
      ...
      const trafficFactor = result.adjustmentFactors.find(
        (f) => f.step === "traffic",
      );
  ...
  describe("weather adjustment", () => {
    it("should apply rain adjustment", () => {
      ...
      const weatherFactor = result.adjustmentFactors.find(
        (f) => f.step === "weather",
      );
  ...
  describe("time-of-day correction", () => {
    it("should increase duration during morning peak hours (7-9 AM)", () => {
      ...
      const timeAdj = result.adjustmentFactors.find(
        (f) => f.step === "time_of_day",
      );
    ...
    it("should apply lower multiplier on weekends", () => {
      ...
      const weekdayAdj = weekdayResult.adjustmentFactors.find(
        (f) => f.step === "time_of_day",
      );
      const weekendAdj = weekendResult.adjustmentFactors.find(
        (f) => f.step === "time_of_day",
      );
  ...
  describe("zone-specific correction", () => {
    it("should apply urban correction", () => {
      ...
      const zoneAdj = result.adjustmentFactors.find(
        (f) => f.step === "zone_specific",
      );
  ...
  describe("ETA calculation", () => {
    it("should calculate correct ETA", () => {
      ...
      const expectedEta = new Date(
        departureTime.getTime() + result.adjustedDurationMin * 60 * 1000,
      );
  ...
  describe("edge cases", () => {
    ...
    it("should produce valid bounds", () => {
      ...
      expect(result.lowerBoundMin).toBeLessThanOrEqual(
        result.adjustedDurationMin,
      );
      expect(result.adjustedDurationMin).toBeLessThanOrEqual(
        result.upperBoundMin,
      );
    });
  });
});
