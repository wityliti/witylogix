...
import type { ReturnItem, ReturnPolicy, ItemCondition } from "../types";
...
describe("Returns Service Tests", () => {
  ...
  describe("calculateRefund()", () => {
    ...
    it("should handle zero restocking fee policy", () => {
      const policyNoFee: ReturnPolicy = {
        ...defaultPolicy,
    ...
    it("should handle high restocking fee policy", () => {
      const policyHighFee: ReturnPolicy = {
        ...defaultPolicy,
  ...
  describe("canTransition()", () => {
    it("should allow REQUESTED to APPROVED", () => {
      expect(canTransition(ReturnStatus.REQUESTED, ReturnStatus.APPROVED)).toBe(
        true,
      );
    });
    ...
    it("should not allow REQUESTED to SHIPPED_BACK", () => {
      expect(
        canTransition(ReturnStatus.REQUESTED, ReturnStatus.SHIPPED_BACK),
      ).toBe(false);
    });

    it("should allow APPROVED to SHIPPED_BACK", () => {
      expect(
        canTransition(ReturnStatus.APPROVED, ReturnStatus.SHIPPED_BACK),
      ).toBe(true);
    });
    ...
    it("should allow SHIPPED_BACK to RECEIVED", () => {
      expect(
        canTransition(ReturnStatus.SHIPPED_BACK, ReturnStatus.RECEIVED),
      ).toBe(true);
    });

    it("should allow SHIPPED_BACK to CLOSED", () => {
      expect(
        canTransition(ReturnStatus.SHIPPED_BACK, ReturnStatus.CLOSED),
      ).toBe(true);
    });

    it("should allow RECEIVED to INSPECTED", () => {
      expect(canTransition(ReturnStatus.RECEIVED, ReturnStatus.INSPECTED)).toBe(
        true,
      );
    });

    it("should allow RECEIVED to CLOSED", () => {
      expect(canTransition(ReturnStatus.RECEIVED, ReturnStatus.CLOSED)).toBe(
        true,
      );
    });

    it("should allow INSPECTED to REFUNDED", () => {
      expect(canTransition(ReturnStatus.INSPECTED, ReturnStatus.REFUNDED)).toBe(
        true,
      );
    });

    it("should allow INSPECTED to REJECTED", () => {
      expect(canTransition(ReturnStatus.INSPECTED, ReturnStatus.REJECTED)).toBe(
        true,
      );
    });

    it("should allow REFUNDED to CLOSED", () => {
      expect(canTransition(ReturnStatus.REFUNDED, ReturnStatus.CLOSED)).toBe(
        true,
      );
    });

    it("should not allow transitions from CLOSED", () => {
      expect(canTransition(ReturnStatus.CLOSED, ReturnStatus.APPROVED)).toBe(
        false,
      );
      expect(canTransition(ReturnStatus.CLOSED, ReturnStatus.CLOSED)).toBe(
        false,
      );
    });

    it("should not allow invalid transitions", () => {
      expect(canTransition(ReturnStatus.REQUESTED, ReturnStatus.REFUNDED)).toBe(
        false,
      );
      expect(canTransition(ReturnStatus.APPROVED, ReturnStatus.RECEIVED)).toBe(
        false,
      );
    });
  });
  ...
  describe("Complete return lifecycle transitions", () => {
    ...
    it("should allow rejection from REQUESTED", () => {
      ...
      expect(canTransition(ReturnStatus.REJECTED, ReturnStatus.CLOSED)).toBe(
        true,
      );
    });
    ...
    it("should allow inspection rejection path", () => {
      ...
      expect(canTransition(status, (status = ReturnStatus.APPROVED))).toBe(
        true,
      );
      expect(canTransition(status, (status = ReturnStatus.SHIPPED_BACK))).toBe(
        true,
      );
      expect(canTransition(status, (status = ReturnStatus.RECEIVED))).toBe(
        true,
      );
      expect(canTransition(status, (status = ReturnStatus.INSPECTED))).toBe(
        true,
      );
