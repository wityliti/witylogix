...
describe("HoltWintersModel", () => {
  ...
  describe("weekly seasonality", () => {
    it("captures weekend peaks", () => {
      ...
      const weekdayAvg =
        (seasonal[0] + seasonal[1] + seasonal[2] + seasonal[3] + seasonal[4]) /
        5;
  ...
  describe("forecastHourly", () => {
    ...
    it("sum of hourly forecast ≈ daily total", () => {
      ...
      const result = model.forecastHourly(
        dailySeries,
  ...
  describe("DEFAULT_HOURLY_PROFILE", () => {
    ...
    it("peak hours have higher demand than off-peak", () => {
      ...
      expect(DEFAULT_HOURLY_PROFILE[9]).toBeGreaterThan(
        DEFAULT_HOURLY_PROFILE[3],
      );
      // Hour 18 (evening peak) should be > hour 2 (night)
      expect(DEFAULT_HOURLY_PROFILE[18]).toBeGreaterThan(
        DEFAULT_HOURLY_PROFILE[2],
      );
    });
  });
});
