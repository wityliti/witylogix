...
describe("PolicyEngine", () => {
  ...
  describe("Wildcard Permissions", () => {
    it("should grant all actions with * wildcard", () => {
      ...
      expect(engine.can(permissions, Resources.ORDERS, Actions.READ)).toBe(
        true,
      );
      expect(engine.can(permissions, Resources.ORDERS, Actions.CREATE)).toBe(
        true,
      );
      expect(engine.can(permissions, Resources.ORDERS, Actions.UPDATE)).toBe(
        true,
      );
      expect(engine.can(permissions, Resources.ORDERS, Actions.DELETE)).toBe(
        true,
      );
      expect(engine.can(permissions, Resources.ORDERS, Actions.EXPORT)).toBe(
        true,
      );
    });
    ...
    it("should grant all resources with * wildcard", () => {
      ...
      expect(engine.can(permissions, Resources.SHIPMENTS, Actions.READ)).toBe(
        true,
      );
      expect(engine.can(permissions, Resources.DRIVERS, Actions.READ)).toBe(
        true,
      );
      expect(engine.can(permissions, "custom_resource", Actions.READ)).toBe(
        true,
      );
    });
    ...
    it("should grant all resources and actions with * wildcard", () => {
      ...
      expect(engine.can(permissions, Resources.SHIPMENTS, Actions.CREATE)).toBe(
        true,
      );
    ...
    it("should handle resource:* wildcard pattern", () => {
      ...
      expect(engine.can(permissions, "orders:pending", Actions.READ)).toBe(
        true,
      );
      expect(engine.can(permissions, "orders:completed", Actions.READ)).toBe(
        true,
      );
      expect(engine.can(permissions, Resources.SHIPMENTS, Actions.READ)).toBe(
        false,
  ...
  describe("Role Hierarchy", () => {
    it("owner can do everything", () => {
      ...
      expect(engine.can(ownerPerms, Resources.ORDERS, Actions.CREATE)).toBe(
        true,
      );
      expect(engine.can(ownerPerms, Resources.BILLING, Actions.DELETE)).toBe(
        true,
      );
    ...
    it("admin has most permissions except billing", () => {
      ...
      expect(engine.can(adminPerms, Resources.ORDERS, Actions.MANAGE)).toBe(
        true,
      );
      expect(engine.can(adminPerms, Resources.DRIVERS, Actions.MANAGE)).toBe(
        true,
      );
      expect(engine.can(adminPerms, Resources.BILLING, Actions.READ)).toBe(
        true,
      );
      expect(engine.can(adminPerms, Resources.BILLING, Actions.DELETE)).toBe(
        false,
      );
    });
    ...
    it("viewer has only read permissions", () => {
      ...
      expect(engine.can(viewerPerms, Resources.ORDERS, Actions.READ)).toBe(
        true,
      );
      expect(engine.can(viewerPerms, Resources.ORDERS, Actions.CREATE)).toBe(
        false,
      );
      expect(engine.can(viewerPerms, Resources.ORDERS, Actions.UPDATE)).toBe(
        false,
      );
      expect(engine.can(viewerPerms, Resources.ORDERS, Actions.DELETE)).toBe(
        false,
      );
    });
    ...
    it("driver has limited read permissions", () => {
      ...
      expect(engine.can(driverPerms, Resources.SHIPMENTS, Actions.READ)).toBe(
        true,
      );
      expect(engine.can(driverPerms, Resources.ORDERS, Actions.READ)).toBe(
        true,
      );
      expect(engine.can(driverPerms, Resources.ORDERS, Actions.CREATE)).toBe(
        false,
      );
      expect(engine.can(driverPerms, Resources.SETTINGS, Actions.READ)).toBe(
        false,
      );
    });
    ...
    it("dispatcher can manage routes", () => {
      ...
      expect(
        engine.can(dispatcherPerms, Resources.ROUTES, Actions.MANAGE),
      ).toBe(true);
      expect(
        engine.can(dispatcherPerms, Resources.SHIPMENTS, Actions.READ),
      ).toBe(true);
      expect(
        engine.can(dispatcherPerms, Resources.SHIPMENTS, Actions.UPDATE),
      ).toBe(true);
      expect(
        engine.can(dispatcherPerms, Resources.SETTINGS, Actions.READ),
      ).toBe(false);
    });
  });
  ...
  describe("Permission Caching", () => {
    ...
    it(
      "should return expired cache as null",
      (done) => {
        const permissions: Permission[] = [
          { resource: Resources.ORDERS, action: Actions.READ },
        ];
  ...
  describe("Custom Roles with Mixed Permissions", () => {
    it("should support custom role with mixed permissions", () => {
      ...
      expect(
        engine.can(customPermissions, Resources.ORDERS, Actions.CREATE),
      ).toBe(true);
      expect(
        engine.can(customPermissions, Resources.SHIPMENTS, Actions.READ),
      ).toBe(true);
      expect(
        engine.can(customPermissions, Resources.DRIVERS, Actions.UPDATE),
      ).toBe(true);
      expect(
        engine.can(customPermissions, Resources.CUSTOMERS, Actions.READ),
      ).toBe(false);
    });
    ...
    it("should support wildcards in custom roles", () => {
      ...
      expect(
        engine.can(customPermissions, "orders:pending", Actions.READ),
      ).toBe(true);
      expect(
        engine.can(customPermissions, "shipments:active", Actions.CREATE),
      ).toBe(true);
      expect(
        engine.can(customPermissions, Resources.DRIVERS, Actions.READ),
      ).toBe(true);
      expect(
        engine.can(customPermissions, Resources.CUSTOMERS, Actions.READ),
      ).toBe(false);
