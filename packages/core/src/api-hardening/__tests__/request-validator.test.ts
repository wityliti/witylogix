...
class RequestValidator {
   ...
   */
  validateBody<T>(body: unknown, schema: z.ZodSchema): { data: T; error?: null } | { data?: null; error: any } {
   ...
   */
  validateQuery<T>(query: unknown, schema: z.ZodSchema): { data: T; error?: null } | { data?: null; error: any } {
  ...
  private decodeHtmlEntities(str: string): string {
    ...
    return str.replace(/&[a-z]+;|&#\d+;|&#x[a-f0-9]+;/gi, match => entities[match] || match);
  }

  /**
   * Validate and coerce pagination parameters
   */
  validatePagination(query: any): { page: number; pageSize: number; error?: null } | { error: any } {
