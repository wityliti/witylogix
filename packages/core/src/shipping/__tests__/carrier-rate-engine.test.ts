...
describe("RateCache", () => {
  ...
  describe("TTL Expiration", () => {
    it("should expire cached entries after TTL", async () => {
      const shortTtlCache = new RateCache({
        ttlMs: 100,
        maxEntries: 10,
        enabled: true,
      });
  ...
  describe("Cache Management", () => {
    ...
    it("should evict old entries when max capacity reached", () => {
      const smallCache = new RateCache({
        ttlMs: 5000,
        maxEntries: 2,
        enabled: true,
      });
      ...
      const request2 = {
        ...createMockRateRequest(),
        destination: { ...mockDestination, zip: "10002" },
      };
      const request3 = {
        ...createMockRateRequest(),
        destination: { ...mockDestination, zip: "10003" },
      };
...
describe("ParallelRateFetcher", () => {
  ...
  beforeEach(() => {
    ...
    fetcher = new ParallelRateFetcher(
      { ttlMs: 5000, enabled: true },
      credentialManager,
    );
  });
  ...
  describe("Parallel Fetching", () => {
    it("should fetch from multiple carriers in parallel", async () => {
      const mockFns = {
        [CarrierCode.UPS]: vi.fn(async () =>
          createMockRates(CarrierCode.UPS, 2),
        ),
        [CarrierCode.FEDEX]: vi.fn(async () =>
          createMockRates(CarrierCode.FEDEX, 2),
        ),
        [CarrierCode.DHL]: vi.fn(async () =>
          createMockRates(CarrierCode.DHL, 2),
        ),
      };

      fetcher.registerMockCarrier(CarrierCode.UPS, mockFns[CarrierCode.UPS]);
      fetcher.registerMockCarrier(
        CarrierCode.FEDEX,
        mockFns[CarrierCode.FEDEX],
      );
  ...
  describe("Ranking", () => {
    it("should rank results by strategy", async () => {
      ...
      const response = await fetcher.fetchRates(
        request,
        "tenant_1",
      ...
      expect(response.rates[0].cost).toBeLessThanOrEqual(
        response.rates[1].cost,
      );
    });
  });
