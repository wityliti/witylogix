...
describe('TomTomSDKClient', () => {
  ...
  describe('fuzzySearch', () => {
    ...
    it('should handle empty results', async () => {
      vi.mocked(global.fetch).mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          summary: { query: 'xyz', queryTime: 50, numResults: 0, offset: 0, totalResults: 0 },
          results: [],
        }),
      } as Response);
    ...
    it('should handle API errors', async () => {
      ...
      await expect(client.fuzzySearch('test')).rejects.toThrow('TomTom fuzzy search error');
    });
  });
  ...
  describe('geocode', () => {
        ...
        results: [
          {
            ...
            address: {
              freeformAddress: '350 5th Avenue, New York, NY 10118, United States',
              streetNumber: '350',
    ...
    it('should return multiple results', async () => {
      vi.mocked(global.fetch).mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          summary: { query: 'new york', queryTime: 100, numResults: 2, offset: 0, totalResults: 2 },
          results: [
            {
  ...
  describe('reverseGeocode', () => {
        ...
        results: [
          {
            ...
            position: { lat: 40.7541, lon: -73.9832 },
            address: {
              freeformAddress: 'Empire State Building, 5th Avenue, New York, NY 10118',
              streetName: '5th Avenue',
              municipality: 'New York',
              countrySubdivision: 'NY',
              country: 'United States',
    ...
    it('should handle missing results', async () => {
      vi.mocked(global.fetch).mockResolvedValueOnce({
        ok: true,
        json: async () => ({ summary: { queryTime: 100, numResults: 0 }, results: [] }),
      } as Response);
            ...
            legs: [
              {
                ...
                points: [
                  ...
                  { latitude: 40.7200, longitude: -74.0 },
  ...
  describe('matrix', () => {
    it('should calculate distance/duration matrix', async () => {
      const mockResponse = {
        ...
        matrix: [
          [
            { response: { routeSummary: { lengthInMeters: 2000, travelTimeInSeconds: 900 } } },
            { response: { routeSummary: { lengthInMeters: 3000, travelTimeInSeconds: 1200 } } },
          ],
          [
            { response: { routeSummary: { lengthInMeters: 1500, travelTimeInSeconds: 600 } } },
            { response: { routeSummary: { lengthInMeters: 2500, travelTimeInSeconds: 900 } } },
          ],
        ],
      };
