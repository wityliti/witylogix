...
export class RoutificClient extends RoutingAdapter {
   ...
   */
  private async solveAsync(
    payload: RoutificVrpRequest,
  ): Promise<RoutificVrpResponse> {
    ...
    while (Date.now() - startTime < this.asyncPollTimeout) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.asyncPollInterval),
      );
    }
  ...
  private async makeRequest<T>(url: string, payload: unknown): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Routific API error: ${response.status} ${response.statusText}`,
        );
      }
