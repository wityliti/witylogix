...
import { type PaymentTransaction, type PaymentMethod, type PaymentRefund } from '../types';
...
describe('BraintreeClient', () => {
  ...
  describe('charge', () => {
    ...
    it('should support idempotency key', async () => {
      vi.spyOn(global, 'fetch').mockResolvedValueOnce({
        ...
        json: async () => ({
          success: true,
          transaction: { id: 'txn-456', amount: '100.00', status: 'authorized' },
        }),
      } as any);
    ...
    it('should include billing address', async () => {
      vi.spyOn(global, 'fetch').mockResolvedValueOnce({
        ...
        json: async () => ({
          success: true,
          transaction: { id: 'txn-789', amount: '100.00', status: 'authorized' },
        }),
      } as any);
  ...
  describe('createSubscription', () => {
    it('should create subscription', async () => {
      ...
      const result = await client.createSubscription('pm-token', 'plan-monthly', {
        billingCycles: 12,
      });
