...
describe('TeamsClient', () => {
  ...
  describe('Channel Operations', () => {
    ...
    it('should unarchive channel', async () => {
      ...
      await expect(teamsClient.unarchiveChannel('ch1')).resolves.toBeUndefined();
    });
  });
  ...
  describe('Message Operations', () => {
    ...
    it('should edit message', async () => {
      ...
      const edited = await teamsClient.editMessage('ch-id/msg-id', 'Updated message');
  ...
  describe('Presence Status Mapping', () => {
    it('should map Teams availability to collaboration presence', () => {
      ...
      expect((client as any).mapTeamsPresenceStatus('Available')).toBe('active');
      expect((client as any).mapTeamsPresenceStatus('Away')).toBe('away');
      expect((client as any).mapTeamsPresenceStatus('DoNotDisturb')).toBe('do_not_disturb');
    ...
    it('should map collaboration presence to Teams availability', () => {
      ...
      expect((client as any).mapPresenceStatusToTeams('active')).toBe('Available');
      expect((client as any).mapPresenceStatusToTeams('away')).toBe('Away');
      expect((client as any).mapPresenceStatusToTeams('do_not_disturb')).toBe('DoNotDisturb');
      expect((client as any).mapPresenceStatusToTeams('offline')).toBe('Offline');
    });
  });
