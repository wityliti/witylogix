...
export class KorberClient extends SupplyChainAdapter {
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      throw new Error(`Körber initialization failed: ${(error as Error).message}`);
    }
  }
  ...
  public async adjustInventory(
    ...
    try {
      ...
      await RetryHandler.execute(async () => {
        const headers = this.buildHeaders();
        const payload = { sku, quantity, reason, adjustmentDate: new Date().toISOString() };
  ...
  public async createInboundShipment(shipment: InboundShipment): Promise<InboundShipment> {
    try {
      ...
      const result = await RetryHandler.execute(async () => {
        ...
        const response = await fetch(`${this.config.baseUrl}/api/inbound-shipments`, {
          method: 'POST',
          headers,
          body: JSON.stringify(payload),
          signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async cancelOutboundOrder(orderId: string, reason: string): Promise<OutboundOrder> {
    try {
      ...
      await RetryHandler.execute(async () => {
        ...
        const response = await fetch(`${this.config.baseUrl}/api/orders/${orderId}/cancel`, {
          method: 'POST',
          headers,
          body: JSON.stringify(payload),
          signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async releaseWave(waveId: string): Promise<WaveDefinition> {
    try {
      ...
      await RetryHandler.execute(async () => {
        ...
        const response = await fetch(`${this.config.baseUrl}/api/waves/${waveId}/release`, {
          method: 'POST',
          headers,
          body: JSON.stringify(payload),
          signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async completeWave(waveId: string): Promise<WaveDefinition> {
    try {
      ...
      await RetryHandler.execute(async () => {
        ...
        const response = await fetch(`${this.config.baseUrl}/api/waves/${waveId}/complete`, {
          method: 'POST',
          headers,
          body: JSON.stringify(payload),
          signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async updatePickTask(
    ...
    try {
      ...
      await RetryHandler.execute(async () => {
        const headers = this.buildHeaders();
        const payload = { pickedQuantity, status, updatedDate: new Date().toISOString() };
  ...
  public async confirmShipment(
    ...
    try {
      ...
      const result = await RetryHandler.execute(async () => {
        ...
        const response = await fetch(`${this.config.baseUrl}/api/ship-confirm`, {
          method: 'POST',
          headers,
          body: JSON.stringify(payload),
          signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async configureZone(
    ...
    try {
      ...
      const result = await RetryHandler.execute(async () => {
        ...
        const response = await fetch(`${this.config.baseUrl}/api/zones/${zone}`, {
          method: 'PUT',
          headers,
          body: JSON.stringify(payload),
          signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async createPurchaseOrder(po: PurchaseOrder): Promise<PurchaseOrder> {
    try {
      ...
      const result = await RetryHandler.execute(async () => {
        ...
        const response = await fetch(`${this.config.baseUrl}/api/purchase-orders`, {
          method: 'POST',
          headers,
          body: JSON.stringify(payload),
          signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async createTransferOrder(to: TransferOrder): Promise<TransferOrder> {
    try {
      ...
      const result = await RetryHandler.execute(async () => {
        ...
        const response = await fetch(`${this.config.baseUrl}/api/transfer-orders`, {
          method: 'POST',
          headers,
          body: JSON.stringify(payload),
          signal: AbortSignal.timeout(this.config.timeout ?? 30000),
