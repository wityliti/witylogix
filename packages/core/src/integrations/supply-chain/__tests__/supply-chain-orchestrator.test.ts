    ...
    it("should route order to best warehouse", async () => {
      ...
      (global.fetch as any)
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
            items: [{ SKU: "SKU-123", QOH: 100, QAL: 50, QAV: 50 }],
          }),
        })
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
            items: [{ SKU: "SKU-456", QOH: 200, QAL: 100, QAV: 100 }],
          }),
        })
    ...
    it("should throw error for insufficient inventory", async () => {
      (global.fetch as any)
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
            items: [{ SKU: "SKU-999", QOH: 5, QAL: 5, QAV: 0 }],
          }),
        })
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
            inventoryId: "INV-BY-999",
            sku: "SKU-999",
            qoh: 3,
            qal: 3,
            qav: 0,
    ...
    it("should sync all inventory across providers", async () => {
      (global.fetch as any)
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
            content: [{ sku: "SKU-789", qoh: 150, qal: 75, qav: 75 }],
          }),
        });
