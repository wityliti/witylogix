/**
 * Blue Yonder WMS/TMS Client
 * OAuth2 multi-tenant client for demand planning, fulfillment optimization, and transportation.
 ...
 * Supports multi-tenant demand planning and fulfillment operations
 */
export class BlueYonderClient extends SupplyChainAdapter {
  private accessToken: string | null = null;
  private tokenExpiresAt: number | null = null;

  /**
   * Initialize Blue Yonder client and authenticate
   */
  ...
  /**
   * Verify connection to Blue Yonder system
   */
  ...
  public async getTransferOrder(toId: string): Promise<TransferOrder> {
    try {
      ...
      return {
        ...
        fromWarehouseId: to.fromFacilityId,
  ...
  public async listTransferOrders(status?: string): Promise<TransferOrder[]> {
    try {
      ...
      return tos.map((t) => ({
        id: t.toId,
        toNumber: t.toNumber,
        fromWarehouseId: t.fromFacilityId,
  ...
  public async createTransferOrder(to: TransferOrder): Promise<TransferOrder> {
    try {
      ...
      const result = await RetryHandler.execute(async () => {
        ...
        const payload = {
          toNumber: to.toNumber,
          fromFacilityId: to.fromWarehouseId,
          toFacilityId: to.toWarehouseId,
