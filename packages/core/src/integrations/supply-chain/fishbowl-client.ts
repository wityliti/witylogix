...
export class FishbowlClient extends SupplyChainAdapter {
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      throw new Error(
        `Fishbowl initialization failed: ${(error as Error).message}`,
      );
    }
  }
    ...
    try {
      ...
      const warehouse = await RetryHandler.execute(async () => {
        ...
        const response = await fetch(
          `${this.config.baseUrl}/api/v1/locations/${warehouseId}`,
          {
            method: "GET",
            headers,
            signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async listWarehouses(): Promise<WarehouseLocation[]> {
    try {
      ...
      const warehouses = await RetryHandler.execute(async () => {
        const headers = this.buildHeaders();
        const response = await fetch(
          `${this.config.baseUrl}/api/v1/locations`,
          {
            method: "GET",
            headers,
            signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async listInventory(
    ...
    try {
      ...
      const items = await RetryHandler.execute(async () => {
        ...
        const response = await fetch(
          `${this.config.baseUrl}/api/v1/inventory?${params}`,
          {
            method: "GET",
            headers,
            signal: AbortSignal.timeout(this.config.timeout ?? 30000),
  ...
  public async adjustInventory(
    ...
    try {
      ...
      await RetryHandler.execute(async () => {
        ...
        const response = await fetch(
          `${this.config.baseUrl}/api/v1/inventory-adjustments`,
          {
            method: "POST",
            headers,
            body: JSON.stringify(payload),
  ...
  public async moveInventory(
    ...
    try {
      ...
      await RetryHandler.execute(async () => {
        ...
        const response = await fetch(
          `${this.config.baseUrl}/api/v1/inventory-moves`,
          {
            method: "POST",
            headers,
            body: JSON.stringify(payload),
