...
export class EpicorClient {
   ...
   * Schedule job
   */
  async scheduleJob(jobId: string, resourceId: string, startDate: Date): Promise<any> {
    const data = {
    ...
    const response = await this.request('POST', `/jobs/${jobId}/scheduling`, data);
    return response;
  }
   ...
   * Get inventory
   */
  async getInventory(partNum: string, warehouse?: string): Promise<ERPInventory> {
  ...
  async getWarehouseBins(warehouseId: string): Promise<any[]> {
    const response = await this.request('GET', `/warehouses/${warehouseId}/bins`);
    return response.value || [];
  }
   ...
   * Execute BAQ
   */
  async executeBAQ(baqId: string, parameters?: Record<string, unknown>): Promise<any> {
  ...
  async getSalesAnalysis(startDate: Date, endDate: Date): Promise<any> {
    ...
    const response = await this.request('GET', `/reports/sales-analysis?${queryParams}`);
    return response;
  }
  ...
  async getProductionSchedule(jobId?: string): Promise<any> {
    const endpoint = jobId ? `/production-schedule?jobNum=${jobId}` : '/production-schedule';
    const response = await this.request('GET', endpoint);
    return response;
  }
}
