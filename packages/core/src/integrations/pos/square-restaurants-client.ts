...
export class SquareRestaurantsClient extends POSAdapter {
  ...
  async getMenuItems(
    ...
    return this.executeWithRetries(async () => {
      ...
      const response = await this.makeRequest(
        `/v2/catalog/search?${params.toString()}`,
        "GET",
      );
  ...
  async getOrder(locationId: string, orderId: string): Promise<POSOrder> {
    return this.executeWithRetries(async () => {
      ...
      const fulfillment = sqOrder.fulfillment as
      ...
      return {
        ...
        payments: ((sqOrder.payments ?? []) as unknown[]).map((p: unknown) => {
          ...
          const moneyObj = pay.amount_money as
            | Record<string, unknown>
            | undefined;
          return {
            id: (pay.id as string) ?? "",
        ...
        subtotal:
          (sqOrder.subtotal_money as Record<string, number> | undefined)
            ?.amount ?? totals.subtotal,
        tax: taxMoney?.amount ?? totals.tax,
        total:
          (sqOrder.total_money as Record<string, number> | undefined)?.amount ??
          totals.total,
        createdAt: new Date((sqOrder.created_at as string) ?? Date.now()),
        updatedAt: new Date((sqOrder.updated_at as string) ?? Date.now()),
      };
    ...
    return this.executeWithRetries(async () => {
      ...
      if (filters?.startDate || filters?.endDate) {
        ...
        if (filters.startDate) {
          payload.query.filter.date_time_filter.created_at.start_at =
            filters.startDate.toISOString();
        }
        if (filters.endDate) {
          payload.query.filter.date_time_filter.created_at.end_at =
  ...
  async getKitchenDisplayStatus(
    ...
    return this.executeWithRetries(async () => {
      const order = await this.getOrder(locationId, orderId);
      const validStatuses: import("./types").KitchenOrderStatus[] = [
        "new",
        "received",
        "in_progress",
        "ready",
  ...
  async closeShift(locationId: string, shiftId: string): Promise<POSShift> {
    return this.executeWithRetries(async () => {
      ...
      const updatedShift = (updateResponse.shift ?? {}) as Record<
        string,
        unknown
      >;
