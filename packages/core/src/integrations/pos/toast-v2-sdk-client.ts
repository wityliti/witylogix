...
import type { ToastV2, RateLimitInfo, POSSDKError, IdempotencyOptions } from './pos-sdk-types';
...
export class ToastV2SDKClient extends POSAdapter {
  ...
  private rateLimitInfo: RateLimitInfo = { remaining: 500, reset: Date.now(), limit: 500 };
  ...
  private initializeRateLimiter(): void {
    setInterval(() => {
      this.rateLimitInfo.remaining = Math.min(500, this.rateLimitInfo.remaining + 500 / 60);
    }, 1000);
  }
  ...
  async getMenuItems(locationId: string, categoryId?: string): Promise<POSMenuItem[]> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        ...
        const items = records.map((item: any) => this.mapToastMenuItemToPOS(item, locationId));
        ...
        const payload = {
          ...
          tab: {
            lineItems: order.lineItems?.map((li: any) => ({
              ...
              modifiers: li.modifiers?.map((m: any) => ({ modifierGuid: m.modifierId })),
            })),
            discounts: order.discounts?.map((d: any) => ({
              name: d.reason || 'Discount',
              type: d.type === 'percentage' ? 'PERCENTAGE' : 'FIXED',
              amount: d.type === 'percentage' ? d.amount : this.formatAmount(d.amount),
            })),
  ...
  async listOrders(locationId: string, filters?: OrderFilters): Promise<POSOrder[]> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        ...
        return records.map((check: any) => this.mapToastCheckToPOS(check, locationId));
      });
    });
  }
   ...
   */
  async applyDiscount(locationId: string, orderId: string, discount: { type: 'fixed' | 'percentage'; amount: number; reason?: string }): Promise<POSOrder> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = {
          name: discount.reason || 'Discount',
          type: discount.type === 'percentage' ? 'PERCENTAGE' : 'FIXED',
          amount: discount.type === 'percentage' ? discount.amount : this.formatAmount(discount.amount),
        };
   ...
   * Add item to order
   */
  async addOrderItem(locationId: string, orderId: string, menuItemId: string, quantity: number): Promise<POSOrder> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = {
          itemGuid: menuItemId,
   ...
   */
  async removeOrderItem(locationId: string, orderId: string, lineItemId: string): Promise<POSOrder> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        await this.makeRequest(`/checks/${orderId}/lineItems/${lineItemId}`, 'DELETE');
        return this.getOrder(locationId, orderId);
      });
    });
  }
   ...
   * Split payment
   */
  async splitPayment(locationId: string, orderId: string, amount: number): Promise<POSPayment> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = {
          checkGuid: orderId,
  ...
  async syncMenu(locationId: string, options?: MenuSyncOptions): Promise<void> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const categoriesResponse = await this.makeRequest(`/locations/${locationId}/menuCategories`, 'GET');
        const categories = Array.isArray(categoriesResponse) ? categoriesResponse : categoriesResponse.records || [];
   ...
   */
  async createMenuCategory(locationId: string, name: string, sortOrder?: number): Promise<{ id: string; name: string }> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = { name, sortOrder: sortOrder || 0 };
        const response = await this.makeRequest(`/locations/${locationId}/menuCategories`, 'POST', payload);
   ...
   * Create menu item
   */
  async createMenuItem(locationId: string, item: { name: string; categoryId: string; price: number; cost?: number }): Promise<POSMenuItem> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = {
          name: item.name,
        ...
        const response = await this.makeRequest(`/locations/${locationId}/menuItems`, 'POST', payload);
        this.invalidateMenuCache(locationId);
   ...
   */
  async updateMenuItemPrice(locationId: string, menuItemId: string, price: number, daypart?: string): Promise<POSMenuItem> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload: Record<string, any> = { price: this.formatAmount(price) };
        if (daypart) {
          payload.daypart = daypart;
        }

        const response = await this.makeRequest(`/locations/${locationId}/menuItems/${menuItemId}`, 'PUT', payload);
        this.invalidateMenuCache(locationId);
        return this.mapToastMenuItemToPOS(response, locationId);
      });
    });
   ...
   */
  async chargePayment(locationId: string, amount: number, method: 'card' | 'cash' | 'gift_card'): Promise<POSPayment> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = {
          amount: this.formatAmount(amount),
   ...
   * Refund payment
   */
  async refundPayment(locationId: string, paymentId: string, amount: number): Promise<POSPayment> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
  ...
  async checkGiftCardBalance(giftCardNumber: string): Promise<number> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const response = await this.makeRequest(`/giftCards/${giftCardNumber}/balance`, 'GET');
   ...
   * Reload gift card
   */
  async reloadGiftCard(giftCardNumber: string, amount: number): Promise<{ balance: number }> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = { amount: this.formatAmount(amount) };
        const response = await this.makeRequest(`/giftCards/${giftCardNumber}/reload`, 'POST', payload);
   ...
   * Redeem gift card
   */
  async redeemGiftCard(giftCardNumber: string, amount: number): Promise<{ balance: number }> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = { amount: this.formatAmount(amount) };
        const response = await this.makeRequest(`/giftCards/${giftCardNumber}/redeem`, 'POST', payload);
        return { balance: this.formatAmount(response.balance || 0) };
      });
    });
  }
  ...
  async closeShift(locationId: string, shiftId: string): Promise<POSShift> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const response = await this.makeRequest(`/shifts/${shiftId}/end`, 'POST');
   ...
   * Clock in time entry
   */
  async clockIn(employeeId: string, locationId: string): Promise<{ shiftId: string; timestamp: Date }> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
   ...
   */
  async getLaborCost(locationId: string, startDate: Date, endDate: Date): Promise<{ totalCost: number; laborCostPercentage: number }> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
   ...
   */
  async getDailySalesReport(locationId: string, date: Date): Promise<{ grossSales: number; netSales: number; taxAmount: number; discountAmount: number }> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const dateStr = date.toISOString().split('T')[0];
        const response = await this.makeRequest(`/locations/${locationId}/reports/sales?date=${dateStr}`, 'GET');
        ...
        return {
          ...
          discountAmount: this.formatAmountToCents(response.discountAmount || 0),
        };
      });
    });
  }
   ...
   */
  async getWeeklySalesReport(locationId: string, startDate: Date, endDate: Date): Promise<any> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const params = new URLSearchParams({
          startDate: startDate.toISOString(),
        ...
        const response = await this.makeRequest(`/locations/${locationId}/reports/sales/weekly?${params}`, 'GET');
        return {
          totalSales: this.formatAmountToCents(response.totalSales || 0),
          daysReporting: response.daysReporting || 0,
          averageDailySales: this.formatAmountToCents((response.totalSales || 0) / (response.daysReporting || 1)),
        };
      });
    });
  }
   ...
   */
  async getProductMixReport(locationId: string, startDate: Date, endDate: Date): Promise<Array<{ name: string; quantity: number; sales: number }>> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const params = new URLSearchParams({
          startDate: startDate.toISOString(),
        ...
        const response = await this.makeRequest(`/locations/${locationId}/reports/productMix?${params}`, 'GET');
        const records = Array.isArray(response) ? response : response.records || [];
   ...
   */
  async getCompVoidReport(locationId: string, startDate: Date, endDate: Date): Promise<{ comps: number; voids: number; totalAmount: number }> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const params = new URLSearchParams({
          startDate: startDate.toISOString(),
        ...
        const response = await this.makeRequest(`/locations/${locationId}/reports/compVoid?${params}`, 'GET');
        return {
   ...
   */
  async markItemPrepared(locationId: string, orderId: string, lineItemId: string): Promise<void> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        await this.makeRequest(`/checks/${orderId}/lineItems/${lineItemId}/prepared`, 'POST');
      });
    });
  }
   ...
   */
  async getKitchenTimingAnalytics(locationId: string, startDate: Date, endDate: Date): Promise<{ avgPrepTime: number; totalOrders: number }> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const params = new URLSearchParams({
          startDate: startDate.toISOString(),
        ...
        const response = await this.makeRequest(`/locations/${locationId}/kitchen/analytics?${params}`, 'GET');
        return {
  ...
  async getRevenueCenter(locationId: string, revenueCenterId: string): Promise<POSRevenueCenter> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const response = await this.makeRequest(`/locations/${locationId}/revenueCenters/${revenueCenterId}`, 'GET');
   ...
   * Open table
   */
  async openTable(locationId: string, tableNumber: string, partySize: number): Promise<{ tableId: string; status: string }> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = { tableName: tableNumber, partySize };
        const response = await this.makeRequest(`/locations/${locationId}/tables`, 'POST', payload);
  ...
  async closeTable(locationId: string, tableId: string): Promise<void> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        await this.makeRequest(`/locations/${locationId}/tables/${tableId}`, 'DELETE');
      });
    });
  }
   ...
   * Transfer table
   */
  async transferTable(locationId: string, tableId: string, newTableId: string): Promise<void> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = { newTableId };
        await this.makeRequest(`/locations/${locationId}/tables/${tableId}/transfer`, 'POST', payload);
      });
    });
  }
   ...
   * Merge tables
   */
  async mergeTables(locationId: string, fromTableId: string, toTableId: string): Promise<void> {
    return this.enqueueRequest(async () => {
      return this.executeWithRetries(async () => {
        const payload = { toTableId };
        await this.makeRequest(`/locations/${locationId}/tables/${fromTableId}/merge`, 'POST', payload);
      });
    });
  }
  ...
  private async makeRequest(
    ...
    try {
      ...
      if (response.headers.has('X-RateLimit-Remaining')) {
        this.rateLimitInfo.remaining = parseInt(response.headers.get('X-RateLimit-Remaining') || '500', 10);
      }
      if (response.headers.has('X-RateLimit-Reset')) {
        this.rateLimitInfo.reset = parseInt(response.headers.get('X-RateLimit-Reset') || Date.now().toString(), 10);
      }
  ...
  private createError(code: string, message: string, details?: Record<string, any>): Error & { code: string; retryable: boolean } {
    ...
    error.code = code;
    error.retryable = !['INVALID_ORDER', 'UNAUTHORIZED', 'NOT_FOUND'].includes(code);
    return error;
  }
  ...
  private mapToastCheckToPOS(check: any, locationId: string): POSOrder {
    const totals = this.calculateOrderTotals({
      lineItems: (check.tab?.lineItems || []).map((li: any) => ({
        ...
        modifiers: li.modifiers?.map((m: any) => ({ price: this.formatAmountToCents(m.price || 0) })),
      })),
    ...
    return {
      ...
      lineItems: (check.tab?.lineItems || []).map((li: any) => ({
        ...
        modifiers: li.modifiers?.map((m: any) => ({ modifierId: m.guid, price: this.formatAmountToCents(m.price) })),
      })),
