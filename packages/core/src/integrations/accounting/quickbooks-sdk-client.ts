...
export class QuickBooksSDKClient {
  ...
  async exchangeAuthorizationCode(
    ...
    try {
      const response = await this.request(
        "POST",
        `${this.authBaseUrl}/oauth2/v1/tokens/bearer`,
        {
          headers: {
  ...
  async revokeToken(token: string): Promise<void> {
    ...
    try {
      await this.request(
        "POST",
        `${this.authBaseUrl}/oauth2/v1/tokens/revoke`,
        {
          headers: {
            Authorization: `Basic ${auth}`,
            "Content-Type": "application/x-www-form-urlencoded",
  ...
  async downloadInvoicePDF(
    ...
    const response = await this.request(
      "GET",
      `${this.apiBaseUrl}/company/${realmId}/invoice/${invoiceId}/pdf`,
      {
        headers: {
  ...
  async queryCustomers(
    ...
    const qr = response.QueryResponse as
      | { Customer?: QBCustomer[] }
      | undefined;
  ...
  async sendEstimateEmail(
    ...
    return {
      success: response.status === "success" || response.id === estimateId,
    };
  }
    ...
    for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        if (contentType?.includes("application/pdf")) {
          return (await response.arrayBuffer()) as unknown as Record<
            string,
            unknown
          >;
        }
