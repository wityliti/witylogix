...
import { QuickBooksAdapter, createQuickBooksConfig } from '../quickbooks-adapter.js';
...
describe('QuickBooksAdapter', () => {
  ...
  describe('OAuth Flow', () => {
    ...
    it('should authenticate with authorization code', async () => {
      const connection = await adapter.authenticate('auth_code_123', 'realm_456');
    ...
    it('should throw error when refreshing without token', async () => {
      ...
      await expect(adapter.refreshToken(connection)).rejects.toThrow('No refresh token available');
    });
  });
  ...
  describe('Invoice Sync', () => {
    ...
    it('should throw error when QB account not configured', async () => {
      ...
      await expect(adapter.createInvoice(mockInvoice, invalidConnection)).rejects.toThrow(
    ...
    it('should handle invoice with no line items', async () => {
      ...
      const result = await adapter.createInvoice(invoiceNoItems, mockConnection);
  ...
  describe('Payment Sync', () => {
    ...
    it('should sync payment to QB', async () => {
      const result = await adapter.syncPayment(mockInvoice, mockPayment, mockConnection);
  ...
  describe('Customer Management', () => {
    ...
    it('should throw error when QB account not configured', async () => {
      ...
      await expect(
        adapter.getOrCreateCustomer('john@example.com', 'John Doe', invalidConnection),
      ).rejects.toThrow('QB account ID not configured');
    });
  ...
  describe('Line Items Sync', () => {
    ...
    it('should throw error when QB account not configured', async () => {
      ...
      await expect(adapter.syncLineItems('inv_123', invalidConnection)).rejects.toThrow(
        'QB account ID not configured',
      );
    });
  });
  ...
  describe('Accounting Status', () => {
    ...
    it('should get accounting status', async () => {
      const status = await adapter.getAccountingStatus('inv_123', mockConnection);
