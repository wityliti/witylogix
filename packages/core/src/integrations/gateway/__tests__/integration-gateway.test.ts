...
describe('IntegrationGateway', () => {
  ...
  describe('HTTP methods', () => {
    ...
    it('should make POST request', async () => {
      const result = await gateway.post('test-provider', '/users', { name: 'John' });
    ...
    it('should make PUT request', async () => {
      const result = await gateway.put('test-provider', '/users/123', { name: 'Jane' });

      expect(result).toEqual({ message: 'ok' });
    });

    it('should make PATCH request', async () => {
      const result = await gateway.patch('test-provider', '/users/123', { status: 'active' });
  ...
  describe('Interceptors', () => {
    ...
    it('should run error interceptors', async () => {
      ...
      gatewayWithInterceptor.registerProvider({
        ...
        baseUrl: 'https://api.example.com',
        retry: { maxRetries: 0, initialBackoffMs: 100, maxBackoffMs: 1000, multiplier: 2, retryableStatusCodes: [] },
      });
