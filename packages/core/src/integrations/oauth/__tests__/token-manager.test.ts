...
import { OAuth2TokenManager, type OAuth2Config, type TokenManagerConfig } from '../token-manager.js';
...
describe('OAuth2TokenManager', () => {
  ...
  describe('getToken', () => {
    ...
    it('should throw when no token available', async () => {
      await expect(manager.getToken()).rejects.toThrow(/No valid token available/);
    });
  });
