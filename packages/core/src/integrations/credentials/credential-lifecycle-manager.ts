...
export class RotationScheduler extends EventEmitter {
  ...
  private scheduleRotation(policy: RotationPolicy): void {
    ...
    const timeoutId = setTimeout(() => {
      this.performRotation(policy.credentialId, "scheduled", policy).catch(
        (error) => {
    ...
    switch (schedule.frequency) {
      ...
      case "weekly":
        const daysUntilWeekly =
          ((schedule.dayOfWeek || 0) - now.getDay() + 7) % 7;
        return new Date(
          now.getTime() + (daysUntilWeekly || 7) * 24 * 60 * 60 * 1000,
        );
      case "monthly":
    ...
    try {
      ...
      if (policy.preRotationValidation) {
        const validationResult =
          await this.validateCredentialBeforeRotation(credentialId);
        if (!validationResult) {
  ...
  private async validateCredentialBeforeRotation(
    credentialId: string,
  ): Promise<boolean> {
    try {
  ...
  private async verifyRotatedCredential(
    credentialId: string,
  ): Promise<boolean> {
    try {
      const credential = await this.vault.get(credentialId);
...
export class SecretScanner extends EventEmitter {
  ...
  async scanContent(content: string, scanId?: string): Promise<ScanResult> {
    ...
    try {
      ...
      scan.findings.push(...this.scanForHighEntropySecrets(content, scan.id));
      ...
      this.emit("scan_completed", {
        scanId: scan.id,
        findingsCount: scan.secretsFound,
      });
      return scan;
    } catch (error) {
      scan.status = "failed";
  ...
  classifyFinding(finding: SecretScanFinding): Severity {
    ...
    if (
      finding.pattern.includes("api_key") ||
      finding.pattern.includes("secret_key")
    ) {
      return "critical";
    }
  ...
  private scanForHighEntropySecrets(
    content: string,
    scanId: string,
  ): SecretScanFinding[] {
...
export class ZeroDowntimeRotator extends EventEmitter {
  private activeShifts: Map<
    string,
    { startedAt: Date; currentPercentage: number }
  > = new Map();
  ...
  async rotateWithTrafficShift(
    ...
    try {
      ...
      for (const stage of this.trafficShiftConfig.stages) {
        ...
        this.activeShifts.set(rotationId, {
          startedAt: new Date(),
          currentPercentage: stage.percentage,
        });
        this.emit("traffic_shift", {
          rotationId,
        ...
        await this.monitorHealth(
          rotationId,
          this.trafficShiftConfig.healthCheckIntervalMs,
        );
...
export class CredentialHealthScorer extends EventEmitter {
  ...
  calculateHealth(
    ...
    return {
      ...
      usageScore,
      lastRotationDays,
      scanFindingsCount: scanFindings.filter(
        (f) => f.remediationStatus === "pending",
      ).length,
      rotationStatus: "pending",
      nextRotationDue: this.calculateNextRotationDue(
        credential,
        rotationPolicy,
      ),
  ...
  private calculateUsageScore(usageData?: {
    lastUsedAt?: Date;
    accessCount?: number;
  }): number {
    ...
    const unusedDays =
      (Date.now() - usageData.lastUsedAt.getTime()) / (24 * 60 * 60 * 1000);
  ...
  private calculateNextRotationDue(
    credential: Credential,
    policy: RotationPolicy | null,
  ): Date | undefined {
  ...
  private generateRecommendations(
    ...
    if (ageDays > 90) {
      recommendations.push(
        "Credential is older than 90 days - schedule immediate rotation",
      );
    }
    ...
    if (criticalFindings.length > 0) {
      recommendations.push(
        `${criticalFindings.length} critical exposures detected - rotate immediately`,
      );
    }

    if (!policy) {
      recommendations.push(
        "No rotation policy configured - set up automated rotation",
      );
    }

    recommendations.push(
      "Review access patterns and revoke if unused for 30+ days",
    );
