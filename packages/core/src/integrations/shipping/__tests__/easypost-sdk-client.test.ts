...
describe('EasyPostSdkClient', () => {
  ...
  describe('Configuration', () => {
    it('should validate configuration successfully', async () => {
      vi.mocked(mockGateway.get).mockResolvedValueOnce({ user: { id: 'user_test' } });
    ...
    it('should return false on validation failure', async () => {
      vi.mocked(mockGateway.get).mockRejectedValueOnce(new Error('Invalid credentials'));
  ...
  describe('Addresses API', () => {
    it('should create address', async () => {
      vi.mocked(mockGateway.post).mockResolvedValueOnce({ address: mockAddress });

      const address = await client.addresses.create({ name: 'John Doe', street1: '123 Main St' });

      expect(address).toEqual(mockAddress);
      expect(mockGateway.post).toHaveBeenCalledWith('easypost', '/addresses', expect.any(Object));
    });

    it('should verify address', async () => {
      vi.mocked(mockGateway.get).mockResolvedValueOnce({ address: mockAddress });
    ...
    it('should detect residential address', async () => {
      ...
      vi.mocked(mockGateway.get).mockResolvedValueOnce({ address: residentialAddress });

      const isResidential = await client.addresses.detectResidential({ id: 'adr_test123' });
  ...
  describe('Parcels API', () => {
    it('should create parcel with dimensions', async () => {
      ...
      const parcel = await client.parcels.create({ weight: 16, length: 12, width: 8, height: 6 });
  ...
  describe('Shipments API', () => {
    it('should create shipment', async () => {
      vi.mocked(mockGateway.post).mockResolvedValueOnce({ shipment: mockShipment });
    ...
    it('should get rates for shipment', async () => {
      vi.mocked(mockGateway.get).mockResolvedValueOnce({ shipment: mockShipment });
    ...
    it('should buy rate and create label', async () => {
      ...
      vi.mocked(mockGateway.post).mockResolvedValueOnce({ shipment: boughtShipment });

      const result = await client.shipments.buyRate('shp_test123', 'rate_test123');
    ...
    it('should refund shipment', async () => {
      ...
      vi.mocked(mockGateway.post).mockResolvedValueOnce({ shipment: refundedShipment });
  ...
  describe('Tracking API', () => {
    it('should create tracker', async () => {
      vi.mocked(mockGateway.post).mockResolvedValueOnce({ tracker: mockTracker });
    ...
    it('should get tracking info', async () => {
      vi.mocked(mockGateway.get).mockResolvedValueOnce({ tracker: mockTracker });
    ...
    it('should register webhook', async () => {
      ...
      const webhook = await client.tracking.registerWebhook('https://example.com/webhook');
  ...
  describe('Webhooks API', () => {
    it('should verify valid HMAC signature', () => {
      ...
      const hmac = require('crypto').createHmac('sha256', mockConfig.webhookSecret!);
      hmac.update(payload);
    ...
    it('should reject invalid HMAC signature', () => {
      ...
      const isValid = client.webhooks.verifySignature(payload, invalidSignature);
    ...
    it('should throw on invalid webhook signature', () => {
      ...
      expect(() => client.webhooks.parse(payload, signature)).toThrow('Invalid webhook signature');
    });
  });
  ...
  describe('Batch API', () => {
    ...
    it('should reject batch exceeding 10K shipments', async () => {
      ...
      await expect(client.batch.create(largeArray)).rejects.toThrow('cannot contain more than 10,000');
    });

    it('should buy batch', async () => {
      const purchasedBatch = { ...mockBatch, state: 'purchased' };
      vi.mocked(mockGateway.post).mockResolvedValueOnce({ batch: purchasedBatch });
    ...
    it('should generate scan form', async () => {
      ...
      vi.mocked(mockGateway.post).mockResolvedValueOnce({ scan_form: scanForm });
