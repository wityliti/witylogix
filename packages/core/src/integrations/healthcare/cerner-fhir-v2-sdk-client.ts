/**
 * Oracle Health/Cerner FHIR R4 v2 SDK Client.
 *
 * Production-grade implementation for Oracle Health (formerly Cerner) FHIR R4 API.
 * Supports OAuth2 system/patient scopes, SMART app launch, scheduling resources, and bulk exports.
 *
 * Features:
 * - OAuth2 authentication (system, patient, user scopes)
 * - SMART app launch support
 * - Comprehensive FHIR resources (Patient, Encounter, Observation, Condition, etc.)
 * - Appointment, Schedule, Slot management
 * - RelatedPerson, CareTeam, Goal resources
 * - Bulk data export ($export operation)
 * - Rate limiting (20 req/sec), retry logic, circuit breaker
...
/**
 * Cerner FHIR v2 SDK Client.
 */
export class CernerFhirV2SdkClient extends EventEmitter {
  ...
  /**
   * Get system scope token (client credentials).
   */
  async getSystemToken(): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(`System token failed: ${response.statusText}`);
    }
  ...
  private async ensureToken(): Promise<void> {
    if (!this.accessToken || Date.now() >= this.tokenExpiresAt) {
      if (this.refreshToken) {
        ...
        throw new Error(
          "No valid token available. Call exchangeCodeForToken or getSystemToken first.",
        );
      }
    }
  }
  ...
  /**
   * Private: Make request with rate limiting and retries.
   */
  ...
  private createError(
    ...
    const error = new HealthcareSDKError(code, statusCode, "cerner", message);
