...
import type { CRMConnection, CRMContact, CRMAccount, CRMDeal } from '../types.js';
...
describe('HubSpotAdapter', () => {
  ...
  describe('Contact Operations', () => {
    ...
    it('should search contacts', async () => {
      ...
      const result = await adapter.search('John Doe', 'contacts', { limit: 10 });
  ...
  describe('Error Handling', () => {
    it('should handle refresh token failures', async () => {
      ...
      await expect(adapterNoToken.refreshToken()).rejects.toThrow('No refresh token available');
    });
  });
});
