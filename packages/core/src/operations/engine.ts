...
type JsonInput = string | number | boolean | null | { [k: string]: JsonInput } | JsonInput[]
...
type ActivityFlow = Awaited<ReturnType<typeof prisma.activityFlow.findUnique>> & {}
...
export function validateGraph (graph: FlowGraph): FlowValidationError[] {
  ...
  for (const stage of graph.stages) {
    ...
    if (keys.has(stage.key)) {
      errors.push({ code: 'DUPLICATE_KEY', message: `Duplicate stage key: ${stage.key}` })
    }
    keys.add(stage.key)
...
export function findStage (graph: FlowGraph, key: string): FlowStage | undefined {
...
export function listAllowedNextStages (graph: FlowGraph, currentKey: string): FlowStage[] {
...
export async function createFlow (input: CreateFlowInput): Promise<ActivityFlow> {
  ...
  return prisma.$transaction(async (tx) => {
    if (input.isDefault) {
      await tx.activityFlow.updateMany({
        where: { shopId: input.shopId, entityType: input.entityType, isDefault: true },
...
export async function updateFlow (input: UpdateFlowInput): Promise<ActivityFlow> {
  if (input.graph) {
    const errs = validateGraph(input.graph)
    if (errs.length > 0) throw new FlowValidationException(errs)
  }
  return prisma.$transaction(async (tx) => {
    const existing = await tx.activityFlow.findUnique({ where: { id: input.id } })
...
export async function getFlow (id: string, shopId: string): Promise<ActivityFlow | null> {
...
export async function transitionShipment (input: TransitionInput): Promise<TransitionResult> {
...
async function transitionEntity (
  ...
  if (!entity) {
    throw new FlowNotFoundError(`${entityType.toLowerCase()} ${entityId} not found`)
  }
  ...
  const fromKey: string | null = entity.currentStageKey ?? initialStage(graph)?.key ?? null
  if (!fromKey) {
  ...
  if (!fromStage) {
    throw new FlowTransitionError(`Current stage "${fromKey}" is not part of the flow`)
  }
  if (fromStage.isTerminal) {
    throw new FlowTransitionError(`Cannot transition out of terminal stage "${fromKey}"`)
  }
  ...
  const eventType = entityType === 'SHIPMENT' ? 'shipment.stage_changed' : 'order.stage_changed'
...
async function loadEntity (
  ...
  if (entityType === 'SHIPMENT') {
    const row = await prisma.shipment.findUnique({
      where: { id },
      select: { id: true, shopId: true, activityFlowId: true, currentStageKey: true },
    })
    return row ?? null
  }
...
async function resolveFlowForEntity (
  ...
  if (entity.activityFlowId) {
    return prisma.activityFlow.findUnique({ where: { id: entity.activityFlowId } })
  }
...
export async function listAllowedNextForEntity (
  entityType: ActivityEntityType,
  entityId: string,
): Promise<{ flow: ActivityFlow; currentStage: string | null; next: FlowStage[] } | null> {
  const entity = await loadEntity(entityType, entityId)
...
export class FlowValidationException extends Error {
  constructor (public readonly errors: FlowValidationError[]) {
    super(`Activity flow graph is invalid: ${errors.map((e) => e.message).join('; ')}`)
