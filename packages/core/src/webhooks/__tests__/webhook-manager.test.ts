...
describe("Webhook Manager", () => {
  ...
  describe("endpoint registration", () => {
    ...
    it("should accept custom secret", () => {
      ...
      const id = manager.registerEndpoint(
  ...
  describe("event matching (exact and wildcard)", () => {
    it("should match exact event type", () => {
      ...
      const endpoint = manager.getEndpoint(id)!;
      expect(
        manager.matchesFilter("order.created", endpoint.eventFilters),
      ).toBe(true);
    });
    ...
    it("should match wildcard event", () => {
      ...
      expect(
        manager.matchesFilter("order.confirmed", endpoint.eventFilters),
      ).toBe(true);
    });
    ...
    it("should not match non-matching event", () => {
      ...
      const endpoint = manager.getEndpoint(id)!;
      expect(
        manager.matchesFilter("delivery.started", endpoint.eventFilters),
      ).toBe(false);
    });
  ...
  describe("delivery status tracking", () => {
    it("should record successful delivery", () => {
      ...
      const delivery = manager.recordDelivery(
        id,
        "order.created",
        "delivered",
        200,
  ...
  describe("endpoint management", () => {
    ...
    it("should update endpoint", () => {
      ...
      const updated = manager.updateEndpoint(id, {
        eventFilters: ["order.*", "delivery.*"],
      });

      expect(updated).toBe(true);
      expect(manager.getEndpoint(id)?.eventFilters).toEqual([
        "order.*",
        "delivery.*",
      ]);
    });
