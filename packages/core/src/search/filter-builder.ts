...
function getDateRange(shortcut: DateShortcut): [Date, Date] {
  ...
  switch (shortcut) {
    ...
    case "this_week":
      ...
      start = new Date(startOfDay.getTime() - (dayOfWeek || 7) * 86400000 + 86400000); // Monday
      end = new Date(startOfDay.getTime() + (7 - (dayOfWeek || 7)) * 86400000 + 86400000); // Next Monday
      break;
...
export class FilterBuilder {
  ...
  private buildSingleFilter(filter: FilterRule, alias: string): string {
    ...
    switch (filter.operator) {
      case "eq":
        ...
        return filter.value === null ? `${fieldRef} IS NULL` : `${fieldRef} = ${paramRef}`;
      ...
      case "in":
        this.params.push(Array.isArray(filter.value) ? filter.value : [filter.value]);
   ...
   */
  static mergeFilters(...filters: (FilterRule | CompositeFilter)[]): CompositeFilter {
    return {
      AND: filters,
    };
  }
   ...
   */
  static orFilters(...filters: (FilterRule | CompositeFilter)[]): CompositeFilter {
    return {
      OR: filters,
    };
  }
