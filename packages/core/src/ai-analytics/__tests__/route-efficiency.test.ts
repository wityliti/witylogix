...
describe('Route Efficiency Scorer', () => {
  ...
  describe('Route Deviations', () => {
    it('should detect deviations from planned path', () => {
      ...
      const result = calculateRouteEfficiency('route_1', planned, deviatingTrace);
  ...
  describe('Batch Processing', () => {
    it('should calculate efficiency for multiple routes', () => {
      const routes = [
        { routeId: 'route_1', planned: createMockPlannedRoute(), gpsTrace: createMockGPSTrace() },
        { routeId: 'route_2', planned: createMockPlannedRoute(), gpsTrace: createMockGPSTrace() },
        { routeId: 'route_3', planned: createMockPlannedRoute(), gpsTrace: createMockGPSTrace() },
      ];

      const result = calculateRouteEfficiencyBatch(routes);

      expect(result.scores).toHaveLength(3);
      expect(result.scores.every((s) => s.score >= 0 && s.score <= 100)).toBe(true);
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle zero planned distance', () => {
      ...
      const result = calculateRouteEfficiency('route_1', planned, createMockGPSTrace());
