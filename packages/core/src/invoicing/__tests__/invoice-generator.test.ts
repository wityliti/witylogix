...
describe('InvoiceGenerator', () => {
  ...
  describe('generateInvoice', () => {
    it('should generate invoice from delivery contexts', async () => {
      const config = createInvoiceGeneratorConfig(
        ...
        new Date('2026-01-31'),
        [
          createBillingRule('rule-1', 'tenant-1', 'Per Delivery Charge', 'per-delivery', {
            baseAmount: 50,
            isActive: true,
          }),
        ],
    ...
    it('should calculate line items from billing rules', async () => {
      const config = createInvoiceGeneratorConfig(
        ...
        new Date('2026-01-31'),
        [
          createBillingRule('rule-1', 'tenant-1', 'Per Mile Charge', 'per-mile', {
            unitRate: 2.5,
            isActive: true,
          }),
          createBillingRule('rule-2', 'tenant-1', 'Per Kilogram Charge', 'per-kg', {
            unitRate: 0.5,
            isActive: true,
          }),
        ],
    ...
    it('should apply discounts correctly', async () => {
      const config = createInvoiceGeneratorConfig(
        ...
        new Date('2026-01-31'),
        [
          createBillingRule('rule-1', 'tenant-1', 'Base Charge', 'per-delivery', {
            baseAmount: 100,
            isActive: true,
          }),
        ],
    ...
    it('should set default due date to 30 days from now', async () => {
      const config = createInvoiceGeneratorConfig(
        ...
        new Date('2026-01-01'),
        new Date('2026-01-31'),
        [createBillingRule('rule-1', 'tenant-1', 'Charge', 'per-delivery', { baseAmount: 50, isActive: true })],
      );

      const invoice = await generator.generateInvoice(config, [{ deliveryId: 'del-1' }]);
  ...
  describe('finalizeInvoice', () => {
    it('should finalize draft invoice with number', async () => {
      ...
      const finalized = await generator.finalizeInvoice(invoice, 'INV-2026-00001');
    ...
    it('should throw error when finalizing non-draft invoice', async () => {
      ...
      await expect(generator.finalizeInvoice(invoice, 'INV-2026-00002')).rejects.toThrow();
    });
  });
  ...
  describe('markAsSent', () => {
    ...
    it('should throw error when sending draft invoice', async () => {
      ...
      await expect(generator.markAsSent(invoice)).rejects.toThrow('Cannot send draft invoice');
    });
  });
  ...
  describe('voidInvoice', () => {
    ...
    it('should throw error when voiding already voided invoice', async () => {
      ...
      await expect(generator.voidInvoice(invoice)).rejects.toThrow('already voided');
    });
  });
      ...
      const invoices: Invoice[] = [
        {
          ...
          discounts: [],
          taxes: [],
          payments: [{ id: 'pay-1', invoiceId: 'inv-1', amount: 110, method: 'bank_transfer', paidAt: new Date(), createdAt: new Date() }],
          createdAt: new Date(),
          updatedAt: new Date(),
        },
        {
