  ...
  async executeWorkflow(execution: WorkflowExecution): Promise<void> {
    try {
      ...
      if (this.workflowEngine && this.workflowEngine.execute) {
        execution.output = await this.workflowEngine.execute(execution.workflowType, execution.input);
      } else {
...
describe('Workflow Integration', () => {
  ...
  describe('auto-trigger on order creation', () => {
    it('should auto-trigger workflow in auto mode', async () => {
      ...
      const executionId = await integration.triggerWorkflow('process_order', { orderId: '123' }, 'tenant_1');
  ...
  describe('tenant config: auto vs manual vs disabled', () => {
    ...
    it('should check auto-trigger eligibility', () => {
      ...
      expect(integration.shouldAutoTrigger('order.created', 'tenant_1')).toBe(true);
      expect(integration.shouldAutoTrigger('delivery.started', 'tenant_1')).toBe(false);
    });
  });
  ...
  describe('graceful fallback on workflow engine failure', () => {
    it('should handle workflow engine failure gracefully', async () => {
      ...
      const executionId = await integrationWithEngine.triggerWorkflow('process_order', { orderId: '123' }, 'tenant_1');
    ...
    it('should emit failure event on engine error', async () => {
      ...
      expect(eventEmitter.emit).toHaveBeenCalledWith('workflow.failed', expect.any(Object));
    });
  });
  ...
  describe('event emission on workflow state changes', () => {
    it('should emit workflow.completed event', async () => {
      ...
      expect(eventEmitter.emit).toHaveBeenCalledWith('workflow.completed', expect.objectContaining({
    ...
    it('should emit workflow.failed event on error', async () => {
      ...
      expect(eventEmitter.emit).toHaveBeenCalledWith('workflow.failed', expect.objectContaining({
        workflowId: executionId,
        error: 'Process error',
      }));
    });
  ...
  describe('concurrent workflow triggers', () => {
    it('should handle multiple concurrent triggers', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        promises.push(
          integration.triggerWorkflow('process_order', { orderId: `${i}` }, 'tenant_1')
        );
      }
    ...
    it('should isolate executions per tenant', async () => {
      ...
      await integration.triggerWorkflow('process_order', { orderId: '1' }, 'tenant_1');
      await integration.triggerWorkflow('process_order', { orderId: '2' }, 'tenant_2');
