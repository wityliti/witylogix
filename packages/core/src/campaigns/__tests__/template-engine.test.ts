...
class TemplateEngine {
  ...
  validate(template: string): { valid: boolean; errors: string[] } {
    ...
    if (ifCount !== endIfCount) {
      errors.push(
        `Mismatched if/endif tags: ${ifCount} opening, ${endIfCount} closing`,
      );
    }
    ...
    if (eachCount !== endEachCount) {
      errors.push(
        `Mismatched each/endeach tags: ${eachCount} opening, ${endEachCount} closing`,
      );
    }
  ...
  parseTemplate(template: string): ParsedTemplate[] {
    ...
    while (position < template.length) {
      ...
      const ifMatch = this.conditionalPattern.exec(
        template.substring(position),
      );
      ...
      const nextMatches = [
        varMatch
          ? {
              match: varMatch,
              type: "variable",
              pos: position + varMatch.index,
            }
          : null,
        ifMatch
          ? { match: ifMatch, type: "condition", pos: position + ifMatch.index }
          : null,
        loopMatch
          ? { match: loopMatch, type: "loop", pos: position + loopMatch.index }
          : null,
      ].filter((m) => m !== null);
...
describe("TemplateEngine", () => {
  ...
  describe("Conditional Blocks", () => {
    ...
    it("should handle nested content in conditionals", () => {
      const template = "{{#if hasDiscount}}Discount: {{discount}}% off{{/if}}";
  ...
  describe("Template Validation", () => {
    ...
    it("should detect empty variable names", () => {
      ...
      const validation = engine.validate(template);
      expect(validation.valid).toBe(false);
      expect(validation.errors.some((e) => e.includes("empty variable"))).toBe(
        true,
      );
    });
