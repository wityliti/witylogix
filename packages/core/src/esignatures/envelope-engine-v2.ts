...
export class EnvelopeManager {
  ...
  async createEnvelope(
    ...
    const envelope: Envelope = {
      ...
      requireAcceptanceBeforeSigning:
        data.requireAcceptanceBeforeSigning ?? false,
  ...
  async cloneEnvelope(
    ...
    const cloned = await this.createEnvelope(original.tenantId, userId, {
      ...
      signers: JSON.parse(JSON.stringify(original.signers)).map(
        (s: Signer) => ({
          ...s,
  ...
  async listEnvelopes(
    ...
    let results = Array.from(this.envelopes.values()).filter(
    ...
    if (filters?.createdByUserId) {
      results = results.filter(
        (e) => e.createdByUserId === filters.createdByUserId,
      );
    }
  ...
  async addSigner(envelopeId: string, signer: Signer): Promise<Signer> {
    ...
    if (envelope.status !== EnvelopeStatus.DRAFT) {
      throw new Error(
        `Cannot add signer to envelope in ${envelope.status} status`,
      );
    }
  ...
  async addDocument(
    ...
    if (envelope.status !== EnvelopeStatus.DRAFT) {
      throw new Error(
        `Cannot add document to envelope in ${envelope.status} status`,
      );
    }
...
export class EnvelopeValidator {
   ...
   */
  validateFieldPlacement(envelope: Envelope): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    for (const field of envelope.fields || []) {
      ...
      if (!doc) {
        errors.push(
          `Field ${field.id}: Document ${field.documentId} not found`,
        );
        continue;
      }
      ...
      if (field.pageNumber < 1 || field.pageNumber > doc.pageCount) {
        errors.push(
          `Field ${field.id}: Page ${field.pageNumber} out of range (1-${doc.pageCount})`,
        );
      }
   ...
   */
  validateSignerRouting(envelope: Envelope): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    for (let i = 0; i < sortedOrders.length; i++) {
      if (sortedOrders[i] !== i + 1) {
        errors.push(
          `Routing order gap: expected ${i + 1}, got ${sortedOrders[i]}`,
        );
      }
    }
    ...
    if (
      envelope.signers.filter((s) => s.role === SignerRole.SIGNER).length === 0
    ) {
  ...
  async processBatch(
    ...
    for (let i = 0; i < csvRows.length; i++) {
      ...
      try {
        ...
        const envelope = await this.substituteVariables(
          { templateId },
          variables,
        );
  ...
  async mergeEnvelopes(
    ...
    try {
      ...
      for (const envelopeId of sourceEnvelopeIds) {
        ...
        result.totalPageCount += envelope.documents.reduce(
        ...
        pageOffset += envelope.documents.reduce(
          (sum, d) => sum + d.pageCount,
          0,
        );
      }
      ...
      const newEnvelope = await envelopeManager.createEnvelope(
        tenantId,
        userId,
        {
