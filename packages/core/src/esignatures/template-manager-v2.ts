...
export class TemplateManager {
  ...
  async publishTemplate(templateId: string): Promise<Template> {
    ...
    const versions =
      this.templateVersions.get(template.previousVersionId || templateId) || [];
  ...
  async shareTemplate(
    ...
    template.sharedWith = [
      ...new Set([...(template.sharedWith || []), ...userIds]),
    ];
...
export class RoleManager {
  ...
  async defineRole(
    ...
    const role: SignerRoleDefinition = {
      ...
      authenticationMethod:
        roleData.authenticationMethod || AuthenticationMethod.EMAIL,
   ...
   */
  async listRolesByTemplate(
    templateId: string,
  ): Promise<SignerRoleDefinition[]> {
    const templateRoles = Array.from(this.roles.values()).filter(
  ...
  async updateRole(
    roleId: string,
    updates: Partial<SignerRoleDefinition>,
  ): Promise<SignerRoleDefinition> {
...
export class TemplateAnalyticsManager {
  ...
  async recordEnvelopeCompleted(
    ...
    if (analytics) {
      ...
      analytics.averageCompletionDays =
        (analytics.averageCompletionDays * (totalCompleted - 1) +
          completionDays) /
        totalCompleted;
      analytics.averageSigningTimeMinutes =
        (analytics.averageSigningTimeMinutes * (totalCompleted - 1) +
          signingTimeMinutes) /
        totalCompleted;
      ...
      analytics.completionRate =
        totalAttempts > 0 ? (totalCompleted / totalAttempts) * 100 : 0;
  ...
  async recordEnvelopeDeclined(
    ...
    if (analytics) {
      ...
      if (reason) {
        analytics.declineReasons[reason] =
          (analytics.declineReasons[reason] || 0) + 1;
      }
      ...
      analytics.declineRate =
        totalAttempts > 0
          ? (analytics.totalEnvelopesDeclined / totalAttempts) * 100
          : 0;
  ...
  async getAnalytics(
    templateId: string,
  ): Promise<TemplateAnalyticsType | null> {
  ...
  async getPeakUsageTime(
    templateId: string,
  ): Promise<{ day?: string; hour?: number } | null> {
    const analytics = this.analytics.get(templateId);
    return analytics
      ? { day: analytics.peakUsageDay, hour: analytics.peakUsageHour }
      : null;
  }
}
