...
describe("SquareGateway", () => {
  ...
  describe("createPaymentIntent", () => {
    ...
    it("should throw error on invalid amount", async () => {
      ...
      expect(gateway.createPaymentIntent("shop-123", 0, "USD")).rejects.toThrow(
        "Invalid amount",
      );
    });
  ...
  describe("captureWithSourceId", () => {
    ...
    it("should throw error when no source ID", async () => {
      expect(gateway.capturePayment("intent-id-123")).rejects.toThrow(
        "sourceId",
      );
    });
  });
  ...
  describe("getPaymentStatus", () => {
    it("should return transaction status", async () => {
      const transaction = await gateway.getPaymentStatus(
        "square-payment-id-123",
      );
      ...
      expect(transaction.status).toMatch(
        /pending|authorized|completed|cancelled|failed/,
      );
    });
  ...
  describe("Invoice Creation", () => {
    it("should create invoice for deferred payment", async () => {
      ...
      expect(invoice.dueDate.toISOString().split("T")[0]).toBe(
  ...
  describe("Idempotency", () => {
    it("should generate unique idempotency keys", async () => {
      const intent1 = await gateway.createPaymentIntent(
        "shop-123",
        5000,
        "USD",
      );
      ...
      const intent2 = await gateway.createPaymentIntent(
        "shop-123",
        5000,
        "USD",
      );
    ...
    it("should include shop ID in idempotency key", async () => {
      const intent = await gateway.createPaymentIntent("shop-456", 5000, "USD");
