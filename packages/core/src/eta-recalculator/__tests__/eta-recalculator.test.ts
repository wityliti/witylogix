...
const mockNotify = vi
  .fn<[NotificationTrigger], Promise<void>>()
...
function makeGPSUpdate(
  overrides: Partial<DriverLocationUpdate> = {},
): DriverLocationUpdate {
  return {
    driverId: "driver-1",
    shopId: "shop-abc",
...
describe("ETARecalculator", () => {
  ...
  describe("basic recalculation", () => {
    ...
    it("uses driver speed to estimate base travel time", async () => {
      const delivery = makeDelivery({
        destination: { lat: 40.72, lng: -74.005 }, // ~0.8km away
      });

      const slowUpdate = makeGPSUpdate({
        lat: 40.7128,
        lng: -74.006,
        speedKmh: 10,
      });
      const fastUpdate = makeGPSUpdate({
        lat: 40.7128,
        lng: -74.006,
        speedKmh: 60,
      });
      ...
      expect(slowResult.newEtaMinutes).toBeGreaterThan(
        fastResult.newEtaMinutes,
      );
    });
  ...
  describe("notification trigger — ETA delta > 5 minutes", () => {
    it("fires notification when ETA increases by more than 5 minutes", async () => {
      ...
      const update = makeGPSUpdate({
        lat: 40.6, // far south of destination at 40.758
        lng: -74.006,
        speedKmh: 15, // slow
      });
    ...
    it("does NOT fire notification when ETA changes by 4 minutes", async () => {
      const delivery = makeDelivery({
        ...
        destination: { lat: 40.72, lng: -74.003 }, // ~0.9km away
      });
    ...
    it("fires notification when ETA improves (decreases) by more than 5 minutes", async () => {
      const delivery = makeDelivery({
        ...
        destination: { lat: 40.715, lng: -74.004 }, // close
      });
    ...
    it("includes push channel in notification when pushToken is present", async () => {
      const delivery = makeDelivery({
        ...
        destination: { lat: 40.9, lng: -74.006 }, // far away
      });
    ...
    it("only sends SMS when no push token is provided", async () => {
      const delivery = makeDelivery({
        ...
        destination: { lat: 40.9, lng: -74.006 },
      });
  ...
  describe("batch processing", () => {
    it("processes multiple deliveries for the same driver", async () => {
      const deliveries = [
        makeDelivery({
          deliveryId: "del-001",
          destination: { lat: 40.72, lng: -74.01 },
        }),
        makeDelivery({
          deliveryId: "del-002",
          destination: { lat: 40.73, lng: -73.99 },
        }),
        makeDelivery({
          deliveryId: "del-003",
          destination: { lat: 40.74, lng: -73.98 },
        }),
      ];
      const update = makeGPSUpdate();
