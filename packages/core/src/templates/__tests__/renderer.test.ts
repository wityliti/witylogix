...
describe("renderTemplate - Nested Properties", () => {
  ...
  it("accesses multiple nested paths", () => {
    const result = renderTemplate("{{order.customer.name}} - {{order.total}}", {
      order: {
...
describe("renderTemplate - Raw HTML (Triple Braces)", () => {
  ...
  it("mixes escaped and raw HTML correctly", () => {
    const result = renderTemplate("<div>{{escaped}} {{{raw}}}</div>", {
      escaped: "<script>",
      raw: "<strong>bold</strong>",
    });
    expect(result.html).toBe("<div>&lt;script&gt; <strong>bold</strong></div>");
  });
});
...
describe("renderTemplate - Conditionals (if/unless)", () => {
  it("if block shows content when true", () => {
    const result = renderTemplate("{{#if delivered}}Delivered!{{/if}}", {
      delivered: true,
    });
  ...
  it("unless block shows content when false", () => {
    const result = renderTemplate("{{#unless cancelled}}Active{{/unless}}", {
      cancelled: false,
    });
  ...
  it("nested conditionals work", () => {
    const result = renderTemplate("{{#if order}}Order: {{order.id}}{{/if}}", {
  ...
  it("if treats non-empty strings as truthy", () => {
    const result = renderTemplate("{{#if text}}Content: {{text}}{{/if}}", {
      text: "hello",
    });
  ...
  it("if treats empty string as falsy", () => {
    const result = renderTemplate("{{#if text}}Content{{/if}}", { text: "" });
    expect(result.html).toBe("");
  });

  it("if treats zero as falsy", () => {
    const result = renderTemplate("{{#if count}}{{count}}{{/if}}", {
      count: 0,
    });
    expect(result.html).toBe("");
  });
  ...
  it("if treats non-empty array as truthy", () => {
    const result = renderTemplate("{{#if items}}Has items{{/if}}", {
      items: [1, 2, 3],
    });
...
describe("renderTemplate - Text Version", () => {
  it("extracts text without HTML tags", () => {
    const result = renderTemplate("Hello <b>{{name}}</b>!", { name: "John" });
...
describe("validateTemplateVariables", () => {
  it("accepts when all required variables provided", () => {
    const errors = validateTemplateVariables("Hello {{name}}", {
      name: "John",
    });
  ...
  it("validates nested variable requirements", () => {
    const errors = validateTemplateVariables("Order: {{order.id}}", {});
...
describe("renderTemplate - XSS Prevention", () => {
  it("prevents script injection in variables", () => {
    const result = renderTemplate("Content: {{userInput}}", {
  ...
  it("prevents event handler injection", () => {
    const result = renderTemplate("{{userInput}}", {
  ...
  it("escapes in conditional blocks", () => {
    const result = renderTemplate("{{#if show}}{{content}}{{/if}}", {
      show: true,
  ...
  it("raw HTML preserves safety responsibility to template author", () => {
    const result = renderTemplate("{{{userHTML}}}", {
...
describe("renderTemplate - Edge Cases", () => {
  ...
  it("handles missing intermediate properties", () => {
    const result = renderTemplate("{{order.customer.name}}", { order: {} });
    expect(result.html).toBe("");
    expect(result.errors.length).toBeGreaterThan(0);
  });
  ...
  it("handles arrays in variable output", () => {
    const result = renderTemplate("List: {{items}}", {
      items: ["a", "b", "c"],
    });
