...
export function transformOrder(orderData: any): any {
  ...
  return {
    ...
    externalOrderId:
      ...
      orderData.name ||
      orderId,
    status: transformOrderStatus(
      orderData.status || orderData.fulfillment_status || "pending",
    ),
    ...
    shippingAddress: transformAddress(
      "shipping_address" in orderData
        ? orderData.shipping_address
        : orderData.shippingAddress,
    ),
    billingAddress: transformAddress(
      "billing_address" in orderData
    ...
    discountCode:
      orderData.discount_codes?.[0]?.code || orderData.discountCode || null,
    discountAmount: parseFloat(
      orderData.total_discounts ?? orderData.discountAmount ?? "0",
    ),
    taxAmount: parseFloat(orderData.total_tax ?? orderData.taxAmount ?? "0"),
    shippingMethod:
      orderData.shipping_lines?.[0]?.title || orderData.shippingMethod || null,
    shippingCost: parseFloat(orderData.shipping_lines?.[0]?.price ?? "0"),
    metadata: orderData.metadata || orderData.metafields || {},
    createdAt: new Date(
      orderData.created_at || orderData.createdAt || new Date(),
    ),
    updatedAt: new Date(
      orderData.updated_at || orderData.updatedAt || new Date(),
    ),
  };
}
...
export function transformShipment(shipment: any): any {
  ...
  return {
    ...
    actualDelivery: shipment.actual_delivery
      ? new Date(shipment.actual_delivery)
      : null,
    ...
    createdAt: new Date(
      shipment.created_at || shipment.createdAt || new Date(),
    ),
    updatedAt: new Date(
      shipment.updated_at || shipment.updatedAt || new Date(),
    ),
  };
}
...
export function transformDriver(driver: any): any {
  ...
  return {
    ...
    rating:
      "rating" in driver
        ? safeFloat(driver.rating ?? "0")
        : safeFloat(driver.rating_score ?? "0"),
    totalDeliveries:
      "totalDeliveries" in driver
        ? safeInt(driver.totalDeliveries ?? "0")
        : safeInt(driver.total_deliveries ?? "0"),
...
export function transformCustomer(customer: any): any {
  ...
  return {
    ...
    defaultAddress: transformAddress(
      customer.default_address || customer.defaultAddress,
    ),
    ...
    createdAt: new Date(
      customer.created_at || customer.createdAt || new Date(),
    ),
    updatedAt: new Date(
      customer.updated_at || customer.updatedAt || new Date(),
    ),
  };
}
...
export function transformZone(zone: any): any {
  ...
  return {
    ...
    postalCodes: (zone.postal_codes || zone.postalCodes || []).filter(
      (p: any) => p,
    ),
    polygonCoordinates:
      zone.polygon_coordinates || zone.polygonCoordinates || null,
    ...
    estimatedDays: parseInt(
      zone.estimated_days ?? zone.estimatedDays ?? "1",
      10,
    ),
...
export function transformRoute(route: any): any {
  ...
  return {
    ...
    shipmentIds: (route.shipment_ids || route.shipmentIds || []).map(
      (id: any) => (typeof id === "string" ? id : id?.toString?.() || id),
    ),
    startTime: route.start_time ? new Date(route.start_time) : null,
    endTime: route.end_time ? new Date(route.end_time) : null,
    startLocation: transformAddress(
      route.start_location || route.startLocation,
    ),
