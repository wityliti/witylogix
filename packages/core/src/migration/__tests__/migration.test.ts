...
const mockMongoZone = {
  ...
  polygon_coordinates: [[[-87.6298, 41.8827]], [[-87.6194, 41.8827]]],
  polygonCoordinates: [[[-87.6298, 41.8827]], [[-87.6194, 41.8827]]],
  base_rate: "5.00",
  baseRate: 5.0,
  per_km_rate: "0.50",
  perKmRate: 0.5,
...
describe("DataMapper", () => {
  ...
  it("should apply built-in transformers", () => {
    const idTransformed = BuiltInTransformers.objectIdToString(
  ...
  it("should preserve default values for missing fields", () => {
    const customMapping: CollectionMapping = {
      ...
      prismaModel: "Test",
      fieldMap: [{ sourceField: "name", targetField: "name" }],
      defaultValues: {
        status: "ACTIVE",
        createdAt: new Date(),
      },
