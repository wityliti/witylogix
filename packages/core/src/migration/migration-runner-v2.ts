...
export class MigrationRunnerV2 {
  ...
  async run(
    ...
    try {
      ...
      for (const collectionMapping of this.config.collections) {
        ...
        if (
          options.skipCollections?.includes(collectionMapping.mongoCollection)
        ) {
          console.log(
  ...
  private async migrateCollectionIdempotent(
    ...
    try {
      ...
      console.log(
        `[COUNT] Found ${totalCount} documents in ${mapping.mongoCollection}`,
      );
      ...
      const rate =
        progress.durationMs > 0
          ? (progress.processed / progress.durationMs) * 1000
          : 0;
      console.log(
  ...
  async rollback(target: any): Promise<void> {
    ...
    for (const entry of this.rollbackLog) {
      try {
        console.log(
          `[ROLLBACK] Removing ${entry.ids.length} records from ${entry.collection}`,
        );
        // Implement deletion based on your target adapter
        console.log(`  → Removed ${entry.ids.length} records`);
      } catch (error) {
        console.error(
          `[ROLLBACK ERROR] Failed to rollback ${entry.collection}:`,
          error,
        );
      }
    }
  ...
  .map(
    (c) =>
      `${c.collection.padEnd(25)} | Processed: ${c.processed
        ...
        .padEnd(
          6,
        )} | Failed: ${c.failed.toString().padEnd(4)} | Skipped: ${c.skipped.toString().padEnd(4)} | ${(c.durationMs || 0).toFixed(0)}ms`,
  )
  .join("\n")}
