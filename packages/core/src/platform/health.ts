...
export class PlatformHealthService {
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('DB timeout')), this.config.databaseQueryTimeout)
        ),
      ]);
  ...
  public async getPrometheusMetrics(): Promise<string> {
    ...
    for (const [serviceName, check] of Object.entries(health.checks.externalServices)) {
      lines.push('');
      lines.push(`# HELP external_service_${serviceName}_latency_ms ${serviceName} latency`);
  ...
  public formatHealthStatusText(health: HealthStatus): string {
    ...
    if (Object.keys(health.checks.externalServices).length > 0) {
      ...
      for (const [name, check] of Object.entries(health.checks.externalServices)) {
   ...
   */
  private determineOverallStatus(checks: HealthCheckResult[]): HealthStatusType {
