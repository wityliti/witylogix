...
const woocommerceProductCreatedWebhook = {
  ...
  price: '49.99',
  price_html: '<span class="woocommerce-Price-amount amount"><bdi><span class="woocommerce-Price-currencySymbol">$</span>49.99</span></bdi>',
  on_sale: false,
  purchasable: true,
  total_sales: 0,
  tax_status: 'taxable',
...
describe('WooCommerce Webhook E2E Tests', () => {
  ...
  describe('WooCommerce Webhook Signature Validation', () => {
    it('should validate correct WooCommerce webhook signature (HMAC-SHA256)', () => {
      const payload = Buffer.from(JSON.stringify(woocommerceOrderCreatedWebhook));
  ...
  describe('WooCommerce Order Processing', () => {
    it('should map order.created webhook to Witylogix format', async () => {
      const mappedOrder = await adapter.mapOrder(woocommerceOrderCreatedWebhook);
    ...
    it('should process order.updated webhook', async () => {
      const mappedOrder = await adapter.mapOrder(woocommerceOrderUpdatedWebhook);
    ...
    it('should process order.deleted webhook', async () => {
      const mappedOrder = await adapter.mapOrder(woocommerceOrderCancelledWebhook);
  ...
  describe('WooCommerce Product Processing', () => {
    it('should map product.created webhook', async () => {
      const mappedProduct = await adapter.mapProduct(woocommerceProductCreatedWebhook);
    ...
    it('should process product.updated webhook', async () => {
      const mappedProduct = await adapter.mapProduct(woocommerceProductUpdatedWebhook);
  ...
  describe('WooCommerce Customer Processing', () => {
    it('should map customer.created webhook', async () => {
      const mappedCustomer = await adapter.mapCustomer(woocommerceCustomerCreatedWebhook);
  ...
  describe('WooCommerce Error Handling', () => {
    ...
    it('should handle null line items', async () => {
      const orderWithoutItems = { ...woocommerceOrderCreatedWebhook, line_items: null };
    ...
    it('should handle empty shipping address', async () => {
      const orderWithoutShipping = { ...woocommerceOrderCreatedWebhook, shipping: {} };
    ...
    it('should handle empty topic field', () => {
      const webhookWithoutTopic = { ...woocommerceOrderCreatedWebhook, topic: '' };
  ...
  describe('Complete WooCommerce E2E Flow', () => {
    it('should process full webhook chain: validate → map → queue → db', async () => {
      ...
      // Step 4: Queue job
      const job = await queue.add('process-webhook', { eventType, data: mappedOrder });
    ...
    it('should handle order lifecycle: created → updated → cancelled', async () => {
      ...
      const cancelled = await adapter.mapOrder(woocommerceOrderCancelledWebhook);
