...
import { ShopifyAdapter, ShopifyOrder, ShopifyProduct, ShopifyCustomer } from '../adapters/shopify';
import { CreateOrderInput, CreateProductInput, CreateCustomerInput } from '../types';
...
function createValidShopifyHmac(payload: string, secret: string): string {
  return crypto.createHmac('sha256', secret).update(payload, 'utf-8').digest('base64');
}

function createValidShopifyPayloadBuffer(obj: Record<string, any>, secret: string): { payload: Buffer; signature: string } {
...
describe('ShopifyAdapter', () => {
  ...
  describe('validateWebhook', () => {
    it('should validate webhook with correct HMAC-SHA256 signature', () => {
      const payload = Buffer.from(JSON.stringify({ id: 123, order_number: 1001 }));
      const signature = createValidShopifyHmac(payload.toString('utf-8'), secret);
    ...
    it('should use timing-safe comparison to prevent timing attacks', () => {
      ...
      const validSignature = createValidShopifyHmac(payload.toString('utf-8'), secret);
    ...
    it('should handle whitespace in signature gracefully', () => {
      ...
      const result = adapter.validateWebhook(payload, signatureWithWhitespace, secret);
    ...
    it('should handle real-world Shopify webhook format', () => {
      ...
      const result = adapter.validateWebhook(Buffer.from(orderPayload), signature, secret);
      expect(result).toBe(true);
    });
  });
    ...
    it('should map order with multiple line items', () => {
      const orderWithManyItems = {
        ...
        line_items: [
          ...
          { id: 999, variant_id: 888, title: 'Item 3', quantity: 1, sku: 'SKU3', price: '25.00' } as any,
        ],
      };
  ...
  describe('mapProduct', () => {
    it('should map full Shopify product with variants', () => {
      ...
      expect(result.description).toBe('<p>Our flagship product with premium features.</p>');
    ...
    it('should extract image URL from images array', () => {
      ...
      expect(result.imageUrl).toBe('https://cdn.shopify.com/s/files/1/0001/1234/5678/premium-widget.jpg');
    });
  ...
  describe('fetchOrder', () => {
    ...
    it('should handle API error responses', async () => {
      ...
      await expect(adapter.fetchOrder('999999999', credentials)).rejects.toThrow();
    });
  ...
  describe('fetchProducts', () => {
    ...
    it('should fetch products with pagination', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        headers: new Map([
          ['Link', '<https://store.myshopify.com/api/2024-01/products.json?cursor=abc123>; rel="next"'],
        ]),
      });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle zero prices', () => {
      const order = {
        ...
        total_tax: '0',
        line_items: [
          { ...MOCK_SHOPIFY_ORDER.line_items[0], price: '0' },
        ],
      };
    ...
    it('should handle very large order values', () => {
      const order = {
        ...
        line_items: [
          { ...MOCK_SHOPIFY_ORDER.line_items[0], price: '999999.99', quantity: 10 },
        ],
      };
