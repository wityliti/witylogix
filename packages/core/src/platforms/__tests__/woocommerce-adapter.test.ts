...
import { WooCommerceAdapter, WooCommerceOrder, WooCommerceProduct, WooCommerceCustomer } from '../adapters/woocommerce';
...
const MOCK_WOOCOMMERCE_ORDER: WooCommerceOrder = {
  ...
  coupon_lines: [],
  refunds: [],
  payment_url: 'https://store.example.com/checkout/order-pay/12345/?pay_for_order=true',
...
const MOCK_WOOCOMMERCE_PRODUCT: WooCommerceProduct = {
  ...
  price_html: '<span class="woocommerce-Price-amount"><bdi><span class="woocommerce-Price-currencySymbol">$</span>39.99</bdi></span>',
  on_sale: true,
  purchasable: true,
  total_sales: 156,
  virtual: false,
...
const MOCK_WOOCOMMERCE_CUSTOMER: WooCommerceCustomer = {
  ...
  meta_data: [
    { id: 1, key: 'loyalty_points', value: '500' },
  ],
  _links: {},
};
...
describe('WooCommerceAdapter', () => {
  ...
  describe('mapOrder', () => {
    ...
    it('should determine financial status from payment', () => {
      const paidOrder = { ...MOCK_WOOCOMMERCE_ORDER, date_paid: '2024-01-15T10:32:00' };
    ...
    it('should determine fulfillment status from order status', () => {
      ...
      const processingOrder = { ...MOCK_WOOCOMMERCE_ORDER, status: 'processing' };
  ...
  describe('mapProduct', () => {
    ...
    it('should extract image URLs', () => {
      ...
      expect(result.imageUrl).toBe('https://store.example.com/wp-content/uploads/product-1.jpg');
    });
