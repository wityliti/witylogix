...
describe('Cross-Platform Webhook E2E Tests', () => {
  ...
  let webhooksProcessed: Array<{ platform: PlatformSource; eventType: string; orderId: string }>;
  ...
  describe('Cross-Platform Order Reconciliation', () => {
    it('should identify same customer across different platforms', async () => {
      ...
      expect(customerKey).toBe(`${wooOrder1.shopId}-${wooOrder1.customerEmail}`);
    });
    ...
    it('should reconcile orders from multiple platforms for same customer', async () => {
      ...
      ordersInDb.set(`${PlatformSource.SHOPIFY}-${shopifyOrder1.externalOrderId}`, {
      ...
      ordersInDb.set(`${PlatformSource.WOOCOMMERCE}-${wooOrder1.externalOrderId}`, {
      ...
      expect(customerOrders.every((o) => o.customerEmail === 'john@example.com')).toBe(true);
    });
  });
  ...
  describe('Concurrent Multi-Platform Webhook Processing', () => {
    ...
    it('should process mixed webhooks (orders, products, customers) concurrently', async () => {
      ...
      const promises = [
        shopifyAdapter.mapOrder(shopifyOrder),
        shopifyAdapter.mapProduct({ shopId: 'shop-123', id: 1, title: 'P1', sku: 'SKU1', price: '50' }),
        shopifyAdapter.mapCustomer({ shopId: 'shop-123', id: 1, email: 'test@example.com', first_name: 'Test', last_name: 'User' }),
        wooAdapter.mapOrder(woocommerceOrder),
        wooAdapter.mapProduct({ shopId: 'shop-123', id: 2, name: 'P2', sku: 'SKU2', price: '60' }),
        wooAdapter.mapCustomer({ shopId: 'shop-123', id: 2, email: 'test2@example.com', first_name: 'Test2', last_name: 'User2' }),
      ];
      ...
      expect(orders.every((r) => 'externalOrderId' in r || 'title' in r || 'email' in r)).toBe(true);
    });
  });
  ...
  describe('Complete Multi-Platform E2E Flow', () => {
    ...
    it('should process complete order lifecycle across platforms', async () => {
      ...
      expect(allOrders.every((o) => o.customerEmail === 'john@example.com')).toBe(true);
