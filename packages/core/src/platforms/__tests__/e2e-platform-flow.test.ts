...
class MockShopifyAdapter implements PlatformAdapter {
  ...
  async fetchOrder(externalOrderId: string): Promise<unknown> {
    return { ...mockShopifyOrder, id: parseInt(externalOrderId.split('/').pop()!) };
  }
...
const ADAPTER_REGISTRY: Map<PlatformSource, () => Promise<PlatformAdapter>> = new Map();
const ADAPTER_CACHE: Map<PlatformSource, PlatformAdapter> = new Map();

function registerMockAdapter(source: PlatformSource, factory: () => Promise<PlatformAdapter>) {
  ADAPTER_REGISTRY.set(source, factory);
}

async function getMockAdapter(source: PlatformSource): Promise<PlatformAdapter> {
...
describe('E2E Platform Adapter Lifecycle - SHOPIFY', () => {
  ...
  beforeEach(async () => {
    registerMockAdapter(PlatformSource.SHOPIFY, async () => new MockShopifyAdapter());
  ...
  it('should detect Shopify webhook event type', () => {
    const eventType = adapter.getWebhookEventType({ event_type: 'orders/create' });
...
describe('E2E Platform Adapter Lifecycle - WOOCOMMERCE', () => {
  ...
  beforeEach(async () => {
    registerMockAdapter(PlatformSource.WOOCOMMERCE, async () => new MockWooCommerceAdapter());
...
describe('E2E Platform Adapter Lifecycle - MAGENTO', () => {
  ...
  beforeEach(async () => {
    registerMockAdapter(PlatformSource.MAGENTO, async () => new MockMagentoAdapter());
  ...
  it('should detect Magento webhook event type', () => {
    const eventType = adapter.getWebhookEventType({ event_type: 'order.create' });
...
describe('E2E Platform Adapter Lifecycle - CUSTOM', () => {
  ...
  beforeEach(async () => {
    registerMockAdapter(PlatformSource.CUSTOM, async () => new MockCustomAdapter());
  ...
  it('should detect custom webhook event type', () => {
    const eventType = adapter.getWebhookEventType({ event_type: 'order.created' });
    expect(eventType).toBe('order.created');
  });
...
describe('Cross-Platform Scenarios', () => {
  ...
  it('should support multiple platforms on same shop', async () => {
    ...
    expect(shopifyCustomer.externalCustomerId).toBe(wooCustomer.externalCustomerId);
  });
  ...
  it('should validate webhooks from all platforms', async () => {
    ...
    expect(shopifyAdapter.validateWebhook(shopifyPayload, shopifySignature)).toBe(true);
    expect(wooAdapter.validateWebhook(wooPayload, wooSignature)).toBe(true);
    expect(magentoAdapter.validateWebhook(magentoPayload, magentoSignature)).toBe(true);
  });
...
describe('Error Handling', () => {
  ...
  it('should handle webhook with invalid signature', () => {
    const payload = Buffer.from(JSON.stringify(mockShopifyOrder));
    const invalidSignature = 'completely-invalid-signature-that-will-never-match';
...
describe('Complete Workflow Scenarios', () => {
  ...
  it('should handle platform migration workflow', async () => {
    ...
    expect(wooOrder.customerEmail).toBe(mockWooCommerceOrder.billing.email || '');
