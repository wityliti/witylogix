...
class ShopifyAdapter implements PlatformAdapter {
  ...
  mapOrder(externalOrder: any): CreateOrderInput {
    ...
    return {
      ...
      customerName:
        externalOrder.customer?.default_address?.name ||
        `${externalOrder.customer?.first_name || ""} ${externalOrder.customer?.last_name || ""}`.trim(),
...
class WooCommerceAdapter implements PlatformAdapter {
  ...
  mapOrder(externalOrder: any): CreateOrderInput {
    ...
    return {
      externalOrderId: externalOrder.id?.toString() || "",
      externalOrderNumber:
        externalOrder.number?.toString() || externalOrder.id?.toString() || "",
      customerName:
...
describe("Platform Adapter Registry", () => {
  ...
  describe("WooCommerce Webhook Validation", () => {
    ...
    it("should reject webhook with invalid signature", () => {
      ...
      const result = wooAdapter.validateWebhook(
        payload,
        invalidSignature,
        secret,
      );
  ...
  describe("WooCommerce Order Mapping", () => {
    ...
    it("should handle WooCommerce order with multiple items", () => {
      ...
      expect(result.items[0].quantity).toBe(2);
      expect(result.items[1].price).toBe(75.5);
    });
  });
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should validate both adapters independently", () => {
      ...
      const wooHmac = crypto
        .createHmac("sha256", secret)
        .update(payload)
        .digest("hex");

      const shopifyResult = shopifyAdapter.validateWebhook(
        payload,
        shopifyHmac,
        secret,
      );
      ...
      const crossShopify = shopifyAdapter.validateWebhook(
        payload,
        wooHmac,
        secret,
      );
