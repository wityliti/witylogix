...
function makeWCOrder(
  overrides: Partial<WooCommerceOrder> = {},
): WooCommerceOrder {
  return {
    id: 42,
    parent_id: 0,
...
describe("persistWCOrder", () => {
  describe("happy path — new order", () => {
    ...
    it("upserts Order with correct shopId and externalOrderId", () => {
      ...
      const call = (db.order.upsert as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
      expect(call.where.shopId_externalOrderId_source.shopId).toBe(SHOP_ID);
      expect(call.where.shopId_externalOrderId_source.externalOrderId).toBe(
        "wc_order_42",
      );
      expect(call.where.shopId_externalOrderId_source.source).toBe(
        "WOOCOMMERCE",
      );
    });
    ...
    it("creates Shipment with PENDING status", () => {
      const call = (db.shipment.create as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
  ...
  describe("status mapping", () => {
    ...
    it.each(cases)(
      'WC status "%s" → Order status "%s"',
      async (wcStatus, expected) => {
        const db = makeDb();
  ...
  describe("null guard — missing billing / shipping fields", () => {
    ...
    it("handles null shipping_lines (no carrier on shipment)", async () => {
      const db = makeDb();
      await persistWCOrder(
        makeWCOrder({ shipping_lines: null as any }),
        SHOP_ID,
        db,
      );
    ...
    it("handles incomplete billing address (falls back to store default for pickup)", async () => {
      ...
      // Should not throw
      await expect(
        persistWCOrder(makeWCOrder({ billing }), SHOP_ID, db),
  ...
  describe("tenant isolation", () => {
    it("scopes every DB write to the provided shopId", async () => {
      ...
      const orderCall = (db.order.upsert as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
      const shipmentCall = (db.shipment.create as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
    ...
    it("queries existing shipment scoped to shopId", async () => {
      ...
      const findCall = (db.shipment.findFirst as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
