...
describe("WooCommerceShipmentService", () => {
  describe("upsertShipment — creation (no existing shipment)", () => {
    ...
    it("creates ShipmentItem rows", async () => {
      ...
      const call = (db.shipmentItem.createMany as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
  ...
  describe("status mapping", () => {
    ...
    it.each(cases)(
      ...
      async (wcStatus, expectedShipmentStatus, expectedOrderStatus) => {
        ...
        const shipmentCall = (db.shipment.create as ReturnType<typeof vi.fn>)
          .mock.calls[0][0];
  ...
  describe("full pipeline integration", () => {
    it("webhook payload → normalizer → upsert → shipment created end-to-end", async () => {
      ...
      const orderCall = (db.order.upsert as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
      expect(
        orderCall.where.shopId_externalOrderId_source.externalOrderId,
      ).toBe("wc_order_9999");
      ...
      const itemsCall = (db.shipmentItem.createMany as ReturnType<typeof vi.fn>)
        .mock.calls[0][0];
      expect(itemsCall.data).toHaveLength(2);
      expect(itemsCall.data[0]).toMatchObject({
        sku: "BOX-HD",
        quantity: 3,
        title: "Heavy Duty Box",
      });
      expect(itemsCall.data[1]).toMatchObject({
        sku: "TAPE-001",
        quantity: 10,
