...
export class DataFuser {
  ...
  public fuseCustomerData(
    ...
    const completedOrders = orders.filter((o) => o.status === 'completed').sort((a, b) => b.date.getTime() - a.date.getTime());
    const successfulPayments = payments.filter((p) => p.status === 'successful');
    const recentActivities = activities.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());

    const totalRevenue = completedOrders.reduce((sum, o) => sum + o.amount, 0);
    const lastActivityDate =
      recentActivities.length > 0
        ? new Date(Math.max(recentActivities[0].timestamp.getTime(), completedOrders[0]?.date.getTime() || 0))
        : null;
...
export class FeatureExtractor {
  ...
  public extractRFM(
    ...
    const tenureDays = Math.floor((referenceDate.getTime() - acquisitionDate.getTime()) / (1000 * 60 * 60 * 24));
    const frequency = tenureDays > 0 ? (completedOrders.length / tenureDays) * 30 : 0;

    // Monetary: average order value & total spend
    const totalSpend = completedOrders.reduce((sum, o) => sum + o.amount, 0);
    const monetary = completedOrders.length > 0 ? totalSpend / completedOrders.length : 0;
    ...
    const growthFactor = this.calculateGrowthFactor(completedOrders, referenceDate);
   ...
   */
  private calculateEngagement(activities: CustomerActivity[], referenceDate: Date): number {
    ...
    const activityLast30 = activities.filter((a) => a.timestamp.getTime() > thirtyDaysAgo).length;
    const activityLast60 = activities.filter((a) => a.timestamp.getTime() > sixtyDaysAgo).length;
    const activityLast90 = activities.filter((a) => a.timestamp.getTime() > ninetyDaysAgo).length;
   ...
   */
  private calculateGrowthFactor(completedOrders: OrderData[], referenceDate: Date): number {
    ...
    const recentOrders = completedOrders.filter((o) => o.date.getTime() > threeMonthsAgo);
    const olderOrders = completedOrders.filter(
      (o) => o.date.getTime() > sixMonthsAgo && o.date.getTime() <= threeMonthsAgo
    );

    if (recentOrders.length === 0 || olderOrders.length === 0) return 1.0;

    const recentAverage = recentOrders.reduce((sum, o) => sum + o.amount, 0) / recentOrders.length;
    const olderAverage = olderOrders.reduce((sum, o) => sum + o.amount, 0) / olderOrders.length;
...
export class LTVPredictor {
  ...
  public predictLTV(customerData: {
    ...
    const rfm = this.featureExtractor.extractRFM(customerData.acquisitionDate, customerData.completedOrders, customerData.activities);
    ...
    const predictedLTV = historicalLTV * rfm.growthFactor * retentionProbability * expansionFactor;

    // Confidence based on data availability and consistency
    const confidence = this.calculateConfidence(customerData.completedOrders, customerData.activities);

    // Determine segment
    const segment = this.determineSegment(rfm, customerData.acquisitionDate);

    // Generate explanations
    const explanation = this.generateExplanations(rfm, retentionProbability, segment);
  ...
  private calculateRetention(rfm: RFMFeatures): number {
    ...
    // Apply RFM weighting
    const rfmScore =
      recencyScore * RECENCY_WEIGHT + frequencyScore * FREQUENCY_WEIGHT + monetaryScore * MONETARY_WEIGHT;

    // Factor in engagement
    const engagementFactor = 0.8 + (rfm.engagement / 100) * 0.2; // 0.8-1.0 multiplier

    // Growth trend impact
    const growthImpact = rfm.growthFactor > 1 ? 1 + (rfm.growthFactor - 1) * 0.2 : rfm.growthFactor; // amplify growth slightly
   ...
   */
  private calculateConfidence(orders: OrderData[], activities: CustomerActivity[]): number {
    ...
    if (activities.length > 0) {
      ...
      const daysSinceActivity = Math.floor((Date.now() - recentActivity.timestamp.getTime()) / (1000 * 60 * 60 * 24));
   ...
   */
  private determineSegment(rfm: RFMFeatures, acquisitionDate: Date): CustomerSegment {
   ...
   */
  private generateExplanations(rfm: RFMFeatures, retention: number, segment: CustomerSegment): string[] {
    const explanations: string[] = [];

    // Recency insights
    if (rfm.recency <= 7) {
      explanations.push('Very recent purchase activity indicates strong current engagement');
    } else if (rfm.recency <= 30) {
      explanations.push('Recent activity shows ongoing engagement');
    } else if (rfm.recency > 180) {
      explanations.push('Long time since last order indicates potential churn risk');
    }
    ...
    if (rfm.frequency >= 2) {
      ...
      explanations.push('Low purchase frequency suggests experimental customer');
    }
    ...
    // Retention risk
    if (retention < 0.3) {
      explanations.push('Retention probability is low - consider re-engagement campaign');
    } else if (retention > 0.8) {
      explanations.push('High retention probability - focus on expansion');
    }

    // Segment specific
    explanations.push(`Segmented as ${segment} based on purchasing behavior and engagement`);
...
export class CohortAnalyzer {
  ...
  public analyzeCohorts(customerProfiles: LTVPrediction[]): CohortAnalysis[] {
    ...
    cohortMap.forEach((profiles, cohort) => {
      ...
      analyses.push({
        ...
        retentionRate: retentionRates.reduce((a, b) => a + b, 0) / profiles.length,
        averageEngagement: profiles.reduce((sum, p) => sum + (p.confidence || 0), 0) / profiles.length,
  ...
  private determineTrend(profiles: LTVPrediction[]): 'increasing' | 'stable' | 'decreasing' {
    const growthFactors = profiles.map((p) => p.growthFactor);
    const average = growthFactors.reduce((a, b) => a + b, 0) / growthFactors.length;
...
export class ChurnPredictor {
   ...
   */
  public predictChurnRisk(ltvPrediction: LTVPrediction, rfm: RFMFeatures): ChurnRisk {
    const riskFactors = {
    ...
    if (riskFactors.longInactivity) daysUntilLikely = Math.min(daysUntilLikely, 30);
    if (riskFactors.decayingEngagement) daysUntilLikely = Math.min(daysUntilLikely, 45);
    if (riskFactors.decreasingSpend) daysUntilLikely = Math.min(daysUntilLikely, 60);

    // Generate recommendations
    const recommendations: string[] = [];

    if (riskFactors.longInactivity) {
      recommendations.push('Re-engagement campaign: send personalized offer or check-in email');
    }
    if (riskFactors.decayingEngagement) {
      recommendations.push('Increase engagement: share relevant content, product updates');
    }
    if (riskFactors.decreasingSpend) {
      recommendations.push('Win-back campaign: highlight new products, exclusive deals');
    }

    if (churnProbability > 0.7) {
      recommendations.push('Consider urgency discount or VIP outreach to prevent churn');
    }
...
let customerLTVPredictorInstance: ReturnType<typeof createCustomerLTVPredictor> | null = null;

/**
 * Get singleton instance of customer LTV predictor
 */
export function getCustomerLTVPredictor(): ReturnType<typeof createCustomerLTVPredictor> {
