...
describe('Intelligent Order Router', () => {
  ...
  describe('ProximityCalculator', () => {
    it('should calculate proximity score based on distance', () => {
      ...
      const scoreClose = calc.calculateScore(nyc, { latitude: 40.7489, longitude: -73.968 });
      expect(scoreClose).toBeGreaterThan(90);

      // Far warehouse
      const scoreFar = calc.calculateScore(laCoord, { latitude: 40.7489, longitude: -73.968 });
  ...
  describe('StockChecker', () => {
    ...
    it('should calculate stock score', () => {
      ...
      // Partial stock
      const partialItems = [{ skuId: 'sku_1', quantity: 60, weight: 120, volume: 6 }];
  ...
  describe('CapacityTracker', () => {
    ...
    it('should track remaining capacity', () => {
      ...
      expect(remaining).toBe(warehouses[0].dailyCapacity - warehouses[0].currentDailyCount);
    });
  });
  ...
  describe('IntelligentOrderRouter', () => {
    ...
    it('should handle capacity constraints', () => {
      ...
      const fullWarehouses = warehouses.map((w) => ({
        ...w,
        currentDailyCount: w.id === 'wh_1' ? w.dailyCapacity - 1 : w.currentDailyCount,
      }));
    ...
    it('should suggest split orders when necessary', () => {
      ...
      const decision = router.routeOrder(splitOrder, warehouses, { considerSplits: true });
