...
describe('IntegrationDependencyGraph', () => {
  ...
  describe('Synergy Types', () => {
    ...
    it('should have complement synergies (maps + routing)', () => {
      const complementary = graph.getComplementaryProviders('google-routes-api');
  ...
  describe('Complementary Provider Selection', () => {
    ...
    it('should exclude specified provider IDs', () => {
      const complementary = graph.getComplementaryProviders('stripe', ['shopify', 'quickbooks']);
  ...
  describe('Synergy Scoring', () => {
    ...
    it('should provide e-commerce synergy example', () => {
      ...
      const shopifyWithShipping = graph.getSynergyScore('shopify', ['shipstation']);
  ...
  describe('Graph Visualization Data', () => {
    ...
    it('should have synergy types in edges', () => {
      ...
      graphData.edges.forEach((edge: any) => {
        expect(['data-flow', 'complement', 'required', 'optional']).toContain(edge.synergyType);
      });
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle duplicate provider IDs', () => {
      ...
      const stripeCount = graphData.nodes.filter((n) => n.id === 'stripe').length;
    ...
    it('should be deterministic', () => {
      ...
      expect(data1.nodes.map((n) => n.id)).toEqual(data2.nodes.map((n) => n.id));
    });
  });
});
