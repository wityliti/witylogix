...
describe("Demand Forecaster", () => {
  ...
  beforeEach(() => {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      const quantity = Math.round(
        baseQuantity * weeklyMultiplier + Math.random() * 20,
      );
  ...
  describe("SeasonalDecomposer", () => {
    it("should extract weekly seasonality", () => {
      ...
      const weekdayAvg =
        (pattern.weeklyPattern.get(1) ??
          1 + pattern.weeklyPattern.get(2) ??
          1 + pattern.weeklyPattern.get(3) ??
          1) / 3;
      const weekendAvg =
        (pattern.weeklyPattern.get(0) ??
          1 + pattern.weeklyPattern.get(6) ??
          1) / 2;
  ...
  describe("DemandPredictor", () => {
    it("should predict demand for future date", () => {
      ...
      const forecast = predictor.predictDemand(
        "sku_1",
        historicalData,
        forecastDate,
      );
    ...
    it("should apply seasonality adjustment", () => {
      ...
      const fridayForecast = predictor.predictDemand(
        "sku_1",
        historicalData,
        fridayDate,
      );
      const saturdayForecast = predictor.predictDemand(
        "sku_1",
        historicalData,
        saturdayDate,
      );
      ...
      expect(fridayForecast.forecastedDemand).toBeGreaterThan(
        saturdayForecast.forecastedDemand,
      );
    });
    ...
    it("should apply external factors", () => {
      ...
      const forecastWithPromo = predictor.predictDemand(
        "sku_1",
        historicalData,
        forecastDate,
        [promotion],
      );
      const forecastWithoutPromo = predictor.predictDemand(
        "sku_1",
        historicalData,
        forecastDate,
      );

      expect(forecastWithPromo.forecastedDemand).toBeGreaterThan(
        forecastWithoutPromo.forecastedDemand,
      );
    });
  ...
  describe("ReorderSuggester", () => {
    ...
    it("should recommend reorder date", () => {
      ...
      expect(suggestion.recommendedReorderDate.getTime()).toBeGreaterThan(
        Date.now(),
      );
    });
  });
  ...
  describe("DemandForecasterService", () => {
    it("should forecast multiple days ahead", () => {
      const forecasts = forecaster.forecastMultipleDays(
        "sku_1",
        historicalData,
        7,
      );
  ...
  describe("Integration scenarios", () => {
    it("should handle complete forecasting workflow", () => {
      ...
      const avgForecastDemand =
        forecasts.reduce((a, b) => a + b.forecastedDemand, 0) /
        forecasts.length;
      const suggestion = forecaster.getReorderSuggestion(
        "sku_1",
        50,
