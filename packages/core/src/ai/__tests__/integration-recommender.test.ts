...
describe("IntegrationRecommender", () => {
  ...
  describe("Personalized Recommendations", () => {
    ...
    it("should score based on industry match", () => {
      ...
      const toastRec = recommendations.find(
        (r) => r.providerId === "toast-pos",
      );
...
describe("Category Gap Analysis", () => {
  ...
  it("should calculate overall coverage percentage", () => {
    const coverage = analyzeCategoryGaps(
      ["stripe", "shipstation"],
      "ecommerce",
    );
...
describe("Recommendation Determinism", () => {
  it("should return consistent results for same input", () => {
    ...
    expect(recs1.map((r) => r.providerId)).toEqual(
      recs2.map((r) => r.providerId),
    );
    expect(recs1.map((r) => r.relevanceScore)).toEqual(
      recs2.map((r) => r.relevanceScore),
    );
  });
});
