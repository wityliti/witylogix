...
export class RecommendationService {
  ...
  async getDependencies(
    ...
    const registry =
      require("../integrations/registry/integration-registry.js").integrationRegistry;
    ...
    const complementary = this.dependencyGraph.getComplementaryProviders(
      providerId,
      connectedIds,
    );
    ...
    const graphData = this.dependencyGraph.getGraphData([
      providerId,
      ...connectedIds,
    ]);
  ...
  async analyzeGaps(
    ...
    const recommendations = coverage.recommendations.map((id) => {
      const registry =
        require("../integrations/registry/integration-registry.js").integrationRegistry;
  ...
  async getFeedbackAnalytics(
    ...
    const setupStarted = feedback.filter(
      (f) => f.action === "setup_started",
    ).length;

    const acceptanceRate =
      feedback.length > 0 ? (accepted / feedback.length) * 100 : 0;
    const conversionRate =
      feedback.length > 0 ? (setupStarted / feedback.length) * 100 : 0;

    const avgDwellTime =
      feedback
        .filter((f) => f.dwellTimeSeconds)
        .reduce((sum, f) => sum + (f.dwellTimeSeconds ?? 0), 0) /
      feedback.filter((f) => f.dwellTimeSeconds).length;

    const byProvider: Record<
      string,
      { accepted: number; dismissed: number; setupStarted: number }
    > = {};
    for (const f of feedback) {
      if (!byProvider[f.providerId]) {
        byProvider[f.providerId] = {
          accepted: 0,
          dismissed: 0,
          setupStarted: 0,
        };
  ...
  private initializeABTests(): void {
    this.abTestConfig.set("relevance", {
      variant: "relevance",
      weight: 0.5,
      enabled: true,
    });
    this.abTestConfig.set("industry-first", {
      variant: "industry-first",
      weight: 0.25,
      enabled: true,
    });
    this.abTestConfig.set("workflow-first", {
      variant: "workflow-first",
      weight: 0.15,
      enabled: true,
    });
    this.abTestConfig.set("popularity", {
      variant: "popularity",
      weight: 0.1,
      enabled: true,
    });
  ...
  private selectABTestVariant(): RecommendationAlgorithm {
    const enabledTests = Array.from(this.abTestConfig.values()).filter(
      (c) => c.enabled,
    );
...
export class RecommendationCache {
  private cache: Map<
    string,
    { data: RecommendationResponse; expiresAt: number }
  > = new Map();
