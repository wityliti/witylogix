...
export interface Anomaly {
  type: 'cost_spike' | 'delay_cluster' | 'carrier_degradation' | 'volume_shift' | 'damage_spike';
...
export class CostAnalyzer {
  ...
  public analyzeCostByService(shipments: ShipmentRecord[]): CostBreakdown[] {
    ...
    for (const shipment of shipments) {
      const current = byService.get(shipment.serviceLevel) || { cost: 0, count: 0 };
      current.cost += shipment.cost;
      current.count += 1;
   ...
   */
  public calculateSavings(shipments: ShipmentRecord[]): { category: string; potentialSavings: number; percentSavings: number; recommendation: string }[] {
    const opportunities = [];
    ...
    if (byCarrier.length > 1) {
      ...
      const savingsPercent = ((expensive.averageCostPerShipment - cheapest.averageCostPerShipment) / expensive.averageCostPerShipment) * 100;
...
export class PerformanceAnalyzer {
   ...
   */
  public analyzeCarrierPerformance(shipments: ShipmentRecord[], period: string): PerformanceMetrics[] {
    ...
    return Array.from(byCarrier.entries()).map(([carrier, carrierShipments]) => {
      ...
      return {
        ...
        averageTransitDays: carrierShipments.filter((s) => s.deliveryDate).length > 0 ? totalTransitDays / carrierShipments.filter((s) => s.deliveryDate).length : 0,
...
export class VolumeAnalyzer {
   ...
   */
  public analyzeVolume(shipments: ShipmentRecord[], period: string): VolumeReport {
    ...
    for (const shipment of shipments) {
      // By carrier
      byCarrier.set(shipment.carrier, (byCarrier.get(shipment.carrier) || 0) + 1);

      // By service
      byService.set(shipment.serviceLevel, (byService.get(shipment.serviceLevel) || 0) + 1);

      // By state
      byState.set(shipment.destinationState, (byState.get(shipment.destinationState) || 0) + 1);
...
export class AnomalyDetector {
   ...
   */
  public detectAnomalies(current: ShipmentRecord[], historical: ShipmentRecord[]): Anomaly[] {
    const anomalies: Anomaly[] = [];

    // 1. Cost spike detection
    const currentAvgCost = current.reduce((sum, s) => sum + s.cost, 0) / current.length;
    const historicalAvgCost = historical.reduce((sum, s) => sum + s.cost, 0) / historical.length;
    const costDeviation = ((currentAvgCost - historicalAvgCost) / historicalAvgCost) * 100;
    ...
    const currentOnTime = current.filter((s) => s.onTime).length / current.length;
    const historicalOnTime = historical.filter((s) => s.onTime).length / historical.length;
    const onTimeDeviation = ((historicalOnTime - currentOnTime) / historicalOnTime) * 100;
    ...
    // 3. Damage rate spike
    const currentDamageRate = current.filter((s) => s.damaged).length / current.length;
    const historicalDamageRate = historical.filter((s) => s.damaged).length / historical.length;
    const damageDeviation = ((currentDamageRate - historicalDamageRate) / (historicalDamageRate + 0.01)) * 100;
...
export class ShippingForecaster {
   ...
   */
  public forecastVolume(historicalMonths: { month: string; shipmentCount: number }[], targetMonth: string): Forecast {
    if (historicalMonths.length < 2) {
      return {
        period: targetMonth,
        forecastType: 'volume',
        predictedValue: historicalMonths.length > 0 ? historicalMonths[historicalMonths.length - 1].shipmentCount : 0,
        confidenceInterval: { low: 0, high: 0 },
        baselineValue: historicalMonths.length > 0 ? historicalMonths[historicalMonths.length - 1].shipmentCount : 0,
        changePercent: 0,
    ...
    const trend = (recent[recent.length - 1].shipmentCount - recent[0].shipmentCount) / recent.length;
   ...
   */
  public forecastCost(historicalMonths: { month: string; totalCost: number }[], targetMonth: string): Forecast {
    if (historicalMonths.length < 2) {
      return {
        period: targetMonth,
        forecastType: 'cost',
        predictedValue: historicalMonths.length > 0 ? historicalMonths[historicalMonths.length - 1].totalCost : 0,
        confidenceInterval: { low: 0, high: 0 },
        baselineValue: historicalMonths.length > 0 ? historicalMonths[historicalMonths.length - 1].totalCost : 0,
        changePercent: 0,
        methodology: 'Insufficient data',
      };
    }
    ...
    // Simple linear trend
    const recent = historicalMonths.slice(-3);
    const trend = (recent[recent.length - 1].totalCost - recent[0].totalCost) / recent.length;
...
export class ShippingAnalyticsEngine {
   ...
   */
  public generateReport(tenantId: string, current: ShipmentRecord[], historical: ShipmentRecord[], period: string): ShippingAnalytics {
    const costAnalysis = this.costAnalyzer.analyzeCostByCarrier(current);
    const performanceByCarrier = this.performanceAnalyzer.analyzeCarrierPerformance(current, period);
    ...
    const insights = this.generateInsights(costAnalysis, performanceByCarrier, volumeReport, anomalies);
  ...
  private generateInsights(
    ...
    if (costAnalysis.length > 0) {
      ...
      insights.push(`Top carrier by spend: ${topCarrier.dimensionValue} (${topCarrier.percentOfTotal.toFixed(0)}% of total)`);
    }

    // Performance insight
    const topPerformer = performance.sort((a, b) => b.onTimeRate - a.onTimeRate)[0];
    if (topPerformer) {
      insights.push(`Best on-time performer: ${topPerformer.carrier} (${(topPerformer.onTimeRate * 100).toFixed(0)}%)`);
    }

    // Volume insight
    if (volume.byDestinationState.length > 0) {
      const topState = volume.byDestinationState[0];
      insights.push(`Top shipping destination: ${topState.state} (${topState.count} shipments)`);
    }
    ...
    if (anomalies.length > 0) {
      ...
      if (highSeverity.length > 0) {
        insights.push(`Alert: ${highSeverity.length} high-severity anomalies detected`);
      }
    }
