...
describe("Calendar Engine - Business Hours", () => {
  ...
  test("isTimeWithinOperatingHours should validate business hours", () => {
    ...
    expect(isTimeWithinOperatingHours(weekday9am, operatingHours, "UTC")).toBe(
      true,
    );
    expect(isTimeWithinOperatingHours(weekday5pm, operatingHours, "UTC")).toBe(
      false,
    );
    expect(isTimeWithinOperatingHours(weekend, operatingHours, "UTC")).toBe(
      false,
    );
  });
...
describe("Calendar Engine - Cutoff Logic", () => {
  ...
  test("calculateTotalPrepTime should add conditional minutes", () => {
    const prepTime: PrepTime = {
      baseMinutes: 60,
      conditionalMinutes: [{ condition: "weekend", additionalMinutes: 30 }],
    };

    const weekdayMinutes = calculateTotalPrepTime(
      prepTime,
      new Date("2024-03-04"),
      "UTC",
    );
    const weekendMinutes = calculateTotalPrepTime(
      prepTime,
      new Date("2024-03-03"),
      "UTC",
    );
...
describe("Validator - Rate Validation", () => {
  ...
  test("validateRates should validate weight tiers", () => {
    ...
    expect(result.errors.some((e) => e.code === "WEIGHT_TIER_ORDER")).toBe(
      true,
    );
  });
});
...
describe("Integration - Complete Rate Calculation", () => {
  test("calculateRate should compute shipping cost end-to-end", async () => {
    ...
    try {
      ...
      expect(result.estimatedDeliveryDays.max).toBeGreaterThan(
...
describe("Integration - Delivery Scheduling", () => {
  test("getNextAvailableSlot should find next available delivery", () => {
    ...
    const result = getNextAvailableSlot(slots, startDate, [], "UTC", 14);
...
describe("Edge Cases - Special Values", () => {
  test("calculatePriceBasedRate should handle exact threshold", () => {
    const tiers = [{ orderAmount: 50, rate: 5, freeShipping: false }];
