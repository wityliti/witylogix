 ...
 */
export function validateProfileCompleteness(profile: ShippingProfile): {
  valid: boolean;
  errors: ValidationError[];
} {
  ...
  if (!profile.shopId) {
    addError(
      errors,
      "MISSING_SHOP_ID",
      "Profile must be associated with a shop",
      "shopId",
    );
  }
  ...
  if (!profile.rateType) {
    addError(
      errors,
      "MISSING_RATE_TYPE",
      "Profile must specify a rate type",
      "rateType",
    );
  }
 ...
 */
function validateDistanceTiers(
  tiers: any[],
  rateIndex: number,
  errors: ValidationError[],
): void {
  ...
  for (let i = 0; i < profile.zones.length; i++) {
    for (let j = i + 1; j < profile.zones.length; j++) {
      ...
      if (zone1.postalCodes && zone2.postalCodes) {
        const shared = zone1.postalCodes.filter((pc) =>
          zone2.postalCodes!.includes(pc),
        );
        if (shared.length > 0) {
...
export function validateCalendarProfiles(profile: ShippingProfile): {
  ...
  for (let i = 0; i < profile.calendarProfiles.length; i++) {
    ...
    if (
      ...
      Object.keys(calendar.operatingHours.byDayOfWeek).length === 0
    ) {
      addWarning(
        warnings,
        "CALENDAR_NO_HOURS",
 ...
 */
export function validateShippingProfile(
