  ...
  return (
    <>
              ...
              {(["grid", "map"] as const).map((v) => (
                <button
                  ...
                  className={cn(
                    ...
                    viewMode === v ? "bg-blue-600 text-white" : "bg-wl-bg-elevated text-wl-text-secondary hover:text-white"
                  )}
                >
                  {v}
                </button>
      ...
      <div className={cn("p-6")}>
        ...
        {viewMode === "map" && (
          filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-96 rounded-xl border border-wl-border-default bg-wl-bg-surface text-wl-text-secondary gap-3">
        ...
        {viewMode === "grid" && (
          ...
          >
            <div className={cn("grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4 content-start")}>
              {filtered.length === 0 ? (
                <div className="col-span-full flex flex-col items-center justify-center h-48 rounded-xl border border-wl-border-default bg-wl-bg-surface text-wl-text-secondary gap-3">
                  <p className="text-sm">No locations match your filters</p>
                </div>
              ) : (
                  ...
                  >
                    ...
                    <div className={cn("flex justify-between items-start")}>
                      <div className={cn("flex-1 min-w-0")}>
                        <div className={cn("flex gap-2 items-center mb-1.5")}>
                    ...
                    <div className={cn("text-xs text-wl-text-secondary")}>
                      <div className={cn("font-medium text-wl-text-primary text-sm mb-0.5")}>{location.addressLine1}</div>
            ...
            {/* Detail Panel */}
            {selectedLocation && (
              <div className={cn("bg-wl-bg-surface border border-wl-border-default rounded-xl p-5 flex flex-col gap-4 h-fit sticky top-4")}>
                <div className={cn("flex justify-between items-start")}>
                  <div>
                    <h3 className={cn("text-base font-bold text-white mb-1")}>{selectedLocation.name}</h3>
                    <div className="flex gap-1.5 flex-wrap">
                      <Badge variant={typeVariant(selectedLocation.type)} dot>
                        {typeLabel(selectedLocation.type)}
                      </Badge>
                      <Badge variant={statusVariant(selectedLocation.status)} dot>
                ...
                <div>
                  ...
                  <div className={cn("text-xs text-wl-text-tertiary mt-0.5")}>{selectedLocation.country}</div>
                </div>
                  ...
                  <>
                    ...
                    <div>
                      ...
                      {selectedLocation.phone && (
                        <div className={cn("text-sm text-wl-text-primary font-mono mb-1")}>{selectedLocation.phone}</div>
                      )}
                      {selectedLocation.email && (
                        <div className={cn("text-sm text-wl-text-secondary")}>{selectedLocation.email}</div>
                      )}
                    </div>
                  </>
                )}
                ...
                <div>
                  <div className={cn("text-xs font-semibold text-wl-text-secondary uppercase mb-2 tracking-wider")}>Performance</div>
                  <div className={cn("grid grid-cols-2 gap-3")}>
                    <div>
                      <div className={cn("text-xs text-wl-text-secondary mb-0.5")}>Active Shipments</div>
                      <div className={cn("text-lg font-bold font-mono text-blue-400")}>{selectedLocation.activeShipments}</div>
                    </div>
                    <div>
                      <div className={cn("text-xs text-wl-text-secondary mb-0.5")}>Total Processed</div>
                      <div className={cn("text-lg font-bold font-mono text-emerald-500")}>{selectedLocation.totalProcessed}</div>
                    </div>
                    <div>
                      <div className={cn("text-xs text-wl-text-secondary mb-0.5")}>Avg Prep Time</div>
                      <div className={cn("text-lg font-bold font-mono text-wl-text-primary")}>{selectedLocation.avgPrepTime}m</div>
                    </div>
                  </div>
                </div>
                ...
                {selectedLocation.operatingHours && (
                  <>
                    <div className={cn("h-px bg-wl-bg-elevated")} />
                    <div>
                      <div className={cn("text-xs font-semibold text-wl-text-secondary uppercase mb-2 tracking-wider")}>Operating Hours</div>
                        ...
                        <tbody>
                          {Object.entries(selectedLocation.operatingHours).map(([day, hours]) => (
                            <tr key={day} className={cn("border-b border-wl-border-default last:border-0")}>
                              <td className={cn("py-1.5 pr-3 text-wl-text-secondary font-medium whitespace-nowrap")}>{day}</td>
                              <td className={cn("py-1.5", hours.open === "closed" ? "text-wl-text-tertiary" : "text-white font-mono")}>
