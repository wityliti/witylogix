...
export default function QueuesPage() {
  ...
  const { data: queuesData, loading, error, refetch } = useApiQuery<QueuesResponse>('/api/v4/admin/queues');
  const { data: jobsData, loading: jobsLoading } = useApiQuery<QueueJobsResponse>(
    selectedQueue ? `/api/v4/admin/queues/${encodeURIComponent(selectedQueue)}/jobs?status=${jobStatusFilter === 'all' ? 'active' : jobStatusFilter}` : null,
  );
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default: return "bg-wl-bg-surface text-gray-800";
    }
  };
    ...
    <div className="min-h-screen bg-wl-bg-surface">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {selectedQueue && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                <Card className="lg:col-span-1">
                  <CardHeader>
                    <CardTitle className="text-sm">Queue Health: {selectedQueue}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {(() => {
                      ...
                      return (
                        <div className="space-y-4">
                          ...
                          <div>
                            <div className="flex justify-between text-sm mb-1">
                              <span>Status</span>
                              <span className="font-semibold">{q.paused ? "Paused" : "Running"}</span>
                            </div>
                          </div>
                        </div>
                      );
        ...
        <Card className="mb-8">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>Jobs {selectedQueue ? `— ${selectedQueue}` : "(select a queue)"}</CardTitle>
              <div className="flex gap-3">
                ...
                <select
                  ...
                  className="px-3 py-2 border border-wl-border-subtle rounded-md text-sm"
                >
                      ...
                      <tr key={job.id} className="border-b hover:bg-wl-bg-surface">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <div className="w-24 bg-wl-bg-surface rounded-full h-2">
                              <div
              ...
              <div className="space-y-3">
                {dlqItems.map(item => (
                  ...
                  >
                    ...
                    <p className="text-sm text-wl-text-tertiary mb-3">{item.failedReason}</p>
                    <div className="flex items-center justify-between">
