...
export default function FreightPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<Shipment>("/api/v4/shipments?type=freight");

  const stats = useMemo(() => {
    const booked = shipments.filter((l) => l.status === "Booked").length;
    const inTransit = shipments.filter((l) =>
      ["In-Transit", "IN_TRANSIT", "PICKED_UP"].includes(l.status),
    ).length;
    const delivered = shipments.filter((l) =>
      ["Delivered", "DELIVERED"].includes(l.status),
    ).length;
    const exceptions = shipments.filter((l) =>
      ["Exception", "FAILED", "FAILED_ATTEMPT"].includes(l.status),
    ).length;
    const ratedShipments = shipments.filter((l) => l.rate && l.rate > 0);
    const avgCostPerMile =
      ratedShipments.length > 0
        ? Math.round(
            (ratedShipments.reduce((sum, l) => sum + (l.rate ?? 0), 0) /
              ratedShipments.length) *
              100,
          ) / 100
        : 0;
  ...
  const topCarriers = useMemo(() => {
    const carrierVolume: Record<
      string,
      { name: string; count: number; rate: number }
    > = {};
    shipments.forEach((s) => {
      if (s.carrier) {
    ...
    return Object.values(carrierVolume)
      ...
      .map((c) => ({
        ...c,
        avgRate: c.count > 0 ? Math.round(c.rate / c.count) : 0,
      }));
  }, [shipments]);
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-wl-text-secondary mt-0.5">
                Shipments, carriers, and cost optimization
              </p>
            </div>
                    ...
                    {topCarriers.map((carrier, idx) => (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-wl-text-tertiary mt-1">
                            {carrier.count} loads · Avg rate: ${carrier.avgRate}
                          </p>
                        </div>
                  ...
                  {[
                    {
                      label: "Booked",
                      count: stats.booked,
                      color: "bg-wl-info-500",
                      border: "border-wl-info-500/30",
                    },
                    {
                      label: "In-Transit",
                      count: stats.inTransit,
                      color: "bg-wl-warning-500",
                      border: "border-wl-warning-500/30",
                    },
                    {
                      label: "Delivered",
                      count: stats.delivered,
                      color: "bg-wl-success-500",
                      border: "border-wl-success-500/30",
                    },
                    {
                      label: "Exceptions",
                      count: stats.exceptions,
                      color: "bg-wl-danger-500",
                    ...
                    <div
                      key={item.label}
                      className={cn(
                        "flex items-center justify-between p-3 bg-wl-bg-overlay rounded-lg border",
                        item.border,
                      )}
                    >
                      <div className="flex items-center gap-3">
              ...
              <Card className="p-6 bg-wl-bg-surface border-wl-border-default">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 rounded-lg bg-wl-bg-overlay border border-wl-border-default">
                    <p className="text-xs font-medium text-wl-text-tertiary mb-2 uppercase">
                      Total Shipments
                    </p>
                  ...
                  <div className="p-4 rounded-lg bg-wl-bg-overlay border border-wl-border-default">
                    ...
                    <p className="text-2xl font-bold text-wl-text-primary">
                      ${stats.avgCostPerMile}
                    </p>
                    <p className="text-xs text-wl-text-tertiary mt-1">
                      Across rated shipments
                    </p>
                  </div>
                </div>
              </Card>
