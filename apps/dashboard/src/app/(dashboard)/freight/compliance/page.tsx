...
export default function FreightCompliancePage() {
  ...
  const stats = useMemo(() => {
    const compliantCount = shipments.filter((s) =>
      s.status === 'compliant' || s.status === 'DELIVERED'
    ).length;
    const avgScore = shipments.length > 0
  ...
  return (
    <div className="flex flex-col min-h-screen bg-wl-bg-primary p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-wl-text-secondary mt-0.5">{shipments.length} shipments monitored</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <Card className="bg-wl-bg-surface border-wl-border-default">
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <span className="text-wl-text-secondary text-sm font-medium">Compliant Shipments</span>
              <CheckCircle className="text-wl-success-500" size={20} />
            </div>
            <p className="text-3xl font-bold text-wl-text-primary">{stats.compliantCount}</p>
            <p className="text-wl-text-tertiary text-xs mt-2">{stats.avgScore}% compliance rate</p>
          </div>
        </Card>
        ...
        <Card className="bg-wl-bg-surface border-wl-border-default">
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <span className="text-wl-text-secondary text-sm font-medium">Avg. Score</span>
              <TrendingUp className="text-wl-info-500" size={20} />
            </div>
            <p className="text-3xl font-bold text-wl-text-primary">{stats.avgScore}%</p>
            <p className="text-wl-text-tertiary text-xs mt-2">Overall compliance</p>
          </div>
        </Card>
        ...
        <Card className="bg-wl-bg-surface border-wl-border-default">
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <span className="text-wl-text-secondary text-sm font-medium">Flagged Items</span>
              <AlertCircle className="text-wl-warning-500" size={20} />
            </div>
            <p className="text-3xl font-bold text-wl-text-primary">{stats.flagged}</p>
            <p className="text-wl-text-tertiary text-xs mt-2">Requiring review</p>
          </div>
        </Card>
      </div>
      ...
      {shipments.length === 0 ? (
        <Card className="bg-wl-bg-surface border-wl-border-default">
          <div className="p-12 text-center text-wl-text-tertiary">No shipments found</div>
        </Card>
      ) : (
        <Card className="bg-wl-bg-surface border-wl-border-default">
          <div className="p-6">
            <h3 className="text-lg font-semibold text-wl-text-primary mb-4">Compliance Status</h3>
            <div className="space-y-3">
              {filtered.slice(0, 15).map((shipment) => {
                const isCompliant = shipment.status === 'compliant' || shipment.status === 'DELIVERED';
                const score = shipment.complianceScore ?? (isCompliant ? 100 : 70);
                return (
                  <div
                    key={shipment.id}
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-wl-text-tertiary">{shipment.carrier || 'Unknown Carrier'}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <p className="text-sm font-semibold text-wl-text-primary">{score}%</p>
