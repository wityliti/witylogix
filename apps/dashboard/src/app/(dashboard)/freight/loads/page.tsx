...
const DeliveryMapView = dynamic(
  () => import("../../delivery/components/delivery-map-view"),
  {
    ssr: false,
    loading: () => (
      <div className="h-[560px] rounded-xl bg-wl-bg-surface animate-pulse" />
    ),
  },
);
...
export default function FreightLoadsPage() {
  ...
  const {
    ...
    refetch,
    setPage,
  } = useApiList<Shipment>("/api/v4/shipments?type=freight&view=loads");

  if (loading) return <LoadingSkeleton />;
  if (error) return <ErrorState message={error.message} onRetry={refetch} />;

  const activeLoads = shipments.filter((s) =>
    ["Booked", "In-Transit", "IN_TRANSIT", "PICKED_UP"].includes(s.status),
  ).length;
  const mappableLoads = shipments.filter(
    (s) => s.deliveryLocation?.lat != null,
  ).length;
        ...
        <Card className="bg-wl-bg-surface border-wl-border-default">
          <div className="p-6">
            ...
            <p className="text-3xl font-bold text-wl-text-primary">
              {
                shipments.filter(
                  (s) => s.status === "Available" || s.status === "PENDING",
                ).length
              }
            </p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-wl-bg-overlay border-b border-wl-border-default">
                <tr>
                  <th className="px-6 py-4 text-left font-semibold text-wl-text-secondary">
                    Load ID
                  </th>
                  <th className="px-6 py-4 text-left font-semibold text-wl-text-secondary">
                    Recipient
                  ...
                  <th className="px-6 py-4 text-center font-semibold text-wl-text-secondary">
                    Status
                  </th>
                </tr>
              </thead>
              <tbody>
                {shipments.length === 0 ? (
                  <tr>
                    <td
                      colSpan={4}
                      className="px-6 py-12 text-center text-wl-text-tertiary"
                    >
                      No loads found
                    </td>
                  </tr>
