...
function HealthGauge({ score, size = "lg" }: GaugeProps) {
  const getColor = (score: number) => {
    ...
    if (score >= 70) return "text-wl-warning-400";
    return "text-wl-danger-400";
  };

  const getBgColor = (score: number) => {
    if (score >= 90) return "from-wl-success-500/10 to-wl-success-500/5";
    if (score >= 70) return "from-wl-warning-500/10 to-wl-warning-500/5";
    return "from-wl-danger-500/10 to-wl-danger-500/5";
  };
        ...
        <Card className="bg-wl-bg-secondary border-wl-neutral-700">
          <CardContent className="pt-6">
            <div className="space-y-2">
              ...
              <p className="text-3xl font-bold text-wl-success-400">
        ...
        <Card className="bg-wl-bg-secondary border-wl-neutral-700">
          <CardContent className="pt-6">
            <div className="space-y-2">
              ...
              <p className="text-3xl font-bold text-wl-warning-400">
          ...
          <CardContent>
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {sortedAlerts.map((alert) => (
                <div
                  key={alert.id}
                  className="flex items-start justify-between gap-3 p-3 rounded-lg bg-wl-bg-tertiary border border-wl-neutral-700"
                >
                  <div className="flex gap-3 flex-1 min-w-0">
                    {alert.severity === "critical" && (
                      <XCircle className="w-5 h-5 text-wl-danger-400 flex-shrink-0 mt-0.5" />
                    )}
                    {alert.severity === "warning" && (
                      <AlertTriangle className="w-5 h-5 text-wl-warning-400 flex-shrink-0 mt-0.5" />
                    )}
                    {alert.severity === "info" && (
            ...
            <div className="h-64 flex items-end gap-2 px-4">
              {health.errorTrend.map((point, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="bg-gradient-to-t from-wl-danger-500 to-wl-danger-400 w-full rounded-t transition-all hover:from-wl-danger-400 hover:to-wl-danger-300"
        ...
        <div className="flex-1 min-w-64">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-lg bg-wl-bg-secondary border border-wl-neutral-700 text-wl-text-primary placeholder-wl-text-secondary focus:outline-none focus:border-wl-primary-500 focus:ring-1 focus:ring-wl-primary-500"
            />
          </div>
        </div>
        ...
        {categories.length > 0 && (
          <div className="flex gap-2">
            ...
            <select
              ...
              className="px-3 py-2 rounded-lg bg-wl-bg-secondary border border-wl-neutral-700 text-wl-text-primary text-sm focus:outline-none focus:border-wl-primary-500"
            >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredProviders.map((provider) => (
            <Card
              key={provider.id}
              className="bg-wl-bg-secondary border-wl-neutral-700 hover:border-wl-neutral-600 transition-colors cursor-pointer"
            >
              <CardContent className="pt-6">
