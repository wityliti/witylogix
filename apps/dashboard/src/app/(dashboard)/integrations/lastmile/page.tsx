...
export default function LastMileIntegrationPage() {
  ...
  const { data: partnerStatsData, loading: statsLoading } = useApiQuery<{
    stats: PartnerStat[];
    ...
    return (
      <>
        <Header
          ...
          subtitle="Courier partner management and delivery tracking"
        />
        <div className="p-6">
          ...
          {partnersLoading ? (
            <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
              {[...Array(3)].map((_, i) => (
                <div
                  key={i}
                  className="h-40 rounded-lg bg-wl-bg-surface animate-pulse"
                />
              ))}
            </div>
          ) : (partners ?? []).length === 0 ? (
            <Card>
              <CardContent>
                <div className="py-8 text-center text-gray-400 text-sm">
                  No courier partners configured. Add a courier to start
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <Badge
                            variant={getHealthVariant(partner.healthStatus)}
                          >
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(partnerStatsData?.stats ?? []).map((stat, idx) => (
                      ...
                      >
                        <td className="p-3 text-white font-semibold capitalize">
                          {stat.provider.replace(/_/g, " ")}
                        </td>
                        <td className="p-3 text-center text-white font-semibold">
                          {stat.activeDeliveries}
                        </td>
                        <td className="p-3 text-center text-white font-semibold">
        ...
        <div className="grid grid-cols-[1fr_1fr] gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <div className="flex gap-2 ml-auto">
                {(["all", "PENDING", "IN_TRANSIT", "DELIVERED"] as const).map(
                  (s) => (
                    <button
                      key={s}
              ...
              {deliveriesLoading ? (
                <div className="space-y-2">
                  {[...Array(4)].map((_, i) => (
                    <div
                      key={i}
                      className="h-16 rounded-md bg-wl-bg-surface animate-pulse"
                    />
                  ))}
                </div>
              ) : deliveriesError ? (
                  ...
                  {(deliveries ?? []).map((d) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-1">
                        ...
                        <Badge
                          variant={getDeliveryStatusVariant(d.status)}
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-400">
                        ...
                        <span>
                          {d.quote?.distanceKm != null
                            ? `${d.quote.distanceKm} km`
                            : ""}
                          {d.actualCost != null
          ...
          <Card>
            <CardHeader>
              ...
              <div className="flex gap-2 ml-auto">
                {(["all", "AVAILABLE", "ON_ROUTE", "OFFLINE"] as const).map(
                  (s) => (
                    <button
                      key={s}
                    ...
                    >
                      ...
                      <div className="flex justify-between text-xs text-gray-400 mt-1">
                        ...
                        {driver.lastLocationAt && (
                          <span>
                            Seen {formatRelative(driver.lastLocationAt)}
                          </span>
                        )}
                      </div>
                    </div>
