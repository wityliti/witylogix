...
export default function PaymentsIntegrationPage() {
  ...
  const { data: summaryData, loading: sumLoading } =
  ...
  } = useApiQuery<{ bankTransactions: ReconciliationItem[] }>(
  ...
  const totalTxns =
    summaryData?.byStatus?.reduce((s, b) => s + b._count, 0) ?? 0;
  const totalFeesPct = gateways.length
    ? gateways.reduce((s, g) => s + g.transactionFeePercent, 0) /
      gateways.length
    : 2.9;

  const reconItems: ReconciliationItem[] = reconData?.bankTransactions ?? [];
  const unmatchedCount = reconItems.filter(
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {gwLoading ? (
            ...
            <div className="rounded-xl bg-wl-bg-surface border border-wl-border-default p-10 text-center">
              ...
              <p className="text-xs text-wl-text-tertiary mb-4">
                Connect Stripe, PayPal, Square, or other providers
              </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {gateways.map((gw) => (
                <Card
                  key={gw.id}
                  className="bg-wl-bg-surface border border-wl-border-default hover:border-wl-border-strong transition-all"
                >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-wl-text-tertiary mt-0.5">
                            {gw.transactionFeePercent}% + $
                            {(gw.fixedFeeInCents / 100).toFixed(2)} per txn
                          </p>
                        </div>
                      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-wl-border-default">
                    {["Date", "Method", "Amount", "Status", "Reference"].map(
                      (h) => (
                        <th
                          key={h}
        ...
        >
          {pmLoading ? (
            ...
            <div className="rounded-xl bg-wl-bg-surface border border-wl-border-default p-8 text-center">
              ...
              <p className="text-xs text-wl-text-tertiary mb-4">
                Add credit cards, bank accounts, or digital wallets
              </p>
              <Link href="/settings/payments">
                <Button variant="primary" size="sm">
                  <Plus className="w-4 h-4 mr-1.5" />
            ...
            <div className="space-y-3">
              {paymentMethods.map((pm) => (
                <div
                  key={pm.id}
                  className="flex items-center justify-between rounded-xl bg-wl-bg-surface border border-wl-border-default px-5 py-4 hover:border-wl-border-strong transition-all"
                >
                  <div className="flex items-center gap-3">
                ...
                <tbody className="divide-y divide-wl-border-default">
                  {reconItems.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 font-mono text-sm text-wl-text-primary">
                        {fmtAmt(item.amount)}
                      </td>
        ...
        <div className="mt-2 flex items-center justify-between rounded-xl bg-wl-bg-surface border border-wl-border-default px-5 py-4">
          <div>
            ...
            <p className="text-xs text-wl-text-tertiary mt-0.5">
              Browse the marketplace for Stripe, PayPal, Square and 20+ more
            </p>
          </div>
          <div className="flex gap-2">
            <Button
              variant="ghost"
              size="sm"
              className="gap-1 text-wl-text-tertiary hover:text-wl-text-secondary"
            >
