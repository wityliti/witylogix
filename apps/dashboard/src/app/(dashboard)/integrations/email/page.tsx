...
import { Mail, CheckCircle2, AlertCircle, FileText, RefreshCw, Plus } from 'lucide-react';
...
const EMAIL_SLUGS = new Set(['sendgrid', 'mailgun', 'aws_ses', 'postmark', 'resend', 'smtp']);
    ...
    <>
      ...
      <div className="p-6 bg-wl-bg-root space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Active Providers', value: stats.total, icon: Mail, color: 'text-wl-info-400' },
            ...
            { label: 'Templates', value: stats.templates, icon: FileText, color: 'text-wl-text-primary' },
          ].map((s) => {
            const Icon = s.icon;
            return (
          ...
          <CardContent className="p-0">
            {providers.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-wl-text-secondary mb-1">No email providers configured</p>
                <p className="text-sm text-wl-text-muted mb-6 max-w-sm mx-auto">
                  Connect SendGrid, Mailgun, Postmark, AWS SES, Resend, and more to send
                  transactional emails for order confirmations and delivery updates.
                </p>
                <Link href="/integrations/marketplace?category=COMMUNICATION">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-wl-text-muted">Last used: {timeSince(conn.lastSyncTime)}</p>
                      </div>
                    </div>
        ...
        <Card className="bg-wl-bg-surface border-wl-border-default">
          ...
          <CardContent>
            {stats.templates === 0 ? (
              <p className="text-sm text-wl-text-muted text-center py-4">No notification templates yet.</p>
            ) : (
              <p className="text-sm text-wl-text-secondary">
                {stats.templates} template{stats.templates !== 1 ? 's' : ''} configured.{' '}
                <Link href="/settings/notifications/templates" className="text-wl-info-400 hover:underline">
                  View all →
                </Link>
              </p>
            )}
