...
function dutyBadge(duty: string) {
  ...
  if (d === "driving")
    return (
      <Badge variant="danger" className="text-xs bg-red-500/10 text-red-400">
        Driving
      </Badge>
    );
  if (d === "on_duty")
  ...
  if (d === "sleeper")
    return (
      <Badge variant="info" className="text-xs bg-blue-500/10 text-blue-400">
        Sleeper
      </Badge>
    );
  return (
...
function HosBar({ remaining, max = 11 }: { remaining: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-mono text-wl-text-tertiary w-10 text-right">
...
export default function ELDIntegrationPage() {
  ...
  const {
    ...
    "/api/v4/integrations/connections?category=eld&limit=20",
  );
  ...
  const { data: compliance, loading: compLoading } = useApiQuery<EldCompliance>(
    "/api/v4/eld/compliance",
  );

  const criticalCount = violations.filter(
    (v) => v.severity.toLowerCase() === "critical",
  ).length;
  const dvirFails = dvirList.filter(
    (d) => d.status === "FAIL" || d.status === "fail",
  ).length;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link href="/integrations">
          <Button
            variant="ghost"
            size="sm"
            className="mb-6 gap-1.5 text-wl-text-tertiary hover:text-wl-text-secondary"
          >
        ...
        >
          {connLoading ? (
            ...
            <ErrorState
              title="Failed to load ELD providers"
              onRetry={connRefetch}
            />
            ...
            <div className="rounded-xl bg-wl-bg-surface border border-wl-border-default p-10 text-center">
              ...
              <p className="text-sm font-medium text-wl-text-secondary mb-1">
                No ELD providers connected
              </p>
              <p className="text-xs text-wl-text-tertiary mb-4">
                Connect Motive, Omnitracs, Azuga, or other ELD providers
              </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {connections.map((conn) => (
                <div
                  key={conn.id}
                  className="rounded-xl bg-wl-bg-surface border border-wl-border-default p-5 hover:border-wl-border-strong transition-all"
                >
                  ...
                  <div className="flex gap-2 pt-3 border-t border-wl-border-default">
                    <Link
                      href={`/integrations/connected/${conn.slug}`}
                      className="flex-1"
                    >
                    ...
                    <Link href="/eld">
                      <Button
                        variant="ghost"
                        size="sm"
                        className="text-wl-text-tertiary hover:text-wl-text-secondary"
                      >
        ...
        >
          {drvLoading ? (
            ...
            <ErrorState
              title="Failed to load driver HOS data"
              onRetry={drvRefetch}
            />
          ) : eldDrivers.length === 0 ? (
            <div className="rounded-xl bg-wl-bg-surface border border-wl-border-default p-8 text-center">
              ...
              <p className="text-sm font-medium text-wl-text-secondary">
                No active drivers with ELD data
              </p>
              <p className="text-xs text-wl-text-tertiary mt-1">
                Connect an ELD provider to see HOS records
              </p>
            </div>
          ) : (
                ...
                <thead>
                  <tr className="border-b border-wl-border-default">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-4 py-3 text-left text-xs font-medium text-wl-text-tertiary uppercase tracking-wide"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {eldDrivers.map((drv) => (
                    <tr
                      ...
                      className={cn(
                        ...
                        drv.violations > 0 && "bg-red-950/20",
                      )}
                    >
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-wl-text-tertiary font-mono">
                          {drv.driverId.slice(0, 8)}
                        </p>
                      </td>
                      <td className="px-4 py-3">
                        {dutyBadge(drv.currentDuty)}
                      </td>
              ...
              <div className="px-4 py-3 border-t border-wl-border-default flex justify-end">
                <Link href="/eld">
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-xs gap-1 text-wl-text-tertiary hover:text-wl-text-secondary"
                  >
                    Full ELD Dashboard{" "}
        ...
        >
          {dvirLoading ? (
            ...
            <ErrorState
              title="Failed to load DVIR reports"
              onRetry={dvirRefetch}
            />
          ) : dvirList.length === 0 ? (
            <div className="rounded-xl bg-wl-bg-surface border border-wl-border-default p-8 text-center">
              ...
              <p className="text-xs text-wl-text-tertiary mt-1">
                Reports appear after drivers complete pre/post-trip inspections
              </p>
            </div>
          ) : (
            <div className="rounded-xl bg-wl-bg-surface border border-wl-border-default overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-wl-border-default">
                    {["Report ID", "Vehicle", "Status", "Defects", "Date"].map(
                      (h) => (
                        <th
                          key={h}
                          className="px-4 py-3 text-left text-xs font-medium text-wl-text-tertiary uppercase tracking-wide"
                ...
                <tbody className="divide-y divide-wl-border-default">
                  {dvirList.map((dvir) => {
                    const fail =
                      dvir.status === "FAIL" || dvir.status === "fail";
                    const pending =
                      dvir.status === "PENDING" || dvir.status === "pending";
                    return (
                      <tr
                        key={dvir.id}
                        className={cn(
                      ...
                      >
                        <td className="px-4 py-3 text-xs text-wl-text-tertiary font-mono">
                          {dvir.id.slice(0, 8)}
                        </td>
        ...
        >
          {violLoading ? (
            ...
            <div className="rounded-xl bg-wl-bg-surface border border-wl-border-default p-8 text-center">
              ...
              <p className="text-xs text-wl-text-tertiary mt-1">
                All drivers are within HOS limits
              </p>
            </div>
          ) : (
        ...
        <div className="mt-2 flex items-center justify-between rounded-xl bg-wl-bg-surface border border-wl-border-default px-5 py-4">
          <div>
            <p className="text-sm font-medium text-wl-text-primary">
              Need another ELD provider?
            </p>
            <p className="text-xs text-wl-text-tertiary mt-0.5">
              Browse Motive, Omnitracs, Azuga, and more in the marketplace
            </p>
          </div>
          <Link href="/integrations/marketplace">
            <Button variant="primary" size="sm">
              <Plus className="w-4 h-4 mr-1.5" />
              Browse ELD Providers
            </Button>
          </Link>
        </div>
      </div>
