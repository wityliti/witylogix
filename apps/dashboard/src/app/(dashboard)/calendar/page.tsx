...
export default function CalendarPage() {
  ...
  const {
    ...
    loading,
    error,
  } = useApiList<CalendarRule>("/api/v4/calendar-rules");
  ...
  const filtered = rules.filter((rule) => {
    ...
    if (selectedZone !== "all" && !rule.zones.includes(selectedZone))
      return false;
    return true;
  });
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-500">
                        ...
                        {rule.capacity && (
                          <span>Capacity: {rule.capacity}</span>
                        )}
                        {rule.recurrencePattern && (
                          <span>Pattern: {rule.recurrencePattern}</span>
                        )}
                      </div>
                    </div>
                    <Button
