...
const FleetVehiclesMapView = dynamic(
  () => import("./components/fleet-vehicles-map-view"),
  {
    ...
    loading: () => (
      <div
        className="w-full rounded-xl bg-wl-bg-elevated border border-wl-border-default flex items-center justify-center"
        style={{ height: 560 }}
      >
...
function FuelBar({ level }: { level: number | null | undefined }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-medium text-wl-text-primary w-8 text-right">
        {pct}%
      </span>
    </div>
  );
...
export default function VehiclesPage() {
  ...
  const filtered = vehicles.filter((v) => {
    ...
    const searchMatch =
      !localSearch ||
      [v.name, v.make, v.model, v.licensePlate, v.vin].some((f) =>
  ...
  const statusCounts: Record<StatusFilter, number> = {
    ...
    OFFLINE: vehicles.filter((v) => v.status.toUpperCase() === "OFFLINE")
      .length,
  ...
  return (
    <>
      ...
      {/* Map view */}
      {viewMode === "map" && <FleetVehiclesMapView vehicles={filtered} />}
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-wl-border-subtle bg-wl-bg-elevated/50">
                  ...
                  <th className="text-center px-4 py-3 text-xs font-semibold text-wl-text-tertiary uppercase tracking-wide hidden lg:table-cell">
                    Location
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-wl-text-tertiary uppercase tracking-wide hidden md:table-cell">
                    Fuel
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((vehicle, idx) => {
                    ...
                    const statusCfg =
                      STATUS_CONFIGS[statusKey] ?? STATUS_CONFIGS.INACTIVE;
                    const displayName =
                      vehicle.name ||
