...
const StoreZoneCoverageMap = dynamic(
  () =>
    import("./components/store-zone-coverage-map").then(
      (m) => m.StoreZoneCoverageMap,
    ),
  {
    ssr: false,
    loading: () => (
...
export default function StoresManagement() {
  const {
    ...
    error: shopError,
    refetch: refetchShop,
  } = useApiQuery<ShopProfile>("/api/v4/shops/me");
  const {
    data: stats,
    loading: statsLoading,
    refetch: refetchStats,
  } = useApiQuery<ShopStats>("/api/v4/shops/me/stats");
      ...
      <div className="px-6 py-8 border-b border-wl-border-default flex justify-between items-center">
        <div>
          ...
          <p className="text-wl-text-secondary text-sm">
            Manage your connected e-commerce store
          </p>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {Array.from({ length: 4 }).map((_, i) => (
              <Card
                key={i}
                className="bg-wl-bg-surface border border-wl-border-default animate-pulse h-24"
              >
                {" "}
              </Card>
            ))}
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-white">
                        {stats?.orders.total?.toLocaleString() ??
                          shop._count?.orders?.toLocaleString() ??
                          "—"}
                      </p>
                    </div>
              ...
              <CardContent className="p-5">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4 pb-4 border-b border-wl-border-default">
                  ...
                  <div>
                    ...
                    <p className="text-white font-semibold">
                      {shop._count?.deliveryZones ?? stats?.zones.total ?? "—"}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                {zonesLoading ? (
                  ...
                  <span className="text-wl-text-secondary text-xs">
                    {zonesGeoJson?.features.length ?? 0} delivery zone
                    {(zonesGeoJson?.features.length ?? 0) !== 1 ? "s" : ""}
                  </span>
                )}
              </div>
              {zonesLoading ? (
