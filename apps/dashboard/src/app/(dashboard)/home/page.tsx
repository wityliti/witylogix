...
function KPICard({
  ...
  if (loading) {
    return (
      <div className="bg-wl-bg-root border border-wl-border-strong rounded-lg p-6 animate-pulse">
  ...
  return (
    <div className="bg-wl-bg-root border border-wl-border-strong rounded-lg p-6 transition-all duration-200 hover:border-wl-border-default hover:bg-wl-bg-root/80">
...
function OrderFeedItem({ order }: { order: ApiOrder }) {
  ...
  return (
    <div className="border-b border-wl-border-strong last:border-0 pb-4 last:pb-0 transition-all duration-200 hover:bg-wl-bg-root/50 px-4 py-3 rounded -mx-4">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-wl-text-tertiary truncate">{destination}</p>
          {eta && <p className="text-xs text-wl-text-tertiary mt-1">ETA: {eta}</p>}
        </div>
...
const DRIVER_STATUS_CONFIG: Record<string, { bg: string; border: string; text: string; dot: string; label: string }> = {
  ...
  OFFLINE: { bg: 'bg-wl-bg-overlay/20', border: 'border-wl-border-default/50', text: 'text-wl-text-secondary', dot: '#6b7280', label: 'OFFLINE' },
};

function DriverStatusCard({ driver, loading = false }: { driver?: ApiDriver; loading?: boolean }) {
  if (loading) {
    return (
      <div className="bg-wl-bg-root border border-wl-border-strong rounded-lg p-4 animate-pulse">
  ...
  return (
    <div className={cn('bg-wl-bg-root border border-wl-border-strong rounded-lg p-4 transition-all duration-200 hover:border-wl-border-default', config.bg)}>
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="text-right">
          <p className="text-sm font-bold text-wl-text-primary">{activeDeliveries}</p>
    ...
    <div className="bg-wl-bg-root min-h-screen">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-wl-text-primary">Dashboard</h1>
                ...
                <div className="-mx-6 px-6">
                  {ordersLoading ? (
                    ...
                    <div className="text-center py-8">
                      <Icon d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" size={40} className="mx-auto text-wl-text-tertiary mb-2" />
                      <p className="text-wl-text-tertiary">No orders yet</p>
                    </div>
                  )}
                </div>
                {recentOrders.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-wl-border-strong">
                ...
                <div className="grid gap-4">
                  {driversLoading
                    ...
                    : displayDrivers.length === 0
                      ? <p className="text-sm text-wl-text-tertiary text-center py-4">No drivers found</p>
