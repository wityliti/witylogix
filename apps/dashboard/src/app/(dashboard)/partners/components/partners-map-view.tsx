...
import {
  ...
  type CoveragePartner,
} from "@/components/map/partner-coverage-layer";
...
export default function PartnersMapView({
  partners,
  onPartnerClick,
}: PartnersMapViewProps) {
  ...
  const totalCities = useMemo(
    () =>
      partners.reduce(
        (sum, p) =>
          sum + p.serviceAreas.filter((c) => lookupCity(c) !== null).length,
        0,
      ),
    [partners],
  );
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-3 left-3 bg-wl-bg-elevated/90 border border-wl-border-default rounded-lg p-3 text-xs flex flex-col gap-1.5 backdrop-blur-sm">
        <p className="font-semibold text-wl-text-secondary uppercase tracking-wider mb-1">
          Coverage
        </p>
