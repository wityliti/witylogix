...
export default function OrdersPage() {
  ...
  const {
    ...
    error: wavesError,
  } = useApiList<WavePlan>("/api/v4/supply-chain/waves");
  const {
    items: batchPicking,
    loading: batchesLoading,
    error: batchesError,
  } = useApiList<BatchPickingTask>("/api/v4/supply-chain/batches");
  const {
    items: returnQueue,
    loading: returnsLoading,
    error: returnsError,
    refetch: refetchReturns,
  } = useApiList<RawReturn>("/api/v4/returns");
  const { items: warehouseItems } = useApiList<{ name: string }>(
  ...
  const filteredOrders = orders.orders.filter((order) => {
    ...
    return (
      matchesSearch && matchesStatus && matchesPriority && matchesWarehouse
    );
  });
  ...
  if (orders.loading || fulfillment.loading) {
    return (
      <>
        <Header
          title="Supply Chain Orders"
          subtitle="Order pipeline and fulfillment"
        />
        <div className="p-6">
  ...
  return (
    <>
      <Header
        title="Supply Chain Orders"
        subtitle={`${orders.orders.length} orders in pipeline`}
      />
      <div className="space-y-8 p-6">
          ...
          {[
            ...
            <Card key={stat.label}>
              <CardContent className="pt-4 text-center">
                ...
                <p className="text-xs text-wl-neutral-300 mt-1">{stat.label}</p>
              </CardContent>
            </Card>
          ))}
        </div>
              ...
              {filteredOrders.map((order) => (
                <Card
                  ...
                  className={cn(
                    ...
                    selectedOrder === order.id && "border-blue-500",
                  )}
                >
                  <CardHeader>
        ...
        {selectedTab === "batches" && (
          <div className="space-y-4">
            ...
            {batchesError && (
              <ErrorState
                message="Failed to load batch picking tasks"
                onRetry={() => window.location.reload()}
              />
            )}
            {!batchesLoading && !batchesError && batchPicking.length === 0 ? (
              <div className="text-center py-10 text-wl-text-secondary">
                No batch picking tasks found.
              </div>
            ) : null}
