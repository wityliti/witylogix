...
import {
  WarehouseLayer,
  type WarehousePin,
} from "@/components/map/warehouse-layer";
...
export function SupplyChainWarehouseMap({
  warehouses,
}: SupplyChainWarehouseMapProps) {
  const pins = warehouses
    .map(toWarehousePin)
      ...
      <div className="flex flex-col items-center justify-center h-full text-center gap-3 bg-wl-bg-surface rounded-xl border border-wl-border-default p-12">
        ...
        <div>
          <p className="text-sm font-medium text-wl-text-secondary">
            No geocoded warehouses
          </p>
          <p className="text-xs text-wl-text-tertiary mt-1">
            Set lat/lng coordinates on your locations to see them on the map.
          </p>
        </div>
      </div>
    );
    ...
    <div className="relative h-full rounded-xl overflow-hidden border border-wl-border-default">
      ...
      <div className="absolute bottom-4 left-4 bg-wl-bg-elevated/90 backdrop-blur rounded-lg border border-wl-border-default p-3 text-xs space-y-1.5">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-wl-text-tertiary">&lt;60% — Low</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-emerald-500 flex-shrink-0" />
          <span className="text-wl-text-tertiary">60–79% — Moderate</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-amber-500 flex-shrink-0" />
          <span className="text-wl-text-tertiary">80–94% — High</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-red-500 flex-shrink-0" />
          <span className="text-wl-text-tertiary">≥95% — Critical</span>
        </div>
      </div>
