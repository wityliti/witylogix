...
import { RevenueZoneLayer, type RevenueZoneEntry } from '@/components/map/revenue-zone-layer';
...
function FitToZones({ zones }: FitToZonesProps) {
  ...
  const coords = useMemo(() => {
    ...
    for (const f of zones.features) {
      ...
      const flatCoords = (g.type === 'Polygon' ? g.coordinates[0] : g.type === 'MultiPolygon' ? g.coordinates.flat(2) : []) as [number, number][];
...
export function FinanceMapView({ days = 30, className }: FinanceMapViewProps) {
  ...
  const { data: zonesData, loading: zonesLoading, error: zonesError } =
    useApiQuery<FeatureCollection>(`/api/v4/zones?format=geojson`);

  const { data: revenueData, loading: revenueLoading, error: revenueError } =
    useApiQuery<{ data: RevenueByZoneData }>(`/api/v4/finance/revenue-by-zone?days=${days}`);
  ...
  const total = revenue?.total ?? { revenue: 0, orderCount: 0, zonesWithOrders: 0 };

  if (zones.features.length === 0) {
    return (
      <div className={cn('flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-wl-border-default bg-wl-bg-surface', className)}>
        <Map className="w-10 h-10 text-wl-text-tertiary" />
        <p className="text-sm text-wl-text-tertiary">No delivery zones configured</p>
      </div>
    );
  }
    ...
    <div className={cn('relative rounded-lg overflow-hidden', className)}>
      ...
      <div className="absolute top-4 right-4 bg-wl-bg-surface/90 backdrop-blur rounded-lg p-3 border border-wl-border-default text-xs space-y-1 min-w-[160px]">
        ...
        <div className="flex justify-between gap-4">
          ...
          <span className="font-medium text-wl-text-primary">{total.orderCount.toLocaleString()}</span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="text-wl-text-tertiary">Active zones</span>
          <span className="font-medium text-wl-text-primary">{total.zonesWithOrders}</span>
        </div>
        {selectedZone && (
