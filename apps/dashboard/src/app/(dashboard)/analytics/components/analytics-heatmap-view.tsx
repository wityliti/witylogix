...
export default function AnalyticsHeatmapView({
  points,
  loading,
  total,
}: Props) {
  ...
  return (
    <div
      className="relative rounded-2xl overflow-hidden border border-wl-border-default bg-wl-bg-surface"
      style={{ height: 560 }}
    >
      {loading && (
      ...
      {!loading && points.length === 0 && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 text-center">
          ...
          <p className="text-sm text-wl-text-tertiary">
            Heatmap populates as deliveries are completed with geo-tagged
            locations.
          </p>
        </div>
      )}
      ...
      <div className="absolute bottom-3 right-3 z-10 rounded-lg bg-black/70 backdrop-blur border border-white/10 px-3 py-2">
        ...
        <div className="flex items-center gap-1">
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, rgba(96,165,250,0.4), rgba(245,158,11,0.7), rgba(239,68,68,1))",
            }}
          />
        </div>
