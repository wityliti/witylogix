...
const CITY_COORDS: Record<string, [number, number]> = {
  ...
  'San Francisco': [37.7749, -122.4194], 'SF': [37.7749, -122.4194], 'Austin': [30.2672, -97.7431],
  'Boston': [42.3601, -71.0589], 'Seattle': [47.6062, -122.3321], 'Miami': [25.7617, -80.1918],
  'Atlanta': [33.749, -84.388], 'Denver': [39.7392, -104.9903], 'Minneapolis': [44.9778, -93.265],
  'Portland': [45.5051, -122.675], 'Las Vegas': [36.1699, -115.1398], 'Nashville': [36.1627, -86.7816],
  'Charlotte': [35.2271, -80.8431], 'Detroit': [42.3314, -83.0458], 'Memphis': [35.1495, -90.049],
  'Baltimore': [39.2904, -76.6122], 'Louisville': [38.2527, -85.7585], 'Milwaukee': [43.0389, -87.9065],
  'Albuquerque': [35.0844, -106.6504], 'Tucson': [32.2226, -110.9747], 'Fresno': [36.7378, -119.7871],
  'Sacramento': [38.5816, -121.4944], 'Kansas City': [39.0997, -94.5786], 'Mesa': [33.4152, -111.8315],
  'Omaha': [41.2565, -95.9345], 'Colorado Springs': [38.8339, -104.8214], 'Raleigh': [35.7796, -78.6382],
  'Long Beach': [33.77, -118.1937], 'Virginia Beach': [36.8529, -75.978], 'Minneapolis–Saint Paul': [44.9778, -93.265],
  'Oakland': [37.8044, -122.2712], 'Minneapolis/St Paul': [44.9778, -93.265], 'Tampa': [27.9506, -82.4572],
  'Arlington': [32.7357, -97.1081], 'New Orleans': [29.9511, -90.0715], 'Wichita': [37.6872, -97.3301],
  'Cleveland': [41.4993, -81.6944], 'Bakersfield': [35.3733, -119.0187], 'Aurora': [39.7294, -104.8319],
  'Anaheim': [33.8366, -117.9143], 'Santa Ana': [33.7455, -117.8677], 'Corpus Christi': [27.8006, -97.3964],
  'Riverside': [33.9806, -117.3755], 'Lexington': [38.0406, -84.5037], 'Stockton': [37.9577, -121.2908],
  'Pittsburgh': [40.4406, -79.9959], 'St. Louis': [38.627, -90.1994], 'Anchorage': [61.2181, -149.9003],
  'Cincinnati': [39.1031, -84.512], 'Indianapolis': [39.7684, -86.1581], 'Columbus': [39.9612, -82.9988],
  'Hartford': [41.7637, -72.6851], 'Salt Lake City': [40.7608, -111.891], 'Jacksonville': [30.3322, -81.6557],
  'Providence': [41.8240, -71.4128], 'Richmond': [37.5407, -77.4360], 'Buffalo': [42.8864, -78.8784],
  'Newark': [40.7357, -74.1724], 'Jersey City': [40.7178, -74.0431], 'El Paso': [31.7619, -106.485],
  'Honolulu': [21.3069, -157.8583], 'Fort Worth': [32.7555, -97.3308], 'Henderson': [36.0395, -114.9817],
  ...
  'Edinburgh': [55.9533, -3.1883], 'Bristol': [51.4545, -2.5879], 'Sheffield': [53.3811, -1.4701],
  'Newcastle': [54.9783, -1.6178], 'Paris': [48.8566, 2.3522], 'Lyon': [45.764, 4.8357],
  'Marseille': [43.2965, 5.3698], 'Toulouse': [43.6047, 1.4442], 'Nice': [43.7102, 7.262],
  'Berlin': [52.52, 13.405], 'Munich': [48.1351, 11.582], 'Hamburg': [53.5753, 10.0153],
  'Frankfurt': [50.1109, 8.6821], 'Cologne': [50.9333, 6.95], 'Stuttgart': [48.7758, 9.1829],
  ...
  'Brussels': [50.8503, 4.3517], 'Warsaw': [52.2297, 21.0122], 'Prague': [50.0755, 14.4378],
  'Budapest': [47.4979, 19.0402], 'Stockholm': [59.3293, 18.0686], 'Oslo': [59.9139, 10.7522],
  'Copenhagen': [55.6761, 12.5683], 'Helsinki': [60.1699, 24.9384], 'Athens': [37.9838, 23.7275],
  'Lisbon': [38.7169, -9.1399], 'Dublin': [53.3498, -6.2603],
  // Asia-Pacific
  'Dubai': [25.2048, 55.2708], 'Abu Dhabi': [24.4539, 54.3773], 'Riyadh': [24.7136, 46.6753],
  'Jeddah': [21.4858, 39.1925], 'Kuwait City': [29.3759, 47.9774], 'Doha': [25.2854, 51.531],
  'Muscat': [23.5880, 58.3829], 'Cairo': [30.0444, 31.2357], 'Nairobi': [1.2921, 36.8219],
  'Lagos': [6.5244, 3.3792], 'Johannesburg': [-26.2041, 28.0473], 'Cape Town': [-33.9249, 18.4241],
  'Mumbai': [19.076, 72.8777], 'Delhi': [28.7041, 77.1025], 'New Delhi': [28.6139, 77.209],
  ...
  'Yokohama': [35.4437, 139.638], 'Nagoya': [35.1815, 136.9066], 'Sapporo': [43.0618, 141.3545],
  ...
  'Hong Kong': [22.3193, 114.1694], 'Seoul': [37.5665, 126.978], 'Busan': [35.1796, 129.0756],
  'Singapore': [1.3521, 103.8198], 'Kuala Lumpur': [3.1390, 101.6869], 'Jakarta': [-6.2088, 106.8456],
  'Bangkok': [13.7563, 100.5018], 'Manila': [14.5995, 120.9842], 'Ho Chi Minh City': [10.8231, 106.6297],
  ...
  'North': [40.8, -74.0], 'South': [40.65, -74.0], 'East': [40.7, -73.9], 'West': [40.7, -74.1],
  'Unknown': [40.7128, -74.006], 'Other': [40.7128, -74.006],
};
...
function resolveCity(name: string, idx: number): [number, number] {
  ...
  const seed = name.split('').reduce((a, c) => a + c.charCodeAt(0), 0) + idx * 17;
...
function FitBoundsController({ cities }: { cities: CityVolume[] }) {
  const map = useWLMap();
  useFitBounds(map, cities.map((c) => ({ lat: c.lat, lng: c.lng })), 80);
  return null;
}
...
// ── Main component ─────────────────────────────────────────────────────────
export default function AnalyticsZoneView({ zones, loading, timeRange }: AnalyticsZoneViewProps) {
  const [selected, setSelected] = useState<string | null>(null);

  const cities: CityVolume[] = useMemo(() => {
    return zones.map((z, i) => {
      const [lat, lng] = resolveCity(z.name, i);
      return { name: z.name, lat, lng, orders: z.orders, pct: z.pct, trend: z.trend };
    });
  }, [zones]);
  ...
  if (loading) {
    return <div className="h-[560px] rounded-2xl bg-wl-bg-surface border border-wl-border-default animate-pulse" />;
  }
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-wl-border-default relative" style={{ height: 560 }}>
      ...
      <div className="absolute bottom-4 left-4 bg-wl-bg-overlay/90 backdrop-blur-sm border border-wl-border-default rounded-xl p-3 flex flex-col gap-1.5 pointer-events-none">
        <p className="text-[10px] font-semibold text-white/40 uppercase tracking-wider mb-1">Trend</p>
        ...
        <p className="text-[10px] text-white/25 mt-1">Circle size = order volume</p>
      </div>
      ...
      <div className="absolute top-4 left-4 bg-wl-bg-overlay/90 backdrop-blur-sm border border-wl-border-default rounded-xl px-3 py-2 pointer-events-none">
        <p className="text-[11px] text-white/40">
          <span className="font-semibold text-white/70">{zones.length}</span> delivery zones · {timeRange}
        </p>
      </div>
      ...
      {selectedZone && (
        <div className="absolute top-4 right-4 bg-wl-bg-elevated/95 backdrop-blur-sm border border-wl-border-default rounded-xl p-4 min-w-[180px]">
          <div className="flex items-start justify-between mb-2">
            <p className="text-sm font-semibold text-white/85">{selectedZone.name}</p>
            <button
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between">
              ...
              <span className="text-[12px] font-mono text-white/70">{selectedZone.orders.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[12px] text-white/40">Share</span>
              <span className="text-[12px] font-mono text-white/70">{selectedZone.pct}%</span>
            </div>
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className={cn(
                  'text-[12px] font-mono',
                  selectedZone.trend > 0 ? 'text-emerald-400' : selectedZone.trend < 0 ? 'text-red-400' : 'text-white/40'
                )}>
                  {selectedZone.trend > 0 ? '+' : ''}{selectedZone.trend}%
                </span>
              </div>
            </div>
          </div>
