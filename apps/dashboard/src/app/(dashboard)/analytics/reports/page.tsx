...
function CreateReportModal({
  ...
  const { execute, loading, error } = useApiMutation<Report>(
    "POST",
    "/api/v4/analytics/reports",
  );
            ...
            {recipients.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {recipients.map((r) => (
                  <span
                    key={r}
                    className="flex items-center gap-1 text-xs bg-wl-bg-overlay text-wl-text-secondary px-2 py-0.5 rounded-full"
                  >
                    {r}
                    <button
                      type="button"
...
export default function ReportsPage() {
  const {
    ...
    error,
    refetch,
  } = useApiList<Report>("/api/v4/analytics/reports");
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {report.description && (
                        <p className="text-sm text-wl-text-secondary mb-3 line-clamp-1">
                          {report.description}
                        </p>
                      )}
