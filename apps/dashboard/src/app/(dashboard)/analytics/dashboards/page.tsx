...
function CreateDashboardModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [isPublic, setIsPublic] = useState(false);
  const [layout, setLayout] = useState(2);
  const { execute, loading, error } = useApiMutation<DashboardItem>('POST', '/api/v4/analytics/dashboards');

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!name.trim()) return;
    await execute({ name: name.trim(), description: description.trim(), isPublic, layout });
    onCreated();
    onClose();
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <Card className="w-full max-w-md p-6 bg-wl-bg-elevated border-wl-border-default">
        <h2 className="text-lg font-semibold text-wl-text-primary mb-4">New Dashboard</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-wl-text-secondary mb-1">Name</label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Operations Overview" required />
          </div>
          <div>
          ...
          <div className="flex items-center gap-3">
            <label className="text-sm font-medium text-wl-text-secondary">Columns:</label>
            {[1, 2, 3, 4].map((n) => (
              <button
                key={n}
                type="button"
                ...
                className={cn(
                  ...
                  layout === n ? 'bg-wl-primary-500 text-white' : 'bg-wl-bg-surface text-wl-text-secondary hover:bg-wl-bg-overlay'
                )}
              >
                {n}
              </button>
...
export default function DashboardsPage() {
  ...
  if (error) return <ErrorState message={error?.message ?? 'Failed to load dashboards'} onRetry={refetch} />;
          ...
          <div className="space-y-6 max-w-7xl">
            {dashboards.length === 0 ? (
              <Card className="p-12 bg-wl-bg-surface border-wl-border-default text-center">
                ...
                <p className="text-wl-text-primary font-medium mb-1">No dashboards yet</p>
                <p className="text-sm text-wl-text-secondary mb-4">Create a custom dashboard to visualise your key metrics.</p>
                ...
                {dashboards.map((dashboard) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <h3 className="font-semibold text-wl-text-primary flex-1 line-clamp-1">{dashboard.name}</h3>
                    ...
                    {dashboard.description && (
                      <p className="text-sm text-wl-text-secondary mb-4 line-clamp-2">{dashboard.description}</p>
                    )}
                    <div className="flex items-center justify-between text-xs text-wl-text-tertiary mt-auto pt-3 border-t border-wl-border-subtle">
                      ...
                      <span>{dashboard.layout} col{dashboard.layout !== 1 ? 's' : ''}</span>
                    </div>
                  </Card>
                ))}
              </div>
