...
function MetricTile({
  label, value, suffix, trend, icon: Icon, accent, delay = 0,
}: {
  label: string; value: string | number; suffix?: string;
  trend?: number; icon: typeof Package; accent: string; delay?: number;
}) {
...
function MiniBarChart({
  ...
  data: Array<{ hour: number; orders: number; deliveries: number }>; color: string; maxVal: number;
}) {
  return (
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {viewMode === 'zones' && (
        <div className="px-6 lg:px-8 pb-8">
          <AnalyticsZoneView zones={topZones} loading={loading} timeRange={timeRange} />
        </div>
      )}
      ...
      {viewMode === 'charts' && (
        <div className="px-6 lg:px-8 pb-8 space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-white/60 tracking-wide">Top Delivery Zones</h3>
                <button
                  onClick={() => setViewMode('zones')}
