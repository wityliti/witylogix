...
export default function WebhooksDebuggerPage() {
  const {
    ...
    error,
    refetch,
  } = useApiList<WebhookEvent>("/api/v4/webhook-deliveries", { limit: 100 });
  ...
  useEffect(() => {
    ...
    const avgDeliveryTime =
      filteredEvents.reduce((sum, e) => sum + (e.duration ?? 0), 0) /
    ...
    if (filteredEvents.length >= 2) {
      ...
      if (spanMs > 0) {
        eventsPerHour = Math.round(
          (filteredEvents.length / spanMs) * 3_600_000,
        );
      }
    }
  ...
  const eventTypes = Array.from(
    new Set(webhookEvents.map((e) => e.eventType)),
  ).sort();
              ...
              <CardContent>
                {selectedEvent ? (
                  <div className="space-y-4">
                    <div>
                      <div className="text-xs font-semibold text-wl-text-secondary mb-1">
                        Event ID
                      </div>
                    ...
                    <div>
                      ...
                      <div className="text-xs text-wl-text-secondary break-all">
