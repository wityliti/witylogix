...
export default function AccountingSettingsPage() {
  ...
  const [disconnectingProvider, setDisconnectingProvider] = useState<
    string | null
  >(null);
  const [manualSyncProvider, setManualSyncProvider] = useState<
    "quickbooks" | "xero"
  >("quickbooks");
  ...
  async function fetchSyncHistory(provider?: string) {
    try {
      ...
      const data = await api.get<{ records: SyncRecord[]; total: number }>(
        `/api/accounting/sync/history?${params}`,
      );
  ...
  async function handleManualSync() {
    try {
      if (
        !connections.some(
          (c) => c.provider === manualSyncProvider && c.isActive,
        )
      ) {
        ...
        {["quickbooks", "xero"].map((provider) => {
          ...
          return (
            <Card
              key={provider}
              className={cn(
                "relative border border-wl-border-default bg-wl-bg-surface",
              ...
              <CardContent className="space-y-4">
                {connection ? (
                  <>
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>
                            Disconnect {getProviderLabel(provider)}
                          </DialogTitle>
                          <DialogDescription>
                            This will stop syncing invoices to{" "}
                            {getProviderLabel(provider)}. You can reconnect
                            anytime.
                          </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                  ...
                  <Button
                    ...
                    className="w-full"
                    onClick={() =>
                      handleConnect(provider as "quickbooks" | "xero")
                    }
                  >
          ...
          <TabsContent value="manual">
            <Card className="border border-wl-border-default bg-wl-bg-surface">
              <CardHeader>
                ...
                <CardDescription>
                  Manually sync invoices to your accounting software
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setManualSyncProvider(v as "quickbooks" | "xero")
                    }
                  >
                ...
                <Alert>
                  <AlertDescription>
                    This will sync all pending invoices to the selected
                    provider. The process may take a few minutes.
                  </AlertDescription>
                </Alert>
          ...
          <TabsContent value="history">
            <Card className="border border-wl-border-default bg-wl-bg-surface">
              <CardHeader>
                ...
                <CardDescription>
                  Recent invoice synchronization events
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <TableBody>
                      {syncHistory.length > 0 ? (
                        ...
                        <TableRow>
                          <TableCell
                            colSpan={5}
                            className="text-center text-wl-text-secondary py-8"
                          >
                            No sync history
                          </TableCell>
                        </TableRow>
