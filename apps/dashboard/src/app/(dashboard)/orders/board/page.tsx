...
function toKanban(o: ApiOrder): KanbanOrder {
  ...
  const destination =
    [addr.city, addr.state].filter(Boolean).join(", ") || addr.street || "";
  const fullAddress = [
    addr.street,
    addr.city,
    addr.state,
...
export default function OrderBoardPage() {
  const {
    ...
    error,
    refetch,
  } = useApiList<ApiOrder>("/api/v4/orders", { limit: 200 });
  ...
  const orders: KanbanOrder[] = useMemo(
    () =>
      apiOrders.map((o) => {
        ...
        return localStatusOverrides[o.id]
          ? { ...base, status: localStatusOverrides[o.id] }
          : base;
      }),
  ...
  const drivers = useMemo(() => {
    ...
    orders.forEach((o) => {
      if (o.driverInitials) set.add(o.driverInitials);
    });
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent<HTMLDivElement>, targetStatus: OrderStatus) => {
      ...
      try {
        ...
        ) as { id: string; sourceStatus: OrderStatus };
  ...
  return (
    <>
      <Header
        ...
        subtitle="Drag and drop orders between stages"
      />
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setSortBy((s) => (s === "time" ? "priority" : "time"))
                  }
                >
