...
export default function OrdersMapView({ orders, selectedOrderId, onOrderClick }: OrdersMapViewProps) {
  return (
    <div className="absolute inset-0">
      <WLMap center={[0, 20]} zoom={2} className="h-full w-full">
        <FitBoundsController pins={orders} />
        {orders.length > 0 && (
          <OrderLayer orders={orders} selectedOrderId={selectedOrderId} onOrderClick={onOrderClick} />
        )}
      </WLMap>
    </div>
  );
