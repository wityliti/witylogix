...
export default function WidgetsPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<Widget>("/api/v4/widgets");
  const { data: catalogResp } = useApiQuery<CatalogResponse>(
    "/api/v4/widgets/catalog",
  );
  ...
  const handleAddWidget = useCallback(
    async (widget: CatalogWidget) => {
      await api.post("/api/v4/widgets", {
        type: widget.type,
        name: widget.name,
      });
      refetch();
        ...
        {loading ? (
          ...
          >
            {Array.from({ length: 4 }).map((_, i) => (
              <Card
                key={i}
                className="bg-wl-bg-surface border border-wl-border-default animate-pulse h-52"
              >
                {" "}
              </Card>
            ))}
              ...
              {activeWidgets.length === 0 ? (
                <Card className="bg-wl-bg-surface border border-wl-border-default p-8 text-center">
                  <p className="text-wl-text-secondary">
                    No active widgets. Add some from the gallery.
                  </p>
                </Card>
              ) : (
                <div
                ...
                >
                  {activeWidgets.map((widget, idx) => (
                    <Card
                      key={widget.id}
                      className="bg-wl-bg-surface border border-wl-border-default flex flex-col"
                    >
