...
function normaliseStatus(s: string): DriverStatus {
  return (
    DRIVER_STATUS_MAP[s] ?? DRIVER_STATUS_MAP[s?.toUpperCase()] ?? "offline"
  );
}
      ...
      {markers.length === 0 ? (
        <div className="w-full h-full bg-wl-bg-surface flex flex-col items-center justify-center gap-3">
          <div className="w-12 h-12 rounded-full bg-wl-bg-elevated flex items-center justify-center">
            <svg
              className="w-6 h-6 text-wl-text-tertiary"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-wl-text-tertiary mt-1 max-w-64">
              Driver positions are reported from the mobile app. Ask drivers to
        ...
        <>
          ...
          <div className="absolute bottom-4 left-4 bg-wl-bg-surface/90 backdrop-blur-sm border border-wl-border-strong rounded-lg p-3 z-10">
            <p className="text-[10px] font-semibold text-wl-text-secondary uppercase mb-2">
              Driver Status
            </p>
              ...
              {STATUS_LEGEND.map(({ status, color, label }) => {
                ...
                return (
                  <div key={status} className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-wl-text-tertiary ml-auto pl-2">
                      {count}
                    </span>
                  </div>
                );
            ...
            <div className="absolute top-4 right-4 bg-wl-bg-surface/95 backdrop-blur-sm border border-wl-border-strong rounded-lg p-4 z-10 min-w-48">
              ...
              <p className="text-xs text-wl-text-secondary mb-1">
                ...
                <span
                  className={cn(
                    "font-medium",
                    normaliseStatus(selectedDriver.status) === "available"
                      ? "text-emerald-400"
                      : normaliseStatus(selectedDriver.status) === "busy"
                        ? "text-amber-400"
              ...
              {selectedDriver.location && selectedDriver.location !== "Hub" && (
                <p
                  className="text-xs text-wl-text-tertiary mt-1 truncate max-w-48"
                  title={selectedDriver.location}
                >
