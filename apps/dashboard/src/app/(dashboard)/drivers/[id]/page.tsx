...
} from "../components/driver-location-map";
...
// Flat API response for driver scoring endpoint
...
> = {
  ...
  silver: {
    bg: "bg-wl-neutral-400/15",
    text: "text-wl-neutral-300",
    border: "border-wl-neutral-300/30",
    label: "Silver",
  },
  bronze: {
    bg: "bg-orange-700/15",
...
function ScoreArc({ score }: { score: number }) {
  ...
  return (
    <svg viewBox="0 0 140 140" className="w-36 h-36">
      <path
        d={`M ${x1} ${y1} A ${r} ${r} 0 ${largeArcBase} 1 ${x2b} ${y2b}`}
        fill="none"
      ...
      {score > 0 && (
        <path
          d={`M ${x1} ${y1} A ${r} ${r} 0 ${largeArcFill} 1 ${x2f} ${y2f}`}
          fill="none"
          stroke={accent}
          strokeWidth="10"
          strokeLinecap="round"
...
export default function DriverDetailPage() {
  ...
  // profile alias used in memos below — profileData may be undefined until loaded
  ...
  const mapProps: DriverLocationMapProps | null = profile
    ? {
        ...
        currentLat,
        currentLng,
        activeOrders: (profile.orders ?? []).map(
          (o): ActiveOrderPin => ({
            id: o.id,
            customerName: o.customerName ?? null,
            externalOrderNumber: o.externalOrderNumber ?? null,
      ...
      <div className="min-h-screen px-6 lg:px-8 pt-8 space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="h-28 bg-white/[0.04] rounded-xl animate-pulse"
            />
          ))}
        </div>
      </div>
  ...
  if (scoreError || !scoreData) {
    return (
      <div className="min-h-screen px-6 lg:px-8 pt-8">
        <Link
          href="/drivers"
          className="inline-flex items-center gap-1.5 text-sm text-white/30 hover:text-white/60 mb-6"
        >
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-sm font-medium text-white/30">
            Driver not found or score unavailable
          </p>
          <p className="text-xs text-white/20">
            {scoreError?.message ?? "No scoring data for this driver"}
          </p>
          <button
            onClick={refetch}
            className="flex items-center gap-1.5 text-xs text-white/40 hover:text-white/60 mt-2"
          >
  ...
  // score/driver/metrics aliases — scoreData is non-null here (early returns above guard it)
  const score = scoreData;
  const driver = scoreData.driver;
  ...
  const scoreDelta =
    scoreData.previousScore != null
      ...
      : null;
  const avgRating = scoreData.metrics.avgCustomerRating
    ? parseFloat(scoreData.metrics.avgCustomerRating)
    : null;
  const podPct = scoreData.metrics.podCompliancePercent
    ? parseFloat(scoreData.metrics.podCompliancePercent)
    : null;
                  ...
                  <span className="flex items-center gap-1.5 text-xs text-white/35">
                    <span
                      className={cn(
                        ...
                        STATUS_DOT[profile.status] ?? "bg-wl-text-tertiary",
                      )}
                    />
              ...
              <div className="flex flex-wrap items-center gap-3 mt-1.5">
                ...
                {profile && (profile.orders?.length ?? 0) > 0 && (
                  <span className="inline-flex items-center gap-1 text-xs text-amber-400/70">
                    ...
                    {profile.orders!.length} active order
                    {profile.orders!.length !== 1 ? "s" : ""}
                  </span>
                )}
                {currentLat != null && (
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-5 flex flex-col items-center">
                ...
                {scoreDelta !== null && (
                  <div
                    className={cn(
                      "flex items-center gap-1.5 mt-3 text-sm font-mono font-medium",
                      scoreDelta > 0
                        ? "text-emerald-400"
                        : scoreDelta < 0
                          ? "text-red-400"
                  ...
                  >
                    ...
                    {scoreDelta > 0 ? "+" : ""}
                    {scoreDelta.toFixed(1)} vs last period
                  </div>
                )}
              </div>
              ...
              <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-5 space-y-4">
                ...
                <BreakdownRow
                  ...
                  value={score.breakdown.onTimeScore}
                  icon={Clock}
                  accent="var(--wl-success-400)"
                />
                <BreakdownRow
                  label="Customer Rating"
                  value={score.breakdown.customerRatingScore}
                  icon={Star}
                  accent="var(--wl-warning-400)"
                />
                <BreakdownRow
                  label="POD Compliance"
                  value={score.breakdown.podComplianceScore}
                  icon={Camera}
                  accent="var(--wl-info-400)"
                />
                <BreakdownRow
                  label="Route Efficiency"
                  value={score.breakdown.routeEfficiencyScore}
                  icon={Route}
                  accent="var(--wl-chart-violet)"
                />
                <BreakdownRow
                  label="Reliability"
                  value={score.breakdown.reliabilityScore}
                  icon={ShieldCheck}
                ...
                {[
                  {
                    ...
                    value: `${scoreData.metrics.avgDeliveryMinutes} min`,
                  },
                  {
                    label: "Late Deliveries",
                    value: scoreData.metrics.lateCount.toString(),
                  },
                  {
                    label: "Incidents",
                    value: scoreData.metrics.incidentCount.toString(),
                  },
                  {
                    label: "Active Orders",
                    value: profileData?.orders?.length?.toString() ?? "—",
                  },
              ...
              <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[
                    ...
                    {
                      label: "POD Missed",
                      value: scoreData.metrics.podMissedCount.toString(),
                    },
                    {
                      label: "Incidents",
                      value: scoreData.metrics.incidentCount.toString(),
                    ...
                    {
                      label: "Vehicle Type",
                      value: profile?.vehicleType ?? driver?.vehicleType ?? "—",
                    },
                    {
                      label: "Member Since",
                ...
                <p className="text-xs text-wl-text-tertiary mt-0.5">
                  ...
                  {profile?.lastLocationAt && (
                    <span className="ml-2 opacity-60">
                      · Last update{" "}
                      {new Date(profile.lastLocationAt).toLocaleTimeString(
                        "en-GB",
                        {
                          hour: "2-digit",
                          minute: "2-digit",
