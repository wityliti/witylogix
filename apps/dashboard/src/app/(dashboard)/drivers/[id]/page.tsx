...
interface DriverProfile {
  ...
  heading: number | null;
  createdAt: string;
  orders?: { id: string; customerName?: string; externalOrderNumber?: string; addressLine1?: string; city?: string; status?: string }[];
}
...
export default function DriverDetailPage() {
  ...
  const { data: profile } = useApiQuery<DriverProfile>(
  ...
  const mapProps: DriverLocationMapProps | null = profile
    ? {
        ...
        currentLat,
        currentLng,
        activeOrders: (profile.orders ?? []).map((o) => ({
          id: o.id,
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { label: 'Active Orders',      value: profile?.orders?.length?.toString() ?? '—' },
