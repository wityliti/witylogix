...
interface DriverScoreData extends DriverScore {
  driverName: string;
  driver: {
    email: string;
    phone?: string;
...
export default function DriverDetailPage() {
  ...
  const currentLat = useMemo(
    () => profileData?.currentLocation?.latitude ?? null,
    [profileData],
  );
  const currentLng = useMemo(
    () => profileData?.currentLocation?.longitude ?? null,
    [profileData],
  );

  const mapProps: DriverLocationMapProps | null = profileData
    ? {
        driverId,
        driverName: profileData.name,
        driverStatus: profileData.status,
        vehicleType: profileData.vehicleType,
        activeDeliveries: profileData.orders?.length ?? 0,
        currentLat,
        currentLng,
        activeOrders: profileData.orders ?? [],
      }
    : null;
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                {profileData && (
                  <span className="flex items-center gap-1.5 text-xs text-white/35">
                    <span className={cn('w-2 h-2 rounded-full', STATUS_DOT[profileData.status] ?? 'bg-wl-text-tertiary')} />
                    {STATUS_LABEL[profileData.status] ?? profileData.status}
                  </span>
                )}
              </div>
              <p className="text-sm text-white/35 mt-0.5">{scoreData.driver.email}</p>
              {/* Vehicle + phone info from profile */}
              <div className="flex flex-wrap items-center gap-3 mt-1.5">
                {(profileData?.phone || scoreData.driver?.phone) && (
                ...
                {profileData && (profileData.orders?.length ?? 0) > 0 && (
                  <span className="inline-flex items-center gap-1 text-xs text-amber-400/70">
                    <Package className="w-3 h-3" />
                    {profileData.orders?.length} active order{(profileData.orders?.length ?? 0) !== 1 ? 's' : ''}
                  </span>
                )}
                {currentLat != null && (
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-5 flex flex-col items-center">
              ...
              <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-5 space-y-4">
                ...
                <BreakdownRow label="On-Time Delivery"    value={scoreData.breakdown.onTimeScore}          icon={Clock}       accent="var(--wl-success-400)" />
                <BreakdownRow label="Customer Rating"     value={scoreData.breakdown.customerRatingScore}   icon={Star}        accent="var(--wl-warning-400)" />
                <BreakdownRow label="POD Compliance"      value={scoreData.breakdown.podComplianceScore}    icon={Camera}      accent="var(--wl-info-400)" />
                <BreakdownRow label="Route Efficiency"    value={scoreData.breakdown.routeEfficiencyScore}  icon={Route}       accent="var(--wl-chart-violet)" />
                <BreakdownRow label="Reliability"         value={scoreData.breakdown.reliabilityScore}      icon={ShieldCheck} accent="var(--wl-chart-purple)" />
              </div>
            </div>
              ...
              {profileData && (profileData.orders?.length ?? 0) > 0 && (
                <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[11px] text-white/20 font-mono">{profileData.orders?.length} total</span>
                  </div>
                  <div className="space-y-2">
                    {profileData.orders!.map((order: ActiveOrderPin) => (
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-wl-text-tertiary mt-0.5">
                  ...
                  {profileData?.lastLocationAt && (
                    <span className="ml-2 opacity-60">
                      · Last update{' '}
                      {new Date(profileData.lastLocationAt).toLocaleTimeString('en-GB', {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </span>
