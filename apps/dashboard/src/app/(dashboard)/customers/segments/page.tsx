...
function GeoMapLayers({
  ...
  const map = useWLMap();
  const coords = customers.map((c) => ({ lat: c.lat, lng: c.lng }));
...
const WLMap = dynamic(
  () => import("@/components/map/wl-map").then((m) => ({ default: m.WLMap })),
  {
    ssr: false,
    loading: () => (
      <div className="h-full bg-wl-bg-root rounded-xl animate-pulse" />
    ),
  },
);
...
export default function CustomerSegmentsPage() {
  ...
  const { data: customerLocations, loading: locLoading } =
    useCustomerLocations();
    ...
    <>
      <Header
        ...
        subtitle={
          loading
            ? "Loading…"
            : `${total} customers across ${Object.keys(stats?.tiers ?? {}).length} tiers`
        }
        actions={
      ...
      <div className="p-6 space-y-6">
          ...
          <>
                      ...
                      <>
                        ...
                        <div className="space-y-2 text-xs border-t border-white/[0.06] pt-3">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="text-wl-text-primary font-semibold">
                              {seg && seg.count > 0
                                ? (seg.totalOrders / seg.count).toFixed(1)
                                : "0"}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            ...
                            <span className="text-wl-text-primary font-semibold">
                              {seg && seg.count > 0
                                ? fmt.format(seg.totalSpent / seg.count)
                                : "—"}
                            </span>
                          </div>
                        </div>
            ...
            <Card className="p-5">
              ...
              <div className="space-y-3 text-sm">
                {(
                  [
                    ...
                    {
                      tier: "standard",
                      rule: "Lifetime spend < $1,000",
                      color: "text-wl-text-secondary",
                    },
                  ] as const
                ).map(({ tier, rule, color }) => (
                  <div
                    key={tier}
                    className="flex items-center justify-between py-2 border-b border-wl-border-default last:border-0"
                  >
                    <div className="flex items-center gap-2">
                      <span
                        className={cn(
              ...
              <p className="text-xs text-wl-text-tertiary mt-3">
                Tiers are computed automatically based on cumulative spend
        ...
        {activeView === "geo" && (
          <div className="space-y-5">
            ...
            <div className="h-[420px] rounded-xl overflow-hidden border border-wl-border-default">
              {locLoading ? (
                <div className="w-full h-full bg-wl-bg-root animate-pulse" />
                ...
                <div className="w-full h-full bg-wl-bg-root flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-sm font-medium text-wl-text-secondary">
                    No geo-located customers
                  </p>
                  <p className="text-xs text-wl-text-tertiary">
                    Customer addresses will appear here once synced
                  </p>
                </div>
              ) : (
                <WLMap
                    ...
                    <table className="w-full border-collapse text-sm">
                      <thead>
                        <tr className="border-b border-wl-border-default bg-wl-bg-surface">
                          <th className="p-3 px-4 text-left text-xs font-semibold text-wl-text-secondary uppercase tracking-wide">
                            #
                          </th>
                          <th className="p-3 px-4 text-left text-xs font-semibold text-wl-text-secondary uppercase tracking-wide">
                            City
                          </th>
                          <th className="p-3 px-4 text-center text-xs font-semibold text-wl-text-secondary uppercase tracking-wide">
                            Customers
                          </th>
                          <th className="p-3 px-4 text-right text-xs font-semibold text-wl-text-secondary uppercase tracking-wide">
                            Revenue
                          </th>
                          <th className="p-3 px-4 text-center text-xs font-semibold text-wl-text-secondary uppercase tracking-wide">
                            Avg Orders
                      ...
                      <tbody>
                        {stats.topCities.map((city, i) => {
                          ...
                          return (
                            ...
                            >
                              <td className="p-3 px-4 text-xs text-wl-text-tertiary font-mono">
                                {i + 1}
                              </td>
              ...
              <div className="space-y-4">
                <Card className="p-5">
                  ...
                  {loading ? (
                    <div className="space-y-3">
                      {[0, 1, 2].map((i) => (
                        <Skeleton key={i} type="rect" className="h-8" />
                      ))}
                    </div>
                  ) : stats?.tiers ? (
                    (() => {
                      const totalRevenue = Object.values(stats.tiers).reduce(
                        (s, t) => s + t.totalSpent,
                        0,
                      );
                      return (
                        <div className="space-y-4">
                          {(["enterprise", "premium", "standard"] as const).map(
                            (tier) => {
                              const seg = stats.tiers[tier];
                              const pct =
                                totalRevenue > 0
                                  ? (seg.totalSpent / totalRevenue) * 100
                                  : 0;
                              const cfg = TIER_CONFIG[tier];
                              return (
                                <div key={tier}>
