    ...
    >
      ...
      <div className="absolute top-3 left-3 z-10 flex items-center gap-2">
        <div className="rounded-lg bg-black/70 backdrop-blur border border-white/10 px-3 py-1.5 flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-white/80">Forecast Accuracy by Zone</span>
        </div>
        <div className="rounded-lg bg-black/70 backdrop-blur border border-white/10 px-3 py-1.5">
          <span className="text-xs text-white/60">{models.length} model{models.length !== 1 ? 's' : ''} blended</span>
        </div>
      </div>
      ...
      {selectedData && (
        <div className="absolute top-3 right-3 z-10 rounded-lg bg-black/80 backdrop-blur border border-white/10 px-3 py-2.5 min-w-[160px]">
          ...
          <div className="space-y-1">
            ...
            <div className="flex justify-between gap-4 text-[11px]">
              <span className="text-white/40">Wt. MAPE</span>
              <span className="font-mono text-white/70">{(100 - selectedData.onTimePercent).toFixed(1)}%</span>
            </div>
            <div className="flex justify-between gap-4 text-[11px]">
              <span className="text-white/40">Wt. MAE</span>
              <span className="font-mono text-white/70">{selectedData.avgDelayMinutes.toFixed(1)}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="absolute bottom-3 right-3 z-10 rounded-lg bg-black/70 backdrop-blur border border-white/10 px-3 py-2">
        <p className="text-[10px] text-white/40 mb-1.5 uppercase tracking-wide">Forecast Accuracy (100 − MAPE)</p>
        <div className="space-y-1">
          {[
