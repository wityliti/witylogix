...
const PIPELINE_STATUSES: NormStatus[] = [
  "pending",
  "approved",
  "received",
  "inspected",
...
function StatusPipelineStrip({ returns }: { returns: Return[] }) {
  const countByStatus = useMemo(() => {
    const acc: Record<NormStatus, number> = {
      pending: 0,
      approved: 0,
      rejected: 0,
      received: 0,
    ...
    returns.forEach((r) => {
      acc[normalize(r.status as string)]++;
    });
    return acc;
  }, [returns]);
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          ...
          <div className="flex items-center gap-2 flex-shrink-0 ml-4 pl-4 border-l border-wl-border-subtle">
            <div className="px-4 py-2.5 rounded-lg border border-wl-error-500/20 bg-wl-error-500/5 text-center min-w-[80px]">
              <div className="text-[10px] font-semibold text-wl-error-400 uppercase tracking-wider mb-1">
                Rejected
              </div>
              <div className="text-xl font-bold text-wl-error-400 font-mono">
    ...
    <div className="bg-wl-bg-surface border border-wl-border-subtle rounded-xl overflow-hidden">
      ...
      <div className="divide-y divide-wl-border-subtle">
        {returns.map((ret) => {
          ...
          const date = new Date(ret.createdAt).toLocaleDateString("en-CA", {
            month: "short",
            day: "numeric",
            year: "numeric",
          });
          ...
          return (
            ...
            >
              ...
              <div className="col-span-1">
                <div className="md:hidden text-xs text-wl-text-tertiary mb-1">
                  Return ID
                </div>
              ...
              <div className="col-span-1">
                ...
                <div className="text-sm font-mono text-wl-text-secondary">
...
export default function ReturnsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return returns.filter(
      (r) => normalize(r.status as string) === statusFilter,
    );
  ...
  const returnsWithCoords = useMemo(
    () =>
      returns.filter(
        (r) =>
          (r as Return & { order?: { deliveryLat?: number } }).order
            ?.deliveryLat != null,
      ),
    [returns],
  );
  ...
  const pendingCount =
    statsData?.counts?.requested ??
    returns.filter((r) => normalize(r.status as string) === "pending").length;
  const refundedCount =
    statsData?.counts?.refunded ??
    returns.filter((r) => normalize(r.status as string) === "refunded").length;
    ...
    <>
            ...
            >
              ...
              {s !== "all" && (
                <span className="ml-1.5 opacity-60">
                  (
                  {
                    returns.filter((r) => normalize(r.status as string) === s)
                      .length
                  }
                  )
                </span>
      ...
      {viewMode === "map" ? (
        <div className="space-y-3">
          <ReturnsMapView
            returns={returns}
            onSelect={(id) => router.push(`/returns/${id}`)}
          />
          {returnsWithCoords.length === 0 && (
            <p className="text-center text-sm text-wl-text-tertiary py-3">
              No returns have delivery coordinates. Coordinates come from the
              associated order's shipment.
            </p>
          )}
        </div>
      ) : (
        <ReturnsTable
          returns={filtered}
          onView={(id) => router.push(`/returns/${id}`)}
        />
      )}
    </>
  );
