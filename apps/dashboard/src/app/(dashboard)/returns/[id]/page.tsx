...
const ReturnLocationMap = dynamic(
  () => import("./components/return-location-map"),
  {
    ssr: false,
    loading: () => (
...
const STATUS_CONFIG: Record<
  NormalStatus,
  {
    badge: "default" | "info" | "primary" | "danger" | "warning" | "success";
    label: string;
...
function normalize(status: string): NormalStatus {
  ...
  if (s === "received" || s === "picked_up" || s === "in_transit")
    return "received";
  ...
  if (status === "rejected") {
    return (
      <div className="flex items-center gap-2 p-3 rounded-lg bg-wl-error-500/10 border border-wl-error-500/20">
        ...
        <span className="text-sm font-medium text-wl-error-400">
          Return Rejected
        </span>
      </div>
    );
            ...
            <div
              className={cn(
                "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors",
                  ...
                  ? "bg-wl-success-500/15 text-wl-success-400 border border-wl-success-500/20"
                  : active
                    ? "bg-wl-primary/20 text-wl-primary border border-wl-primary/30"
            ...
            {i < PIPELINE.length - 1 && (
              <div
                className={cn(
                  ...
                  done ? "bg-wl-success-500/40" : "bg-wl-border-subtle",
                )}
              />
            )}
          </div>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-wl-border-subtle">
            <th className="text-left py-2 px-3 text-xs font-medium text-wl-text-tertiary uppercase tracking-wider">
              Item
            </th>
            <th className="text-center py-2 px-3 text-xs font-medium text-wl-text-tertiary uppercase tracking-wider">
              Qty
            </th>
            <th className="text-left py-2 px-3 text-xs font-medium text-wl-text-tertiary uppercase tracking-wider">
              Condition
            </th>
            <th className="text-right py-2 px-3 text-xs font-medium text-wl-text-tertiary uppercase tracking-wider">
              Unit Price
            </th>
            <th className="text-right py-2 px-3 text-xs font-medium text-wl-text-tertiary uppercase tracking-wider">
              Refund
        ...
        <tbody className="divide-y divide-wl-border-subtle">
          {items.map((item, i) => (
            <tr
              key={item.id ?? i}
              className="hover:bg-wl-bg-elevated/50 transition-colors"
            >
              <td className="py-2.5 px-3 text-wl-text-primary font-medium">
                {item.productName ?? item.name ?? `Item ${i + 1}`}
              </td>
              <td className="py-2.5 px-3 text-center text-wl-text-secondary">
                {item.quantity}
              </td>
              <td className="py-2.5 px-3">
                <span
                  className={cn(
                    ...
                    item.condition === "good"
                      ? "bg-wl-success-500/15 text-wl-success-400"
                      : item.condition === "damaged"
                        ? "bg-wl-error-500/15 text-wl-error-400"
                        : item.condition === "defective"
                          ? "bg-wl-warning-500/15 text-wl-warning-400"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="h-40 bg-wl-bg-elevated animate-pulse rounded-xl"
              />
            ))}
          </div>
  ...
  if (error || !ret) {
    return (
      <div className="p-6">
        <Link
          href="/returns"
          className="inline-flex items-center gap-2 text-sm text-wl-text-tertiary hover:text-wl-text-primary mb-6 transition-colors"
        >
        ...
        <div className="flex flex-col items-center justify-center py-20 rounded-xl border border-wl-border-subtle bg-wl-bg-elevated gap-4">
          ...
          <div className="text-center">
            <p className="font-medium text-wl-text-primary">
              {error ? "Failed to load return" : "Return not found"}
            </p>
            {error && (
    ...
    <div className="p-6 space-y-6">
      ...
      {viewMode === "map" && hasCoords && (
        <ReturnLocationMap
          ...
          label={
            ret.order?.customerName ?? ret.customerName ?? "Return location"
          }
          status={status}
        />
      )}
            ...
            <Card>
              ...
              <CardContent className="pt-0 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-wl-text-primary capitalize">
                ...
                {ret.trackingNumber && (
                  <div>
                    ...
                    <p className="text-sm font-mono text-wl-primary">
                      {ret.trackingNumber}
                    </p>
                  </div>
                )}
                    ...
                    .map((event, i, arr) => (
                      <div key={event.label} className="flex items-start gap-3">
                        <div className="flex flex-col items-center">
                          ...
                          />
                          {i < arr.length - 1 && (
                            <div className="w-px h-6 bg-wl-border-subtle mt-1" />
                          )}
                        </div>
                        <div>
              ...
              <CardContent className="pt-0 space-y-3">
                ...
                {ret.order && (
                  <div className="pt-2 border-t border-wl-border-subtle">
                    <p className="text-xs text-wl-text-tertiary uppercase tracking-wider mb-1.5">
                      Related Order
                    </p>
                    <Link
                ...
                <CardContent className="pt-0 space-y-2">
                  {status === "pending" && (
                    <>
                      ...
                      <Button
                        ...
                        className="w-full"
                        onClick={() =>
                          doAction(reject, { reason: "Rejected by admin" })
                        }
                  ...
                  {status === "received" && (
                    <Button
                      ...
                      className="w-full"
                      onClick={() =>
                        doAction(inspect, { condition: "good", notes: "" })
                      }
            ...
            <Card>
              ...
              <CardContent className="pt-0 space-y-3">
                {[
                  ...
                  {
                    label: "Refund Amount",
                    value: fmtCurrency(refundTotal),
                    mono: true,
                  },
                  ret.restockingFee
                    ? {
                        label: "Restocking Fee",
                        value: fmtCurrency(ret.restockingFee),
                        mono: true,
                      }
                    : null,
                ]
