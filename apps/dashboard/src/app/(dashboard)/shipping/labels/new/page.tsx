...
export default function CreateLabelPage() {
  ...
  const {
    ...
    loading,
    error,
  } = useApiMutation("POST", "/api/v4/shipments");
  ...
  return (
    <>
      <Header
        ...
        subtitle="Step-by-step label creation"
      />

      <main className="min-h-screen bg-wl-bg-root p-6 max-w-2xl mx-auto">
        {/* Progress Indicator */}
        <div className="flex gap-2 mb-8">
          {(["package", "carrier", "rates", "review"] as const).map(
            (s, idx) => (
              <div
                key={s}
                className={`flex-1 h-2 rounded-full transition-colors ${
                  (["package", "carrier", "rates", "review"] as const).indexOf(
                    step,
                  ) >= idx
                    ? "bg-blue-500"
                    : "bg-wl-bg-elevated"
                  ...
                  <div
                    ...
                    className={`p-3 border-2 rounded cursor-pointer transition-colors ${
                      formData.carrier === carrier
                        ...
                        : "border-wl-border-default bg-wl-bg-sunken"
                    }`}
                  >
            ...
            {step === "review" && (
              <div className="space-y-4">
                <div className="p-4 bg-wl-bg-elevated rounded border border-wl-border-default">
                  ...
                  <p className="text-sm text-wl-text-secondary">
                    Carrier: {formData.carrier} {formData.service}
                  </p>
                </div>
                {error && (
