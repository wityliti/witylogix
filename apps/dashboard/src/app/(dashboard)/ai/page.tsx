...
const FEATURE_CARDS: FeatureCard[] = [
  {
    name: "ETA Prediction",
    description:
      "AI-powered delivery time estimates using route history and live traffic.",
  ...
  },
  {
    ...
    description:
      "Detect anomalies and inefficiencies across delivery routes in real time.",
  ...
  },
  {
    ...
    description:
      "Score and rank drivers based on performance, safety, and on-time delivery.",
    href: "/drivers",
  ...
  },
  {
    ...
    description:
      "Predict order volumes by zone so you can staff and route proactively.",
    href: "/demand",
  ...
  },
  {
    ...
    description:
      "Recommend optimal delivery windows to balance capacity and customer preference.",
    href: "/time-slots",
  ...
  },
  {
    name: "AI Co-pilot",
    description:
      "Conversational AI assistant for logistics decisions, queries, and planning.",
    href: "/ai/copilot",
    statsPath: null,
    status: "active",
  },
...
function useFeatureUsageCounts() {
  ...
  const leaderboard = useApiQuery<LeaderboardStats>(
    "/api/ai/analytics/leaderboard",
  );
  const demand = useApiQuery<DemandStats>("/api/ai/demand");
  const slots = useApiQuery<SlotStats>("/api/ai/slots/recommend");

  const isLoading =
    eta.loading || leaderboard.loading || demand.loading || slots.loading;

  return {
    isLoading,
    counts: {
      "/api/ai/eta/statistics":
        eta.data?.totalPredictions30d ?? eta.data?.count ?? 0,
      "/api/ai/analytics/leaderboard":
        leaderboard.data?.totalEntries ?? leaderboard.data?.count ?? 0,
      "/api/ai/demand": demand.data?.totalForecasts ?? demand.data?.count ?? 0,
      "/api/ai/slots/recommend":
        slots.data?.totalRecommendations ?? slots.data?.count ?? 0,
  ...
  const inner = (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <Badge
          variant={isComingSoon ? "default" : "success"}
