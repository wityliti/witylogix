    ...
    >
      <span
        className={cn(
          ...
          isTop ? "text-emerald-400" : grade.color,
        )}
      >
        {score}
      </span>
...
export default function SlotAIPage() {
  ...
  const { items: zones, loading: zonesLoading } = useApiList<Zone>(
    "/api/v4/zones?limit=50",
  );
  ...
  } = useApiQuery<RecommendResponse>(queryUrl);

  const slots: ScoredSlot[] = data?.recommendations ?? [];

  const handleSearch = () => {
    if (!customerId.trim()) return;
    const dateIso = encodeURIComponent(
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/35 mt-0.5">
              AI-powered slot recommendations for customers
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 lg:px-8 pb-8 space-y-5">
        ...
        <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-5">
          <h3 className="text-sm font-semibold text-white/60 tracking-wide mb-4">
            Preview Slot Recommendations
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {/* Customer ID */}
            <div>
              <label className="block text-xs text-white/30 mb-1.5">
                Customer ID <span className="text-white/15">(optional)</span>
              </label>
              <input
                type="text"
        ...
        {!queryUrl ? (
          <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-12 text-center">
            ...
            <p className="text-sm text-white/25">
              Enter a customer ID and click "Get Recommendations"
            </p>
            <p className="text-xs text-white/15 mt-1">
              Customer ID is required to personalise slot scoring
            </p>
          </div>
        ) : recommendError ? (
          ...
          <div className="space-y-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="h-28 bg-wl-bg-surface border border-white/[0.06] rounded-xl animate-pulse"
              />
            ))}
          </div>
        ) : slots.length === 0 ? (
          <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] p-12 text-center">
            <CalendarDays className="w-10 h-10 text-white/10 mx-auto mb-3" />
            <p className="text-sm text-white/25">
              No slot recommendations available
            </p>
            <p className="text-xs text-white/15 mt-1">
              Try a different zone or date
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            {slots.length > 0 && (
              <div className="flex items-center gap-2 px-1">
                ...
                <span className="text-xs font-medium text-amber-400/80">
                  Top pick:{" "}
                  <span className="text-amber-300">{slots[0].name}</span> ·
                  Score {slots[0].score}
                </span>
              </div>
            )}
            ...
            {slots.map((slot, idx) => {
              ...
              const driverPct =
                slot.driverAvailability.total > 0
                  ? (slot.driverAvailability.available /
                      slot.driverAvailability.total) *
                    100
                  : 0;
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-xs text-white/35 mb-3">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {slot.driverAvailability.available}/
                          {slot.driverAvailability.total} drivers available
                        </span>
                        <span className="flex items-center gap-1">
                          <BarChart3 className="w-3 h-3" />
                          {slot.demandForecast.predicted} orders predicted
                          <span className="text-white/20">
                            ({(slot.demandForecast.confidence * 100).toFixed(0)}
                            % conf.)
                          </span>
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-3">
                        ...
                        <div>
                          <div className="flex items-center justify-between text-[10px] text-white/25 mb-1">
                            ...
                            <span>
                              {(slot.demandForecast.confidence * 100).toFixed(
                                0,
                              )}
                              %
                            </span>
                          ...
                          <div className="h-1.5 bg-white/[0.04] rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${slot.demandForecast.confidence * 100}%`,
                              }}
                            />
                          </div>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {slot.reasoning.map((r, i) => (
                          <span
                            key={i}
                            className="flex items-center gap-1 text-[10px] text-white/30 bg-white/[0.03] border border-white/[0.06] rounded-full px-2.5 py-0.5"
                          >
        ...
        {queryUrl && slots.length > 0 && (
          <p className="text-[11px] text-white/20 text-center">
            Recommendations scored by demand forecast · driver availability ·
            customer preferences · historical success rates
          </p>
        )}
      </div>
    </div>
