...
const DriverLayer = dynamic(
  () =>
    import("@/components/map/driver-layer").then((m) => ({
      default: m.DriverLayer,
    })),
  { ssr: false },
);
  ...
  switch (tier) {
    ...
    case "silver":
      return {
        ...
        border: "border-wl-neutral-400/30",
      };
    case "bronze":
      return {
        bg: "bg-orange-700/15",
...
export default function DriverInsightsPage() {
  ...
  const [sortBy, setSortBy] = useState<
    "score" | "onTime" | "efficiency" | "rating"
  >("score");
  ...
  const entries = [...rawEntries].sort((a, b) => {
    switch (sortBy) {
      case "onTime":
        return b.breakdown.onTimeScore - a.breakdown.onTimeScore;
      case "efficiency":
        return (
          b.breakdown.routeEfficiencyScore - a.breakdown.routeEfficiencyScore
        );
      case "rating":
        return (
          b.breakdown.customerRatingScore - a.breakdown.customerRatingScore
        );
      default:
  ...
  const avgOnTime =
    entries.length > 0
      ? entries.reduce((s, e) => s + e.breakdown.onTimeScore, 0) /
        entries.length
      : 0;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/35 mt-0.5">
                AI-scored performance leaderboard
              </p>
            </div>
          </div>
      ...
      <div className="px-6 lg:px-8 pb-8 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            {
              label: "Top Score",
              value: top ? top.compositeScore.toFixed(1) : "—",
              suffix: "/100",
              icon: Trophy,
            ...
            },
            {
              ...
              sub: `${entries.length} drivers`,
            },
            {
              label: "Avg On-Time",
              value: loading ? "…" : avgOnTime.toFixed(1),
              suffix: "%",
              icon: Target,
              accent: "var(--wl-success-400)",
              sub: `${toScoringPeriod(period)} window`,
            },
            {
              label: "Platinum Tier",
              value: loading
                ? "…"
                : entries.filter((e) => e.tier === "platinum").length,
              icon: Star,
          ...
          <div className="rounded-xl bg-wl-bg-surface border border-white/[0.06] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
              <h3 className="text-sm font-semibold text-white/60 tracking-wide">
                Driver Locations by Performance Tier
              </h3>
              <div className="flex items-center gap-4 text-[11px] text-white/30">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-wl-neutral-500" />
                  Bronze
                </span>
              </div>
            </div>
            {mapDrivers.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 gap-3">
                ...
                <p className="text-xs text-white/15">
                  Driver GPS data appears here once drivers are active
                </p>
              </div>
            ) : (
            ...
            {loading ? (
              <div className="space-y-px p-2">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-14 bg-white/[0.02] rounded animate-pulse"
                  />
                ))}
              </div>
            ) : entries.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                ...
                <p className="text-xs text-white/15">
                  Scores are calculated once drivers complete deliveries
                </p>
              </div>
            ) : (
              <div>
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p
                          className={cn(
                            ...
                            entry.compositeScore >= 90
                              ? "text-indigo-300"
                              : entry.compositeScore >= 80
                                ? "text-emerald-400"
                                : "text-amber-400",
                          )}
                        >
                      ...
                      <div className="text-right flex flex-col items-end gap-1">
                        <span
                          className={cn(
                            "text-xs font-mono",
                            entry.breakdown.onTimeScore >= 90
                              ? "text-emerald-400"
                              : entry.breakdown.onTimeScore >= 75
                                ? "text-amber-400"
                                : "text-red-400",
                          )}
                        >
                        ...
                        <ScoreBar
                          value={entry.breakdown.onTimeScore}
                      ...
                      <div className="text-right flex flex-col items-end gap-1">
                        <span
                          className={cn(
                            "text-xs font-mono",
                            entry.breakdown.routeEfficiencyScore >= 85
                              ? "text-emerald-400"
                              : entry.breakdown.routeEfficiencyScore >= 70
                                ? "text-amber-400"
                                : "text-red-400",
                          )}
                        >
                        ...
                        <ScoreBar
                          value={entry.breakdown.routeEfficiencyScore}
                          color="var(--wl-chart-violet)"
                        />
                      </div>
                      ...
                      <div className="text-right">
                        <div className="flex items-center justify-end gap-0.5">
                          <span className="text-xs font-mono text-amber-400">
                            {(entry.breakdown.customerRatingScore / 20).toFixed(
                              1,
                            )}
                          </span>
                      ...
                      <div className="text-right">
                        <span
                          className={cn(
                            "text-xs font-mono",
                            entry.breakdown.reliabilityScore >= 90
                              ? "text-emerald-400"
                              : "text-amber-400",
                          )}
                        >
        ...
        <div className="flex items-center gap-6 text-[11px] text-white/25">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-wl-neutral-400" />
            Silver (70–79)
          </span>
          <span className="flex items-center gap-1.5">
          ...
          <span className="ml-auto">
            Powered by AI composite scoring · {toScoringPeriod(period)} window
          </span>
        </div>
      </div>
    </div>
