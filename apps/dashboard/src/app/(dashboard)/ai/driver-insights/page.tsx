      ...
      <div className="px-6 lg:px-8 pt-6 pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-wl-text-primary tracking-tight">Driver Insights</h1>
              <p className="text-sm text-wl-text-tertiary mt-0.5">AI-scored performance leaderboard</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center rounded-lg border border-wl-border-default overflow-hidden">
              <button
                ...
                className={cn('px-3 py-1.5 text-xs flex items-center gap-1.5 transition-all', view === 'list' ? 'bg-white/10 text-wl-text-primary' : 'text-wl-text-tertiary hover:text-wl-text-secondary')}
              ...
              <button
                onClick={() => setView('map')}
                className={cn('px-3 py-1.5 text-xs flex items-center gap-1.5 transition-all', view === 'map' ? 'bg-white/10 text-wl-text-primary' : 'text-wl-text-tertiary hover:text-wl-text-secondary')}
                aria-label="Map view"
              >
      ...
      <div className="px-6 lg:px-8 pb-8 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <div key={label} className="relative overflow-hidden rounded-xl bg-wl-bg-surface border border-wl-border-subtle p-5 group hover:border-wl-border-default transition-all">
              ...
              <div className="flex items-baseline gap-1.5">
                <span className="text-[28px] font-bold text-wl-text-primary leading-none tracking-tight font-mono">{value}</span>
                {suffix && <span className="text-sm text-wl-text-tertiary">{suffix}</span>}
              </div>
              {sub && <p className="text-[11px] text-wl-text-tertiary mt-2 font-mono">{sub}</p>}
            </div>
          ))}
        </div>
          ...
          <div className="rounded-xl bg-wl-bg-surface border border-wl-border-subtle overflow-hidden">
            ...
            {mapDrivers.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 gap-3">
                ...
                <p className="text-sm text-wl-text-tertiary">No driver locations available</p>
                <p className="text-xs text-wl-text-tertiary">Driver GPS data appears here once drivers are active</p>
              </div>
            ) : (
        ...
        <div className="rounded-xl bg-wl-bg-surface border border-wl-border-subtle overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-wl-border-subtle">
            <h3 className="text-sm font-semibold text-wl-text-secondary tracking-wide">Performance Leaderboard</h3>
            <div className="flex items-center gap-1">
              <span className="text-[11px] text-wl-text-tertiary mr-2">Sort by</span>
              {([
                ...
                <button
                  ...
                  className={cn(
                    ...
                    sortBy === key ? 'bg-white/10 text-wl-text-secondary' : 'text-wl-text-tertiary hover:text-wl-text-tertiary',
                  )}
                >
                  {label}
                </button>
          ...
          <div className="grid grid-cols-[2.5rem_1fr_5rem_5rem_5rem_5rem_5rem_4rem] gap-2 px-5 py-2 text-[10px] text-wl-text-tertiary font-medium uppercase tracking-wider border-b border-wl-border-subtle">
            <span>#</span>
            <span>Driver</span>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <Users className="w-10 h-10 text-wl-text-tertiary" />
              <p className="text-sm font-medium text-wl-text-tertiary">No driver data yet</p>
              <p className="text-xs text-wl-text-tertiary">Scores are calculated once drivers complete deliveries</p>
            </div>
          ) : (
            <div>
              {entries.map((entry) => {
                ...
                return (
                  <div
                    key={entry.driverId}
                    className="grid grid-cols-[2.5rem_1fr_5rem_5rem_5rem_5rem_5rem_4rem] gap-2 px-5 py-3.5 items-center hover:bg-white/[0.02] transition-colors border-b border-wl-border-subtle last:border-0"
                  >
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-8 h-8 rounded-full bg-white/[0.05] flex items-center justify-center shrink-0">
                        <span className="text-xs font-medium text-wl-text-tertiary">
                          {entry.driverName.split(' ').map((n) => n[0]).join('')}
                        </span>
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm text-wl-text-secondary truncate font-medium">{entry.driverName}</p>
                    ...
                    <div className="text-right flex flex-col items-end gap-1">
                      <span className={cn('text-xs font-mono', entry.breakdown.onTimeScore >= 90 ? 'text-wl-success-400' : entry.breakdown.onTimeScore >= 75 ? 'text-wl-warning-400' : 'text-wl-danger-400')}>
                    ...
                    <div className="text-right flex flex-col items-end gap-1">
                      <span className={cn('text-xs font-mono', entry.breakdown.routeEfficiencyScore >= 85 ? 'text-wl-success-400' : entry.breakdown.routeEfficiencyScore >= 70 ? 'text-wl-warning-400' : 'text-wl-danger-400')}>
                    ...
                    <div className="text-right">
                      <div className="flex items-center justify-end gap-0.5">
                        <span className="text-xs font-mono text-wl-warning-400">{(entry.breakdown.customerRatingScore / 20).toFixed(1)}</span>
                        <Star className="w-3 h-3 text-wl-warning-400 fill-amber-400" />
                      </div>
                    </div>

                    <div className="text-right">
                      <span className={cn('text-xs font-mono', entry.breakdown.reliabilityScore >= 90 ? 'text-wl-success-400' : 'text-wl-warning-400')}>
                    ...
                    <div className="flex items-center justify-end gap-1">
                      {isStable ? (
                        <Minus className="w-3.5 h-3.5 text-wl-text-tertiary" />
                      ) : isUp ? (
                      ...
                      {!isStable && trendPct > 0 && (
                        <span className={cn('text-[10px] font-mono', isUp ? 'text-wl-success-400' : 'text-wl-danger-400')}>
