    ...
    methods: [
      {
        name: "getRates",
        description:
          "Retrieve shipping rates for a given origin, destination, and package dimensions.",
        parameters: [
          {
            name: "origin",
            type: "Address",
          ...
          },
          {
            ...
            type: "Package[]",
            required: true,
            description: "Array of package dimensions and weights",
          },
        ],
        returns: "RateQuote[]",
        example:
          "const rates = await fedex.getRates({ origin, destination, packages });",
      },
      {
        name: "createShipment",
        ...
        parameters: [
          {
            ...
            required: true,
            description: "Full shipment details",
          },
        ],
    ...
    methods: [
      {
        ...
        example:
          "const rate = await ups.getRates({ origin, destination, package });",
      },
    ],
  },
};
...
const WEBHOOK_CATALOGS: WebhookCatalog[] = [
  {
    ...
    events: [
      ...
      },
      {
        ...
        example:
          '{ "shipmentId": "SHP-001", "deliveredAt": "2026-03-23T14:00:00Z" }',
      },
    ],
  },
  {
    ...
    events: [
      {
        ...
        example:
          '{ "trackingNumber": "1Z999AA10123456784", "status": "in_transit" }',
      },
    ],
  },
];
...
const RATE_LIMITS: RateLimitInfo[] = [
  {
    ...
    burstCapacity: 20,
    window: "sliding",
    notes: "Production credentials required for full limits",
  },
  {
    provider: "UPS",
    requestsPerSecond: 5,
  ...
  },
  {
    ...
    burstCapacity: 10,
    window: "sliding",
    notes: "Web Tools API key required",
  },
];
...
const TROUBLESHOOTING_PLAYBOOKS: TroubleshootingPlaybook[] = [
  ...
  },
  {
    ...
    description:
      "Rate request did not receive a response within the configured timeout.",
    steps: [
  ...
  },
  {
    ...
    errorCode: "LABEL_422",
    description:
      "The carrier rejected the shipment data during label generation.",
    steps: [
      ...
      changes: [
        {
          type: "added",
          description:
            "Batch label generation endpoint for multi-package shipments",
        },
        {
          type: "changed",
          description:
            "Rate response now includes estimated delivery date range",
        },
      ],
    },
    {
      ...
      changes: [
        {
          type: "added",
          description: "Hazmat declarations support in shipment creation",
        },
        {
          type: "deprecated",
          description:
            "Legacy v3 tracking endpoint — migrate to v4 by June 2026",
        },
      ],
    },
  ],
  ups: [
    {
      ...
      changes: [
        {
          type: "added",
          description: "OAuth 2.0 authentication (replaces API key auth)",
        },
        {
          type: "changed",
          description: "Unified REST endpoint replaces legacy XML API",
        },
      ],
    },
  ],
...
export function useWebhookCatalog() {
  const searchEvents = useMemo(
    () =>
      (
      ...
      ): { provider: string; event: WebhookEvent }[] => {
        const catalogs = providerFilter
          ? WEBHOOK_CATALOGS.filter(
              (c) => c.provider.toLowerCase() === providerFilter.toLowerCase(),
            )
          : WEBHOOK_CATALOGS;
        ...
        return catalogs.flatMap((c) =>
          c.events
            .filter(
              (e) =>
                e.type.includes(lq) || e.description.toLowerCase().includes(lq),
            )
  ...
  const playbooks = useMemo(() => {
    ...
    return TROUBLESHOOTING_PLAYBOOKS.filter(
      (p) =>
        ...
        (p.errorCode ?? "").toLowerCase().includes(lq),
    );
  }, [query]);
