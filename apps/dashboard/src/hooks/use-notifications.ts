...
export function useNotifications(filters?: ApiFilters) {
  ...
  const markReadMutation = useApiMutation<Notification>(
    "PATCH",
    "/api/v4/notifications/:id/read",
  );
  const markUnreadMutation = useApiMutation<Notification>(
    "PATCH",
    "/api/v4/notifications/:id/unread",
  );
  const deleteMutation = useApiMutation<void>(
    "DELETE",
    "/api/v4/notifications/:id",
  );
