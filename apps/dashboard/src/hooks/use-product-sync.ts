...
export function useSyncScheduleQuery(platformId: string): UseApiQueryResult<SyncSchedule> {
...
export function useSyncSchedule(platformId: string) {
  const queryResult = useApiQuery<SyncSchedule>(`/api/v4/sync/platforms/${platformId}/schedule`);
  const updateMutation = useApiMutation<SyncSchedule>('PATCH', `/api/v4/sync/platforms/${platformId}/schedule`);
...
export function useFieldMappings(platformId: string) {
  const listResult = useApiList<Mapping>(`/api/v4/sync/platforms/${platformId}/mappings`);
  const createMutation = useApiMutation<Mapping>('POST', `/api/v4/sync/platforms/${platformId}/mappings`);
  const deleteMutation = useApiMutation<void>('DELETE', `/api/v4/sync/platforms/${platformId}/mappings/:id`);
  const updateMutation = useApiMutation<Mapping>('PATCH', `/api/v4/sync/platforms/${platformId}/mappings/:id`);
  ...
  const removeMapping = (id: string) => {
    setLocalMappings((localMappings ?? listResult.items).filter((m) => m.id !== id));
  };

  const updateTransformer = (id: string, transformer: Mapping['transformer']) => {
    setLocalMappings((localMappings ?? listResult.items).map((m) =>
...
export function useProductPreview(platformId: string, mappings: Mapping[]) {
  const previewMutation = useApiMutation<PreviewResult>('POST', `/api/v4/sync/platforms/${platformId}/preview`);
  const [previewProduct, setPreviewProduct] = useState<PreviewResult | null>(null);
