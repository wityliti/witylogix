import { formatDistanceToNow, formatDate as fnFormatDate, differenceInSeconds } from 'date-fns';
 ...
 */
export function formatDistance(meters: number, locale: LocaleKey = 'en'): string {
 ...
 */
export function formatDuration(seconds: number, locale: LocaleKey = 'en'): string {
  const units = {
    en: {
      day: 'd',
      hour: 'h',
  ...
  if (remaining > 0 || parts.length === 0) parts.push(`${remaining}${unitLabels.second}`);
 ...
 */
export function getTimeAgoLabel(date: Date | string | number, locale: LocaleKey = 'en'): string {
  const dateObj = typeof date === 'string' || typeof date === 'number' ? new Date(date) : date;
