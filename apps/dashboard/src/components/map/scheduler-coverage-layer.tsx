...
export function SchedulerCoverageLayer({ zones, coverageData, onSelect }: SchedulerCoverageLayerProps) {
  const map = useWLMap();
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (map.getSource('scheduler-coverage')) map.removeSource('scheduler-coverage');
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [map]);

  useEffect(() => {
    const src = map.getSource('scheduler-coverage') as GeoJSONSource | undefined;
