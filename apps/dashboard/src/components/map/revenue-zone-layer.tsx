...
export function RevenueZoneLayer({ zones, revenueData, onSelect }: RevenueZoneLayerProps) {
  const map = useWLMap();
  ...
  useEffect(() => {
    const setup = () => {
      ...
      map.on('click', 'revenue-zones-fill', (e) => {
        ...
        const popup = new (window as any).maplibregl.Popup({ closeButton: true })
          .setLngLat(e.lngLat)
          .setHTML(
    ...
    return () => {
      if (map.getLayer('revenue-zones-fill')) map.removeLayer('revenue-zones-fill');
      if (map.getLayer('revenue-zones-line')) map.removeLayer('revenue-zones-line');
