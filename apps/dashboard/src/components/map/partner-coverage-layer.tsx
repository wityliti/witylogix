...
export function PartnerCoverageLayer({ partners, onCityClick }: PartnerCoverageLayerProps) {
  const map = useWLMap();
  ...
  useEffect(() => {
    ...
    const geojson: GeoJSON.FeatureCollection = {
      ...
      features: features.map((f) => ({
        ...
        properties: { id: f.id, partnerName: f.partnerName, city: f.city, status: f.status },
      })),
    };
    ...
    const init = () => {
      ...
      if (onCityClick) {
        map.on('click', LAYER_CIRCLE, (e) => {
          ...
          if (!props) return;
          const partner = partners.find((p) => p.id === props.id.split('::')[0]);
