...
export function WarehouseLayer({ warehouses }: WarehouseLayerProps) {
  ...
  useFitBounds(map, warehouses.map((w) => ({ lng: w.lng, lat: w.lat })));
      ...
      map.addLayer({
        ...
        paint: {
          ...
          'circle-color': [
            'interpolate', ['linear'],
            ['get', 'utilization'],
            0, 'var(--wl-info-500, #3b82f6)',
            50, 'var(--wl-success-500, #22c55e)',
            85, 'var(--wl-warning-500, #f59e0b)',
            100, 'var(--wl-error-500, #ef4444)',
          ],
          'circle-opacity': 0.8,
