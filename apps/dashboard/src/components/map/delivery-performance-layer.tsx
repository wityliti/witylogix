...
function pinColor(pin: DeliveryPin): string {
  ...
  if (pin.status === "OUT_FOR_DELIVERY" || pin.status === "ARRIVED")
    return "#f59e0b"; // amber
...
export function DeliveryPerformanceLayer({
  pins,
  onPinClick,
}: DeliveryPerformanceLayerProps) {
  const map = useWLMap();
  ...
  useEffect(() => {
    ...
    return () => {
      [
        "delivery-perf-cluster",
        "delivery-perf-cluster-count",
        HALO_ID,
        CIRCLE_ID,
      ].forEach((id) => {
  ...
  useEffect(() => {
    ...
    if (src)
      src.setData({ type: "FeatureCollection", features: buildFeatures(pins) });
  }, [map, pins]);
