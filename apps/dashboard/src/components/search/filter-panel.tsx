...
export function FilterPanel({
  ...
  return (
    <div className="flex h-full flex-col border-r border-wl-border-default bg-wl-bg-root">
      {/* Header */}
      <div className="border-b border-wl-border-default p-4">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="flex w-full items-center justify-between text-lg font-semibold text-white hover:text-wl-text-secondary"
        >
          <span>Filters</span>
          <svg
            className={cn(
              ...
              {(STATUS_OPTIONS[entityType] || []).map((status) => (
                <label key={status} className="flex cursor-pointer items-center gap-2">
                  <input
                    ...
                    className="h-4 w-4 rounded border-wl-border-default bg-wl-bg-elevated checked:bg-blue-600"
                  />
      ...
      {showSavePreset && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 p-4">
          <div className="rounded-lg bg-wl-bg-root p-6 shadow-xl">
