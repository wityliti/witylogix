    ...
    <div className={cn('relative w-full h-full', className)}>
      ...
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/30 backdrop-blur-[2px] z-10 pointer-events-none">
          <div className="flex items-center gap-2 bg-wl-bg-root/90 border border-wl-border-default rounded-lg px-4 py-2">
      ...
      {!isLoading && allPoints.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
          <div className="text-center bg-wl-bg-root/80 border border-wl-border-default rounded-xl px-6 py-4 backdrop-blur-sm">
      ...
      <div className="absolute top-3 right-3 z-10 pointer-events-none">
        <div className="bg-wl-bg-root/90 border border-wl-border-default rounded-lg p-2.5 backdrop-blur-sm space-y-1.5 text-xs">
      ...
      <div className="absolute bottom-3 left-3 z-10 pointer-events-none flex gap-2">
        {driverMarkers.length > 0 && (
          <div className="bg-wl-bg-root/90 border border-wl-border-default rounded-md px-2.5 py-1 backdrop-blur-sm flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-xs font-mono text-wl-text-primary">{driverMarkers.length} on map</span>
          </div>
        )}
        {orderPins.length > 0 && (
          <div className="bg-wl-bg-root/90 border border-wl-border-default rounded-md px-2.5 py-1 backdrop-blur-sm">
