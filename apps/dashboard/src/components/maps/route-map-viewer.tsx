...
const STOP_STATUS_BADGES: Record<
  string,
...
export function RouteMapViewer({
  ...
  const drawRoute = (map: google.maps.Map) => {
    ...
    if (displayActualRoute) {
      ...
      if (plannedPath.type === "LineString") {
        coordinates = (plannedPath.coordinates as Array<[number, number]>).map(
          ([lng, lat]) => ({
            lat,
            lng,
          }),
  ...
  return (
    <div
      className={cn(
        "relative w-full h-full flex flex-col bg-wl-bg-surface",
        className,
      )}
    >
      {/* Map */}
          ...
          <div className="space-y-2 text-xs text-wl-text-secondary">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-wl-success-400">
                {summary.completedStops}
              </span>
            </div>
            {summary.failedStops > 0 && (
              <div className="flex justify-between">
                <span>Failed:</span>
                <span className="font-medium text-wl-danger-400">
                  {summary.failedStops}
                </span>
              </div>
            )}
          ...
          {route.stops.map((stop, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-1">
                ...
                <Badge
                  variant={STOP_STATUS_BADGES[stop.status]}
                  className="text-xs px-1.5"
                >
                  {stop.status}
                </Badge>
              </div>
              <p className="text-wl-text-secondary line-clamp-2">
                {stop.address}
              </p>
              {stop.customerName && (
