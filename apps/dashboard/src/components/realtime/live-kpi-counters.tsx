...
function buildMetrics(stats: DashboardStats): KPIMetric[] {
  const ordersToday = stats.deliveredToday ?? 0;
  const activeDeliveries = stats.pendingOrders ?? 0;
  const availableDrivers = stats.activeDrivers ?? 0;
  const slaPerformance = stats.totalOrders > 0
    ? Math.round((stats.deliveredToday / stats.totalOrders) * 100)
    : 0;
