...
export function NotificationCenter({
  ...
  const {
    ...
    loading: isLoading,
    refetch,
  } = useApiList<ApiNotification>("/api/v4/notifications", { limit: 20 });

  const apiNotifs = useMemo<Notification[]>(
    () =>
      rawNotifs.map((n: ApiNotification) => {
        const inferredType = inferType(n.eventType);
        return {
          id: n.id,
          type: inferredType,
          category: CATEGORY_MAP[inferredType] ?? "SYSTEM",
          channel: n.channel ?? "EMAIL",
          title: inferTitle(n.eventType, n.channel),
          message: `Sent to ${n.recipient}`,
          severity: (n.status === "FAILED" || n.status === "BOUNCED"
            ? "warning"
            : "info") as Notification["severity"],
          status: (n.status === "DELIVERED" || n.status === "SENT"
            ? "READ"
