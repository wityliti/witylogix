...
function NotificationItem({
  ...
  const colorCls = notificationTypeColors[notification.type] ?? "bg-wl-bg-surface text-wl-text-secondary";
...
export function NotificationCenter({
  ...
  const notifications = useMemo<Notification[]>(() =>
    rawNotifs
      ...
      .map((n) => {
        const base = toNotification(n);
        return readIds.has(n.id) ? { ...base, read: true } : base;
      }),
  [rawNotifs, readIds, deletedIds]);

  // Poll for new notifications every 60 seconds while open
  useEffect(() => {
    if (!isOpen) return;
