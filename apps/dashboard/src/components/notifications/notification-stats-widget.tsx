...
export function NotificationStatsWidget({ className }: NotificationStatsWidgetProps) {
  const { data: raw, loading } = useApiQuery<{ data: NotificationStatsData }>('/api/v4/notifications/stats?days=7');
  const stats = raw?.data ?? (raw as unknown as NotificationStatsData | undefined);
      ...
      <div className={cn("space-y-6", className)}>
        <Card className="border border-[var(--wl-border)]">
          <CardContent className="pt-6 text-center py-12">
            <p className="text-sm text-[var(--wl-text-secondary)]">No notification data available</p>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <Card className="border border-[var(--wl-border)]">
        ...
        <CardContent>
          {daily.length > 0 ? (
            ...
            <p className="text-xs text-[var(--wl-text-secondary)] text-center py-4">No data</p>
          )}
        </CardContent>
      </Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {failedTemplates.map((template, idx) => (
                <div
                  key={idx}
                  className="flex items-start justify-between p-3 bg-wl-bg-elevated rounded-lg border border-wl-border-default"
                >
