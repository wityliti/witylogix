...
function Stat({
  ...
  return (
    <div>
      ...
      <div className="text-sm" style={{ color: color ?? 'var(--wl-neutral-100)' }}>
        {value}
      </div>
    </div>
  );
...
function ConfigureForm({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Field label="Per-km rate" name="perKmRate" defaultValue={zone.perKmRate} onSave={onSave} />
      <Field label="Min order" name="minOrder" defaultValue={zone.minOrder} onSave={onSave} />
      <Field label="Free above" name="freeAbove" defaultValue={zone.freeAbove} onSave={onSave} />
      ...
      {confirmDelete ? (
        <div className="mt-3 flex gap-2">
          ...
          <button
            onClick={onDelete}
            className="flex-1 text-[11px] rounded py-1 text-wl-text-primary"
