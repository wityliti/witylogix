      ...
      <div className="border border-wl-border-subtle rounded-lg bg-wl-bg-surface p-6">
        ...
        <div className="mb-8 pb-8 border-b border-wl-border-subtle">
            ...
            {comparisonProviders.map((provider) => (
              <div key={provider.id}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-wl-text-primary">{provider.name}</span>
                  <span className="text-sm text-wl-text-secondary">No data</span>
                </div>
          ...
          <p className="mt-3 text-xs text-wl-text-secondary">
            Metric data will appear once providers report performance statistics.
          </p>
        </div>
        ...
        <div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-wl-border-subtle">
                  <th className="text-left py-3 px-3 font-semibold text-wl-text-primary">Provider</th>
                  <th className="text-center py-3 px-3 font-semibold text-wl-text-primary">Status</th>
                  <th className="text-center py-3 px-3 font-semibold text-wl-text-primary">Category</th>
                </tr>
              </thead>
              <tbody>
                {comparisonProviders.map((provider, idx) => (
                  ...
                  >
                    <td className="py-3 px-3 text-wl-text-primary font-medium">{provider.name}</td>
                    <td className="py-3 px-3 text-center text-wl-text-secondary">{provider.status}</td>
                    <td className="py-3 px-3 text-center text-wl-text-secondary">{provider.category}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <p className="mt-3 text-xs text-wl-text-secondary">
            Detailed feature matrix requires provider capability data from the API.
          </p>
        </div>
