...
export function HealthMonitor({
  ...
  const healthScore = useMemo(() => {
    ...
    return Math.round(
      (healthyCount * 100 + degradedCount * 50) / healthChecks.length,
    );
  }, [healthChecks]);
  ...
  const generateResponseTimeTrend = () => {
    ...
    const points = responseTimeHistory.map((value, idx) => {
      const x =
        padding +
        (idx / (responseTimeHistory.length - 1 || 1)) * (width - padding * 2);
          ...
          <div className="space-y-3">
            {healthChecks.map((check) => {
              ...
              return (
                <div
                  key={check.name}
                  className="flex items-center justify-between p-2 rounded bg-wl-surface-hover"
                >
                  <div>
            ...
            <div>
              ...
              <div className="text-xs text-wl-text-secondary">
                {healthChecks.length > 0
                  ? healthChecks[0].checkedAt.toLocaleString()
                  : "Never"}
              </div>
            </div>
            <button
