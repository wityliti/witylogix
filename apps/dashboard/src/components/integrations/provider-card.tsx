    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <h3 className="text-base font-semibold text-wl-text-primary mb-1">{provider.name}</h3>
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-xs text-wl-text-secondary">{getStatusLabel()}</span>
          </div>
        </div>
        <span
          className={cn(
      ...
      <div className="grid grid-cols-3 gap-3 mb-4 pb-4 border-b border-wl-border-subtle">
        {/* Success rate */}
        <div>
          <div className="text-xs text-wl-text-secondary mb-1">Success Rate</div>
      ...
      <div className="mb-4">
        <div className="text-xs text-wl-text-secondary mb-2">Last 24h Trend</div>
        <svg
          width="100%"
          height="24"
            ...
            <>
              ...
              <path
                d={`M ${sparklineData.points[0][0]} ${sparklineData.height} L ${sparklineData.points
                  .map((p) => `${p[0]} ${p[1]}`)
                  .join(' L ')} L ${sparklineData.points[sparklineData.points.length - 1][0]} ${
              ...
              {/* Line */}
              <polyline
                points={sparklineData.points.map((p) => `${p[0]},${p[1]}`).join(' ')}
                fill="none"
