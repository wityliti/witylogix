...
export function BatchOperations({ operations, onCancel, onRetry, className }: BatchOperationsProps) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-wl-surface-hover border-b border-wl-border-subtle p-4">
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-wl-text-primary mb-3">Operation Queue</h3>
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
            <div>
              <div className="text-xs text-wl-text-secondary mb-1">Total</div>
              <div className="text-lg font-semibold text-wl-text-primary">{stats.total}</div>
            </div>
            <div>
              <div className="text-xs text-wl-text-secondary mb-1">Pending</div>
              <div className="text-lg font-semibold" style={{ color: 'var(--wl-border-subtle)' }}>
                {stats.pending}
              </div>
            </div>
            <div>
              <div className="text-xs text-wl-text-secondary mb-1">Running</div>
              <div className="text-lg font-semibold" style={{ color: 'var(--wl-primary-500)' }}>
                {stats.running}
              </div>
            </div>
            <div>
              <div className="text-xs text-wl-text-secondary mb-1">Completed</div>
              <div className="text-lg font-semibold" style={{ color: 'var(--wl-success-500)' }}>
                {stats.completed}
              </div>
            </div>
            <div>
              <div className="text-xs text-wl-text-secondary mb-1">Failed</div>
              <div className="text-lg font-semibold" style={{ color: 'var(--wl-danger-500)' }}>
                {stats.failed}
              </div>
            </div>
            <div>
              <div className="text-xs text-wl-text-secondary mb-1">Processed</div>
              <div className="text-lg font-semibold text-wl-text-primary">
                {stats.processedTotal.toLocaleString()} / {stats.totalItems.toLocaleString()}
              </div>
            </div>
          </div>
        </div>
        ...
        {stats.running > 0 && (
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-wl-text-secondary">Overall Progress</span>
            ...
            <div className="w-full h-2 rounded-full bg-wl-border-subtle overflow-hidden">
              <div
                ...
                style={{ width: `${(stats.processedTotal / stats.totalItems) * 100}%` }}
              />
            </div>
          </div>
        )}
      ...
      <div className="divide-y divide-wl-border-subtle">
        {operations.length === 0 ? (
          <div className="p-8 text-center text-wl-text-tertiary">
            ...
            <div className="text-xs mt-1">Start a batch operation to see it here</div>
          </div>
        ) : (
          operations.map((operation) => {
            const isExpanded = expandedOperationId === operation.id;
            const elapsedMs = (operation.completedAt || new Date()).getTime() - operation.startedAt.getTime();
            const eta = estimateETA(operation.processed, operation.total, elapsedMs);
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h4 className="font-semibold text-wl-text-primary">{operation.name}</h4>
                      <p className="text-xs text-wl-text-secondary mt-1">
                        Started: {operation.startedAt.toLocaleString()}
                      </p>
                    </div>
                    <span className={cn('px-2 py-1 rounded text-xs font-medium', getStatusBadgeClass(operation.status))}>
                  ...
                  <div className="mb-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-wl-text-secondary">
                        {operation.processed.toLocaleString()} / {operation.total.toLocaleString()} items
                      </span>
                      <span className="text-xs text-wl-text-secondary">
                        {operation.progress}%
                        {operation.status === 'running' && eta !== '--' && ` (ETA: ${eta})`}
                      </span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-wl-border-subtle overflow-hidden">
                      <div
                        className={cn(
                          ...
                          operation.status === 'completed' && 'bg-wl-success-500',
                          operation.status === 'failed' && 'bg-wl-danger-500',
                          operation.status === 'running' && 'bg-gradient-to-r from-wl-primary-500 to-wl-primary-600',
                          (operation.status === 'pending' || operation.status === 'cancelled') && 'bg-wl-border-subtle'
                        )}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <span className="text-wl-text-tertiary transition-transform" style={{ fontSize: '12px' }}>
                        {isExpanded ? '▾' : '▸'}
                      </span>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="bg-wl-surface-hover border-t border-wl-border-subtle p-4">
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <div className="text-xs text-wl-text-secondary mb-1">Operation ID</div>
                        <div className="text-xs font-mono text-wl-text-primary break-all">{operation.id}</div>
                      </div>
                      <div>
                    ...
                    {operation.error && (
                      <div className="p-3 rounded-lg bg-wl-danger-500/10 border border-wl-danger-500/20">
                        ...
                        <div className="text-xs text-wl-text-secondary font-mono">{operation.error}</div>
                      </div>
                    )}
