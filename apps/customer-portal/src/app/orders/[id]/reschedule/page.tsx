...
const STEPS: RescheduleStep[] = ['select-date', 'select-time', 'confirm', 'success'];
...
export default function ReschedulePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: orderEnvelope, loading: orderLoading, error: orderError } =
  ...
  const handleNext = async () => {
    if (step === 'select-date' && selectedDate) {
      ...
      try {
        ...
        setRescheduleError(
          err instanceof Error ? err.message : 'Failed to reschedule. Please try again.',
        );
      }
    }
  };
  ...
  if (orderError) {
    return (
      <div className="page-container">
        <Link href={`/orders/${id}`} className="btn btn-ghost inline-flex items-center gap-2 mb-4">
          <ArrowLeft size={18} />
          Back
        </Link>
  ...
  const currentDate = order?.deliveryDate ? new Date(order.deliveryDate) : new Date();

  return (
    <div className="page-container animate-fade-in">
      {/* Header */}
      <div className="flex items-center gap-4">
        <Link href={`/orders/${id}`} className="btn btn-ghost p-2" aria-label="Back to order">
      ...
      <div className="max-w-2xl">
        {step === 'select-date' && (
          <div className="section-card stagger-1">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-2xl font-bold text-wl-text-primary">Select a New Date</h2>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {availableDates.map((date) => (
                ...
                >
                  <p className="font-medium text-wl-text-primary">{format(date, 'EEE, MMM d')}</p>
                  <p className="text-xs text-wl-text-tertiary mt-1">{format(date, 'PPP')}</p>
                </button>
              ))}
            </div>
          </div>
        ...
        {step === 'select-time' && (
          <div className="section-card stagger-1">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-2xl font-bold text-wl-text-primary">Select a Time Slot</h2>
            </div>
        ...
        {step === 'confirm' && (
          <div className="section-card stagger-1">
            <h2 className="text-2xl font-bold text-wl-text-primary mb-6">Confirm Reschedule</h2>
            ...
            {rescheduleError && (
              <p className="mt-3 text-sm text-wl-error-500">{rescheduleError}</p>
            )}
          </div>
        )}
        ...
        {step === 'success' && (
          <div className="section-card border-l-2 border-wl-success-500 text-center stagger-1">
            ...
            <p className="text-wl-text-secondary mb-6">
              ...
              {selectedSlot ? ` at ${selectedSlot.startTime} – ${selectedSlot.endTime}` : ''}.
            </p>
        ...
        <div className="flex gap-3 max-w-2xl">
          ...
          <button
            ...
            className={cn(
              ...
              (isNextDisabled || rescheduling) && 'opacity-50 cursor-not-allowed',
            )}
          >
            {rescheduling ? (
              <>
