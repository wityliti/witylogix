...
export default function OrderDetailPage({
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery<OrderDetailEnvelope>(ROUTES.ORDER(id));
  ...
  if (error) {
    return (
      <div className="page-container">
        <Link
          href="/orders"
          className="btn btn-ghost p-2 inline-flex items-center gap-2 mb-4"
        >
          <ArrowLeft size={18} />
          Back to Orders
        </Link>
  ...
  const orderNumber =
    order.externalOrderNumber ?? `#${id.slice(-8).toUpperCase()}`;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          {order.proofOfDelivery &&
            order.proofOfDelivery.photoUrls.length > 0 && (
              <div className="section-card animate-fade-in stagger-4">
                <h2 className="font-semibold text-wl-text-primary mb-4">
                  Proof of Delivery
                </h2>
