...
import { Mail, MessageCircle, Phone, HelpCircle, Send, Loader2, Check, Ticket } from 'lucide-react';
...
export default function SupportPage() {
  ...
  const { data: ticketsData, loading: ticketsLoading, error: ticketsError, refetch: refetchTickets } =
    useQuery<PaginatedTickets>(ROUTES.SUPPORT_TICKETS);

  const { mutate: createTicket, loading: creating, error: createError } =
  ...
  return (
    <div className="page-container animate-fade-in">
      ...
      <div className="page-header">
        ...
        <p className="page-subtitle">Get help with your deliveries and account</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 stagger-1">
        <a
          ...
          className={cn('section-card flex items-start gap-4', 'hover:border-wl-primary-500 transition-colors')}
        >
          <Mail size={24} className="text-wl-primary-500 flex-shrink-0" />
          <div>
            <h3 className="font-bold text-wl-text-primary">Email</h3>
            <p className="text-sm text-wl-text-secondary mt-1">support@witylogix.com</p>
            <p className="text-xs text-wl-text-tertiary mt-2">Response in 2 hours</p>
          </div>
        </a>
        ...
        <div className={cn('section-card flex items-start gap-4', 'hover:border-wl-primary-500 transition-colors')}>
          <MessageCircle size={24} className="text-wl-primary-500 flex-shrink-0" />
          <div>
            <h3 className="font-bold text-wl-text-primary">Chat</h3>
            <p className="text-sm text-wl-text-secondary mt-1">Live chat available</p>
            <p className="text-xs text-wl-text-tertiary mt-2">During business hours</p>
          </div>
        </div>
      </div>
            ...
            <div className="mb-6 flex flex-wrap gap-2">
              {categories.map((category) => (
                <button
                  ...
                  className={cn(selectedCategory === category.id ? 'btn btn-primary' : 'btn btn-ghost')}
                >
                  {category.label}
                </button>
              ))}
                ...
                >
                  ...
                  >
                    <p
                      className={cn(
                        'font-medium',
                        expandedFAQ === index ? 'text-wl-primary-400' : 'text-wl-text-primary',
                      )}
                    >
                      {faq.question}
                    </p>
        ...
        <div className="flex flex-col gap-6 stagger-3">
          ...
          <form onSubmit={handleSubmitTicket} className="section-card">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-bold text-wl-text-primary">Submit a Ticket</h2>
            </div>
            ...
            {ticketSent && (
              ...
              >
                ...
                <span className="text-wl-text-primary">Ticket submitted! We'll respond shortly.</span>
              </div>
            )}

            {createError && (
              <p className="text-sm text-wl-error-500 mb-3">{createError.message}</p>
            )}
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className={cn(
                  ...
                  (creating || !subject.trim() || !description.trim()) && 'opacity-50 cursor-not-allowed',
                )}
              >
                {creating ? (
                  <>
          ...
          <div className="section-card">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-wl-text-primary">My Tickets</h2>
            </div>
            ...
            {!ticketsLoading && ticketsError && (
              <ErrorMessage message={ticketsError.message} onRetry={refetchTickets} />
            )}

            {!ticketsLoading && !ticketsError && (!ticketsData || ticketsData.data.length === 0) && (
              <p className="text-sm text-wl-text-tertiary">No tickets yet. Submit one above.</p>
            )}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span
                        className={cn(
                          ...
                          TICKET_STATUS_COLORS[ticket.status] ?? 'status-default',
                        )}
                      >
          ...
          {/* Response Time Info */}
          <div>
            <p className="text-sm font-medium text-wl-text-secondary mb-1">Response Time</p>
            <p className="text-xs text-wl-text-tertiary">
              We typically respond within 2 hours during business hours (9 AM–6 PM EST, Monday to
              Friday).
            </p>
          </div>
        </div>
