...
import { forTenant, forTenantInOrg, forOrg } from "@witylogix/db";
...
export async function tenantContext(
  ...
  if (!request.auth?.shopId) {
    throw new UnauthorizedError("Tenant context requires authentication");
  }

  const shopId = request.auth.shopId;
  const orgId = request.auth.orgId;

  // Use dual-scoped client if org exists, otherwise shop-only
  if (orgId) {
    request.tenantDb = forTenantInOrg(shopId, orgId);
    request.orgId = orgId;
  } else {
    request.tenantDb = forTenant(shopId);
  }

  // Tenant-scoped Redis client (always shop-prefixed)
  request.tenantRedis = new TenantRedis(shopId);
...
export async function orgContext(
  ...
  if (!request.auth?.orgId) {
    throw new UnauthorizedError("Organization context requires org membership");
  }

  request.orgDb = forOrg(request.auth.orgId);
