...
class MotiveELDClient {
  ...
  async getDriverLogs(driverId: string, date: string): Promise<{ logs?: any[]; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
  ...
  async updateHOSEvent(driverId: string, eventType: string, timestamp: string): Promise<{ success?: boolean; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
  ...
  async getVehicles(): Promise<{ vehicles?: Array<{ id: string; vin: string }>; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
  ...
  async getDVIRDefects(vehicleId: string): Promise<{ defects?: any[]; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
  ...
  async submitDVIR(vehicleId: string, inspection: Record<string, any>): Promise<{ dvir_id?: string; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
...
class OmnitracELDClient {
  ...
  async authenticateWithCredentials(username: string, password: string): Promise<{ access_token?: string; error?: string }> {
  ...
  async getDriverData(driverId: string): Promise<{ driver?: Record<string, any>; error?: string }> {
    if (!this.accessToken) {
      return { error: 'not_authenticated' };
    }
  ...
  async getELDRecords(driverId: string, startDate: string, endDate: string): Promise<{ records?: any[]; error?: string }> {
    if (!this.accessToken) {
      return { error: 'not_authenticated' };
    }
  ...
  async createELDRecord(driverId: string, record: Record<string, any>): Promise<OmnitracELDRecord> {
    if (!this.accessToken) {
      return { error: 'not_authenticated' };
    }
  ...
  async getDispatchData(driverId: string): Promise<{ assignments?: any[]; error?: string }> {
    if (!this.accessToken) {
      return { error: 'not_authenticated' };
    }
    ...
    return {
      assignments: [
        { id: 'assign-1', origin: 'Warehouse A', destination: 'Customer B', status: 'en_route' },
      ],
    };
  }

  async updateDispatchStatus(assignmentId: string, status: string): Promise<{ success?: boolean; error?: string }> {
    if (!this.accessToken) {
      return { error: 'not_authenticated' };
    }
...
class AzugaELDClient {
  ...
  async authenticate(apiKey: string, accountId: string): Promise<{ success?: boolean; error?: string }> {
    if (!apiKey || !accountId) {
      return { error: 'invalid_credentials' };
    }
  ...
  async getDriverStatus(driverId: string): Promise<{ status?: string; location?: Record<string, any>; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
  ...
  async getComplianceStatus(driverId: string): Promise<{ violations?: any[]; compliant?: boolean; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
  ...
  async recordTrip(driverId: string, trip: Record<string, any>): Promise<AzugaTrip> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
...
class ELDComplianceEngine {
  ...
  constructor(motive: MotiveELDClient, omnitracs: OmnitracELDClient, azuga: AzugaELDClient) {
    this.motive = motive;
    this.omnitracs = omnitracs;
    this.azuga = azuga;
  }

  async validateHOSCompliance(driverId: string): Promise<{ compliant: boolean; violations: ComplianceViolation[] }> {
    ...
    for (const vehicle of vehicles.vehicles || []) {
      ...
      if (defects.defects && defects.defects.length > 0) {
        allViolations.push({
          ...
          driverId,
          timestamp: new Date().toISOString(),
          details: { vehicleId: vehicle.id, defectCount: defects.defects.length },
        });
      }
    }
  ...
  async generateAuditReport(driverId: string, startDate: string, endDate: string): Promise<{ report: Record<string, any>; error?: string }> {
    const driverViolations = this.violations.filter(v => v.driverId === driverId);
  ...
  async aggregateComplianceStatus(driverIds: string[]): Promise<{ compliant: number; violations: number; total: number }> {
    let compliant = 0;
    let violations = 0;
...
describe('Motive ELD Client', () => {
  ...
  describe('Authentication', () => {
    it('should authenticate with valid API key', async () => {
      const response = await motiveClient.authenticateWithAPIKey('valid-api-key-12345');
  ...
  describe('Driver Logs', () => {
    ...
    it('should get driver logs', async () => {
      const response = await motiveClient.getDriverLogs('driver-123', '2024-01-15');
  ...
  describe('HOS Events', () => {
    ...
    it('should update HOS event', async () => {
      const response = await motiveClient.updateHOSEvent('driver-123', 'driving', '09:00');
      expect(response.success).toBe(true);
    });
...
describe('Omnitracs ELD Client', () => {
  ...
  describe('Authentication', () => {
    it('should authenticate with credentials', async () => {
      const response = await omnitracClient.authenticateWithCredentials('username', 'password');
  ...
  describe('ELD Records', () => {
    ...
    it('should get ELD records', async () => {
      const response = await omnitracClient.getELDRecords('driver-123', '2024-01-15', '2024-01-16');
  ...
  describe('Dispatch', () => {
    ...
    it('should update dispatch status', async () => {
      const response = await omnitracClient.updateDispatchStatus('assign-123', 'delivered');
      expect(response.success).toBe(true);
    });
  });
});
...
describe('Azuga ELD Client', () => {
  ...
  describe('Authentication', () => {
    it('should authenticate with valid credentials', async () => {
      const response = await azugaClient.authenticate('valid-key', 'valid-account');
      expect(response.success).toBe(true);
    });
...
describe('ELD Compliance Engine', () => {
  ...
  describe('Audit Reporting', () => {
    it('should generate audit report', async () => {
      await engine.recordELDEvent('driver-123', 'vehicle_inspection', new Date().toISOString(), {});

      const report = await engine.generateAuditReport('driver-123', '2024-01-15', '2024-01-16');
  ...
  describe('Status Aggregation', () => {
    it('should aggregate compliance status', async () => {
      const result = await engine.aggregateComplianceStatus(['driver-1', 'driver-2', 'driver-3']);
