...
class BraintreeClient {
  ...
  constructor(merchantId: string, publicKey: string, privateKey: string, environment: string = 'sandbox') {
  ...
  async authorize(amount: number, paymentMethodNonce: string): Promise<BraintreeTransaction> {
  ...
  async capture(transactionId: string): Promise<BraintreeTransaction> {
    ...
    return { id: transactionId, status: 'captured', amount: transaction.amount };
  }

  async refund(transactionId: string, amount?: number): Promise<BraintreeTransaction> {
    const transaction = this.transactions.get(transactionId);
    if (!transaction) {
      return { error: 'transaction_not_found' };
    }
    ...
    return { id: transactionId, status: 'refunded', amount: amount || transaction.amount };
  }

  async createCustomer(firstName: string, lastName: string, email: string): Promise<BraintreeCustomer> {
    if (!firstName || !email) {
  ...
  async createPaymentMethod(customerId: string, paymentMethodNonce: string): Promise<{ paymentMethodToken?: string; error?: string }> {
  ...
  async createSubscription(planId: string, paymentMethodToken: string): Promise<BraintreeSubscription> {
  ...
  async cancelSubscription(subscriptionId: string): Promise<BraintreeSubscription> {
...
class AuthorizeNetClient {
  ...
  constructor(apiLoginId: string, transactionKey: string, environment: string = 'sandbox') {
  ...
  async createCustomerProfile(description: string): Promise<AuthorizeNetProfile> {
    if (!this.apiLoginId) {
      return { error: 'invalid_credentials' };
    }
  ...
  async addPaymentProfile(profileId: string, cardNumber: string, expirationDate: string): Promise<{ paymentProfileId?: string; error?: string }> {
  ...
  async createRecurringBilling(profileId: string, paymentProfileId: string, amount: number, interval: number): Promise<{ subscriptionId?: string; error?: string }> {
  ...
  async refundTransaction(transactionId: string, amount?: number): Promise<AuthorizeNetResponse> {
    const transaction = this.transactions.get(transactionId);
    if (!transaction) {
      return { error: 'transaction_not_found' };
    }
  ...
  async checkFraud(ipAddress: string, email: string, amount: number): Promise<{ riskLevel?: string; error?: string }> {
    if (!ipAddress || !email) {
      return { error: 'invalid_request' };
    }
...
class AdyenClient {
  ...
  constructor(apiKey: string, clientKey: string, merchantAccount: string, hmacKey: string) {
    this.apiKey = apiKey;
  ...
  async initiateCheckout(amount: number, currency: string, reference: string): Promise<AdyenCheckoutResponse> {
  ...
  async submitPayment(paymentMethod: Record<string, any>, amount: number, currency: string, reference: string): Promise<AdyenCheckoutResponse> {
  ...
  async splitPayment(pspReference: string, splits: Array<{ amount: number; account: string }>): Promise<{ success?: boolean; error?: string }> {
  ...
  async refund(originalReference: string, amount: number, currency: string): Promise<{ pspReference?: string; error?: string }> {
  ...
  verifyWebhookSignature(signature: string, body: string): boolean {
    const expectedSignature = Buffer.from(`${body}${this.hmacKey}`).toString('base64');
...
describe('Braintree Client', () => {
  ...
  beforeEach(() => {
    braintreeClient = new BraintreeClient('merchant-id', 'public-key', 'private-key');
  });

  describe('Transaction Lifecycle', () => {
    it('should authorize transaction', async () => {
      const response = await braintreeClient.authorize(100.00, 'nonce-123');
    ...
    it('should capture authorized transaction', async () => {
      const authorize = await braintreeClient.authorize(100.00, 'nonce-123');
    ...
    it('should reject invalid nonce', async () => {
      const response = await braintreeClient.authorize(100.00, '');
  ...
  describe('Customer Vault', () => {
    it('should create customer', async () => {
      const response = await braintreeClient.createCustomer('John', 'Doe', 'john@example.com');
      expect(response.id).toBeDefined();
      expect(response.paymentMethods).toEqual([]);
    });

    it('should add payment method to customer', async () => {
      const customer = await braintreeClient.createCustomer('Jane', 'Doe', 'jane@example.com');
      const response = await braintreeClient.createPaymentMethod(customer.id || '', 'nonce-456');
  ...
  describe('Subscriptions', () => {
    it('should create subscription', async () => {
      const response = await braintreeClient.createSubscription('plan-monthly', 'token-123');
      expect(response.id).toBeDefined();
      expect(response.status).toBe('active');
    });

    it('should cancel subscription', async () => {
      const subscription = await braintreeClient.createSubscription('plan-monthly', 'token-123');
      const response = await braintreeClient.cancelSubscription(subscription.id || '');
...
describe('Authorize.Net Client', () => {
  ...
  describe('Card Charging', () => {
    ...
    it('should reject invalid card number', async () => {
      const response = await authorizeNetClient.chargeCard('', '', '', 100.00);
      expect(response.error).toBe('invalid_request');
    });
  ...
  describe('Customer Information Manager (CIM)', () => {
    it('should create customer profile', async () => {
      const response = await authorizeNetClient.createCustomerProfile('Test Customer');
  ...
  describe('Automated Recurring Billing (ARB)', () => {
    ...
    it('should validate recurring billing params', async () => {
      const response = await authorizeNetClient.createRecurringBilling('', '', -50, 0);
      expect(response.error).toBe('invalid_request');
    });
  });
  ...
  describe('Refunds', () => {
    it('should refund transaction', async () => {
      ...
      const response = await authorizeNetClient.refundTransaction(transId, 100.00);
      expect(response.transactionResponse?.responseCode).toBe('1');
    });
  });
  ...
  describe('Fraud Detection', () => {
    it('should assess fraud risk', async () => {
      const response = await authorizeNetClient.checkFraud('192.168.1.1', 'user@example.com', 100);
...
describe('Adyen Client', () => {
  ...
  beforeEach(() => {
    adyenClient = new AdyenClient('api-key', 'client-key', 'TestMerchant', 'hmac-secret');
  });

  describe('Checkout Flow', () => {
    it('should initiate checkout', async () => {
      const response = await adyenClient.initiateCheckout(100.00, 'USD', 'order-123');
  ...
  describe('Refunds', () => {
    it('should refund payment', async () => {
      const response = await adyenClient.refund('original-ref', 100.00, 'USD');
      expect(response.pspReference).toBeDefined();
    });
...
describe('Payment Adapter Base', () => {
  ...
  describe('Idempotency', () => {
    ...
    it('should create different transactions for different keys', async () => {
      const result1 = await adapter.processTransaction('braintree', 100.00, 'USD', 'key-1', {});
      const result2 = await adapter.processTransaction('braintree', 100.00, 'USD', 'key-2', {});
  ...
  describe('Data Encryption', () => {
    it('should encrypt sensitive data', () => {
      const encrypted = adapter.encryptSensitiveData('card-number', 'encryption-key');
  ...
  describe('Amount Validation', () => {
    it('should validate amount and currency', () => {
      const valid = adapter.validateAmount(100.00, 'USD');
      expect(valid).toBe(true);
    });
  ...
  describe('Transaction Tracking', () => {
    ...
    it('should retrieve transaction details', async () => {
      const result = await adapter.processTransaction('authorize.net', 150, 'EUR', 'key-x', { test: true });
      const transaction = await adapter.getTransaction(result.transactionId || '');
