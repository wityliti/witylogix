...
class ToastPOSClient {
  ...
  async createOrder(
    locationId: string,
    orderData: {
      items: Array<{ guid: string; quantity: number }>;
      total: number;
    },
  ): Promise<ToastOrder> {
  ...
  async getRevenueCenters(locationId: string): Promise<{
    centers?: Array<{ guid: string; name: string }>;
    error?: string;
  }> {
    if (!this.accessToken) {
      return { error: "not_authed" };
  ...
  async registerWebhook(
    url: string,
    events: string[],
  ): Promise<{ webhookGuid?: string; error?: string }> {
    if (!this.accessToken) {
      return { error: "not_authed" };
    }
...
class SquareRestaurantsClient {
  ...
  async createOrder(
    locationId: string,
    orderData: {
      lineItems: Array<{ catalogObjectId: string; quantity: number }>;
      totalMoney: number;
    },
  ): Promise<SquareOrder> {
    if (!this.accessToken || !locationId) {
  ...
  async listOrders(
    locationId: string,
    cursor?: string,
  ): Promise<{ orders?: SquareOrder[]; cursor?: string; error?: string }> {
    if (!this.accessToken) {
      return { error: "unauthorized" };
    }
      ...
      items: [
        {
          id: "item-1",
          type: "ITEM",
          itemData: { name: "Burger", variations: [{ price: 1299 }] },
        },
        {
          id: "item-2",
          type: "ITEM",
          itemData: { name: "Salad", variations: [{ price: 999 }] },
        },
      ],
    };
  }
  ...
  async getTerminals(locationId: string): Promise<{
    terminals?: Array<{ id: string; deviceName: string }>;
    error?: string;
  }> {
    if (!this.accessToken) {
      return { error: "unauthorized" };
  ...
  verifyWebhookSignature(
    ...
    return (
      signature === Buffer.from(`${body}${signatureKey}`).toString("base64")
    );
  }
}
...
class POSAdapterBase {
  ...
  async getLocationInfo(
    locationId: string,
  ): Promise<{ name?: string; timezone?: string; error?: string }> {
  ...
  async updateOrderStatus(
    ...
    if (
      !["pending", "confirmed", "delivered", "cancelled"].includes(newStatus)
    ) {
  ...
  async getOrdersByLocation(locationId: string): Promise<POSOrder[]> {
    return Array.from(this.orders.values()).filter(
      (o) => o.location === locationId,
    );
  }

  async handleModifierSelection(
    itemId: string,
    modifiers: string[],
...
describe("Toast POS Client", () => {
  ...
  describe("Order Operations", () => {
    beforeEach(async () => {
      await toastClient.getAccessTokenFromCode(
        "code123",
        "client-id",
        "client-secret",
      );
    ...
    it("should update order", async () => {
      ...
      const response = await toastClient.updateOrder(order.guid || "", {
        status: "confirmed",
      });
...
describe("Square Restaurants Client", () => {
  ...
  describe("Order Management", () => {
    beforeEach(async () => {
      await squareClient.getAccessTokenFromCode(
        "code123",
        "client-id",
        "client-secret",
      );
    ...
    it("should update order", async () => {
      ...
      const response = await squareClient.updateOrder(order.id || "", {
        state: "COMPLETED",
      });
...
describe("POS Adapter Base", () => {
  ...
  describe("Order Status Management", () => {
    it("should update order status", async () => {
      ...
      const response = await adapter.updateOrderStatus(
        order.orderId || "",
        "confirmed",
      );
      expect(response.success).toBe(true);
  ...
  describe("Modifier Handling", () => {
    it("should validate modifier selection", async () => {
      const response = await adapter.handleModifierSelection("item-1", [
        "cheese",
        "bacon",
      ]);
    ...
    it("should handle non-existent items", async () => {
      const response = await adapter.handleModifierSelection("non-existent", [
        "cheese",
      ]);
  ...
  describe("Location-Based Queries", () => {
    it("should get orders by location", async () => {
      await adapter.createOrderWithModifiers("loc-123", [
        { itemId: "item-1", quantity: 1 },
      ]);
      await adapter.createOrderWithModifiers("loc-456", [
        { itemId: "item-2", quantity: 1 },
      ]);
