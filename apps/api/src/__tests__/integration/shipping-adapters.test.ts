...
interface ShipmentRecord {
  id: string;
  fromAddress: { name: string; email: string; street1: string; city: string; state: string; zip: string; country: string };
  toAddress: { name: string; email: string; street1: string; city: string; state: string; zip: string; country: string };
...
class ShippoClient {
  ...
  async getRates(shipmentId: string): Promise<{ rates?: ShippoRate[]; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
  ...
  async trackShipment(trackingNumber: string, carrier: string): Promise<{ status?: string; events?: any[]; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
    ...
    return {
      status: 'in_transit',
      events: [
        { status: 'picked_up', timestamp: new Date().toISOString(), location: 'Origin Facility' },
        { status: 'in_transit', timestamp: new Date().toISOString(), location: 'Distribution Center' },
      ],
    };
  }

  async batchCreateShipments(shipmentsList: Array<{ from: any; to: any; parcels: any[] }>): Promise<{ batchId?: string; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
  ...
  async updateShipmentStatus(shipmentId: string, status: string): Promise<{ success?: boolean; error?: string }> {
    if (!this.apiKey) {
      return { error: 'not_authenticated' };
    }
...
class CarrierComparisonEngine {
  ...
  async rateShopping(shipmentId: string): Promise<{ optimal: ShippoRate; all: ShippoRate[]; error?: string }> {
    ...
    const optimalByPrice = rates.reduce((min, r) => (parseFloat(r.amount || '999') < parseFloat(min.amount || '999') ? r : min));
  ...
  async selectOptimalCarrier(
    ...
    if (criteria === 'price') {
      selected = rates.reduce((min, r) => (parseFloat(r.amount || '999') < parseFloat(min.amount || '999') ? r : min));
    } else if (criteria === 'speed') {
      selected = rates.reduce((min, r) => ((r.estimatedDays || 999) < (min.estimatedDays || 999) ? r : min));
    } else {
  ...
  async compareRates(shipmentId: string): Promise<{ comparison: Array<{ carrier: string; price: number; days: number }>; error?: string }> {
    const result = await this.shippo.getRates(shipmentId);
    if (result.error) {
...
describe('Shippo Client', () => {
  ...
  describe('Rate Retrieval', () => {
    ...
    it('should get rates for shipment', async () => {
      const shipment = await shippoClient.createShipment(
        { street1: '123 Main', city: 'NY', state: 'NY', zip: '10001', country: 'US' },
        { street1: '456 Oak', city: 'LA', state: 'CA', zip: '90001', country: 'US' },
        [{ length: 10, width: 10, height: 10, weight: 2 }]
      );
  ...
  describe('Label Creation', () => {
    ...
    it('should create shipping label', async () => {
      ...
      const response = await shippoClient.createLabel(firstRate?.objectId || '');
      expect(response.objectId).toBeDefined();
  ...
  describe('Shipment Tracking', () => {
    ...
    it('should track shipment', async () => {
      const response = await shippoClient.trackShipment('1Z999AA10123456784', 'UPS');
  ...
  describe('Batch Operations', () => {
    ...
    it('should batch create shipments', async () => {
      const shipments = [
        {
          from: { street1: '123', city: 'NY', state: 'NY', zip: '10001', country: 'US' },
          to: { street1: '456', city: 'LA', state: 'CA', zip: '90001', country: 'US' },
          parcels: [{ length: 10, width: 10, height: 10, weight: 2 }],
        },
        {
          from: { street1: '789', city: 'NY', state: 'NY', zip: '10002', country: 'US' },
          to: { street1: '321', city: 'SF', state: 'CA', zip: '94105', country: 'US' },
...
describe('Carrier Comparison Engine', () => {
  ...
  describe('Optimal Carrier Selection', () => {
    it('should select optimal carrier by price', async () => {
      ...
      const result = await engine.selectOptimalCarrier(shipment.objectId || '', 'price');
    ...
    it('should select optimal carrier by speed', async () => {
      ...
      const result = await engine.selectOptimalCarrier(shipment.objectId || '', 'speed');
    ...
    it('should select optimal carrier by reliability', async () => {
      ...
      const result = await engine.selectOptimalCarrier(shipment.objectId || '', 'reliability');
