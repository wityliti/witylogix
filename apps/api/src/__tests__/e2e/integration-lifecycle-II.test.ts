...
class IntegrationOrchestrator {
  ...
  async executeFullWorkflow(
    ...
    try {
      ...
      stages.signature_webhook = await this.processSignatureWebhook(
        workflow.envelope?.id || "",
      );
...
describe("End-to-End Integration Lifecycle II", () => {
  ...
  describe("Webhook Chain Testing", () => {
    it("should log envelope creation webhook", async () => {
      ...
      const envelopeCreated = webhooks.find(
        (w) => w.event === "envelope_created",
      );
    ...
    it("should log envelope completed webhook", async () => {
      ...
      const webhooks = orchestrator.getWebhookLog();
      const envelopeCompleted = webhooks.find(
        (w) => w.event === "envelope_completed",
      );
    ...
    it("should log delivery creation webhook", async () => {
      ...
      const webhooks = orchestrator.getWebhookLog();
      const deliveryCreated = webhooks.find(
        (w) => w.event === "delivery_created",
      );
  ...
  describe("Error Recovery and Retry Flows", () => {
    it("should retry failed operation", async () => {
      ...
      try {
        const result = await orchestrator.retryFailedOperation(
          "payment_process",
          { amount: 1000 },
        );
    ...
    it("should handle max retries exceeded", async () => {
      try {
        await orchestrator.retryFailedOperation(
          "failing_op",
          { test: true },
          1,
        );
  ...
  describe("Multi-Step Transaction Management", () => {
    it("should handle sequential stages in order", async () => {
      ...
      if (workflow?.status === "completed") {
        ...
        expect(
          new Date(workflow.contact.createdAt).getTime(),
        ).toBeLessThanOrEqual(
          new Date(
            workflow.deal?.createdAt || workflow.contact.createdAt,
          ).getTime(),
        );
        ...
        expect(
          new Date(workflow.invoice?.createdAt || "").getTime(),
        ).toBeLessThanOrEqual(
          new Date(
            workflow.envelope?.createdAt || workflow.invoice?.createdAt || "",
          ).getTime(),
        );
        // Signature before payment
        expect(
          new Date(workflow.envelope?.createdAt || "").getTime(),
        ).toBeLessThanOrEqual(
          new Date(
            workflow.payment?.createdAt || workflow.envelope?.createdAt || "",
          ).getTime(),
        );
      }
    });
