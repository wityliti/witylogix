...
export default async function paymentRoutesV2(
  ...
  fastify.post<{ Params: { id: string }; Body: CapturePaymentRequest }>(
    ...
    async (request: any, reply: FastifyReply) => {
      try {
        ...
        const intent = await processor.getPaymentIntent(paymentIntentId, request.shopId);
        if (!intent) {
          throw new PaymentError(
            'INTENT_NOT_FOUND',
        ...
        await processor.updatePaymentIntentStatus(paymentIntentId, 'captured', request.shopId);

        router.recordTransaction(intent.providerName || 'square', intent.amount);
  ...
  fastify.post<{ Params: { id: string }; Body: RefundPaymentRequest }>(
    ...
    async (request: any, reply: FastifyReply) => {
      try {
        const { amount, reason, description } = request.body as RefundPaymentRequest;
        const transactionId = request.params.id;

        // Lookup transaction to get provider
        const transaction = await processor.getTransaction(transactionId, request.shopId);
        if (!transaction) {
          throw new PaymentError(
        ...
        const gateway = router.getGateway(transaction.providerName?.toLowerCase() || 'square');
        if (!gateway) {
          throw new PaymentError(
            'GATEWAY_NOT_FOUND',
  ...
  fastify.post<{ Params: { gateway: string } }>(
    ...
    async (request: any, reply: FastifyReply) => {
      try {
        ...
        if (!gateway) {
          fastify.log.warn(`Webhook received for unknown gateway: ${gatewayCode}`);
          return reply.status(404).send({ success: false });
        }

        // Verify webhook signature
        const signature = request.headers['x-paypal-transmission-sig'] ||
                          request.headers['x-square-hmac-sha256-signature'] ||
                          request.headers['authorization'] ||
                          '';

        const isValid = await gateway.verifyWebhookSignature(request.body, signature);
        if (!isValid) {
          fastify.log.warn(`Webhook signature verification failed for ${gatewayCode}`);
        ...
        fastify.log.error('Webhook processing failed', { error: error.message });
