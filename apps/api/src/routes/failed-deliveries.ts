...
async function failedDeliveriesRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/:shipmentId/attempts', async (request: FastifyRequest, reply: FastifyReply) => {
    const shopId: string = (request as any).shopId;
    const db: any = (request as any).tenantDb;
    ...
    const shipment = await db.shipment.findFirst({
      ...
      select: { id: true, shipmentNumber: true, status: true, attemptCount: true },
    });
  ...
  fastify.post('/:shipmentId/reassign', async (request: FastifyRequest, reply: FastifyReply) => {
    const shopId: string = (request as any).shopId;
    const db: any = (request as any).tenantDb;
    const { shipmentId } = request.params as { shipmentId: string };
    ...
    const updated = await db.shipment.update({
      ...
      select: { id: true, shipmentNumber: true, status: true, driverId: true },
    });
  ...
  fastify.post('/:shipmentId/return', async (request: FastifyRequest, reply: FastifyReply) => {
    const shopId: string = (request as any).shopId;
    const db: any = (request as any).tenantDb;
    const { shipmentId } = request.params as { shipmentId: string };
