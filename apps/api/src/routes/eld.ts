...
const defectsListQuery = listQuery.extend({
  status: z
    .enum(["REPORTED", "ACKNOWLEDGED", "REPAIRED", "CERTIFIED", "ALL"])
    .default("ALL"),
...
function deriveComplianceStatus(rec: {
  ...
  if (drivingPct >= 0.85 || onDutyPct >= 0.85 || rec.breakStatus === "REQUIRED")
    return "WARNING";
  return "COMPLIANT";
}
...
export default async function eldRoutes(
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const [hosRecords, dvirInspections, openDefects, criticalDefects] =
        await Promise.all([
      ...
      const activeViolations = hosRecords.filter(
        (r: any) => r.isViolation,
      ).length;
      ...
      const dvirCompleted = dvirInspections.filter(
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const data = drivers.map((d: any) => {
        ...
        const currentDuty =
          hos?.currentStatus ?? dutyStatusMap[d.status] ?? "OFF_DUTY";

        return {
          driverId: d.id,
          name: d.name,
          status: hos
            ? deriveComplianceStatus(hos)
            : d.status === "OFFLINE"
              ? "OFFLINE"
              : "COMPLIANT",
          currentDuty,
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return reply.send({
        ...
        cycleHours: 70,
        cycleHoursUsed: hos
          ? Math.round((hos.cycleMinutesUsed / 60) * 10) / 10
          : 0,
        breakStatus: hos?.breakStatus ?? "NOT_REQUIRED",
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const data = records.map((r: any) => ({
        ...
        severity:
          r.drivingMinutesToday >= MAX_DRIVING_MINUTES ? "CRITICAL" : "WARNING",
        timestamp: r.lastStatusChangeAt,
        duration: Math.round(
          (Date.now() - new Date(r.lastStatusChangeAt).getTime()) / 60000,
        ),
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const updated = await (prisma as any).dvirDefect.update({
        ...
        data: {
          ...
          certifiedAt:
            body.status === "CERTIFIED" ? new Date() : existing.certifiedAt,
          updatedAt: new Date(),
        },
        include: { driver: { select: { name: true } } },
      });
  ...
  fastify.get("/dvir", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const data = inspections.map((i: any) => ({
      ...
      criticalDefects: i.defects.filter((d: any) => d.severity === "CRITICAL")
        .length,
      items: i.items,
      photos: i.photos,
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!orgId) {
        return reply
          .code(422)
          .send({ error: "Organization context required for ELD operations" });
      }
      ...
      const hasFailedItems =
        body.items.some((i) => i.status === "FAIL") || body.defects.length > 0;
      ...
      return reply
        .code(201)
        .send({ id: inspection.id, status: inspectionStatus });
    },
  );
}
