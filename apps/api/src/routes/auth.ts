...
async function authRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        fastify.log.warn(
          { err: redisErr },
          "Redis unavailable — refresh token not persisted",
        );
      }
      ...
      if (!isFirstUser) {
        try {
          ...
          fastify.log.info(
            { userId: result.user.id, code },
            "[register] Verification code (logged for dev)",
          );
        } catch (err) {
          fastify.log.warn(
            { err },
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        fastify.log.warn(
          { err: redisErr },
          "Redis unavailable — refresh token not persisted, login proceeds",
        );
      }
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        var { phone, password, shopDomain } = driverLoginSchema.parse(
          request.body,
        );
      } catch (err) {
        if (err instanceof z.ZodError) {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const accessToken = fastify.jwt.sign(payload, {
        expiresIn:
          tokenData.role === "DRIVER" ? "7d" : getConfig().JWT_EXPIRES_IN,
      });
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!auth?.userId) {
        throw new UnauthorizedError(
          "Only user accounts can update their profile here",
        );
      }
