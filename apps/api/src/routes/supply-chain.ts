 ...
 *   GET /transfers         Inventory movements of type TRANSFER
 */
...
async function supplyChainRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const productIds = [
        ...new Set(items.map((i: any) => i.productId as string)),
      ];
      const products =
        productIds.length > 0
      ...
      const locationIds = [
        ...new Set(items.map((i: any) => i.locationId as string)),
      ];
      const locations =
        locationIds.length > 0
          ? await (request.tenantDb as any).location
              ?.findMany({
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const mapped = orders.map((o) => ({
        ...
        warehouse: "Main",
        priority: (o.tags as string[]).includes("expedited")
          ? "expedited"
          : "standard",
      }));
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const orders = await request.tenantDb.order.findMany({
        where: {
          shopId,
          status: {
            in: ["PICKED_UP", "IN_TRANSIT", "PENDING", "ASSIGNED"] as any,
          },
        },
        skip: (page - 1) * limit,
        take: limit,
      ...
      const enriched = await Promise.all(
        locations.map(async (loc: any) => {
          ...
          const totalQty: number = await (request.tenantDb as any).inventoryItem
            .aggregate({
              where: { shopId, locationId: loc.id },
              _sum: { quantity: true },
            })
          ...
          const coords = loc.coordinates as {
            lat?: number;
            lng?: number;
          } | null;
          return {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const [items, total] = await Promise.all([
        (request.tenantDb as any).inventoryItem.findMany({
          where: {
            shopId,
            quantity: {
              lte: (request.tenantDb as any).inventoryItem.fields?.reorderPoint,
            },
          },
          skip: (page - 1) * limit,
          take: limit,
      ...
      const filteredLow = lowItems.filter(
        (i: any) => i.quantity <= i.reorderPoint,
      );
      const paged = filteredLow.slice((page - 1) * limit, page * limit);

      const productIds = [
        ...new Set(paged.map((i: any) => i.productId as string)),
      ];
      const products =
        productIds.length > 0
          ? await request.tenantDb.product.findMany({
      ...
      return {
        ...
        pagination: {
          ...
          totalPages: Math.ceil(filteredLow.length / limit),
        },
      };
    },
  );
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const gauges = items.map((item: any) => {
        ...
        const percentageFilled =
          maximum > 0
            ? Math.min(100, Math.round((item.quantity / maximum) * 100))
            : 0;
        let status: "critical" | "warning" | "optimal" | "overstock" =
          "optimal";
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const data = waves.map((w: any) => {
        const orderIds = [
          ...new Set(w.items.map((i: any) => i.orderId as string)),
        ];
        const status =
          w.status === "DONE"
            ? "completed"
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const locations =
        locationIds.length > 0
          ...
          : [];
      const locationMap = new Map(
        (locations as any[]).map((l: any) => [l.id, l.name as string]),
      );

      // Enrich with product title for SKU display
      const productIds = [
        ...new Set(
          movements.map((m: any) => m.item?.productId).filter(Boolean),
        ),
      ] as string[];
      const products =
        productIds.length > 0
      ...
      const data = movements.map((m: any) => ({
        id: m.id,
        fromWarehouse: m.fromLocationId
          ? (locationMap.get(m.fromLocationId) ?? m.fromLocationId)
          : "Unknown",
        toWarehouse: m.toLocationId
          ? (locationMap.get(m.toLocationId) ?? m.toLocationId)
          : "Unknown",
        sku: m.item?.productId ?? "Unknown",
        productName: m.item?.productId
          ? (productMap.get(m.item.productId) ?? "Unknown")
          : "Unknown",
        qty: m.quantity,
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const batches = await (request.tenantDb as any).pickList.findMany({
        ...
        include: {
          items: { select: { id: true, binLocation: true, zone: true } },
        },
        orderBy: { createdAt: "desc" as const },
        take: 50,
      });
      ...
      const data = batches.map((b: any) => {
        ...
        const doneItems =
          b.status === "DONE"
            ? b.items.length
            : b.status === "IN_PROGRESS"
              ? Math.floor(b.items.length * 0.5)
              : 0;
        const completionRate =
          b.items.length > 0
            ? Math.round((doneItems / b.items.length) * 100)
            : 0;
