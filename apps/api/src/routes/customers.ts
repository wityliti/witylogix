...
const listCustomersQuery = paginationSchema.extend({
  ...
  sortBy: z
    .enum(["firstName", "email", "totalSpent", "lastSyncAt", "ordersCount"])
    .default("lastSyncAt"),
...
async function customerRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const [total, totalPrev, syncedToday, topSpenders, stats, lastSync] =
          await Promise.all([
            ...
            request.tenantDb.customer.count({
              where: { shopId, lastSyncAt: { gte: today } },
            }),
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        for (const c of allCustomers) {
          ...
          if (c.ordersCount > 0) statuses.active++;
  ...
  fastify.delete(
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      return {
        data: normalizeCustomer(deleted as unknown as Record<string, unknown>),
      };
    },
  );
}
