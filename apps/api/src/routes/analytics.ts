...
const driverPerformanceQuerySchema = dateRangeSchema.extend({
  sortBy: z
    .enum(["totalDeliveries", "onTimeRate", "avgDeliveryMinutes", "rating"])
    .default("totalDeliveries"),
  limit: z.number().int().positive().max(100).default(20),
});

const zonePerformanceQuerySchema = dateRangeSchema.extend({
  sortBy: z
    .enum(["deliveryCount", "onTimeRate", "avgDeliveryTime", "revenue"])
    .default("deliveryCount"),
  limit: z.number().int().positive().max(100).default(20),
});
...
function getGranularityInterval(granularity: string): {
  unit: string;
  days: number;
} {
  switch (granularity) {
...
async function analyticsRoutes(fastify: FastifyInstance): Promise<void> {
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const failedOrders = orders.filter(
          (o: any) => o.status === "FAILED" || o.status === "CANCELLED",
        );
        ...
        const onTimeDeliveries = deliveredOrders.filter(
          (o: any) =>
            o.actualDelivery &&
            o.deliveryDate &&
            o.actualDelivery <= o.deliveryDate,
        ).length;
        const onTimeRate =
          totalDeliveries > 0
        ...
        if (timedDeliveries.length > 0) {
          const totalMins = timedDeliveries.reduce((sum: number, o: any) => {
            const diffMs =
              (o.actualDelivery as Date).getTime() -
              (o.deliveryDate as Date).getTime();
        ...
        const metrics = {
          ...
          customerSatisfaction:
            totalDeliveries > 0 ? Math.round((onTimeRate / 20) * 10) / 10 : 0,
        ...
        const weeklyMap: Record<
          number,
          { orders: number; deliveries: number; revenue: number }
        > = {};
        for (let d = 0; d < 7; d++) {
        ...
        for (const o of orders) {
          ...
          if (o.status === "DELIVERED") {
            ...
            weeklyMap[d].revenue += o.totalPrice
              ? parseFloat(o.totalPrice.toString())
              : 0;
          }
        }
        ...
        const driverMap: Record<
          string,
          { name: string; total: number; onTime: number }
        > = {};
        ...
        const topDrivers = Object.values(driverMap)
          ...
          .map((d) => ({
            ...
            deliveries: d.total,
            onTime:
              d.total > 0 ? Math.round((d.onTime / d.total) * 1000) / 10 : 0,
            // On-time rate mapped to 0–5.0 delivery score
            rating:
              d.total > 0 ? Math.round((d.onTime / d.total) * 50) / 10 : 0,
          }));
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        for (const shipment of shipments) {
          const dateKey = formatDateForGrouping(
            shipment.createdAt as any,
            granularity,
          );
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const allDriverShipments = await (
          request as any
        ...
        for (const driver of drivers) {
          ...
          const totalDeliveries = shipments.filter(
          ...
          if (deliveredShipments.length > 0) {
            const onTime = deliveredShipments.filter(
              (s: any) =>
                s.actualDelivery &&
                s.deliveryDate &&
                s.actualDelivery <= s.deliveryDate,
            ).length;
            onTimeRate =
              Math.round((onTime / deliveredShipments.length) * 10000) / 100;
          }

          let avgDeliveryMinutes = 0;
          if (deliveredShipments.length > 0) {
            const totalMinutes = deliveredShipments.reduce(
              (sum: number, s: any) => {
                if (s.pickedUpAt && s.actualDelivery) {
                  const diffMs =
                    s.actualDelivery.getTime() - s.pickedUpAt.getTime();
            ...
            avgDeliveryMinutes =
              Math.round((totalMinutes / deliveredShipments.length) * 100) /
              100;
          }
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        for (const zone of zones) {
          ...
          let avgDeliveryTime = 0;
          if (deliveredShipments.length > 0) {
            const totalHours = deliveredShipments.reduce(
              (sum: number, s: any) => {
                if (s.deliveryDate && s.actualDelivery) {
                  const diffMs =
                    s.actualDelivery.getTime() - s.deliveryDate.getTime();
            ...
            avgDeliveryTime =
              Math.round((totalHours / deliveredShipments.length) * 100) / 100;
          }

          const revenue = shipments.reduce(
            (sum: number, s: any) => sum + (Number(s.shippingCost) || 0),
            0,
          );
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const laborCost = Math.round(totalCost * 0.5 * 100) / 100; // ~50% labor
        const carrierFees = Math.round(totalCost * 0.15 * 100) / 100; // ~15% carrier
        const otherCosts = Math.round(totalCost * 0.1 * 100) / 100; // ~10% other
        ...
        const costPerDelivery =
          deliveryCount > 0
            ? Math.round((totalShippingCost / deliveryCount) * 100) / 100
            : 0;
        ...
        for (const shipment of shipments) {
          ...
          dailyBreakdown[dateKey] =
            (dailyBreakdown[dateKey] || 0) + (shipment.shippingCost || 0);
        }
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        if (type === "deliveries") {
          ...
          // CSV Header
          csvContent =
            "Shipment ID,Tracking Number,Status,Delivery Date,Actual Delivery,Driver,Zone,Cost\n";
          ...
          for (const shipment of shipments) {
            const row = [
              ...
              shipment.deliveryDate
                ? shipment.deliveryDate.toISOString().split("T")[0]
                : "",
          ...
          csvContent =
            "Driver ID,Name,Total Deliveries,On-Time Rate %,Avg Delivery Minutes,Rating\n";
          ...
          const allExportDriverShipments = await (
            request as any
          ).tenantDb.shipment.findMany({
            where: {
          ...
          csvContent =
            "Zone ID,Zone Name,Delivery Count,On-Time Rate %,Avg Delivery Hours,Revenue\n";
          ...
          for (const zone of zones) {
            ...
            if (deliveredShipments.length > 0) {
              ...
              avgDeliveryHours =
                Math.round((totalHours / deliveredShipments.length) * 100) /
                100;
            }
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const [zones, currentOrders, priorOrders, timeSlots] =
          await Promise.all([
        ...
        for (const o of currentOrders) {
          const zid = o.timeSlotId
            ? (slotToZone.get(o.timeSlotId) ?? "unassigned")
            : "unassigned";
          currentByZone.set(zid, (currentByZone.get(zid) ?? 0) + 1);
          if (
            ["DELIVERED", "delivered", "completed", "COMPLETED"].includes(
              o.status,
            )
          ) {
        ...
        const share =
          zones.length > 0 ? Math.floor(unassignedCurrent / zones.length) : 0;
        const priorShare =
          zones.length > 0 ? Math.floor(unassignedPrior / zones.length) : 0;
        ...
        const zoneData = zones.map((zone: any) => {
          ...
          const predicted = Math.max(
            1,
            Math.round(
              actualVolume * Math.min(Math.max(growthFactor, 0.5), 2.0),
            ),
          );
          const confidence =
            actualVolume > 0
          ...
          const trend: "up" | "down" | "stable" =
            actualVolume > priorVolume * 1.1
              ? "up"
              : actualVolume < priorVolume * 0.9
                ? "down"
                : "stable";
          const anomalyCount =
            priorVolume > 0 &&
            Math.abs(actualVolume - priorVolume) / priorVolume > 0.5
              ? 1
              : 0;
          return {
            id: zone.id,
        ...
        const anomalies = zoneData
          ...
          .map((z: any) => ({
            id: `anomaly-${z.id}`,
            type: (z.actualVolume > z.predictedVolume ? "spike" : "drop") as
              | "spike"
              | "drop",
            zone: z.name,
            severity:
              Math.abs(z.actualVolume - z.predictedVolume) /
                Math.max(z.predictedVolume, 1) >
              1
                ? ("high" as const)
                : ("medium" as const),
            description:
              z.actualVolume > z.predictedVolume
        ...
        const totalPredicted = zoneData.reduce(
          (s: number, z: any) => s + z.predictedVolume,
          0,
        );
        const totalActual = zoneData.reduce(
          (s: number, z: any) => s + z.actualVolume,
          0,
        );
        ...
        if (error instanceof z.ZodError)
          throw new ValidationError("Invalid query", error.errors);
        throw error;
      }
    },
  );
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const slotToZone = new Map<string, string>(
          timeSlots
            .filter((ts: any) => ts.deliveryZoneId)
            .map((ts: any) => [ts.id, ts.deliveryZoneId]),
        );
        ...
        for (const zone of zones) {
          ...
          const firstOrder = currentOrders.find(
            (o) =>
              (o.timeSlotId ? slotToZone.get(o.timeSlotId) : null) === zone.id,
          );
          ...
          anomalies.push({
            ...
            value: current,
            previousValue: prior,
            timestamp: firstOrder
              ? firstOrder.createdAt.toISOString()
              : now.toISOString(),
            resolved: false,
            metadata: {
              confidence: Math.min(98, 60 + Math.round(absDeviation * 30)),
            },
          });
        }

        return reply.send({
          data: { items: anomalies, total: anomalies.length },
        });
      } catch (error) {
        throw error;
      }
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        for (const r of recentRoutes) {
          if (r.driverId) {
            driverRouteCount.set(
              r.driverId,
              (driverRouteCount.get(r.driverId) ?? 0) + 1,
            );
          }
        }
        ...
        const avgUtilization =
          drivers.length > 0 && totalRoutes > 0
            ? Math.min(
                100,
                Math.round((totalRoutes / (drivers.length * 5)) * 100),
              )
            : 0;
        const optimizationScore =
          completedRoutes > 0 && totalRoutes > 0
        ...
        const schedule = drivers.map((d: any, di: number) => {
          ...
          const isActive = ["AVAILABLE", "ON_ROUTE", "ON_DELIVERY"].includes(
            d.status,
          );
          return {
            driverId: d.id,
            ...
            timeSlots: slotHours.map((hour, hi) => {
              const status: "scheduled" | "available" | "off" = !isActive
                ? "off"
                : routeCount > hi
                  ? "scheduled"
                  : "available";
              return {
                hour,
                zone:
                  zones[(di + hi) % Math.max(zones.length, 1)]?.name ??
                  "Default",
                status,
              };
            }),
        ...
        const totalCapacity = Array.from(zoneOrderLoad.values()).reduce(
          (s, v) => s + v,
          0,
        );
        const avgCapacity =
          zones.length > 0 ? totalCapacity / zones.length : 50;
        ...
        const recommendations = zones.slice(0, 3).map((z: any, i: number) => {
          ...
          const additionalDrivers = needsMore
            ? Math.ceil((zoneCap - avgCapacity) / 20)
            : 0;
          ...
          return {
            title: needsMore
              ? `Increase coverage in ${z.name}`
              : `Optimize coverage in ${z.name}`,
            ...
            impact: needsMore
              ? `+${Math.min(20, additionalDrivers * 5)}% on-time rate`
              : "-8% cost",
        ...
        return reply.send({
          data: {
            ...
            metrics: {
              ...
              avgUtilization,
              recommendedAdjustments: recommendations.filter((r) =>
                r.impact.startsWith("+"),
              ).length,
              optimizationScore,
            },
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        for (const o of recentOrders) {
          const zid = o.timeSlotId
            ? (slotToZone.get(o.timeSlotId) ?? null)
            : null;
        ...
        const driversPerZone = Math.max(
          1,
          Math.floor(activeDriverCount / Math.max(zones.length, 1)),
        );
        ...
        const slots = zones.flatMap((z: any, zi: number) => {
          ...
          const slotCapTotal = zoneSlots.reduce(
            (s, ts) => s + ts.maxCapacity,
            0,
          );
          ...
          return hours.map((hour) => {
            ...
            const demandPredicted = Math.round(
              (zoneOrders > 0
                ? zoneOrders / hours.length
                : slotCapTotal / Math.max(zoneSlots.length, 1) / hours.length) *
                mult,
            );
        ...
        const zoneSummary = zones.map((z: any) => {
          ...
          const totalCurrent = zSlots.reduce(
            (s: number, x: any) => s + x.currentDrivers,
            0,
          );
          const totalRecommended = zSlots.reduce(
            (s: number, x: any) => s + x.recommendedDrivers,
            0,
          );
          const avgUtil =
            zSlots.length > 0
              ? Math.round(
                  zSlots.reduce(
                    (s: number, x: any) => s + x.utilizationRate,
                    0,
                  ) / zSlots.length,
                )
              : 0;
        ...
        const totalCurrent = zoneSummary.reduce(
          (s: number, z: any) => s + z.totalCurrent,
          0,
        );
        const totalRecommended = zoneSummary.reduce(
          (s: number, z: any) => s + z.totalRecommended,
          0,
        );
          ...
          data: {
            ...
            metrics: {
              ...
              potentialCostSavings: Math.round(
                Math.abs(totalCurrent - totalRecommended) * 150,
              ),
  ...
  fastify.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const settings = (shop?.settings as any) || {};
      const dashboards: any[] = (settings.analyticsDashboards ?? []).filter(
        (d: any) => d.id !== id,
      );
      await (request as any).tenantDb.shop.update({
        where: { id: (request as any).shopId },
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const newReport = {
        ...
        lastGenerated: null,
        nextGenerated:
          body.frequency === "oneTime" ? null : nextDate.toISOString(),
  ...
  fastify.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const settings = (shop?.settings as any) || {};
      const reports: any[] = (settings.analyticsReports ?? []).filter(
        (r: any) => r.id !== id,
      );
      await (request as any).tenantDb.shop.update({
        where: { id: (request as any).shopId },
