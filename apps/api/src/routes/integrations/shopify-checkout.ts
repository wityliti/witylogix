 ...
 */
export default async function shopifyCheckoutRoutes(app: FastifyInstance): Promise<void> {
  /**
   * GET /slots
   * Get available delivery slots for a specific date
   */
  app.get<{ Querystring: any }>('/slots', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
   ...
   */
  app.post<{ Body: any }>('/reserve', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { slotId, cartId, shopDomain, customerEmail, deliveryAddress } = ReserveSlotSchema.parse(request.body);
   ...
   */
  app.get<{ Querystring: any }>('/rates', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
   ...
   */
  app.get<{ Querystring: any }>('/availability', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
   ...
   */
  app.post<{ Body: any }>('/webhook/order', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
