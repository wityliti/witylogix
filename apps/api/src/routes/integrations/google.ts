 ...
 */
export default async function googleIntegrationRoutes(app: FastifyInstance): Promise<void> {
  const googleMapsApiKey = process.env.GOOGLE_MAPS_API_KEY;

  if (!googleMapsApiKey) {
    app.log.warn('GOOGLE_MAPS_API_KEY not configured. Google Maps features disabled.');
  }
  ...
  app.get<{ Querystring: any }>('/geocode', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      if (!googleMapsApiKey) {
        return reply.status(503).send({ error: 'Google Maps not configured' });
      }
   ...
   */
  app.get<{ Querystring: any }>('/distance', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      if (!googleMapsApiKey) {
        return reply.status(503).send({ error: 'Google Maps not configured' });
      }

      const { origin, destination } = DistanceQuerySchema.parse(request.query);
      const mapsService = createGoogleMapsService(googleMapsApiKey);
   ...
   */
  app.get<{ Querystring: any }>('/directions', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      if (!googleMapsApiKey) {
        return reply.status(503).send({ error: 'Google Maps not configured' });
      }

      const { origin, destination, waypoints } = DirectionsQuerySchema.parse(request.query);
      const mapsService = createGoogleMapsService(googleMapsApiKey);

      const waypointArray = waypoints ? waypoints.split('|') : undefined;
      const result = await mapsService.getDirections(origin, destination, waypointArray);
   ...
   */
  app.post<{ Body: any }>('/calendar/auth', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
   ...
   */
  app.get<{ Querystring: any }>('/calendar/callback', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.redirect(`${process.env.DASHBOARD_URL || 'http://localhost:3001'}/integrations/google-calendar/success`);
    } catch (error) {
      app.log.error(error);
      return reply.status(400).send({
   ...
   */
  app.post<{ Body: any }>('/calendar/sync', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { locationId, startDate, endDate } = CalendarSyncSchema.parse(request.body);
      ...
      if (!integration) {
        return reply.status(404).send({ error: 'Google Calendar not configured' });
      }
   ...
   */
  app.post<{ Body: any }>('/zone/validate', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      if (!googleMapsApiKey) {
        return reply.status(503).send({ error: 'Google Maps not configured' });
      }
