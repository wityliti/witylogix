...
async function dispatchRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const mapped = orders.map((o) => {
        const address = [o.addressLine1, o.addressLine2, o.city]
          .filter(Boolean)
          .join(", ");
        const ageMs = Date.now() - new Date(o.createdAt).getTime();
        const ageMinutes = Math.floor(ageMs / 60000);
        const priority =
          ageMinutes > 60 ? "high" : ageMinutes > 30 ? "medium" : "low";
        const windowBase = o.deliveryDate
          ? new Date(o.deliveryDate)
          : new Date(Date.now() + 2 * 3600000);
        const windowStart = windowBase.toLocaleTimeString("en-US", {
          hour: "2-digit",
          minute: "2-digit",
          hour12: false,
        });
        const windowEnd = new Date(
          windowBase.getTime() + 3 * 3600000,
        ).toLocaleTimeString("en-US", {
          hour: "2-digit",
          minute: "2-digit",
          hour12: false,
        });
        const delivLoc = o.deliveryLocation as {
          latitude?: number;
          longitude?: number;
        } | null;
        return {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const mapped = drivers.map((d) => {
        const loc = d.currentLocation as {
          latitude?: number;
          longitude?: number;
          address?: string;
        } | null;
        return {
          ...
          location:
            loc?.address ?? (loc?.latitude != null ? "En Route" : "Hub"),
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      const orders = await request.tenantDb.order.findMany({
        where: {
          status: {
            in: ["ASSIGNED", "PICKED_UP", "OUT_FOR_DELIVERY"] as const,
          },
        },
      ...
      const mapped = orders.map((o) => {
        const loc = o.deliveryLocation as {
          latitude?: number;
          longitude?: number;
        } | null;
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const [unassigned, inTransit, completedToday, allDeliveredToday] =
        await Promise.all([
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const orderDeliveryLoc = order.deliveryLocation as {
        latitude?: number;
        longitude?: number;
      } | null;
      const deliveryCoord =
        orderDeliveryLoc?.latitude != null &&
        orderDeliveryLoc?.longitude != null
          ? {
              latitude: orderDeliveryLoc.latitude,
              longitude: orderDeliveryLoc.longitude,
            }
          : DEFAULT_LOCATION;
