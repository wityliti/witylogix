      ...
      const [
        ...
        request.tenantDb.order.count({
          where: {
            ...
            status: {
              in: ["ASSIGNED", "PICKED_UP", "OUT_FOR_DELIVERY", "ARRIVED"],
            },
          },
        }),
        request.tenantDb.order.count({
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const formatted = orders.map((o) => {
        ...
        const startTime = base.toLocaleTimeString("en-US", {
          hour: "2-digit",
          minute: "2-digit",
          hour12: false,
        });
        ...
        return {
          jobId: o.id,
          jobNumber: o.externalOrderNumber
            ? `#${o.externalOrderNumber}`
            : `#${o.id.slice(0, 6).toUpperCase()}`,
          customerName: o.customerName ?? "Unknown",
          location:
            [o.addressLine1, o.city].filter(Boolean).join(", ") ||
            "Address unknown",
          startTime,
          endTime,
