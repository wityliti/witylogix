...
async function getScenarios(db: FastifyRequest['tenantDb'], shopId: string): Promise<StoredScenario[]> {
...
async function chaosRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/scenarios', async (request: FastifyRequest) => {
    try {
      ...
      await saveScenarios(request.tenantDb, request.shopId, [...scenarios, scenario]);
  ...
  fastify.delete('/scenarios/:id', async (request: FastifyRequest) => {
    ...
    if (updated.length === scenarios.length) throw new NotFoundError('ChaosScenario', id);
    ...
    await request.tenantDb.activityLog.create({
      data: {
        ...
        metadata: {
          ...
          metrics: { requestsImpacted: 0, errorRate: 0, latencyMs: 0, circuitBreakerTrips: 0 },
          assertions,
        },
      },
    });
  ...
  fastify.get('/executions/:id', async (request: FastifyRequest) => {
    ...
    const log = await request.tenantDb.activityLog.findFirst({
      where: { shopId: request.shopId, entityType: 'CHAOS_EXECUTION', entityId: id },
    ...
    return {
      data: {
        ...
        status,
        progress,
        metrics: meta.metrics ?? { requestsImpacted: 0, errorRate: 0, latencyMs: 0, circuitBreakerTrips: 0 },
