...
function generateOTP(): { plain: string; hashed: string } {
  ...
  const hashed = createHmac('sha256', process.env.OTP_SECRET ?? 'witylogix-otp-secret')
    .update(plain)
    .digest('hex');
  return { plain, hashed };
}
...
async function deliveryOTPRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/otp/send', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const order = await db.order.findFirst({
      ...
      select: { id: true, requireOTPConfirmation: true, customerPhone: true, customerEmail: true },
    });

    if (!order) {
      reply.status(404);
      return { data: null, error: { code: 'NOT_FOUND', message: 'Order not found' } };
    }
    ...
    if (!order.requireOTPConfirmation) {
      ...
      return {
        data: null,
        error: { code: 'OTP_NOT_REQUIRED', message: 'This order does not require OTP confirmation' },
      };
    }
    ...
    await db.deliveryAuditLog.create({
      data: {
        ...
        action: 'otp_sent',
        actorId: userId,
        metadata: { otpId: otp.id, channel: order.customerPhone ? 'sms' : 'email' },
      },
    });
  ...
  fastify.post('/otp/verify', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!otp) {
      reply.status(404);
      return { data: null, error: { code: 'OTP_NOT_FOUND', message: 'No OTP found for this order. Use POST /otp/send first.' } };
    }

    if (otp.verified) {
      return reply.send({ data: { verified: true, message: 'OTP already verified' } });
    }

    if (new Date() > otp.expiresAt) {
      reply.status(410);
      return { data: null, error: { code: 'OTP_EXPIRED', message: 'OTP has expired. Use POST /otp/send to generate a new one.' } };
    }

    if (otp.attempts >= MAX_ATTEMPTS) {
      reply.status(429);
      return { data: null, error: { code: 'OTP_LOCKED', message: 'Too many failed attempts. Use POST /otp/send to generate a new OTP.' } };
    }
