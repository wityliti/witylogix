...
const listOrdersQuery = paginationSchema.extend({
  ...
  sortBy: z
    .enum(["createdAt", "deliveryDate", "status", "customerName"])
    .default("createdAt"),
...
function transformOrder(order: any) {
  return {
    ...
    totalWeight: order.totalWeight
      ? parseFloat(order.totalWeight.toString())
      : null,
    currency: "USD",
      ...
      const [orders, total] = await Promise.all([
        request.tenantDb.order.findMany({
          ...
          include: {
            ...
            proofOfDelivery: {
              select: { id: true, photoUrls: true, deliveredAt: true },
            },
          },
        }),
  ...
  fastify.patch(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const existing = await request.tenantDb.order.findUnique({
          where: { id },
        });
        if (!existing) {
          throw new NotFoundError("Order", id);
  ...
  fastify.patch(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const { id } = request.params as { id: string };
        const { status: newStatus, notes } = updateOrderStatusSchema.parse(
          request.body,
        );
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        if (order.status !== "DELIVERED") {
          throw new ValidationError(
            "Rating can only be submitted for delivered orders",
          );
        }
        ...
        try {
          ...
          throw new ValidationError(
            err instanceof Error ? err.message : "Invalid rating data",
          );
        }
