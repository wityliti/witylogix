...
describe("resolveGeofenceRadius", () => {
  ...
  it("returns default when geofenceRadiusMeters is not a number", () => {
    expect(resolveGeofenceRadius({ geofenceRadiusMeters: "big" })).toBe(
      GEOFENCE_DEFAULT_RADIUS_M,
    );
  });
  ...
  it("clamps to min when value is below minimum", () => {
    expect(resolveGeofenceRadius({ geofenceRadiusMeters: 10 })).toBe(
      GEOFENCE_MIN_RADIUS_M,
    );
  });

  it("clamps to max when value is above maximum", () => {
    expect(resolveGeofenceRadius({ geofenceRadiusMeters: 9999 })).toBe(
      GEOFENCE_MAX_RADIUS_M,
    );
  });
  ...
  it("accepts exact min boundary", () => {
    expect(
      resolveGeofenceRadius({ geofenceRadiusMeters: GEOFENCE_MIN_RADIUS_M }),
    ).toBe(GEOFENCE_MIN_RADIUS_M);
  });

  it("accepts exact max boundary", () => {
    expect(
      resolveGeofenceRadius({ geofenceRadiusMeters: GEOFENCE_MAX_RADIUS_M }),
    ).toBe(GEOFENCE_MAX_RADIUS_M);
  });
});

describe("geofenceDedupKey", () => {
  it("returns expected key format", () => {
    expect(geofenceDedupKey("driver-1", "ship-2")).toBe(
      "geofence:dedup:driver-1:ship-2",
    );
  });
...
vi.mock("@witylogix/db", () => ({
  prisma: {
    get shipment() {
      return mockPrismaShipment;
    },
    get deliveryEvent() {
      return mockPrismaDeliveryEvent;
    },
...
describe("runGeofenceCheck", () => {
  ...
  it("returns zero counts when no active shipments", async () => {
    ...
    const { runGeofenceCheck } =
      await import("../../workers/geofence-worker.js");
  ...
  it("respects custom shop geofence radius (50m — driver 111m away is outside)", async () => {
    const shipment = makeShipment({
      shop: { settings: { geofenceRadiusMeters: 50 } },
    });
