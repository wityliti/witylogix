...
describe('Webhooks Routes', () => {
  ...
  describe('HMAC Verification', () => {
    ...
    it('should reject modified payload with valid HMAC', () => {
      const originalPayload = JSON.stringify({ id: 123456, order_number: 1001 });
      const hmac = generateShopifyHmac(originalPayload, SHOPIFY_SECRET);

      const modifiedPayload = JSON.stringify({ id: 654321, order_number: 1001 });
      const isValid = generateShopifyHmac(modifiedPayload, SHOPIFY_SECRET) === hmac;
  ...
  describe('Error Handling', () => {
    ...
    it('should continue on notification queue failure', async () => {
      const payload = JSON.stringify({
        ...
        shipping_address: { address1: '123 St', city: 'NYC', province: 'NY', zip: '10001', country_code: 'US' },
      });
      const hmac = generateShopifyHmac(payload, SHOPIFY_SECRET);
