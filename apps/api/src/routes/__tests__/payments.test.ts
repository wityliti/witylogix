...
describe('Payment Routes', () => {
  ...
  describe('GET / - List Payments', () => {
    ...
    it('should filter payments by amount range', async () => {
      ...
      mockRequest.query = { page: 1, limit: 25, minAmount: 100, maxAmount: 200 };
    ...
    it('should order results by createdAt descending', async () => {
      ...
      mockTenantDb.paymentTransaction.findMany.mockResolvedValue([payment2, payment1]);
    ...
    it('should handle empty payment list', async () => {
      ...
      const result = { success: true, data: [], pagination: { total: 0, page: 1, limit: 25, totalPages: 0 } };
  ...
  describe('POST / - Create Payment', () => {
    ...
    it('should set initial payment status to PENDING', async () => {
      mockTenantDb.shipment.findFirst.mockResolvedValue(mockShipment);
      mockTenantDb.paymentTransaction.create.mockResolvedValue({ ...mockPayment, status: 'PENDING' });
    ...
    it('should reject invalid payment method', async () => {
      ...
      const validMethods = ['CREDIT_CARD', 'DEBIT_CARD', 'BANK_TRANSFER', 'CASH'];
    ...
    it('should reject missing required fields', async () => {
      ...
      const hasRequired = mockRequest.body.paymentType && mockRequest.body.amount;
  ...
  describe('PATCH /:id/status - Update Payment Status', () => {
    it('should update payment status from PENDING to PROCESSING', async () => {
      ...
      mockTenantDb.paymentTransaction.findFirst.mockResolvedValueOnce(pendingPayment);
      mockTenantDb.paymentTransaction.update.mockResolvedValue(processingPayment);
    ...
    it('should update payment status from PROCESSING to COMPLETED', async () => {
      ...
      mockTenantDb.paymentTransaction.findFirst.mockResolvedValueOnce(processingPayment);
      mockTenantDb.paymentTransaction.update.mockResolvedValue(completedPayment);
    ...
    it('should update payment status from COMPLETED to REFUNDED', async () => {
      ...
      mockTenantDb.paymentTransaction.findFirst.mockResolvedValueOnce(completedPayment);
    ...
    it('should return 409 for invalid state transition', async () => {
      ...
      mockTenantDb.paymentTransaction.findFirst.mockResolvedValue(cancelledPayment);
      ...
      const canTransition = validTransitions[cancelledPayment.status]?.includes(mockRequest.body.status);
    ...
    it('should set completedAt timestamp on COMPLETED status', async () => {
      mockTenantDb.paymentTransaction.findFirst.mockResolvedValue({ ...mockPayment, status: 'PROCESSING' });
      mockTenantDb.paymentTransaction.update.mockResolvedValue({
        ...mockPayment,
        status: 'COMPLETED',
      ...
      const result = { ...mockPayment, status: 'COMPLETED', completedAt: new Date() };
    ...
    it('should set completedAt timestamp on FAILED status', async () => {
      ...
      const result = { ...mockPayment, status: 'FAILED', completedAt: new Date() };
      expect(result.completedAt).toBeDefined();
    });
  });
  ...
  describe('POST /:id/refund - Initiate Refund', () => {
    it('should create refund transaction for completed payment', async () => {
      mockTenantDb.paymentTransaction.findFirst.mockResolvedValueOnce(mockPayment);
    ...
    it('should return 409 when refunding non-completed payment', async () => {
      const pendingPayment = { ...mockPayment, status: 'PENDING' };
      mockTenantDb.paymentTransaction.findFirst.mockResolvedValue(pendingPayment);
    ...
    it('should include shop details in refund response', async () => {
      ...
      const result = { id: 'refund-123', shop: { id: 'shop-456', name: 'Test Shop' } };
      expect(result.shop.name).toBe('Test Shop');
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle database connection errors', async () => {
      mockTenantDb.paymentTransaction.findMany.mockRejectedValue(new Error('Database error'));

      await expect(mockTenantDb.paymentTransaction.findMany()).rejects.toThrow('Database error');
    });
  ...
  describe('Tenant Isolation', () => {
    it('should filter payments by shopId', async () => {
      ...
      const allBelongToShop = result.every((p) => p.shopId === mockRequest.shopId);
  ...
  describe('Transaction Integrity', () => {
    ...
    it('should prevent double refunds of same payment', async () => {
      const refundedPayment = { ...mockPayment, status: 'REFUNDED' };
      mockTenantDb.paymentTransaction.findFirst.mockResolvedValue(refundedPayment);
