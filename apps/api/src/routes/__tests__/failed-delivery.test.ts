...
function simulateBatchHandler(opts: {
  ...
  const runningAttemptCount = new Map(
  ...
  for (const event of sorted) {
    ...
    if (event.eventType === "failed_delivery") {
      const payloadParsed = failedDeliveryPayloadSchema.safeParse(
        event.payload,
      );
      if (!payloadParsed.success) {
        results.push({
          id: event.id,
          status: "error",
          message: "Invalid failed_delivery payload: failureReason is required",
        });
        continue;
      }
      ...
      if (
        !allowedFromCurrent ||
        (!allowedFromCurrent.has("FAILED") &&
          !allowedFromCurrent.has("FAILED_ATTEMPT"))
      ) {
        results.push({
          id: event.id,
          status: "conflict",
      ...
      const currentAttempts = runningAttemptCount.get(
        event.deliveryId as string,
      )!;
      const newAttemptCount = currentAttempts + 1;
      const targetStatus =
        newAttemptCount >= maxDeliveryAttempts ? "FAILED" : "FAILED_ATTEMPT";
      const { failureReason } = payloadParsed.data;

      createdAttempts.push({
        shipmentId: event.deliveryId as string,
  ...
  return {
    results,
    createdAttempts,
    finalState: Object.fromEntries(runningState),
    finalAttemptCount: Object.fromEntries(runningAttemptCount),
  };
}
...
describe("failedDeliveryPayloadSchema", () => {
  it("accepts all valid failure reasons", () => {
    ...
    for (const reason of reasons) {
      expect(
        failedDeliveryPayloadSchema.safeParse({ failureReason: reason })
          .success,
      ).toBe(true);
    }
  });
  ...
  it("rejects unknown failureReason", () => {
    expect(
      failedDeliveryPayloadSchema.safeParse({ failureReason: "wrong" }).success,
    ).toBe(false);
  });

  it("accepts optional note up to 500 chars", () => {
    expect(
      failedDeliveryPayloadSchema.safeParse({
        failureReason: "other",
        note: "x".repeat(500),
      }).success,
    ).toBe(true);
  ...
  it("processes events in chronological order regardless of submission order", () => {
    const events = [
      makeEvent({
        id: "evt-later",
        deliveryId: "ship-uuid-1",
        deviceCapturedAt: "2026-03-30T12:00:00Z",
      }),
      makeEvent({
        id: "evt-earlier",
        deliveryId: "ship-uuid-1",
        deviceCapturedAt: "2026-03-30T10:00:00Z",
      }),
    ];
    ...
    const { results } = simulateBatchHandler({
      events,
      shipments: [
        makeShipment({ status: "OUT_FOR_DELIVERY", attemptCount: 0 }),
      ],
      maxDeliveryAttempts: 3,
    });
...
describe("simulateBatchHandler — multiple deliveries in one batch", () => {
  it("handles events for different shipments independently", () => {
    const events = [
      ...
      {
        ...makeEvent({ id: "evt-2", deliveryId: "ship-uuid-2" }),
        payload: { failureReason: "address_not_found" },
      },
    ];
