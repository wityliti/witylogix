...
const makeOrder = (overrides: Partial<{ status: string; shopId: string; id: string }> = {}) => ({
  id: 'order-uuid-1234',
...
describe('PATCH /token/:token/preferences — upsert behaviour', () => {
  ...
  it('creates preferences when none exist', async () => {
    ...
    const result = await mockPrisma.customerDeliveryPreferences.upsert({
      where: { orderId: 'order-uuid-1234' },
      create: { orderId: 'order-uuid-1234', shopId: 'shop-uuid-5678', instructions: 'Ring bell' },
  ...
  it('updates existing preferences', async () => {
    const updated = makePrefs({ instructions: 'Call on arrival', contactPref: 'whatsapp' });
    mockPrisma.customerDeliveryPreferences.upsert.mockResolvedValue(updated);

    const result = await mockPrisma.customerDeliveryPreferences.upsert({
      where: { orderId: 'order-uuid-1234' },
      create: { orderId: 'order-uuid-1234', shopId: 'shop-uuid-5678', instructions: 'Call on arrival' },
  ...
  it('persists dropoffNote correctly', async () => {
    ...
    const result = await mockPrisma.customerDeliveryPreferences.upsert({
      where: { orderId: 'order-uuid-1234' },
      create: { orderId: 'order-uuid-1234', shopId: 'shop-uuid-5678', dropoffNote: 'Leave in blue box by gate' },
...
describe('GET /token/:token/slots', () => {
  ...
  it('returns available slots for an unlocked delivery', async () => {
    const slots = [makeSlot(), makeSlot({ id: 'slot-uuid-2', name: 'Afternoon' })];
    mockPrisma.order.findUnique.mockResolvedValue(makeOrder({ status: 'ASSIGNED' }));
  ...
  it('returns locked:true and empty slots when driver is en route', async () => {
    mockPrisma.order.findUnique.mockResolvedValue(makeOrder({ status: 'OUT_FOR_DELIVERY' }));
    ...
    const rescheduleLockedStatuses = ['OUT_FOR_DELIVERY', 'ARRIVED', 'DELIVERED'];
