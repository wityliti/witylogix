...
const createMockShift = (
  overrides?: Partial<MockDriverShift>,
): MockDriverShift => ({
...
describe("Workflow Drivers Routes", () => {
  ...
  describe("POST /api/workflow/drivers/assign - Assign Driver", () => {
    it("should successfully assign driver to order", async () => {
      ...
      const result = await mockWorkflowEngine.executeWorkflow(
    ...
    it("should consider weight and value constraints", async () => {
      ...
      expect(mockRequest.body.value).toBe(1500.0);
    });
    ...
    it("should handle special requirements", async () => {
      ...
      expect(mockRequest.body.specialRequirements).toContain(
        "TEMPERATURE_CONTROLLED",
      );
    });
  ...
  describe("Auto-Assignment Rules", () => {
    ...
    it("should apply capacity-based scoring", async () => {
      ...
      const availableCapacity1 =
        drivers[0].maxCapacity - drivers[0].currentLoad;
      const availableCapacity2 =
        drivers[1].maxCapacity - drivers[1].currentLoad;
  ...
  describe("Driver Performance Tracking", () => {
    ...
    it("should track average delivery time per order", async () => {
      ...
      const avgTime =
        deliveries.reduce((a, b) => a + b.time, 0) / deliveries.length;
    ...
    it("should track cancellation rate", async () => {
      ...
      const cancelRate =
        (stats.cancelledDeliveries / stats.totalDeliveries) * 100;
  ...
  describe("Break and Rest Compliance", () => {
    it("should enforce minimum rest duration between shifts", async () => {
      ...
      const restHours =
        (nextShiftStart.getTime() - shiftEnd.getTime()) / (1000 * 60 * 60);
    ...
    it("should prevent assignment during break time", async () => {
      ...
      const isOnBreak =
        checkTime >= shift.breakStartTime! && checkTime <= shift.breakEndTime!;
