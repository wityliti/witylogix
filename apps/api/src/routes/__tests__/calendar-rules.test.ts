...
describe('Calendar Rules Routes', () => {
  ...
  describe('GET / - List Calendar Rules', () => {
    it('should list all calendar rules with pagination', async () => {
      mockTenantDb.calendarRule.findMany.mockResolvedValueOnce([mockCalendarRule]);
    ...
    it('should filter rules by shipping profile', async () => {
      ...
      mockRequest.query = { page: 1, limit: 20, shippingProfileId: 'profile-123' };
    ...
    it('should order rules by priority then creation date', async () => {
      ...
      const rule3 = { ...mockCalendarRule, id: 'rule-3', priority: 1, createdAt: new Date('2024-01-02') };

      mockTenantDb.calendarRule.findMany.mockResolvedValueOnce([rule1, rule3, rule2]);
      ...
      expect(result.data[0].priority).toBeLessThanOrEqual(result.data[1].priority);
    });
    ...
    it('should handle pagination with multiple pages', async () => {
      ...
      mockTenantDb.calendarRule.findMany.mockResolvedValueOnce(rules.slice(0, 20));
  ...
  describe('GET /:id - Get Single Rule', () => {
    it('should get a calendar rule by id', async () => {
      mockTenantDb.calendarRule.findUnique.mockResolvedValueOnce(mockCalendarRule);
  ...
  describe('Rule Conflict Detection', () => {
    ...
    it('should handle rules with same priority for conflict resolution', async () => {
      const rule1 = { ...mockCalendarRule, id: 'rule-1', priority: 1, createdAt: new Date('2024-01-01') };
      const rule2 = { ...mockCalendarRule, id: 'rule-2', priority: 1, createdAt: new Date('2024-01-02') };
  ...
  describe('Recurring Rules - Monthly', () => {
    it('should handle monthly recurring rules', async () => {
      ...
      const result = {
        data: { ...mockCalendarRule, type: 'RECURRING_MONTHLY', daysOfWeek: [15] },
      };
    ...
    it('should support last day of month', async () => {
      ...
      const result = {
        data: { ...mockCalendarRule, type: 'RECURRING_MONTHLY', daysOfWeek: [31] },
      };
