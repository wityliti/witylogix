...
const makeOTPRecord = (
  overrides: Partial<MockOTPRecord> = {},
): MockOTPRecord => ({
  id: "otp-rec-001",
  ...
  describe("generateAndSend", () => {
    it("generates a 6-digit numeric OTP", async () => {
      const { generateDeliveryOTP } =
        await import("../../services/delivery-otp.js").catch(() => ({
...
describe("POST /api/deliveries/:deliveryId/otp/verify", () => {
  it("returns 200 and marks OTP verified for correct code", async () => {
    const db = makeTenantDb({ requireOTPConfirmation: true });
    db.deliveryOTP.findFirst = vi
      .fn()
      .mockResolvedValue(makeOTPRecord({ code: "483920" }));
    db.deliveryOTP.update = vi
      .fn()
      .mockResolvedValue(
  ...
  it("returns 422 for wrong OTP code", async () => {
    ...
    db.deliveryOTP.findFirst = vi
      .fn()
      .mockResolvedValue(makeOTPRecord({ code: "483920", attempts: 0 }));
  ...
  it("returns 429 after 5 failed attempts", async () => {
    ...
    db.deliveryOTP.findFirst = vi
      .fn()
      .mockResolvedValue(makeOTPRecord({ code: "483920", attempts: 5 }));
...
describe("Delivery completion — OTP gate", () => {
  it("blocks completion when requireOTPConfirmation=true and OTP not verified", async () => {
    ...
    db.deliveryOTP.findFirst = vi
      .fn()
      .mockResolvedValue(makeOTPRecord({ verified: false }));
...
async function mockSendOTPRoute(params: {
  ...
  const order = await params.tenantDb.order.findUnique({
  ...
  if (!order.customerPhone)
    return {
      status: 400,
      body: { error: "No phone number on file for this order" },
    };
  ...
  const masked = order.customerPhone.replace(
    /(\+\d{2})\d+(\d{2})$/,
    "$1****$2",
  );
  return {
    ...
    body: {
      success: true,
      data: { channel: params.channel, maskedPhone: masked },
    },
  };
}
...
async function mockVerifyOTPRoute(params: {
  ...
  if (!record)
    return { status: 404, body: { error: "No OTP found for this order" } };
  ...
  await params.tenantDb.deliveryOTP.update({
...
async function mockCompleteDeliveryRoute(params: {
  ...
  if (order.requireOTPConfirmation) {
    ...
    if (!otp || !otp.verified) {
      return {
        status: 403,
        body: { error: "OTP confirmation required", code: "OTP_REQUIRED" },
      };
    }
  }
  ...
  return {
    ...
    body: {
      success: true,
      data: { orderId: params.orderId, status: "DELIVERED" },
    },
  };
}
...
async function mockSupervisorOverrideRoute(params: {
  ...
  if (!["SUPERVISOR", "ADMIN", "MANAGER"].includes(params.supervisorRole)) {
    return {
      status: 403,
      body: { error: "Insufficient role to perform supervisor override" },
    };
  }
