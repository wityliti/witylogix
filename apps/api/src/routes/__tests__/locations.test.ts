...
describe('Locations Routes', () => {
  ...
  const mockZones = [
    { zoneId: 'zone-1', priority: 0, isDefault: true, name: 'Zone A', type: 'DELIVERY' },
    { zoneId: 'zone-2', priority: 1, isDefault: false, name: 'Zone B', type: 'PICKUP' },
  ];
  ...
  describe('GET / - List Locations', () => {
    ...
    it('should filter locations by type', async () => {
      ...
      const whereClause = 'WHERE shop_id = $1::uuid AND is_active = true AND type = $2::text';
    ...
    it('should filter locations by distance', async () => {
      ...
      mockRequest.query = { page: 1, limit: 20, lat: 40.7128, lng: -74.006, maxDistance: 50 };
    ...
    it('should apply pagination correctly', async () => {
      ...
      const pageNum = Math.max(1, parseInt(mockRequest.query.page as string) || 1);
      const pageSize = Math.min(100, Math.max(1, parseInt(mockRequest.query.limit as string) || 20));
    ...
    it('should handle empty location list', async () => {
      ...
      const result = { data: [], pagination: { page: 1, limit: 20, total: 0, pages: 0 } };
  ...
  describe('POST / - Create Location', () => {
    ...
    it('should return 400 for invalid coordinates', async () => {
      ...
      const isValidLat = mockRequest.body.latitude >= -90 && mockRequest.body.latitude <= 90;
    ...
    it('should reject missing required fields', async () => {
      ...
      const hasRequired = mockRequest.body.name && mockRequest.body.addressLine1;
      expect(hasRequired).toBeFalsy();
    });

    it('should set default values', async () => {
      mockTenantDb.$queryRawUnsafe.mockResolvedValue([
        { ...mockLocation, type: 'WAREHOUSE', serviceLevel: 'standard', country: 'US' },
      ]);
      ...
      const result = { ...mockLocation, type: 'WAREHOUSE', serviceLevel: 'standard', country: 'US' };
      expect(result.type).toBe('WAREHOUSE');
    });

    it('should store metadata with location', async () => {
      mockTenantDb.$queryRawUnsafe.mockResolvedValue([{ ...mockLocation, metadata: { zone: 'downtown' } }]);
  ...
  describe('PATCH /:id - Update Location', () => {
    ...
    it('should update location coordinates', async () => {
      const updatedLocation = { ...mockLocation, latitude: 41.0, longitude: -73.0 };
      mockTenantDb.$queryRawUnsafe.mockResolvedValue([updatedLocation]);
  ...
  describe('DELETE /:id - Delete Location', () => {
    ...
    it('should return deleted location in response', async () => {
      ...
      const result = { message: 'Location deactivated', location: deletedLocation };
  ...
  describe('GET /:id/hours - Get Working Hours', () => {
    ...
    it('should order working hours by day of week', async () => {
      ...
      const result = mockWorkingHours;
      expect(result[0].dayOfWeek).toBeLessThan(result[result.length - 1].dayOfWeek);
    });
  ...
  describe('PATCH /:id/capacity - Update Capacity', () => {
    ...
    it('should validate slots do not exceed total', async () => {
      ...
      const exceed = mockRequest.body.usedSlots + mockRequest.body.reservedSlots > mockRequest.body.totalSlots;
    ...
    it('should update capacity category', async () => {
      ...
      mockRequest.params = { id: 'loc-123' };
      mockRequest.body = { totalSlots: 100, usedSlots: 60, reservedSlots: 20, category: 'premium' };
  ...
  describe('GET /nearest - Find Nearest Location', () => {
    it('should find nearest locations by coordinates', async () => {
      ...
      mockRequest.query = { latitude: 40.7128, longitude: -74.006, maxDistance: 50, limit: 5 };
    ...
    it('should limit results count', async () => {
      ...
      mockRequest.query = { latitude: 40.7128, longitude: -74.006, maxDistance: 50, limit: 3 };
    ...
    it('should validate latitude range', async () => {
      mockRequest.query = { latitude: 200, longitude: -74.006, maxDistance: 50, limit: 5 };
    ...
    it('should validate longitude range', async () => {
      mockRequest.query = { latitude: 40.7128, longitude: 500, maxDistance: 50, limit: 5 };
    ...
    it('should handle no nearby locations', async () => {
      ...
      mockRequest.query = { latitude: 40.7128, longitude: -74.006, maxDistance: 5, limit: 5 };
    ...
    it('should apply default limit', async () => {
      ...
      mockRequest.query = { latitude: 40.7128, longitude: -74.006, maxDistance: 50 };
  ...
  describe('Tenant Isolation', () => {
    it('should filter all locations by shopId', async () => {
      ...
      mockTenantDb.$queryRawUnsafe.mockResolvedValueOnce([{ count: 1 }]);
      mockTenantDb.$queryRawUnsafe.mockResolvedValueOnce([{ ...mockLocation, shopId: 'shop-456' }]);
