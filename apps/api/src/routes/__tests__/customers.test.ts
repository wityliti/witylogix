...
describe("Customer Routes", () => {
  ...
  describe("POST /sync - Sync Customers", () => {
    ...
    it("should update lastSyncAt timestamp on sync", async () => {
      ...
      expect(result[0].lastSyncAt.getTime()).toBeGreaterThanOrEqual(
        beforeSync.getTime(),
      );
    });
    ...
    it("should return sync message with count", async () => {
      ...
      mockRequest.body = {
        customers: [{ externalId: "ext-123", email: "john@example.com" }],
      };
  ...
  describe("GET /stats - Customer Statistics", () => {
      ...
      const topSpenders = [
        {
          ...
          lastName: "Doe",
          email: "john@example.com",
          totalSpent: new Prisma.Decimal("5000.00"),
        },
        {
          id: "cust-2",
          firstName: "Jane",
          lastName: "Smith",
          email: "jane@example.com",
          totalSpent: new Prisma.Decimal("3500.00"),
        },
      ];
    ...
    it("should retrieve last sync timestamp", async () => {
      ...
      mockTenantDb.customer.findFirst.mockResolvedValue({
        lastSyncAt: lastSync,
      });
  ...
  describe("Input Validation", () => {
    ...
    it("should handle negative pagination values", async () => {
      ...
      const limit = Math.max(
        1,
        parseInt(mockRequest.query.limit as string) || 10,
      );
    ...
    it("should reject invalid sort columns", async () => {
      ...
      const isValid = validSortFields.includes(
        mockRequest.query.sortBy as string,
      );
    ...
    it("should reject invalid sort order", async () => {
      ...
      const isValid = validOrders.includes(
        mockRequest.query.sortOrder as string,
      );
  ...
  describe("Error Handling", () => {
    it("should handle database connection errors", async () => {
      mockTenantDb.customer.findMany.mockRejectedValue(
    ...
    it("should handle transaction rollback on sync failure", async () => {
      ...
      await expect(mockTenantDb.$transaction()).rejects.toThrow(
        "Transaction failed",
      );
    });
    ...
    it("should handle not found errors gracefully", async () => {
      ...
      const customer = await mockTenantDb.customer.findUnique({
  ...
  describe("Tenant Isolation", () => {
    it("should only return customers for the requesting shop", async () => {
      ...
      const allBelongToShop = customer.every(
        (c) => c.shopId === mockRequest.shopId,
      );
