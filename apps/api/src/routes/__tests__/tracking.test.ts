...
describe('Tracking Routes', () => {
  ...
  describe('POST /api/v4/tracking/events (Create Tracking Event)', () => {
    ...
    it('should require status from valid enum', () => {
      ...
      expect(
        validStatuses.includes(String(mockRequest.body.status))
      ).toBe(false);
    });
    ...
    it('should validate latitude and longitude are valid coordinates', () => {
      ...
      const isValidLat = Math.abs(Number(mockRequest.body.location.latitude)) <= 90;
  ...
  describe('PATCH /api/v4/tracking/:shipmentId/status (Status Update)', () => {
    ...
    it('should validate state machine transitions', () => {
      ...
      const validStates = ['pending', 'picked_up', 'in_transit', 'out_for_delivery', 'delivered'];
    ...
    it('should prevent invalid state transitions', () => {
      ...
      const isValidTransition =
        validStates.indexOf(currentStatus) <=
        validStates.indexOf(newStatus);
