...
async function esignaturesRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get("/envelopes", async (request: FastifyRequest) => {
    ...
    const { page = "1", limit = "25" } = request.query as Record<
      string,
      string
    >;
  ...
  fastify.get("/esig/analytics", async (request: FastifyRequest) => {
    ...
    const [
      ...
      request.tenantDb.activityLog.count({
        where: { shopId, entityType: "envelope" },
      }),
      request.tenantDb.activityLog.count({
        where: { shopId, entityType: "envelope", action: "COMPLETED" },
      }),
      request.tenantDb.activityLog.count({
        where: { shopId, entityType: "envelope", action: "SENT" },
      }),
      request.tenantDb.activityLog.count({
        where: { shopId, entityType: "envelope", action: "DECLINED" },
      }),
