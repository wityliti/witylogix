...
export async function loader({ request, params }) {
    ...
    const [orderRes, timelineRes, driversRes] = await Promise.allSettled([
        api.get(`/api/v4/orders/${params.id}`),
        api.get(`/api/v4/orders/${params.id}/timeline`),
        api.get("/api/v4/drivers", { status: "AVAILABLE", limit: 50 }),
    ]);
...
export default function OrderDetailPage() {
    const { order, timeline, availableDrivers } = useLoaderData();
      ...
      <div style={columnsStyle}>
        ...
        <div style={leftColStyle}>
          ...
          <div style={cardStyle}>
            ...
            {order.latitude && order.longitude && (<div style={{ ...subtextStyle, marginTop: 8 }}>
          ...
          <div style={cardStyle}>
            ...
            {order.notes && (<div style={{ marginTop: 12 }}>
                <span style={labelStyle}>Notes</span>
                <p style={{ margin: "4px 0 0", fontSize: 14 }}>{order.notes}</p>
              </div>)}
            {order.tags.length > 0 && (<div style={{ marginTop: 12, display: "flex", gap: 6, flexWrap: "wrap" }}>
                {order.tags.map((tag) => (<span key={tag} style={tagStyle}>{tag}</span>))}
              </div>)}
          </div>

          {/* Line Items */}
          {order.lineItems.length > 0 && (<div style={cardStyle}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                ...
                <tbody>
                  {order.lineItems.map((item, i) => (<tr key={i} style={{ borderBottom: "1px solid var(--p-color-border-subdued, #e1e3e5)" }}>
          ...
          {/* Proof of Delivery */}
          {order.proofOfDelivery && (<div style={cardStyle}>
              <h3 style={cardTitleStyle}>Proof of Delivery</h3>
              {order.proofOfDelivery.signatureUrl && (<div style={{ marginBottom: 12 }}>
                  <span style={labelStyle}>Signature</span>
                  <img src={order.proofOfDelivery.signatureUrl} alt="Customer signature" style={{ maxWidth: 300, border: "1px solid var(--p-color-border-subdued, #e1e3e5)", borderRadius: 8, marginTop: 4 }}/>
                </div>)}
              {order.proofOfDelivery.photos.length > 0 && (<div>
                  <span style={labelStyle}>Photos</span>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4 }}>
                    {order.proofOfDelivery.photos.map((url, i) => (<img key={i} src={url} alt={`Delivery photo ${i + 1}`} style={{ width: 120, height: 120, objectFit: "cover", borderRadius: 8, border: "1px solid var(--p-color-border-subdued, #e1e3e5)" }}/>))}
                  </div>
                </div>)}
              {order.proofOfDelivery.notes && (<div style={{ marginTop: 12 }}>
        ...
        <div style={rightColStyle}>
          ...
          <div style={cardStyle}>
            ...
            {/* Status Update */}
            {nextStatuses.length > 0 && (<Form method="post" style={{ marginBottom: 16 }}>
                <input type="hidden" name="intent" value="update-status"/>
                <div style={{ marginBottom: 8 }}>
                  <label style={labelStyle}>Update Status</label>
                  <select name="status" style={selectStyle}>
                    {nextStatuses.map((s) => (<option key={s} value={s}>
                        {s.replace(/_/g, " ")}
                      </option>))}
                  </select>
                </div>
                <input type="text" name="notes" placeholder="Add a note (optional)" style={{ ...inputStyle, marginBottom: 8 }}/>
            ...
            {/* Cancel */}
            {STATUS_TRANSITIONS[order.status]?.includes("CANCELLED") && (<Form method="post" style={{ marginTop: 16, borderTop: "1px solid var(--p-color-border-subdued, #e1e3e5)", paddingTop: 16 }}>
          ...
          {/* Current Driver Card */}
          {order.driver && (<div style={cardStyle}>
          ...
          {/* Route Card */}
          {order.route && (<div style={cardStyle}>
...
// ─── Styles ────────────────────────────────────────────────
const headerStyle = { padding: "24px 0 16px" };
const headingStyle = { fontSize: 20, fontWeight: 600, color: "var(--p-color-text, #202223)", margin: 0 };
...
const leftColStyle = { display: "flex", flexDirection: "column", gap: 16 };
const rightColStyle = { display: "flex", flexDirection: "column", gap: 16 };
const cardStyle = {
...
const cardTitleStyle = { fontSize: 14, fontWeight: 600, margin: "0 0 12px", color: "var(--p-color-text, #202223)" };
const fieldRowStyle = { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 };
const fieldStyle = { display: "flex", flexDirection: "column", gap: 2 };
const labelStyle = { fontSize: 12, fontWeight: 500, color: "var(--p-color-text-subdued, #6d7175)", textTransform: "uppercase", letterSpacing: "0.5px" };
const valueStyle = { fontSize: 14, color: "var(--p-color-text, #202223)" };
const addressBlockStyle = { fontSize: 14, lineHeight: 1.6, color: "var(--p-color-text, #202223)" };
const tagStyle = {
...
const itemTdStyle = { padding: "8px 0", verticalAlign: "top" };
const selectStyle = {
...
//# sourceMappingURL=orders.$id.js.map