...
export async function loader({ request }) {
    ...
    const response = await api.get("/api/v4/time-slots", {
        page,
        limit,
    });
...
export async function action({ request }) {
    ...
    if (intent === "create") {
        ...
        const result = await api.post("/api/v4/time-slots", timeSlot);
...
export default function TimeSlotsList() {
    const { timeSlots, meta } = useLoaderData();
    ...
    return (<div style={{ maxWidth: 1200, margin: "0 auto" }}>
      ...
      {/* Content */}
      {timeSlots.length === 0 ? (<EmptyState title="No time slots found" description="Create delivery time slots to define available delivery windows." actionLabel="Create Time Slot" onAction={() => setShowCreateModal(true)}/>) : (<>
          <div style={tableContainerStyle}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              ...
              <tbody>
                {timeSlots.map((slot) => (<tr key={slot.id} style={trStyle}>
                    <td style={tdStyle}>
      ...
      {showCreateModal && (<TimeSlotCreateModal onClose={() => setShowCreateModal(false)}/>)}
    </div>);
}
// ─── Sub-components ────────────────────────────────────────
function TimeSlotCreateModal({ onClose }) {
      ...
      <div style={modalStyle}>
        ...
        <Form method="post" onSubmit={onClose}>
          ...
          <div style={formFieldStyle}>
            ...
            <input name="label" required style={formInputStyle} placeholder="e.g., Morning (8am - 12pm)"/>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={formFieldStyle}>
              <label style={formLabelStyle}>Start Time *</label>
              <input name="startTime" type="time" required style={formInputStyle}/>
            </div>
            <div style={formFieldStyle}>
              <label style={formLabelStyle}>End Time *</label>
              <input name="endTime" type="time" required style={formInputStyle}/>
            </div>
          </div>

          <div style={formFieldStyle}>
            <label style={formLabelStyle}>Max Orders</label>
            <input name="maxOrders" type="number" defaultValue={10} min={1} style={formInputStyle}/>
          </div>
...
//# sourceMappingURL=time-slots._index.js.map