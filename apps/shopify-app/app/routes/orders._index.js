...
export default function OrdersList() {
    ...
    return (<div style={{ maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={filtersBarStyle}>
        {/* Search */}
        <input type="text" placeholder="Search orders..." value={currentSearch} onChange={(e) => updateFilter("search", e.target.value)} style={searchInputStyle}/>
        ...
        {/* Date filters */}
        <input type="date" value={searchParams.get("dateFrom") ?? ""} onChange={(e) => updateFilter("dateFrom", e.target.value)} style={selectStyle} placeholder="From date"/>
        <input type="date" value={searchParams.get("dateTo") ?? ""} onChange={(e) => updateFilter("dateTo", e.target.value)} style={selectStyle} placeholder="To date"/>
      </div>

      {/* Orders Table */}
      {orders.length === 0 ? (<EmptyState title="No orders found" description={currentSearch || currentStatus
                ? "Try adjusting your filters to find what you're looking for."
                : "Orders will appear here when they're created from Shopify or the API."} actionLabel={currentSearch || currentStatus ? "Clear filters" : undefined} onAction={currentSearch || currentStatus
          ...
          <div style={tableContainerStyle}>
            <table style={tableStyle}>
              ...
              <tbody>
                {orders.map((order) => (<tr key={order.id} style={trStyle}>
                    <td style={tdStyle}>
                    ...
                    <td style={tdStyle}>
                      {order.driverName ?? (<span style={cellSubtext}>Unassigned</span>)}
                    </td>
                    <td style={tdStyle}>
                      {order.deliveryDate
          ...
          <div style={paginationStyle}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage <= 1} style={paginationButtonStyle} type="button">
                Previous
              </button>
              <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage >= meta.totalPages} style={paginationButtonStyle} type="button">
                Next
              </button>
            </div>
          </div>
...
//# sourceMappingURL=orders._index.js.map