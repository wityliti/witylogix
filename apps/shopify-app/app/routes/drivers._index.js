...
export async function loader({ request }) {
    ...
    const response = await api.get("/api/v4/drivers", {
        page,
        limit,
        status,
        search,
...
export async function action({ request }) {
    ...
    if (intent === "create") {
        ...
        const result = await api.post("/api/v4/drivers", driver);
...
export default function DriversList() {
    const { drivers, meta } = useLoaderData();
    ...
    return (<div style={{ maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={headerStyle}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <div style={toggleGroupStyle}>
            <button onClick={() => setViewMode("table")} style={viewMode === "table" ? toggleActiveStyle : toggleInactiveStyle} type="button">
              Table
            </button>
            <button onClick={() => setViewMode("map")} style={viewMode === "map" ? toggleActiveStyle : toggleInactiveStyle} type="button">
              Map
            </button>
          </div>
      ...
      <div style={filtersStyle}>
        <input type="text" placeholder="Search drivers..." value={currentSearch} onChange={(e) => updateFilter("search", e.target.value)} style={searchInputStyle}/>
      ...
      {/* Content */}
      {drivers.length === 0 ? (<EmptyState title="No drivers found" description={currentSearch || currentStatus
                ? "Try adjusting your filters."
                : "Add drivers so you can assign them to deliveries."} actionLabel="Add Driver" onAction={() => setShowCreateModal(true)}/>) : viewMode === "table" ? (<>
          <div style={tableContainerStyle}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              ...
              <tbody>
                {drivers.map((driver) => (<tr key={driver.id} style={trStyle}>
                    <td style={tdStyle}>
                    ...
                    <td style={tdStyle}>
                      ...
                      {driver.vehiclePlate && (<div style={cellSubStyle}>{driver.vehiclePlate}</div>)}
                    </td>
                    <td style={tdStyle}>
      ...
      {/* Create Driver Modal */}
      {showCreateModal && (<DriverCreateModal onClose={() => setShowCreateModal(false)}/>)}
    </div>);
}
// ─── Sub-components ────────────────────────────────────────
...
function DriverCreateModal({ onClose }) {
...
const searchInputStyle = { flex: "1 1 200px", padding: "8px 12px", fontSize: 14, border: "1px solid var(--p-color-border, #c9cccf)", borderRadius: 8, backgroundColor: "white" };
...
const toggleGroupStyle = { display: "flex", border: "1px solid var(--p-color-border, #c9cccf)", borderRadius: 8, overflow: "hidden" };
const toggleActiveStyle = { padding: "6px 14px", fontSize: 13, fontWeight: 500, border: "none", backgroundColor: "var(--p-color-bg-fill-brand, #005bd3)", color: "white", cursor: "pointer" };
const toggleInactiveStyle = { padding: "6px 14px", fontSize: 13, fontWeight: 500, border: "none", backgroundColor: "white", color: "var(--p-color-text, #303030)", cursor: "pointer" };
...
const cellSubStyle = { fontSize: 12, color: "var(--p-color-text-subdued, #6d7175)", marginTop: 2 };
...
const mapPlaceholderStyle = { padding: 40, backgroundColor: "white", borderRadius: 12, border: "1px solid var(--p-color-border-subdued, #e1e3e5)" };
...
//# sourceMappingURL=drivers._index.js.map