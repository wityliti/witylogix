...
vi.mock('~/lib/rate-limiter.server', () => ({
  checkRateLimit: vi.fn(() => ({ allowed: true, remaining: 199, resetAt: Date.now() + 60_000 })),
...
describe('app.webhooks.receive — action handler', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(checkRateLimit).mockReturnValue({ allowed: true, remaining: 199, resetAt: Date.now() + 60_000 });
  });
  ...
  it('returns 429 when rate limit is exceeded', async () => {
    vi.mocked(checkRateLimit).mockReturnValue({ allowed: false, remaining: 0, resetAt: Date.now() + 5_000, retryAfter: 5 });
  ...
  it('uses x-shopify-shop-id header as rate-limit key', async () => {
    const req = makeWebhookRequest(makeShopifyOrder(), { shopId: 'shop-xyz.myshopify.com' });
    await action({ request: req } as any);
    expect(checkRateLimit).toHaveBeenCalledWith('webhook:shop-xyz.myshopify.com', 200, 60_000);
  });
  ...
  it('returns 401 for an invalid HMAC signature', async () => {
    const req = makeWebhookRequest(makeShopifyOrder(), { hmac: 'totally-wrong-AAAA==' });
    const res = await action({ request: req } as any);
    expect(res.status).toBe(401);
  ...
  it('returns 200 for a valid orders/create webhook', async () => {
    const req = makeWebhookRequest(makeShopifyOrder(), { topic: 'orders/create' });
    const res = await action({ request: req } as any);
  ...
  it('returns 200 but success:false when orders/create payload is missing order id', async () => {
    const req = makeWebhookRequest({ order_number: 1001 /* no id */ }, { topic: 'orders/create' });
    const res = await action({ request: req } as any);
  ...
  it('returns 200 for orders/fulfilled with fulfillment_id in payload', async () => {
    const req = makeWebhookRequest(
      makeShopifyOrder({ fulfillment_status: 'fulfilled', fulfillments: [{ id: 555 }] }),
