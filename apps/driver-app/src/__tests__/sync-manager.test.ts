...
function resolveEventStatus(retryCount: number): "pending" | "error" {
...
describe("event priority rules", () => {
  ...
  it("delivered status transitions are Tier 1", () => {
    ...
    const tierForDelivered = (
      ["delivered", "failed_delivery"] as string[]
    ).includes("delivered")
      ? 1
      : 2;
