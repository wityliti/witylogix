...
describe('append', () => {
  ...
  it('never overwrites an existing event — appends a new record instead', () => {
    const q = makeQueue();
    const id1 = q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd1', payload: { status: 'in_transit' }, driverGPS: null });
    const id2 = q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd1', payload: { status: 'delivered' }, driverGPS: null });
  ...
  it('stores driverGPS alongside the event', () => {
    const q = makeQueue();
    const gps = makeGPS();
    const id = q.append({ type: 'POD_SIGNATURE', deliveryId: 'd1', payload: { signatureBase64: 'abc' }, driverGPS: gps });
    expect(q.get(id)!.driverGPS).toEqual(gps);
  });

  it('allows driverGPS to be null', () => {
    const q = makeQueue();
    const id = q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd1', payload: { status: 'in_transit' }, driverGPS: null });
...
describe('event types', () => {
  ...
  it('accepts POD_SIGNATURE', () => {
    const q = makeQueue();
    const id = q.append({ type: 'POD_SIGNATURE', deliveryId: 'd1', payload: { signatureBase64: 'xyz' }, driverGPS: null });
  ...
  it('accepts PHOTO_EVIDENCE with base64 data and enforces 1 MB size guard', () => {
    ...
    const id = q.append({ type: 'PHOTO_EVIDENCE', deliveryId: 'd1', payload: { photoBase64: smallPhoto }, driverGPS: null });
    expect(q.get(id)!.type).toBe('PHOTO_EVIDENCE');

    // Exceeds 1 MB base64 limit → should throw
    const bigPhoto = 'A'.repeat(1_400_000);
    expect(() =>
      q.append({ type: 'PHOTO_EVIDENCE', deliveryId: 'd1', payload: { photoBase64: bigPhoto }, driverGPS: null })
    ).toThrow(/photo.*size/i);
  });

  it('accepts RTS_WORKFLOW', () => {
    const q = makeQueue();
    const id = q.append({ type: 'RTS_WORKFLOW', deliveryId: 'd1', payload: { reason: 'failed_all_attempts' }, driverGPS: null });
...
describe('sync priority', () => {
  ...
  it('pendingByPriority returns events sorted by priority then timestamp', async () => {
    ...
    q.append({ type: 'RTS_WORKFLOW', deliveryId: 'd1', payload: {}, driverGPS: null });
    await new Promise((r) => setTimeout(r, 2));
    q.append({ type: 'STOP_RESEQUENCED', deliveryId: 'd1', payload: { stopOrder: [] }, driverGPS: null });
    await new Promise((r) => setTimeout(r, 2));
    q.append({ type: 'PHOTO_EVIDENCE', deliveryId: 'd1', payload: { photoBase64: 'x' }, driverGPS: null });
    await new Promise((r) => setTimeout(r, 2));
    q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd1', payload: { status: 'in_transit' }, driverGPS: null });
  ...
  it('within the same priority tier, earlier timestamps come first', async () => {
    ...
    q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd1', payload: { status: 'in_transit' }, driverGPS: null });
    await new Promise((r) => setTimeout(r, 2));
    q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd2', payload: { status: 'delivered' }, driverGPS: null });
...
describe('idempotency', () => {
  it('each appended event gets a unique eventId', () => {
    ...
    for (let i = 0; i < 20; i++) {
      ids.add(q.append({ type: 'DELIVERY_STATUS', deliveryId: `d${i}`, payload: {}, driverGPS: null }));
    }
...
describe('conflict resolution', () => {
  it('markConflict sets status to "conflict" and stores a message', () => {
    const q = makeQueue();
    const id = q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd1', payload: { status: 'in_transit' }, driverGPS: null });
    q.markConflict(id, 'Delivery #d1 was already marked delivered remotely. Your update was discarded.');
    const evt = q.get(id)!;
  ...
  it('conflicts returns all conflict events for UI notification', () => {
    const q = makeQueue();
    const id1 = q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd1', payload: {}, driverGPS: null });
    const id2 = q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd2', payload: {}, driverGPS: null });
  ...
  it('acknowledgeConflict removes the event from conflicts()', () => {
    const q = makeQueue();
    const id = q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd1', payload: {}, driverGPS: null });
...
describe('summary', () => {
  it('summary returns correct counts for each status', () => {
    ...
    const id1 = q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd1', payload: {}, driverGPS: null });
    const id2 = q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd2', payload: {}, driverGPS: null });
    q.append({ type: 'DELIVERY_STATUS', deliveryId: 'd3', payload: {}, driverGPS: null });
    q.markSynced(id1);
