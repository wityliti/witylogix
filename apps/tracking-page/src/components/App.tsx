...
const App: React.FC = () => {
      ...
      .catch((err: unknown) => {
        const message =
          err instanceof Error
            ? err.message
            : "Unable to find that tracking number.";
        setError(message);
      })
      .finally(() => {
        setIsLoading(false);
  ...
  const timelineEvents = trackingData.timeline
    ...
    .map((step, idx) => ({
      id: String(idx),
      status: step.label as Parameters<
        typeof DeliveryTimeline
      >[0]["events"][number]["status"],
  ...
  const estimatedDelivery =
    (trackingData.actualDelivery ?? trackingData.estimatedArrival)
      ? new Date(
          trackingData.actualDelivery ?? trackingData.estimatedArrival!,
