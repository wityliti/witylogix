...
const WORKSPACE_PATTERNS = ["apps/*", "packages/*", "extensions/*"];
...
function validateDependencyVersions(): string[] {
  ...
  for (const [dep, versions] of Object.entries(versionMap)) {
    ...
    if (uniqueVersions.size > 1) {
      ...
      issues.push(`⚠ ${dep} version mismatch: ${versionList}`);
    }
  }
...
function printTable(results: PackageInfo[]): void {
  ...
  console.log(
    `${" Package Name".padEnd(35)} │ ${"Status".padEnd(12)} │ Missing Scripts`,
  );
...
async function main(): Promise<void> {
  ...
  const { results: scriptResults, issues: scriptIssues } =
